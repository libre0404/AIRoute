...
import { resolveNestedComboTargets } from "@AIRoute/open-sse/services/combo.ts";
    ...
    try {
      res = await fetch(internalUrl, {
        ...
        headers: {
          ...
          // AIRoute's semantic cache or other request reuse layers.
          "X-AIRoute-No-Cache": "true",
          ...(target.connectionId ? { "X-AIRoute-Connection": target.connectionId } : {}),
