 ...
 * fetches the live /v1/models catalog from a (possibly remote) AIRoute and
 ...
 *
 * Launch a profile with:  AIRoute launch --profile <name>
...
export function buildProfileSettings(modelId, baseUrl, cfg) {
  ...
  // NOTE: ANTHROPIC_AUTH_TOKEN is intentionally NOT written here — `AIRoute
...
export async function runSetupClaudeCommand(opts = {}) {
  ...
  const baseUrl = (opts.remote ?? `http://localhost:${port}`)
    .replace(/\/+$/, "")
    .replace(/\/v1$/, "");
  const apiKey = opts.apiKey ?? opts["api-key"] ?? process.env.AIRoute_API_KEY ?? "";
  const claudeHome = opts.claudeHome ?? opts["claude-home"] ?? join(os.homedir(), ".claude");
  const profilesRoot = join(claudeHome, "profiles");
  const dryRun = Boolean(opts.dryRun ?? opts["dry-run"]);

  printHeading("AIRoute → Claude Code profile generator");
  ...
  try {
    ...
    printInfo(
      "Make sure AIRoute is running and the --remote URL is correct.\n" +
        "You may also need --api-key if AIRoute requires authentication."
    );
    return 1;
  }
  ...
  if (!dryRun) {
    ...
    console.log("\nTo use a profile:");
    console.log("  AIRoute launch --profile <name>     # e.g. AIRoute launch --profile glm52");
    console.log(
...
export function registerSetupClaude(program) {
  program
    ...
    .description(
      "Fetch the live model catalog from AIRoute (local or remote VPS) and generate " +
