...
export function registerPlugin(program) {
  const plugin = program
    .command("plugin")
    .description(t("plugin.description") || "Manage CLI plugins (AIRoute-cmd-*)");
  ...
  plugin
    ...
    .action(async (opts, cmd) => {
      ...
      if (plugins.length === 0) {
        ...
        process.stdout.write(`Install: AIRoute plugin install <name>\n`);
      }
    });
  ...
  plugin
    ...
    .action(async (name, opts) => {
      ...
      const pkgName = isLocal ? name : `AIRoute-cmd-${name}`;

      if (!opts.yes) {
        process.stderr.write(
          `⚠ WARNING: Plugins run with the same privileges as AIRoute CLI.\n` +
  ...
  plugin
    ...
    .action(async (name, opts) => {
      const pkgName = name.startsWith("AIRoute-cmd-") ? name : `AIRoute-cmd-${name}`;
      if (!opts.yes) {
  ...
  plugin
    ...
    .action(async (name, opts, cmd) => {
      const plugins = await discoverPlugins();
      const p = plugins.find((x) => x.name === name || x.name === `AIRoute-cmd-${name}`);
      if (!p) {
  ...
  plugin
    ...
    .action(async (query) => {
      const q = query ? `AIRoute-cmd-${query}` : "AIRoute-cmd";
      ...
      try {
        ...
        if (rows.length === 0) {
          process.stdout.write(`No plugins found for '${query || "AIRoute-cmd"}'.\n`);
        } else {
          rows.forEach((r) =>
  ...
  plugin
    ...
    .action(async (name) => {
      ...
      const dir = join(process.cwd(), `AIRoute-cmd-${safeName}`);
      if (existsSync(dir)) {
      ...
      writeFileSync(
        ...
        JSON.stringify(
          {
            name: `AIRoute-cmd-${safeName}`,
            version: "0.1.0",
            type: "module",
            main: "index.mjs",
            description: `AIRoute CLI plugin: ${safeName}`,
      ...
      writeFileSync(
        join(dir, "README.md"),
        `# AIRoute-cmd-${safeName}\n\nAn AIRoute CLI plugin.\n\n## Install\n\n\`\`\`bash\nAIRoute plugin install ${safeName}\n\`\`\`\n`
      );
      process.stdout.write(`✓ Scaffolded: ${dir}\n`);
      process.stdout.write(`  Run: cd ${dir} && AIRoute plugin install .\n`);
    });
}
