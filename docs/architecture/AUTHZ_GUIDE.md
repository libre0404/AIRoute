...
Validated by `isValidApiKey()` / `extractApiKey()` in `src/sse/services/auth.ts` and re-exported through `src/shared/utils/apiAuth.ts`. The validator also accepts the `AIRoute_API_KEY` / `ROUTER_API_KEY` env vars as persistent passthrough keys (issue #1350).
...
Incoming request → src/proxy.ts
  → runAuthzPipeline() in src/server/authz/pipeline.ts
    1. Strip trusted internal headers (x-AIRoute-auth-*, x-AIRoute-route-class)
    ...
    8. Otherwise: POLICIES[routeClass].evaluate(ctx)
       - allow  → stamp x-AIRoute-auth-{kind,id,label,scopes} → NextResponse.next()
       - reject → JSON error w/ correlation_id (dashboard pages → 302 /login)
```

Trusted internal headers (defined in `src/server/authz/headers.ts`) are **stripped from incoming requests** before classification — clients cannot pre-populate `x-AIRoute-auth-*` to impersonate a subject.
...
request metadata, or `AIRoute_MCP_SCOPES`.
...
x-AIRoute-route-class:    PUBLIC | CLIENT_API | MANAGEMENT
```

For authenticated requests the upstream (handler-side) request headers also include:

```
x-AIRoute-auth-kind:      client_api_key | dashboard_session | management_key | anonymous
x-AIRoute-auth-id:        key_<last-4> | "dashboard" | "anonymous"
x-AIRoute-auth-label:     (optional)
x-AIRoute-auth-scopes:    comma-separated list
```
