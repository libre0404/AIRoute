...
/**
 * Strip the AIRoute provider prefix from versioned built-in tool model
 ...
 * leaked AIRoute prefix makes Anthropic reject the request. Mutates in place.
 */
...
export class BaseExecutor {
  ...
  async execute(input: ExecuteInput) {
    ...
    for (let urlIndex = 0; urlIndex < fallbackCount; urlIndex++) {
      ...
      // When AIRROUTE_REGION=cn, check if the upstream domain is allowed
      // under 数据安全法 §31 / 个人信息保护法 §38 cross-border rules.
      try {
        const { checkDataExport } = await import("@/lib/compliance/dataExportControl");
        const urlObj = new URL(url);
        const exportCheck = checkDataExport({
          domain: urlObj.hostname,
          provider: this.provider,
          model: model || "unknown",
          userId: (requestCredentials as any)?.userId,
          requestId: (requestCredentials as any)?.requestId,
        });
        if (!exportCheck.allowed) {
          const err = new Error(exportCheck.reason || "Data export blocked");
          (err as any).status = 403;
          throw err;
        }
      } catch (err: unknown) {
        // Re-throw our own 403 block; swallow import failures (compliance module optional)
        if (err && typeof err === "object" && "status" in err && (err as any).status === 403) {
          throw err;
        }
        // Import error — continue without export control (don't break existing setups)
      }
      ...
      try {
        ...
        if (
          ...
          if (Array.isArray(tb.tools)) {
            ...
            // Also strip AIRoute provider prefix from versioned built-in tool
          ...
          //   x-AIRoute-effort:   low | medium | high | xhigh | max | off
          //   x-AIRoute-thinking: adaptive | off
          // A header value applies only when the corresponding body field is
          // not already set; "off" force-strips the field.
          const headerEffort = (
            clientHeaders?.["x-AIRoute-effort"] ?? clientHeaders?.["X-AIRoute-Effort"]
          )
            ?.trim()
            .toLowerCase();
          const headerThinking = (
            clientHeaders?.["x-AIRoute-thinking"] ?? clientHeaders?.["X-AIRoute-Thinking"]
          )
            ?.trim()
            .toLowerCase();
          ...
          if (effThinking === "adaptive") {
            ...
            // must opt in with x-AIRoute-thinking; force-injecting adaptive thinking
          ...
          // Anthropic correlate accounts behind one AIRoute.
