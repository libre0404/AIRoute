...
import { isDomesticProviderId } from "../regionProviders";
...
export interface ScoringFactors {
  ...
  /** Regional affinity: 1.0 = domestic provider (lower cross-border latency), 0.0 = overseas. **/
...
export interface ScoringWeights {
  ...
  resetWindowAffinity: number;
  connectionDensity: number;
  /** Weight for region affinity factor. 0 in default weights (no effect). **/
  regionAffinity: number;
}
...
/**
 * China-region optimized scoring weights.
 *
 * Key differences from DEFAULT_WEIGHTS:
 * - regionAffinity: 0.12  → Strongly prefer domestic providers (lower cross-border
 *   latency, PIPL/DSL compliance, GFW stability)
 * - latencyInv: 0.15     → Boosted because cross-border latency variance is
 *   significant in China; domestic providers often 3-5x faster
 * - costInv: 0.08        → Reduced because domestic models are already price-
 *   competitive; over-weighting cost could penalize premium domestic providers
 * - quota: 0.12          → Slightly reduced to make room for regionAffinity
 * - health: 0.18         → Slightly reduced to make room for regionAffinity
 *
 * Sum = 1.00
 */
export const CN_REGION_WEIGHTS: ScoringWeights = {
  quota: 0.12,
  health: 0.18,
  costInv: 0.08,
  latencyInv: 0.15,
...
export function calculateScore(factors: ScoringFactors, weights: ScoringWeights): number {
  ...
  return clamp01(
    weights.quota * factors.quota +
      ...
      (weights.regionAffinity ?? 0) * factors.regionAffinity
  );
}
...
export function calculateFactors(
  ...
  return {
    ...
    regionAffinity: isDomesticProviderId(candidate.provider) ? 1.0 : 0.0,
  };
}
...
export function scorePool(
  pool: ProviderCandidate[],
  taskType: string,
  weights: ScoringWeights = resolveRegionWeights(),
...
/**
 * Resolve scoring weights based on the current region.
 * When AIRROUTE_REGION=cn, returns CN_REGION_WEIGHTS that boost domestic
 * provider selection and cross-border latency optimization.
 * Otherwise returns DEFAULT_WEIGHTS.
 */
export function resolveRegionWeights(): ScoringWeights {
  const region = process.env.AIRROUTE_REGION;
  if (region === "cn") return CN_REGION_WEIGHTS;
