 ...
 *
 * Covers `createAIRouteConfigHook(opts, deps)`:
 *   - happy path: valid auth.json → mutates input.provider[id] with the
 *     stripped per-model shape (mirroring `@AIRoute/opencode-provider`).
...
import {
  ...
  createAIRouteConfigHook,
  createAIRouteProviderHook,
  AIRoutePlugin,
  resolveAIRoutePluginOptions,
  type AIRouteCombosFetcher,
  type AIRouteEnrichmentEntry,
  type AIRouteEnrichmentFetcher,
  type AIRouteEnrichmentMap,
  type AIRouteFetchCache,
  type AIRouteModelsFetcher,
  type AIRouteProviderConnection,
  type AIRouteProvidersFetcher,
  type AIRouteRawCombo,
  type AIRouteRawModelEntry,
  type AIRouteReadAuthJson,
  type AIRouteStaticProviderEntry,
} from "../src/index.js";
...
const MODEL_CLAUDE: AIRouteRawModelEntry = {
...
const MODEL_GEMINI: AIRouteRawModelEntry = {
  id: "gemini-3-flash",
...
const COMBO_CLAUDE_TIER: AIRouteRawCombo = {
...
): AIRouteReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: AIRouteReadAuthJson = async () => {
    n++;
...
function throwingReadAuthJson(): AIRouteReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: AIRouteReadAuthJson = async () => {
    n++;
...
function stubModelsFetcher(
  payload: AIRouteRawModelEntry[]
): AIRouteModelsFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: AIRouteModelsFetcher = async (baseURL, apiKey) => {
    n++;
...
function stubCombosFetcher(
  payload: AIRouteRawCombo[]
): AIRouteCombosFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: AIRouteCombosFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingModelsFetcher(): AIRouteModelsFetcher & { callCount: () => number } {
  let n = 0;
  const f: AIRouteModelsFetcher = async () => {
    n++;
...
function throwingCombosFetcher(): AIRouteCombosFetcher & { callCount: () => number } {
  let n = 0;
  const f: AIRouteCombosFetcher = async () => {
    n++;
...
function stubEnrichmentFetcher(payload: AIRouteEnrichmentMap): AIRouteEnrichmentFetcher & {
  callCount: () => number;
  callsBy: () => Array<[string, string]>;
} {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: AIRouteEnrichmentFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingEnrichmentFetcher(): AIRouteEnrichmentFetcher & { callCount: () => number } {
  let n = 0;
  const f: AIRouteEnrichmentFetcher = async () => {
    n++;
...
test("config: with valid auth.json + apiKey + baseURL → mutates input.provider[id] with stripped models block", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-AIRoute": { type: "api", key: "sk-test-1", baseURL: "https://or.example.com/v1" },
  });
  ...
  const hook = createAIRouteConfigHook(
    { providerId: "AIRoute" },
  ...
  const provider = (input as { provider: Record<string, AIRouteStaticProviderEntry> }).provider;
  const entry = provider["opencode-AIRoute"];
  assert.ok(entry, "input.provider['opencode-AIRoute'] set");
  assert.equal(entry.npm, "@ai-sdk/openai-compatible");
  assert.equal(entry.name, "AIRoute");
  ...
  const claude = entry.models["opencode-AIRoute/claude-sonnet-4-6"];
  ...
  const combo = entry.models["opencode-AIRoute/claude-tier"];
...
//     prefixed (`opencode-AIRoute`). Without the fallback the lookup misses
...
test("config: auth.json under bare key (pre-prefix login) resolves via dual-key fallback", async () => {
  // Stored under bare `AIRoute` (the key OC wrote before the auto-prefix fix),
  // but the resolved providerId is now `opencode-AIRoute`.
  const readAuthJson = stubReadAuthJson({
    AIRoute: { type: "api", key: "sk-bare-1", baseURL: "https://or.example.com/v1" },
  });
  ...
  const hook = createAIRouteConfigHook(
    { providerId: "AIRoute" }, // resolves to opencode-AIRoute internally
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: prefixed key wins over bare key when both present (dual-key precedence)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-AIRoute": { type: "api", key: "sk-prefixed", baseURL: "https://pref.example/v1" },
    AIRoute: { type: "api", key: "sk-bare", baseURL: "https://bare.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const entry = (input as { provider: Record<string, AIRouteStaticProviderEntry> }).provider[
    "opencode-AIRoute"
  ];
  assert.ok(entry);
  assert.equal(
...
test("config: existing input.provider[id] → no overwrite (respect manual override)", async () => {
  ...
  const readAuthJson = stubReadAuthJson({
    "opencode-AIRoute": { type: "api", key: "sk-test", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const input = makeInput({ "opencode-AIRoute": manual });
  await hook(input);

  const provider = (input as { provider: Record<string, unknown> }).provider;
  assert.equal(provider["opencode-AIRoute"], manual, "manual override preserved by reference");
...
test("config: fetchers throw → warn + emit stub entry with models: {}", async () => {
  ...
  // test below).
  const hook = createAIRouteConfigHook(
    { providerId: "AIRoute", features: { diskCache: false } },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: combos fetcher throws → emit models-only catalog (no combos in models block)", async () => {
  ...
  assert.deepEqual(ids, [
    "opencode-AIRoute/claude-sonnet-4-6",
    "opencode-AIRoute/gemini-3-flash",
  ]);
  assert.equal(entry.models["opencode-AIRoute/claude-tier"], undefined, "no combo entry");
  assert.ok(
    logger.entries.some((e) => String(e[0]).includes("/api/combos fetch failed")),
    "combos-fetch breadcrumb emitted"
  );
...
test("config: baseURL from auth.json takes precedence when opts.baseURL absent", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-AIRoute": { type: "api", key: "sk-test", baseURL: "https://creds.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createAIRouteConfigHook(
    { providerId: "AIRoute" }, // NO opts.baseURL
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: opts.baseURL wins over auth.json's stored baseURL", async () => {
  ...
  const hook = createAIRouteConfigHook(
    { providerId: "AIRoute", baseURL: "https://opts.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: no baseURL resolvable (no opts, no auth.json baseURL) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-AIRoute": { type: "api", key: "sk-test" }, // NO baseURL on the credential
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
...
test("config: multi-instance — two plugins with different providerIds publish to their own keys without collision", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-AIRoute-prod": {
      type: "api",
      key: "sk-prod",
      baseURL: "https://prod.example/v1",
    },
    "opencode-AIRoute-preprod": {
      type: "api",
      key: "sk-preprod",
  ...
  const hookA = createAIRouteConfigHook(
    { providerId: "AIRoute-prod" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const hookB = createAIRouteConfigHook(
    { providerId: "AIRoute-preprod" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  ...
  const provider = (input as { provider: Record<string, AIRouteStaticProviderEntry> }).provider;
  assert.ok(provider["opencode-AIRoute-prod"], "prod block present");
  assert.ok(provider["opencode-AIRoute-preprod"], "preprod block present");
  assert.equal(provider["opencode-AIRoute-prod"].options.apiKey, "sk-prod");
  assert.equal(provider["opencode-AIRoute-preprod"].options.apiKey, "sk-preprod");
  assert.equal(provider["opencode-AIRoute-prod"].options.baseURL, "https://prod.example/v1");
  assert.equal(
    provider["opencode-AIRoute-preprod"].options.baseURL,
    "https://preprod.example/v1"
  );
  assert.notEqual(
    provider["opencode-AIRoute-prod"],
    provider["opencode-AIRoute-preprod"],
...
test("config + provider share cache: second call uses cached fetch result (single fetch per TTL)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-AIRoute": { type: "api", key: "sk-shared", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([COMBO_CLAUDE_TIER]);
  ...
  const configHook = createAIRouteConfigHook(
    { providerId: "AIRoute", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, cache: sharedCache, logger }
  );
  const providerHook = createAIRouteProviderHook(
    { providerId: "AIRoute", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
...
test("provider → config order also dedupes (cache populated by provider, consumed by config)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-AIRoute": { type: "api", key: "sk-reverse", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const sharedCache: AIRouteFetchCache = new Map();
...
test("buildStaticProviderEntry: stripped per-model shape matches sibling @AIRoute/opencode-provider", () => {
  const resolved = resolveAIRoutePluginOptions({
    providerId: "AIRoute",
    displayName: "AIRoute",
  });
  ...
  assert.equal(block.name, "AIRoute");
  ...
  const claude = block.models["opencode-AIRoute/claude-sonnet-4-6"];
...
test("buildStaticProviderEntry: empty fetch results → stub block with models: {}", () => {
  const resolved = resolveAIRoutePluginOptions({ providerId: "AIRoute" });
...
test("buildStaticProviderEntry: hidden combos are excluded", () => {
  ...
  assert.equal(block.models["opencode-AIRoute/claude-tier"], undefined);
  assert.ok(block.models["opencode-AIRoute/claude-sonnet-4-6"]);
});
...
test("buildStaticProviderEntry: emits release_date when raw carries it; omits when null", () => {
  const resolved = resolveAIRoutePluginOptions({ providerId: "AIRoute" });
  const withDate: AIRouteRawModelEntry = {
    ...MODEL_CLAUDE,
    id: "claude-with-date",
  ...
  assert.equal(block.models["opencode-AIRoute/claude-with-date"].release_date, "2026-02-19");
  assert.equal(block.models["opencode-AIRoute/gemini-3-flash"].release_date, undefined);
});

test("buildStaticProviderEntry: combo modalities = intersection of members (LCD)", () => {
  const resolved = resolveAIRoutePluginOptions({ providerId: "AIRoute" });
  const TEXT_ONLY: AIRouteRawModelEntry = {
    id: "text-only",
  ...
  const combo = block.models["opencode-AIRoute/mixed-tier"];
...
// ────────────────────────────────────────────────────────────────────────────
// Integration: AIRoutePlugin factory now exposes config hook
// ────────────────────────────────────────────────────────────────────────────

test("AIRoutePlugin factory exposes config hook alongside auth + provider", async () => {
  const hooks = await AIRoutePlugin({} as never, { providerId: "AIRoute" });
...
test("config: auth.json entry of wrong type (oauth) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-AIRoute": { type: "oauth", refresh: "r", access: "a", expires: 0 },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createAIRouteConfigHook(
    { providerId: "AIRoute", baseURL: "https://or.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: initialises input.provider when undefined", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-AIRoute": { type: "api", key: "sk", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  assert.ok(provider!["opencode-AIRoute"]);
});
...
test("config: enrichment fetched + name overlaid on raw-model entries", async () => {
  ...
  const enrichmentFetcher = stubEnrichmentFetcher(
    new Map<string, AIRouteEnrichmentEntry>([
  ...
  assert.ok(entry);
  assert.equal(entry.models["opencode-AIRoute/claude-sonnet-4-6"].name, "Claude Sonnet 4.6");
  assert.equal(entry.models["opencode-AIRoute/gemini-3-flash"].name, "Gemini 3 Flash");
  // Combo names still come from /api/combos — enrichment overlay does NOT touch combos.
  assert.equal(entry.models["opencode-AIRoute/claude-tier"].name, "Claude Tier");
...
test("config: features.enrichment=false skips enrichment fetch + keeps raw-id names", async () => {
  ...
  const hook = createAIRouteConfigHook(
    { providerId: "AIRoute", features: { enrichment: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
  ...
  assert.equal(
    entry.models["opencode-AIRoute/claude-sonnet-4-6"].name,
    "claude-sonnet-4-6",
    "raw id retained"
  );
});
...
function stubProvidersFetcher(payload: AIRouteProviderConnection[]): AIRouteProvidersFetcher & {
  callCount: () => number;
} {
  let n = 0;
  const f: AIRouteProvidersFetcher = async () => {
    n++;
    return payload;
  };
  return Object.assign(f, { callCount: () => n });
...
function throwingProvidersFetcher(): AIRouteProvidersFetcher & { callCount: () => number } {
  let n = 0;
  const f: AIRouteProvidersFetcher = async () => {
    n++;
    throw new Error("ETIMEDOUT");
...
test("config: usableOnly=true → drops models for non-usable providers, keeps usable + unknown", async () => {
  ...
  const hook = createAIRouteConfigHook(
    { providerId: "AIRoute", baseURL: "https://or.example/v1", features: { usableOnly: true } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, providersFetcher }
  );
...
test("config: diskCache hydrates stale snapshot when /v1/models throws", async () => {
  ...
  const hook = createAIRouteConfigHook(
    { providerId: "AIRoute", features: { diskCache: true } },
    {
      readAuthJson,
      fetcher,
      combosFetcher,
  ...
  assert.ok(
    entry.models["opencode-AIRoute/claude-sonnet-4-6"],
...
test("config: providerTag (default-on) prepends '<provider> - ' to enriched raw-model names", async () => {
  ...
  assert.equal(entry.models["opencode-AIRoute/gemini-3-flash"].name, "Gemini - Gemini 3 Flash");
...
test("config: providerTag=false suppresses the suffix", async () => {
  ...
  const hook = createAIRouteConfigHook(
    { providerId: "AIRoute", features: { providerTag: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: providerTag falls back to UPPER(alias) when providerDisplayName missing", async () => {
  ...
  // a slot AIRoute hasn't curated a human label for yet. We still
  ...
  assert.equal(entry.models["opencode-AIRoute/claude-sonnet-4-6"].name, "CC - Claude Sonnet 4.6");
});
...
test("config: providerTag is idempotent — second hook call doesn't double-suffix", async () => {
  ...
  const hook = createAIRouteConfigHook(
    { providerId: "AIRoute", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, cache: sharedCache, logger }
  );

  const inputA = makeInput();
  await hook(inputA);
  const entryA = (inputA as { provider: Record<string, AIRouteStaticProviderEntry> }).provider[
    "opencode-AIRoute"
  ];
  assert.equal(
    entryA.models["opencode-AIRoute/claude-sonnet-4-6"].name,
    "Claude - Claude Sonnet 4.6"
  );
  ...
  await hook(inputB);
  const entryB = (inputB as { provider: Record<string, AIRouteStaticProviderEntry> }).provider[
    "opencode-AIRoute"
  ];
  assert.equal(
    entryB.models["opencode-AIRoute/claude-sonnet-4-6"].name,
    "Claude - Claude Sonnet 4.6"
  );
});
...
test("buildStaticProviderEntry: nested combo-ref context is the bottleneck across the graph", () => {
  const resolved = resolveAIRoutePluginOptions({ providerId: "AIRoute" });
  const rawModels: AIRouteRawModelEntry[] = [
    {
      id: "raw-big",
      context_length: 200_000,
      max_output_tokens: 64_000,
  ...
  const rawCombos: AIRouteRawCombo[] = [
    {
      id: "tiny-combo",
      name: "TinyCombo",
  ...
  const parent = block.models["opencode-AIRoute/parent"];
