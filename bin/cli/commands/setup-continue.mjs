 ...
 * `apiKey: ${{ secrets.AIRoute_API_KEY }}` reference (secret never written to
...
const SECRET_REF = "${{ secrets.AIRoute_API_KEY }}";
...
/**
 * Merge AIRoute models into an existing Continue config object: drop any prior
...
export function mergeContinueConfig(existing, newModels, apiBase) {
  ...
  if (!cfg.name) cfg.name = "AIRoute Config";
...
export async function runSetupContinueCommand(opts = {}) {
  ...
  printHeading("AIRoute → Continue (config.yaml)");
  ...
  if (dryRun) {
    ...
    printInfo(`[dry-run] ${models.length} AIRoute model(s) → ${configPath}`);
    return 0;
  }

  mkdirSync(join(configPath, ".."), { recursive: true });
  writeFileSync(configPath, out, "utf8");
  printSuccess(`Wrote ${configPath} (${models.length} AIRoute models)`);
  printInfo("\nProvide the key (config.yaml references it, not stores it):");
  printInfo("  cn CLI:  export AIRoute_API_KEY=...   (read from your shell)");
  printInfo("  IDE:     echo 'AIRoute_API_KEY=...' >> ~/.continue/.env");
...
export function registerSetupContinue(program) {
  program
    ...
    .description(
      "Generate ~/.continue/config.yaml (Continue / cn CLI) from the AIRoute model catalog"
    )
