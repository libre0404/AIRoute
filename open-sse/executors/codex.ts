...
function consumeResponsesStoreMarker(body: Record<string, unknown>): unknown {
  const marker = body._AIRouteResponsesStore;
  delete body._AIRouteResponsesStore;
  return marker;
}

/**
 * Global Codex WebSocket kill-switch (feature flag AIRoute_CODEX_WS_ENABLED,
...
function isCodexWsGloballyEnabled(): boolean {
  try {
    return isFeatureFlagEnabled("AIRoute_CODEX_WS_ENABLED");
  } catch {
    return true;
  }
}
...
export function isCodexResponsesWebSocketRequired(_model: string, credentials: unknown): boolean {
  ...
  // AIRoute is an HTTP→SSE gateway — WebSocket transport is unnecessary and
...
function codexDropNonstandardEvents(): boolean {
  const v = process.env.AIRoute_CODEX_DROP_NONSTANDARD_EVENTS;
...
// streaming Response when the AIRoute_CODEX_DROP_NONSTANDARD_EVENTS kill-switch is on.
...
export function encodeResponseSseEvent(raw: string): { sse: string; terminal: boolean } {
  ...
  // AIRoute_CODEX_DROP_NONSTANDARD_EVENTS (the HTTP transport is handled
...
export class CodexExecutor extends BaseExecutor {
  ...
  transformRequest(
    ...
    // These are often injected by AIRoute's fallback logic for store=true,
    ...
    const RESPONSES_API_ALLOWLIST = new Set([
      ...
      "text",
      // Internal markers used by AIRoute pipeline
      "_AIRouteResponsesStore",
    ]);
