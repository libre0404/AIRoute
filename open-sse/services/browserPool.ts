 ...
 * startup cost. Set AIRoute_BROWSER_POOL=off to fully disable.
 */
...
// (and the AIRoute_browser_pool_status MCP tool), giving the previously
...
function isPoolEnabled(): boolean {
  const flag = process.env.AIRoute_BROWSER_POOL;
...
export async function acquireBrowserContext(
  ...
  if (!isPoolEnabled()) {
    throw new Error(
      "browserPool: AIRoute_BROWSER_POOL=off — context requested but pool is disabled"
    );
  }
 ...
 * failures, shutdowns). Surfaced via the AIRoute_browser_pool_status MCP tool.
 */
