...
const RUNTIME_DIR = join(homedir(), ".AIRoute", "runtime");
...
export async function loadSqliteRuntime() {
  ...
  try {
    ...
    console.warn(`[AIRoute] runtime install failed: ${err.message}`);
  }
  ...
  if (existsSync(buildDir)) {
    ...
    if (nodeFile) {
      ...
      if (!magic || (magic !== expected && magic !== "macho-le" && magic !== "macho-fat")) {
        console.warn(
          `[AIRoute] runtime sqlite binary magic mismatch (${magic} ≠ ${expected}) — skipping`
        );
        return null;
      }
    }
...
function ensureRuntimeDir() {
  ...
  if (!existsSync(pkg)) {
    writeFileSync(
      pkg,
      JSON.stringify({ name: "AIRoute-runtime", private: true, type: "commonjs" }),
      "utf-8"
    );
  }
}
