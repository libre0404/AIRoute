...
function getLHCState() {
  if (!globalThis.__AIRouteLocalHC) {
    globalThis.__AIRouteLocalHC = {
      initialized: false,
      sweepTimer: null,
      healthCache: new Map(),
  ...
  return globalThis.__AIRouteLocalHC;
}
...
function isLocalHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("AIRoute_DISABLE_LOCAL_HEALTHCHECK") ||
    isBuildProcess() ||
