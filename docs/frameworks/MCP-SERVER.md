---
title: "AIRoute MCP Server Documentation"
version: 3.8.40
lastUpdated: 2026-06-28
---

# AIRoute MCP Server Documentation
...
AIRoute MCP is built-in. Start it with:
...
AIRoute --dev  # MCP auto-starts on /mcp endpoint
```
...
| :------------------------------ | :-------------------- | :------------------------------------------------------------ |
| `AIRoute_get_health`          | `read:health`         | Uptime, memory, circuit breakers, rate limits, cache stats    |
| `AIRoute_list_combos`         | `read:combos`         | All configured combos with strategies (optional metrics)      |
| `AIRoute_get_combo_metrics`   | `read:combos`         | Performance metrics for a specific combo                      |
| `AIRoute_switch_combo`        | `write:combos`        | Activate or deactivate a combo                                |
| `AIRoute_check_quota`         | `read:quota`          | Quota used/total, percent remaining, reset time, token health |
| `AIRoute_route_request`       | `execute:completions` | Send a chat completion through AIRoute routing              |
| `AIRoute_cost_report`         | `read:usage`          | Cost report by period (session/day/week/month)                |
| `AIRoute_list_models_catalog` | `read:models`         | Full model catalog with capabilities, status, pricing         |

## Phase 1 — Search

| Tool                   | Scopes           | Description                                                                                                                        |
| :--------------------- | :--------------- | :--------------------------------------------------------------------------------------------------------------------------------- |
| `AIRoute_web_search` | `execute:search` | Web search through AIRoute search gateway (Serper/Brave/Perplexity/Exa/Tavily/Google PSE/Linkup/SearchAPI/SearXNG) with failover |

## Advanced Tools (11) — Phase 2

| Tool                               | Scopes                               | Description                                                                               |
| :--------------------------------- | :----------------------------------- | :---------------------------------------------------------------------------------------- |
| `AIRoute_simulate_route`         | `read:health`, `read:combos`         | Dry-run routing simulation with fallback tree                                             |
| `AIRoute_set_budget_guard`       | `write:budget`                       | Session budget with degrade/block/alert action                                            |
| `AIRoute_set_routing_strategy`   | `write:combos`                       | Update combo strategy at runtime (priority/weighted/auto/etc.)                            |
| `AIRoute_set_resilience_profile` | `write:resilience`                   | Apply `aggressive` / `balanced` / `conservative` resilience preset                        |
| `AIRoute_test_combo`             | `execute:completions`, `read:combos` | Live test of every provider in a combo using a real upstream call                         |
| `AIRoute_get_provider_metrics`   | `read:health`                        | Per-provider metrics with p50/p95/p99 latency and circuit breaker state                   |
| `AIRoute_best_combo_for_task`    | `read:combos`, `read:health`         | Recommend combo by task type with budget/latency constraints                              |
| `AIRoute_explain_route`          | `read:health`, `read:usage`          | Explain why a request was routed to a provider (scoring factors + fallbacks)              |
| `AIRoute_get_session_snapshot`   | `read:usage`                         | Full session snapshot: cost, tokens, top models/providers, errors, budget guard           |
| `AIRoute_db_health_check`        | `read:health`, `write:resilience`    | Diagnose (and optionally auto-repair) database drift like broken combo refs / orphan rows |
| `AIRoute_sync_pricing`           | `pricing:write`                      | Sync pricing data from external sources (LiteLLM); supports `dryRun`                      |

## Cache Tools (2)

| Tool                    | Scopes        | Description                                         |
| :---------------------- | :------------ | :-------------------------------------------------- |
| `AIRoute_cache_stats` | `read:cache`  | Semantic cache, prompt-cache, and idempotency stats |
| `AIRoute_cache_flush` | `write:cache` | Flush cache globally or by signature/model          |

## Compression Tools (5)

| Tool                                | Scopes              | Description                                                                                                              |
| :---------------------------------- | :------------------ | :----------------------------------------------------------------------------------------------------------------------- |
| `AIRoute_compression_status`      | `read:compression`  | Compression settings, analytics summary, and cache-aware stats (includes `analytics.mcpDescriptionCompression` metadata) |
| `AIRoute_compression_configure`   | `write:compression` | Configure compression mode, threshold, target ratio, system-prompt preservation, MCP description compression toggle      |
| `AIRoute_set_compression_engine`  | `write:compression` | Pick the active engine (off/caveman/rtk/stacked) and Caveman/RTK intensity                                               |
| `AIRoute_list_compression_combos` | `read:compression`  | List named compression combos and their engine pipelines                                                                 |
| `AIRoute_compression_combo_stats` | `read:compression`  | Analytics grouped by compression combo and engine                                                                        |

`AIRoute_compression_status` reports MCP description compression separately under
...
Separate from the 5 compression tools above, AIRoute includes a post-execution filter that
...
| Tool                        | Scopes         | Description                                                                             |
| :-------------------------- | :------------- | :-------------------------------------------------------------------------------------- |
| `AIRoute_oneproxy_fetch`  | `read:proxies` | Fetch free proxies from the 1proxy marketplace (protocol/country/quality/limit filters) |
| `AIRoute_oneproxy_rotate` | `read:proxies` | Get the next available proxy by strategy (`random` / `quality` / `sequential`)          |
| `AIRoute_oneproxy_stats`  | `read:proxies` | Pool stats, sync status, distribution by protocol and country                           |
...
| Tool                      | Scopes         | Description                                                                         |
| :------------------------ | :------------- | :---------------------------------------------------------------------------------- |
| `AIRoute_memory_search` | `read:memory`  | Search memories by query / type / API key with token-budget enforcement             |
| `AIRoute_memory_add`    | `write:memory` | Add a new memory entry (`factual` / `episodic` / `procedural` / `semantic`)         |
| `AIRoute_memory_clear`  | `write:memory` | Clear memories for an API key, optionally filtered by type or `olderThan` timestamp |
...
| Tool                          | Scopes           | Description                                                                       |
| :---------------------------- | :--------------- | :-------------------------------------------------------------------------------- |
| `AIRoute_skills_list`       | `read:skills`    | List registered skills with optional filtering by API key, name, or enabled state |
| `AIRoute_skills_enable`     | `write:skills`   | Enable or disable a specific skill by ID                                          |
| `AIRoute_skills_execute`    | `execute:skills` | Execute a skill with provided input and return the execution record               |
| `AIRoute_skills_executions` | `read:skills`    | List recent skill execution history                                               |
...
| Tool                              | Scopes         | Description                                                                                                      |
| :-------------------------------- | :------------- | :--------------------------------------------------------------------------------------------------------------- |
| `AIRoute_agent_skills_list`     | `read:catalog` | List all 42 agent skills with optional `category` (api\|cli) and `area` filters; returns metadata + coverage     |
| `AIRoute_agent_skills_get`      | `read:catalog` | Get full metadata + SKILL.md content for a single skill by canonical `id`                                        |
| `AIRoute_agent_skills_coverage` | `read:catalog` | Coverage stats: how many of the 22 API and 20 CLI skills have SKILL.md files on the filesystem vs catalog totals |
...
AIRoute through the same connection model used for LLM providers. They are exposed via
...
| :-------------------------------------- | :--------------------------------- | :----------------------------------------------------------------------------------------------------------------------- |
| `AIRoute_BASE_URL`                    | `http://localhost:20128`           | Base URL the MCP server uses when calling AIRoute internal APIs                                                        |
| `AIRoute_API_KEY`                     | (empty)                            | API key forwarded as `Authorization: Bearer` to internal API calls                                                       |
| `AIRoute_MCP_ENFORCE_SCOPES`          | `false` (only `"true"` enables it) | When enabled, missing scopes deny tool calls and log `scope_denied:<reason>` in audit log                                |
| `AIRoute_MCP_SCOPES`                  | (empty)                            | Comma-separated allowlist of scopes considered "available" by default (used when caller does not provide its own scopes) |
| `AIRoute_MCP_COMPRESS_DESCRIPTIONS`   | (unset = on)                       | When set to `0/false/off/no`, disables MCP description compression at registration time                                  |
| `AIRoute_MCP_DESCRIPTION_COMPRESSION` | (unset = on)                       | Alternate alias for the same toggle as above                                                                             |
...
| `DATA_DIR`                              | `~/.AIRoute`                     | Heartbeat file is written to `${DATA_DIR}/runtime/mcp-heartbeat.json`                                                    |
...
- Toggle process-wide via either `AIRoute_MCP_COMPRESS_DESCRIPTIONS=false` or `AIRoute_MCP_DESCRIPTION_COMPRESSION=false`.
- Realtime stats are surfaced via `AIRoute_compression_status` under `analytics.mcpDescriptionCompression` and tagged `source: "mcp_metadata_estimate"` to disambiguate from real provider usage receipts.
...
MCP_TOOL_DENY="AIRoute_get_health,AIRoute_list_combos" AIRoute --mcp

# Announce only the routing + quota tools (allow-list mode)
MCP_TOOL_ALLOW="AIRoute_route_request,AIRoute_check_quota" AIRoute --mcp
```
