...
description: "Maintainer attestation for the AI-detected potential-malware findings raised against AIRoute and the v3.8.6 mitigations applied at each flagged call site."
---

# Socket.dev / supply-chain finding attestation

This document is the maintainer-authored attestation for the six
`AI-detected potential malware` findings raised against `AIRoute@3.8.5` and
the mitigations applied in `AIRoute@3.8.6`. It exists so:
...
Caddy. The fact that they exist in AIRoute is documented at
...
A `AIRoute_ZED_IMPORT_LEGACY_ONE_STEP=true` env flag preserves the v3.8.5
...
who already use Zed and want to mirror their provider keys into AIRoute
without re-pasting.
...
toolkit." Within AIRoute they are only used by the MITM cert install path
...
(`AIRoute_BUILD_PROFILE=minimal`) replaces
...
## §5 — AIRoute Cloud Sync credential write-back (`api/keys/[id]/route.js`)
...
1. **HMAC verification**: `verifyCloudSignature(rawBody, sigHeader)` checks
   the `X-Cloud-Sig` header (`HMAC-SHA256(AIRoute_CLOUD_SYNC_SECRET,
...
2. **Secret-field opt-in**: `accessToken` / `refreshToken` /
   ...
   `AIRoute_CLOUD_SYNC_SECRETS=true`. The default mode syncs only
...
**Why we keep it**: Cloud Sync is the only way for an AIRoute Cloud tenant
...
```bash
AIRoute_BUILD_PROFILE=minimal npm run build
```
...
The resulting bundle is intended to be published as `AIRoute-secure`. See
