 ...
 * support for the Factory Droid CLI tool. Adapted to AIRoute branding
 * (`custom:AIRoute-<i>`) and extracted as a pure helper so the route-handler
...
/** True when a `customModels` entry was written by AIRoute (any index). */
export function isAIRouteCustomModel(entry: { id?: unknown } | null | undefined): boolean {
  return typeof entry?.id === "string" && entry.id.startsWith("custom:AIRoute");
}
