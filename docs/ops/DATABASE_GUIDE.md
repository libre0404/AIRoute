...
> **TL;DR**: AIRoute uses **SQLite with WAL journaling** as its primary store, with **AES-256-GCM** encryption at rest for sensitive fields. This guide covers the schema, migrations, backup/recovery, and operational runbooks.
...
AIRoute chose SQLite over PostgreSQL/MySQL for several reasons:
...
For **single-user, single-instance** deployments (the primary AIRoute use case), SQLite is simpler and faster.
...
| OS      | Path                                                     |
| ------- | -------------------------------------------------------- |
| Linux   | `~/.AIRoute/storage.sqlite`                            |
| macOS   | `~/.AIRoute/storage.sqlite`                            |
| Windows | `%USERPROFILE%\.AIRoute\storage.sqlite`                |
...
AIRoute's database has **94 domain modules** in `src/lib/db/`. Each module:
...
AIRoute has **94 module files** in `src/lib/db/`. Below is a sampling of core modules; see the directory listing for the complete list:
...
AIRoute uses **versioned, idempotent migrations** in `src/lib/db/migrations/`. Each migration is a single SQL file named `NNN_description.sql`.
...
1. Creates `_AIRoute_migrations` table if not exists
...
> **Backwards-incompatible changes** (e.g., dropping columns) are tricky. AIRoute does NOT support downgrade — once a migration is applied, the schema change is permanent. Plan accordingly.
...
AIRoute uses **`migrateLegacyEncryptedString()`** to handle two encryption schemes transparently:
...
AIRoute backup create --name pre-migration
...
AIRoute backup auto enable --cron "0 2 * * *" --retention 7
```
...
```bash
sqlite3 ~/.AIRoute/storage.sqlite ".backup /backups/AIRoute-hot.db"
```

This uses SQLite's online backup API — safe to run while AIRoute is running.
...
Long-running AIRoute instances benefit from occasional `VACUUM`:

```bash
sqlite3 ~/.AIRoute/storage.sqlite "VACUUM;"
```
...
```bash
sqlite3 ~/.AIRoute/storage.sqlite "PRAGMA integrity_check;"
# Should print: ok
```
...
```bash
sqlite3 ~/.AIRoute/storage.sqlite ".recover" > recovered.sql
sqlite3 recovered.db < recovered.sql
mv recovered.db ~/.AIRoute/storage.sqlite
```
...
```bash
AIRoute sync pull --merge   # or: AIRoute backup restore <backup-id>
```
...
sqlite3 ~/.AIRoute/storage.sqlite "PRAGMA wal_checkpoint(TRUNCATE);"
```
...
```bash
sqlite3 ~/.AIRoute/storage.sqlite "SELECT * FROM api_keys LIMIT 5;"
```

### Count Rows in All Tables

```bash
sqlite3 ~/.AIRoute/storage.sqlite <<EOF
...
1. **Stop AIRoute** (prevent further attempts)
2. **Check the DB state** with `sqlite3`
3. **Manually fix** the partial migration
4. **Re-run** AIRoute (the migration will be retried)
