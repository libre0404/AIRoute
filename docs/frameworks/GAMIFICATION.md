...
AIRoute includes a local-first gamification layer that rewards users for
...
| Community Servers | Federate with external AIRoute instances                      |
...
All tables live in the main AIRoute SQLite database, created by migration
...
Follows the standard AIRoute pattern — imports `getDbInstance()` from
...
The raw token is returned to the user exactly once at creation time. AIRoute
...
All routes follow the standard AIRoute pattern:
