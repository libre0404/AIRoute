...
Traffic Inspector is AIRoute's built-in HTTPS traffic debugger — a Charles Proxy / mitmweb / HTTP Toolkit-like tool that is **LLM-aware** and **agent-aware**. It lives at `/dashboard/tools/traffic-inspector` and receives live traffic from up to 5 simultaneous capture sources.
...
| Feature                                                             | mitmweb | Charles | Fiddler | **AIRoute Traffic Inspector** |
| ------------------------------------------------------------------- | :-----: | :-----: | :-----: | :-----------------------------: |
| Web-based                                                           |    ✓    |    ✗    |    ✗    |                ✓                |
| Open-source                                                         |    ✓    |    ✗    | partial |                ✓                |
...
| **Integrated with AIRoute** routing, fallback, cost               |    ✗    |    ✗    |    ✗    |                ✓                |
...
interface LlmMetadata {
  ...
  mappedTo: string | null; // x-AIRoute-mapped header
  costEstimateUsd: number | null; // estimated cost based on AIRoute pricing
}
```
...
If AIRoute crashes while system-wide proxy mode is active:
...
| POST   | `/requests/{id}/replay`     | Re-execute the same request through AIRoute router                               |
