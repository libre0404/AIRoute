...
} from "@AIRoute/open-sse/services/accountFallback.ts";
import { getModelInfo, getComboForModel } from "../services/model";
import { resolveBareModelToConnectionDefault } from "@AIRoute/open-sse/services/model.ts";
import { errorResponse } from "@AIRoute/open-sse/utils/error.ts";
import { acceptHeaderForcesStream } from "@AIRoute/open-sse/utils/aiSdkCompat.ts";
import { isSelfInflictedUpstreamTimeout } from "@AIRoute/open-sse/handlers/chatCore/cooldownClassification.ts";
import { applyNoThinkingAlias } from "@AIRoute/open-sse/utils/noThinkingAlias.ts";
import { handleComboChat } from "@AIRoute/open-sse/services/combo.ts";
import {
  resolveRequestModePack,
  parseRequestBudgetCap,
} from "@AIRoute/open-sse/services/autoCombo/requestControls.ts";
import { resolveComboConfig } from "@AIRoute/open-sse/services/comboConfig.ts";
import { injectHandoffIntoBody } from "@AIRoute/open-sse/services/contextHandoff.ts";
import {
  HTTP_STATUS,
  ANTIGRAVITY_PRE_RESPONSE_TIMEOUT_CODE,
} from "@AIRoute/open-sse/config/constants.ts";
import { getTargetFormat } from "@AIRoute/open-sse/services/provider.ts";
import {
  getModelTargetFormat,
  PROVIDER_ID_TO_ALIAS,
} from "@AIRoute/open-sse/config/providerModels.ts";
import type { AutoVariant } from "@AIRoute/open-sse/services/autoCombo/autoPrefix.ts";
import {
  AUTO_TEMPLATE_VARIANTS,
  VALID_AUTO_VARIANTS,
} from "@AIRoute/open-sse/services/autoCombo/builtinCatalog.ts";
import {
  parseAutoSuffix,
  type AutoCategory,
  type AutoTier,
} from "@AIRoute/open-sse/services/autoCombo/suffixComposition.ts";
...
} from "@AIRoute/open-sse/services/taskAwareRouter.ts";
import {
  hasNativeWebSearchTool,
  resolveWebSearchRouteOverride,
} from "@AIRoute/open-sse/services/webSearchRouting.ts";
import {
...
} from "@AIRoute/open-sse/services/sessionManager.ts";
import { startQuotaMonitor } from "@AIRoute/open-sse/services/quotaMonitor.ts";
import {
  isFallbackDecision,
  shouldUseFallback,
} from "@AIRoute/open-sse/services/emergencyFallback.ts";
import {
  registerCodexConnection,
  registerCodexQuotaFetcher,
} from "@AIRoute/open-sse/services/codexQuotaFetcher.ts";
import { registerBailianCodingPlanQuotaFetcher } from "@AIRoute/open-sse/services/bailianQuotaFetcher.ts";
import { registerCrofUsageFetcher } from "@AIRoute/open-sse/services/crofUsageFetcher.ts";
import { registerDeepseekQuotaFetcher } from "@AIRoute/open-sse/services/deepseekQuotaFetcher.ts";
import { registerOpencodeQuotaFetcher } from "@AIRoute/open-sse/services/opencodeQuotaFetcher.ts";
import { registerGenericQuotaFetchers } from "@AIRoute/open-sse/services/genericQuotaFetcher.ts";
import {
...
export async function handleChat(
  ...
  // a clear AIRoute-level error before any routing or upstream call (#5110).
  ...
  const requestedConnectionId = request.headers.get("x-AIRoute-connection")?.trim() || null;
  if (sessionId) {
  ...
  if (isAutoRouting) {
    ...
    try {
      const { parseAutoPrefix } =
        await import("@AIRoute/open-sse/services/autoCombo/autoPrefix.ts");
  ...
  if (isAutoRouting && combo === null) {
    ...
    try {
      const { createVirtualAutoCombo } =
        await import("@AIRoute/open-sse/services/autoCombo/virtualFactory.ts");
  ...
  if (combo) {
    ...
    const requestModeHeader = request.headers.get("x-AIRoute-mode")?.trim() || null;
    const requestBudgetHeader = request.headers.get("x-AIRoute-budget")?.trim() || null;
  ...
  requestAttemptLoop: while (true) {
    ...
    while (true) {
      ...
      if (
        ...
        body?._AIRouteSkipContextRelay !== true
      ) {
      ...
      if (runtimeOptions.sessionId && body?._AIRouteInternalRequest !== "context-handoff") {
