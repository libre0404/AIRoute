 ...
 *
 * Covers `createAIRouteFetchInterceptor` (URL-prefix gating, header merge,
...
import { createAIRouteAuthHook, createAIRouteFetchInterceptor } from "../src/index.js";
...
test("createAIRouteFetchInterceptor: targets baseURL → Authorization header injected", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAIRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAIRouteFetchInterceptor: targets baseURL → Authorization OVERRIDES caller-supplied Bearer", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAIRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
    await f(`${BASE}/chat/completions`, {
...
test("createAIRouteFetchInterceptor: targets baseURL + body → Content-Type defaults to application/json", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAIRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
    await f(`${BASE}/chat/completions`, {
...
test("createAIRouteFetchInterceptor: caller-set Content-Type is NOT overwritten", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAIRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAIRouteFetchInterceptor: non-baseURL host → passthrough, no Authorization injected", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAIRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAIRouteFetchInterceptor: refuses suffix-spoof — `${base}-attacker.evil` does NOT match baseURL", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAIRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAIRouteFetchInterceptor: URL object input is handled", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAIRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAIRouteFetchInterceptor: Request input is handled (reads .url)", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAIRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createAIRouteFetchInterceptor: trailing slash in baseURL is normalized", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAIRouteFetchInterceptor({
      apiKey: KEY,
      baseURL: `${BASE}////`,
    });
    await f(`${BASE}/models`, {});
...
test("createAIRouteFetchInterceptor: GET without body does NOT set Content-Type", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createAIRouteFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("loader: returns fetch fn when apiKey + baseURL both present (via opts)", async () => {
  const hook = createAIRouteAuthHook({ baseURL: BASE });
...
test("loader: returns fetch fn when baseURL is stashed on the auth credential", async () => {
  ...
  const hook = createAIRouteAuthHook();
...
test("loader: omits fetch fn when baseURL missing (apiKey-only return)", async () => {
  const hook = createAIRouteAuthHook(); // no baseURL opt
  const result = await hook.loader!(async () => ({ type: "api", key: KEY }) as never, {} as never);
