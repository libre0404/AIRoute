...
import { buildAIRouteSseMetadataComment } from "@/domain/AIRouteResponseMeta";
import {
...
export const PENDING_REQUEST_CLEARED_MARKER = "__AIRoutePendingRequestCleared";
...
export function createSSEStream(options: StreamOptions = {}) {
  ...
  const emitFinalSseMetadata = async (
    ...
    const comment = buildAIRouteSseMetadataComment({
      provider,
      model,
      cacheHit: false,
  ...
  const ensureVisibleResponsesReasoningSummary = (payload: Record<string, unknown>): boolean => {
    ...
    item.summary = [
      {
        type: "summary_text",
        text: "Codex is reasoning, but the upstream Responses API exposed this reasoning block only as encrypted state. AIRoute cannot recover the private reasoning text.",
      },
    ];
    return true;
  };
            ...
            if (trimmed.startsWith("data:") && trimmed.slice(5).trim() !== "[DONE]") {
              try {
                ...
                if (isResponsesSSE) {
                  ...
                  // chunk like "[AIRoute] Upstream returned an empty response. Please
