...
Complete reference for all AIRoute API endpoints.
...
| ------------------------ | --------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `X-AIRoute-No-Cache`   | Request   | Set to `true` to bypass cache                                                                                                |
| `x-AIRoute-no-memory`  | Request   | Set to `true` to skip memory + skills injection for this request (mirrors no-cache; avoids the per-call token/cost overhead) |
| `X-AIRoute-Progress`   | Request   | Set to `true` for progress events                                                                                            |
| `X-Session-Id`           | Request   | Sticky session key for external session affinity                                                                             |
| `x_session_id`           | Request   | Underscore variant also accepted (direct HTTP)                                                                               |
| `Idempotency-Key`        | Request   | Dedup key (5s window)                                                                                                        |
| `X-Request-Id`           | Request   | Alternative dedup key                                                                                                        |
| `X-AIRoute-Cache`      | Response  | `HIT` or `MISS` (non-streaming)                                                                                              |
| `X-AIRoute-Idempotent` | Response  | `true` if deduplicated                                                                                                       |
| `X-AIRoute-Progress`   | Response  | `enabled` if progress tracking on                                                                                            |
| `X-AIRoute-Session-Id` | Response  | Effective session ID used by AIRoute                                                                                       |
| `X-AIRoute-Request-Id` | Response  | Request correlation id (when known)                                                                                          |
| `X-AIRoute-Version`    | Response  | AIRoute build version (always present)                                                                                     |
| `X-AIRoute-Cost-Saved` | Response  | USD the cache avoided on a HIT (cache hits only)                                                                             |

> Nginx note: if you rely on underscore headers (for example `x_session_id`), enable `underscores_in_headers on;`.

> **Cost telemetry headers:** non-streaming success responses also carry the `X-AIRoute-*` cost-telemetry set — `X-AIRoute-Response-Cost` (USD, fixed 10 decimals; `0.0000000000` for free/unpriced), `X-AIRoute-Tokens-In` / `X-AIRoute-Tokens-Out`, `X-AIRoute-Model`, `X-AIRoute-Provider`, `X-AIRoute-Latency-Ms`, `X-AIRoute-Cache-Hit`, and `X-AIRoute-Fallback-Attempts` (only when > 0), plus `X-AIRoute-Request-Id` and `X-AIRoute-Version`. These are emitted by chat completions, `/v1/responses`, `/v1/messages`, **and the media endpoints** — `/v1/embeddings`, `/v1/images/generations`, `/v1/audio/speech`, `/v1/audio/transcriptions`, `/v1/rerank`, `/v1/videos/generations`, `/v1/music/generations`, and `/v1/moderations` (always cost `0`). Media cost is computed per modality (per-image, per-second, per-character, per search-unit) when pricing is available, otherwise `0` (fail-open).

> **Cache-hit cost semantics:** on a semantic-cache HIT (`X-AIRoute-Cache-Hit: true`) no upstream call is made, so `X-AIRoute-Response-Cost` is `0.0000000000` (the **incremental** cost of serving the hit). The original/would-have-been cost is reported separately in `X-AIRoute-Cost-Saved`. Billing consumers should sum `X-AIRoute-Response-Cost` (hits cost nothing); cache analytics can aggregate `X-AIRoute-Cost-Saved`.

### `x-AIRoute-compression`
...
```
X-AIRoute-Compression: <mode>; source=<source>
```
...
For thinking-capable Claude models, `/v1/models` also advertises a **no-thinking** variant whose id is prefixed with `claude-3-AIRoute-no-thinking/`:

```
claude-3-AIRoute-no-thinking/<provider>/<model>
```
...
For clients that cannot attach `Authorization: Bearer ...`, AIRoute also accepts API keys in the URL via either query-string compatibility (`?token=...`, `?apiKey=...`, `?api_key=...`, `?key=...`) or the dedicated `/api/v1/vscode/{token}/...` endpoints documented below.
...
wscat -c "ws://localhost:20128/v1/responses?api_key=<AIRoute_API_KEY>"
# (or: -H "Authorization: Bearer <AIRoute_API_KEY>")
...
a ChatGPT account`). Send the **bare** id (e.g. `gpt-5.5`). AIRoute's bridge is
...
Point the Codex CLI at AIRoute by adding a custom provider with WebSocket
...
env_key = "AIRoute_API_KEY"             # holds the AIRoute API key (Bearer)
```

```bash
export AIRoute_API_KEY=sk-...           # an AIRoute API key (any key if REQUIRE_API_KEY=false)
codex exec "Responda apenas: PONG"
```

The CLI upgrades `base_url + /responses` to a WebSocket and AIRoute tunnels it
...
```json
{
  ...
  "backupPath": "/home/user/.AIRoute/backups/env-repair-2026-04-11.bak"
}
```
...
- URL-based tokens may appear in reverse-proxy logs, browser history, and telemetry outside AIRoute. Treat them as a compatibility option, not the default authentication mode.
...
Outbound webhook subscriptions for AIRoute events (request completion, quota exhaustion, key rotation, etc.).
...
Cloud agent tasks (Claude Code, Codex Cloud, OpenHands, etc.) executed remotely on behalf of AIRoute users.
...
AIRoute exposes three independent temporary-failure mechanisms; the management endpoints below let operators read and override them:
...
Skill framework for extending AIRoute with custom executable handlers, plus marketplace integrations.
...
AIRoute ships an embedded Model Context Protocol server with 3 transports (stdio, SSE, streamable-http) and scoped tools. The dashboard endpoints below read status/audit data and proxy the HTTP transports.
...
AIRoute exposes an A2A (Agent-to-Agent) JSON-RPC 2.0 endpoint plus a REST wrapper for inspection/dashboard use.

### JSON-RPC

```bash
POST /a2a
Authorization: Bearer your-api-key   # optional unless AIRoute_API_KEY is set
Content-Type: application/json
...
**Auth:** the REST helpers run without management auth (dashboard-readable); the JSON-RPC `/a2a` route uses Bearer `AIRoute_API_KEY` if configured.
...
Manage CLI tools that integrate with AIRoute (antigravity, chipotle, commandCode,
...
Manage AIRoute plugins (third-party extensions).
...
Inspect the runtime guardrails (PII detection, prompt injection detection, vision bridging). Guardrails run on every request; per-call opt-out is via the `x-AIRoute-disabled-guardrails` request header — there is no persisted enable/disable surface.
