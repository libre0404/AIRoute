...
> **TL;DR**: AIRoute ranks its **free** providers by model quality using **Arena AI
...
AIRoute aggregates 160+ providers, many of which expose a **free tier** (no-auth,
...
Leaderboard categories map to AIRoute task categories:

| Arena leaderboard | AIRoute task categories                         |
| ----------------- | ------------------------------------------------- |
...
- It is **non-blocking and never fatal** — if the upstream fetch fails, AIRoute keeps
...
stripped and a small alias map expands canonical names into the variants AIRoute uses
