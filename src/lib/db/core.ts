  ...
  try {
    ...
    console.warn(
      `[DB] Cannot create data directory '${DATA_DIR}': ${msg}\n` +
        ...
        `[DB]   DATA_DIR=/path/to/writable/dir AIRoute`
    );
  }
}
...
declare global {
  var __AIRouteDb: SqliteAdapter | undefined;
}

function getDb(): SqliteDatabase | null {
  return globalThis.__AIRouteDb ?? null;
}

function setDb(db: SqliteDatabase | null): void {
  if (db) {
    globalThis.__AIRouteDb = db;
  } else {
    delete globalThis.__AIRouteDb;
  }
}

function checkpointDb(db: SqliteDatabase, mode: CheckpointMode = "TRUNCATE"): boolean {
  // PostgreSQL does not use WAL checkpoint
  if (db.driver === "postgresql") return false;
...
function shouldRunStartupDbHealthCheck(): boolean {
  if (process.env.AIRoute_FORCE_DB_HEALTHCHECK === "1") return true;
...
function createManagedDbBackup(db: SqliteDatabase, reason: string): boolean {
  ...
  // PostgreSQL: use pg_dump via adapter.backup() instead of VACUUM INTO
  if (db.driver === "postgresql") {
    try {
      ...
      const backupPath = path.join(backupDir, `db_${timestamp}_${reason}.sql`);
      db.backup(backupPath).then(
        () => console.log(`[DB] PostgreSQL backup created (${reason}): ${backupPath}`),
        (err: unknown) => {
          const msg = err instanceof Error ? err.message : String(err);
          console.warn(`[DB] PostgreSQL backup failed: ${msg}`);
        }
      );
      return true;
    } catch (error: unknown) {
...
function getDbHealthCheckIntervalMs(): number {
  const rawValue = process.env.AIRoute_DB_HEALTHCHECK_INTERVAL_MS;
...
function startDbHealthCheckScheduler(db: SqliteDatabase) {
  ...
  dbHealthCheckTimer = setInterval(() => {
    try {
      ...
      runDbHealthCheck(db, {
        autoRepair: true,
        skipIntegrityCheck: process.env.AIRoute_SKIP_DB_HEALTHCHECK === "1",
...
export function getDbInstance(): SqliteDatabase {
  ...
  // ── PostgreSQL path ──
  // PostgreSQL bypasses all SQLite-specific logic (file probes, WAL pragmas, VACUUM,
  // file-based backup, db.json migration, sqlite_master inspection, etc.)
  const dbType = resolveDbType();
  if (dbType === "postgresql") {
    // PostgreSQL is async — cannot open synchronously in getDbInstance().
    // This path should only be reached after ensureDbInitialized() has already
    // opened the PG adapter and stored it via setDb().
    throw new Error(
      `[DB] PostgreSQL requires async initialization. Call ensureDbInitialized() before getDbInstance().`
    );
  }
  ...
  db.exec(`
    ...
    INSERT OR IGNORE INTO _AIRoute_migrations (version, name)
  ...
  if (shouldRunStartupDbHealthCheck()) {
    const skipIntegrityCheck = process.env.AIRoute_SKIP_DB_HEALTHCHECK === "1";
    if (skipIntegrityCheck) {
      console.log("[DB] Health check skipped (AIRoute_SKIP_DB_HEALTHCHECK=1)");
    }
    runDbHealthCheck(db, {
      autoRepair: true,
      expectedSchemaVersion: "1",
...
export function closeDbInstance(options?: { checkpointMode?: CheckpointMode | null }): boolean {
  ...
  const isPg = db.driver === "postgresql";
  const checkpointMode = options?.checkpointMode ?? "TRUNCATE";

  try {
    if (checkpointMode && !isPg) {
      try {
    ...
    if (isPg) {
      console.log("[DB] PostgreSQL connection closing (no WAL checkpoint needed).");
    }
  } finally {
    try {
      if (db.open) db.close();
 ...
 * can place it in ~/.AIRoute/runtime/ without blocking a synchronous caller.
 *
...
export async function ensureDbInitialized(): Promise<void> {
  ...
  if (dbType === "postgresql") {
    const { createPgAdapter } = await import("./adapters/pgAdapter");
    const connectionString = process.env.DB_CONNECTION_STRING;
    if (!connectionString) {
      throw new Error(
        `[DB] PostgreSQL selected (DB_TYPE=postgresql) but DB_CONNECTION_STRING is not set. ` +
          `Set it to a PostgreSQL connection string, e.g. postgresql://user:pass@host:5432/airoute`
      );
    }
    console.log("[DB] Initializing PostgreSQL adapter...");
    const pgAdapter = await createPgAdapter(connectionString);

    // Apply schema — uses same SCHEMA_SQL (PG adapter translates SQLite → PG dialect)
    pgAdapter.exec(SCHEMA_SQL);
    ensureProviderConnectionsColumns(pgAdapter);
    ensureUsageHistoryColumns(pgAdapter);
    ensureCallLogsColumns(pgAdapter);

    // Versioned migrations — use PG-specific migration runner
    // (handles SQL transpilation, FTS5→tsvector, PG introspection)
    const { runPgMigrations } = await import("./pgMigrationRunner");
    runPgMigrations(pgAdapter, { isNewDb: true });
    applyStoredDatabaseOptimizationSettings(pgAdapter);

    // Store schema version
    const versionStmt = pgAdapter.prepare(
      "INSERT INTO db_meta (key, value) VALUES ('schema_version', '1') ON CONFLICT (key) DO UPDATE SET value = '1'"
    );
    versionStmt.run();

    setDb(pgAdapter);
    setDriverInfo({ source: "postgresql", kind: "postgresql" });
    startDbHealthCheckScheduler(pgAdapter);
    console.log(`[DB] PostgreSQL database ready: ${pgAdapter.name}`);
    return;
  }
