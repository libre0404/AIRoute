---
title: "Claude Code CLI — Configuration with AIRoute"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Claude Code CLI — Configuration with AIRoute

Point the **Claude Code** CLI (`claude`) at AIRoute — local or a remote VPS —
...
```bash
# Launch Claude Code against a local AIRoute (auto-detects the active context)
AIRoute launch

# Against a remote AIRoute (after `AIRoute connect <host>`, this is automatic)
AIRoute launch --remote http://192.168.0.15:20128 --api-key oma_live_xxx

# Generate per-model profiles, then launch one
AIRoute setup-claude            # writes ~/.claude/profiles/<name>/settings.json
AIRoute launch --profile glm52  # Claude Code using glm/glm-5.2 via AIRoute
```
...
| `ANTHROPIC_AUTH_TOKEN`                       | Sent as `Authorization: Bearer …` — use your AIRoute access token / API key          |
...
`AIRoute launch` sets all of these for you: it resolves the base URL + token
from the active context (so `AIRoute connect <vps>` then `AIRoute launch`
...
`AIRoute setup-claude` fetches the live `/v1/models` catalog and writes one
...
> `AIRoute launch --profile <name>` (it injects `ANTHROPIC_AUTH_TOKEN` from the
> active context), or export `ANTHROPIC_AUTH_TOKEN` yourself and run
> `CLAUDE_CONFIG_DIR=~/.claude/profiles/<name> claude`.

**Auto-sync after model discovery (opt-in).** AIRoute can regenerate these same
...
auto-sync" → Claude Code), or set `AIRoute_AUTO_SYNC_CLAUDE_PROFILES=true` (it also honors
...
AIRoute setup-claude --remote http://192.168.0.15:20128 --api-key oma_live_xxx

# Only some providers
AIRoute setup-claude --only glm,kimi

# Preview without writing
AIRoute setup-claude --dry-run

# Launch a profile
AIRoute launch --profile kimi-k27
```
...
Claude Code routes to capability tiers. Map each to an AIRoute model via env /
...
Once you've run `AIRoute connect <host>` (see
[Remote Mode](./REMOTE-MODE.md)), `AIRoute launch` and `AIRoute setup-claude`
...
`/v1`** and restart `claude` (env is read once at startup). `AIRoute launch`
...
**Auth errors** — the profile holds no token. Use `AIRoute launch --profile`
