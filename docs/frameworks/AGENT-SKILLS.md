---
title: "AIRoute Agent Skills Catalog"
version: 3.8.40
...
Agent Skills are structured SKILL.md files that teach external agents, MCP clients, and A2A orchestrators how to use AIRoute's REST API and CLI. Unlike [Omni Skills](./SKILLS.md) (which are LLM tool definitions executed inside AIRoute), Agent Skills are a _documentation catalog_ — static markdown that can be fed directly into agent context.
...
- A **GitHub raw URL** for live fetch: `https://raw.githubusercontent.com/diegosouzapw/AIRoute/refs/heads/main/skills/{id}/SKILL.md`
...
| Tool                              | Description                                        |
| :-------------------------------- | :------------------------------------------------- |
| `AIRoute_agent_skills_list`     | List skills (optional `category` / `area` filters) |
| `AIRoute_agent_skills_get`      | Get metadata + SKILL.md for one skill by `id`      |
| `AIRoute_agent_skills_coverage` | Coverage stats (API/CLI have/total)                |
...
| :------------------- | :----------------- | :---------------------- |
| `cli-serve`          | cli-serve          | `AIRoute serve`       |
| `cli-health`         | cli-health         | `AIRoute health`      |
| `cli-providers`      | cli-providers      | `AIRoute providers`   |
| `cli-keys`           | cli-keys           | `AIRoute keys`        |
| `cli-models`         | cli-models         | `AIRoute models`      |
| `cli-chat`           | cli-chat           | `AIRoute chat`        |
| `cli-routing`        | cli-routing        | `AIRoute routing`     |
| `cli-resilience`     | cli-resilience     | `AIRoute resilience`  |
| `cli-compression`    | cli-compression    | `AIRoute compression` |
| `cli-contexts`       | cli-contexts       | `AIRoute contexts`    |
| `cli-cost-usage`     | cli-cost-usage     | `AIRoute cost`        |
| `cli-mcp`            | cli-mcp            | `AIRoute mcp`         |
| `cli-a2a`            | cli-a2a            | `AIRoute a2a`         |
| `cli-tunnel`         | cli-tunnel         | `AIRoute tunnel`      |
| `cli-backup-sync`    | cli-backup-sync    | `AIRoute backup`      |
| `cli-policy-audit`   | cli-policy-audit   | `AIRoute policy`      |
| `cli-batches`        | cli-batches        | `AIRoute batch`       |
| `cli-eval`           | cli-eval           | `AIRoute eval`        |
| `cli-plugins-skills` | cli-plugins-skills | `AIRoute plugins`     |
| `cli-setup`          | cli-setup          | `AIRoute setup`       |
...
curl "http://your-AIRoute/api/agent-skills" | jq '.skills[] | {id, name, category}'

# Get SKILL.md for context injection
curl "http://your-AIRoute/api/agent-skills/omni-providers/raw" > omni-providers.md
```
...
const result = await client.callTool("AIRoute_agent_skills_list", { category: "api" });
...
resp = requests.post("http://your-AIRoute/a2a", json={
    "jsonrpc": "2.0", "id": "1",
    "method": "message/send",
...
```bash
BASE="https://raw.githubusercontent.com/diegosouzapw/AIRoute/refs/heads/main/skills"
...
- [MCP-SERVER.md](./MCP-SERVER.md) — MCP tool catalog (`AIRoute_agent_skills_*` tools)
