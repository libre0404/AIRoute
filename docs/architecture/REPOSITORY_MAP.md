...
> Last updated: 2026-06-28 — AIRoute v3.8.40
>
...
├── @AIRoute/           # Publishable companion packages (opencode-plugin, opencode-provider)
├── skills/               # CLI/agent skill packs (cli-* + omni-* + config-codex-cli)
├── examples/             # Sample plugins + AIRoute-cmd-hello starter
...
Separate npm workspace (`@AIRoute/open-sse`). Handles request processing + provider execution.
...
| `preload.js`     | IPC bridge (contextBridge → `window.AIRoute`)                                   |
...
| File                                                                                                        | Purpose                                                                                                                    |
| ----------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `AIRoute.mjs`                                                                                             | Main CLI entry — `AIRoute serve`, `AIRoute setup`, `AIRoute doctor`, `AIRoute providers`, `AIRoute combos`, etc. |
...
| File                         | Purpose                                                                            |
| ---------------------------- | ---------------------------------------------------------------------------------- |
| `skills/AIRoute*/SKILL.md` | 10 skill manifests for external AI agents (Claude Desktop, ChatGPT, Cursor, Cline) |
...
| `CLI-TOOLS.md`              | External CLI integrations + Internal AIRoute CLI                                    |
...
- **`_mono_repo/`** — historic subprojects (AIRouteCloud, AIRouteSite, vscode-extension)
