...
// Known approximate input cost ($/M tokens) from AIRoute's default-pricing constants.
...
//
// AIRoute's sortModelsByCost uses getPricingForModel which merges
...
async function scenarioCostOptimized(healthy) {
  ...
  try {
    ...
    // Both direct match and provider-substring match are accepted since AIRoute
...
//   404, AIRoute calls recordProviderCooldown(providerA, undefined) — a provider-
...
//   1. SSH to read STORAGE_ENCRYPTION_KEY from /root/.AIRoute/.env
//   2. Encrypt a wrong API key using scryptSync(key,"AIRoute-field-encryption-v1",32)+AES-256-GCM
...
// ---------------------------------------------------------------------------
async function main() {
  console.log("=== AIRoute combo-live-vps scenario driver ===");
