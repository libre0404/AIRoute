 ...
 *
 * Validates that two `AIRoutePlugin(input, opts)` invocations with
 ...
 *
 *   "plugin": [
 *     ["@AIRoute/opencode-plugin", {"providerId": "AIRoute-prod",    "baseURL": "https://or.example/v1"}],
 *     ["@AIRoute/opencode-plugin", {"providerId": "AIRoute-preprod", "baseURL": "https://or-preprod.example/v1"}]
 *   ]
 *
 * Assertions:
...
import { AIRoutePlugin } from "../src/index.js";

const fakeInput = {} as Parameters<typeof AIRoutePlugin>[0];

test("multi-instance: two plugin invocations bind to their own providerId", async () => {
  const a = await AIRoutePlugin(fakeInput, {
    providerId: "AIRoute-prod",
    baseURL: "https://a.example/v1",
  });
  const b = await AIRoutePlugin(fakeInput, {
    providerId: "AIRoute-preprod",
    baseURL: "https://b.example/v1",
  });

  assert.equal(a.auth?.provider, "opencode-AIRoute-prod");
  assert.equal(b.auth?.provider, "opencode-AIRoute-preprod");
});
...
test("multi-instance: identical opts twice still yield independent objects", async () => {
  ...
  const first = await AIRoutePlugin(fakeInput, { ...opts });
  const second = await AIRoutePlugin(fakeInput, { ...opts });
...
test("multi-instance: invalid opts on one instance does not poison the other", async () => {
  ...
  await assert.rejects(
    () => AIRoutePlugin(fakeInput, { providerId: "bad id!" } as never),
    /providerId/
  );
  const ok = await AIRoutePlugin(fakeInput, {
