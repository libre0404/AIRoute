...
import { validateMitmScopeForCn, getMergedBypassList, getRegionDnsResolver } from "./cnScopeLimitation.ts";
import { logMitmOperation } from "./mitmAuditLogger.ts";
...
export function interpretMitmStartupError(stderr: string, port: number): string {
  ...
  if (lower.includes("router_api_key")) {
    return "MITM server failed to start: no API key was provided (ROUTER_API_KEY is required). Set a router API key in AIRoute and retry.";
  }
...
export function writeBypassJson(userPatterns?: string[]): void {
  ...
  // Merge CN-specific bypass domains when in CN region
  const mergedPatterns = getMergedBypassList(patterns);
  const payload = {
    version: 1,
    generatedAt: new Date().toISOString(),
...
/**
 * Enumerate every hostname AIRoute may have written to /etc/hosts during
...
/**
 * Start MITM proxy
 * @param {string} apiKey - AIRoute API key
...
async function startMitmInternal(
  ...
  // ── CN region scope validation ──
  // Before any MITM operation, validate that the requested scope is compliant
  // with AIRROUTE_REGION=cn requirements (《网络安全法》《数据安全法》).
  const allTargetHosts = ALL_TARGETS.map((t) => t.hostname);
  const scopeCheck = validateMitmScopeForCn({
    operation: "start",
    targets: allTargetHosts,
    features: ["mitm-proxy"],
  });
  if (!scopeCheck.allowed) {
    logMitmOperation({
      ...
      targets: allTargetHosts,
      status: "blocked",
      reason: scopeCheck.reason || "CN region scope violation",
    });
    throw new Error(
      `[MITM] Blocked in CN region: ${scopeCheck.reason}. ` +
  ...
  try {
    ...
    logMitmOperation({
      operation: "cert-install",
      status: certResult.installed ? "success" : "failure",
      reason: certResult.installed ? undefined : certResult.reason,
    });
  } catch (err) {
  ...
  // is shared with the AIRoute process: getIngestTokenForBootstrap() returns
