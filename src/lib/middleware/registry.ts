...
declare global {
  var __AIRoutePreRequestRegistry:
    | {
        initialized: boolean;
...
function getRegistryState() {
  if (!globalThis.__AIRoutePreRequestRegistry) {
    globalThis.__AIRoutePreRequestRegistry = {
      initialized: false,
      hooks: new Map(),
      middlewares: new Map(),
      logs: [],
  ...
  return globalThis.__AIRoutePreRequestRegistry;
}
...
function compileHookCode(code: string, hookName: string): HookMiddleware {
  ...
  try {
    script = new vm.Script(`(async () => { ${code} })();`, {
      filename: `AIRoute-hook:${hookName}`,
    });
  } catch (err: unknown) {
