...
- [ ] If source English docs changed, run `npm run i18n:run` (requires `AIRoute_TRANSLATION_API_KEY` in `.env`) before tagging
...
- [ ] Test on fresh install: delete `~/.AIRoute/AIRoute.db` and run `npm run dev`
...
> **Operator note:** the remote VPS image directory remains `/usr/lib/node_modules/AIRoute/app/`.
...
- [ ] `AIRoute --tray` boots on macOS (systray2 installed into `~/.AIRoute/runtime/`)
- [ ] `AIRoute --tray` boots on Linux (requires DISPLAY; graceful error if not set)
- [ ] `AIRoute --tray` boots on Windows (PowerShell NotifyIcon, no extra binaries)
- [ ] `AIRoute config tray enable` creates autostart entry; disable removes it
- [ ] `npm install -g AIRoute@<this-version>` runs postinstall without fatal exit
- [ ] Update path keeps optional deps: `AIRoute update --apply` and the auto-updater
...
- [ ] `AIRoute status` works with no `.env` (CLI token path, loopback only)
...
- [ ] All 10 `skills/AIRoute*/SKILL.md` files are publicly fetchable via raw GitHub URL
