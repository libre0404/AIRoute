# AIRoute MCP Server Documentation (Português (Portugal))
...
| Tool                            | Description                              |
| :------------------------------ | :--------------------------------------- |
| `AIRoute_get_health`          | Gateway health, circuit breakers, uptime |
| `AIRoute_list_combos`         | All configured combos with models        |
| `AIRoute_get_combo_metrics`   | Performance metrics for a specific combo |
| `AIRoute_switch_combo`        | Switch active combo by ID/name           |
| `AIRoute_check_quota`         | Quota status per provider or all         |
| `AIRoute_route_request`       | Send a chat completion through AIRoute |
| `AIRoute_cost_report`         | Cost analytics for a time period         |
| `AIRoute_list_models_catalog` | Full model catalog with capabilities     |

## Advanced Tools (8)

| Tool                               | Description                                                 |
| :--------------------------------- | :---------------------------------------------------------- |
| `AIRoute_simulate_route`         | Dry-run routing simulation with fallback tree               |
| `AIRoute_set_budget_guard`       | Session budget with degrade/block/alert actions             |
| `AIRoute_set_resilience_profile` | Apply conservative/balanced/aggressive preset               |
| `AIRoute_test_combo`             | Live-test all models in a combo via a real upstream request |
| `AIRoute_get_provider_metrics`   | Detailed metrics for one provider                           |
| `AIRoute_best_combo_for_task`    | Task-fitness recommendation with alternatives               |
| `AIRoute_explain_route`          | Explain a past routing decision                             |
| `AIRoute_get_session_snapshot`   | Full session state: costs, tokens, errors                   |
