...
AIRoute can expose its local server (`http://localhost:20128`) to the public
...
All three backends are managed in-process — AIRoute starts/stops the underlying
...
- **Auto-install.** On first use, AIRoute downloads the latest `cloudflared`
...
login` + `cloudflared tunnel route dns ...`) are not orchestrated by
  AIRoute. URLs are ephemeral and will change every restart.
...
| Variable                                             | Purpose                                                                               |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `CLOUDFLARED_BIN`                                    | Override the binary path. If set and valid, AIRoute uses it instead of downloading. |
...
must be configured in the ngrok dashboard — AIRoute itself only forwards the
...
1. Install Tailscale (or let AIRoute do it — see `install` endpoint below).
2. Sign in (`tailscale login` or via AIRoute's `login` endpoint).
...
AIRoute's MITM password cache (`getCachedPassword` / `setCachedPassword`) for
...
When you expose AIRoute through a tunnel, the dashboard and OAuth flows must
...
and restart AIRoute before initiating OAuth. For ephemeral Cloudflare Quick
...
than one backend simultaneously is allowed; AIRoute will track each
independently.
...
AIRoute attempts to auto-install on first use. If the install is blocked
