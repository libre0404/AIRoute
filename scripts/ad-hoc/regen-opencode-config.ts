...
const baseURL = process.env.AIRoute_URL ?? "http://localhost:20128";
const apiKey = process.env.AIRoute_KEY ?? process.env.OPENCODE_API_KEY ?? "";

if (!apiKey) {
  console.error(
    "AIRoute_KEY (or OPENCODE_API_KEY) env var is required. " +
      "Find it in AIRoute dashboard → Settings → API Keys."
  );
  process.exit(1);
}
