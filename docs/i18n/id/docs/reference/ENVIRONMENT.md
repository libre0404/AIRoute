...
> Referensi lengkap untuk setiap variabel lingkungan yang dikenali oleh AIRoute.
...
AIRoute menggunakan **SQLite** (melalui `better-sqlite3`) untuk semua persistensi data. Variabel-variabel ini mengontrol lokasi data, enkripsi, dan siklus hidup data.

| Variable                         | Default              | Source File                                     | Deskripsi                                                                                                                          |
| -------------------------------- | -------------------- | ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `DATA_DIR`                       | `~/.AIRoute/`      | `src/lib/db/core.ts`                            | Direktori utama untuk DB SQLite, cadangan, dan file data. Override untuk volume Docker atau path khusus.                           |
...
| `AIRoute_CRYPT_KEY`            | _(unset)_            | `src/lib/db/encryption.ts`                      | **Alias legacy** untuk `STORAGE_ENCRYPTION_KEY`. Diterima sebagai fallback ketika variabel utama tidak ada.                        |
| `AIRoute_API_KEY_BASE64`       | _(unset)_            | `src/lib/db/encryption.ts`                      | **Alias legacy** (bentuk yang dikodekan Base64) diterima sebagai fallback. Didekode secara otomatis sebelum digunakan.             |

### Skenario

| Skenario                   | Konfigurasi                                                                                      |
| -------------------------- | ------------------------------------------------------------------------------------------------ |
| **Pengembangan lokal**     | Biarkan semua nilai default. DB berada di `~/.AIRoute/AIRoute.db`.                           |
...
| **CI/Testing**             | `DATA_DIR=/tmp/AIRoute-test` — bersifat sementara, tidak perlu enkripsi.                       |
...
| `AIRoute_PORT`      | _(unset)_    | `src/lib/runtime/ports.ts` | Mengambil prioritas di atas `PORT` saat berjalan di dalam Electron atau wrapper lainnya.              |
...
AIRoute menyediakan pertahanan dua lapis: pemindaian injeksi di sisi permintaan dan penghapusan PII di sisi respons.
...
> [!IMPORTANT]
> Saat melakukan deployment di balik reverse proxy (nginx, Caddy), `NEXT_PUBLIC_BASE_URL` **harus** diatur ke URL publik Anda (misalnya, `https://AIRoute.example.com`). Tanpa ini, callback OAuth akan gagal karena redirect_uri tidak akan cocok.
...
Mengontrol bagaimana AIRoute menemukan dan menjalankan sidecar CLI (Claude Code, Codex, dll.).
...
| `CLI_ALLOW_CONFIG_WRITES` | `false`    | `src/shared/services/cliRuntime.ts` | Izinkan AIRoute menulis file konfigurasi CLI (penyegaran token, data sesi).                    |
...
```bash
# Mount biner host ke kontainer dan beri tahu AIRoute lokasinya:
...
| Variable                                | Default          | Source File                                 | Deskripsi                                                                                                                              |
| --------------------------------------- | ---------------- | ------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `AIRoute_BASE_URL`                    | deteksi otomatis | `open-sse/mcp-server/server.ts`             | URL eksplisit agar alat MCP/A2A dapat menjangkau AIRoute. Menimpa deteksi otomatis localhost.                                        |
| `AIRoute_API_KEY`                     | _(unset)_        | MCP/A2A modules                             | Kunci API untuk panggilan alat MCP internal dan skill A2A.                                                                             |
| `AIRoute_API_KEY_ID`                  | _(unset)_        | `open-sse/mcp-server/audit.ts`              | ID kunci untuk atribusi log audit MCP.                                                                                                 |
| `ROUTER_API_KEY`                        | _(unset)_        | Legacy                                      | Alias legacy untuk `AIRoute_API_KEY`.                                                                                                |
| `AIRoute_MCP_ENFORCE_SCOPES`          | `false`          | `open-sse/mcp-server/server.ts`             | Terapkan kontrol akses berbasis scope pada panggilan alat MCP.                                                                         |
| `AIRoute_MCP_SCOPES`                  | _(all)_          | `open-sse/mcp-server/server.ts`             | Scope yang dipisahkan koma: `admin`, `combos`, `health`, `models`, `routing`, `budget`, `metrics`, `pricing`, `memory`, `skills`.      |
...
| `AIRoute_DISABLE_BACKGROUND_SERVICES` | `false`          | `src/instrumentation-node.ts`               | Nonaktifkan semua layanan latar belakang (sinkronisasi, harga, pembaruan model). Berguna untuk CI/pengujian.                           |
| `AIRoute_BOOTSTRAPPED`                | `false`          | `src/app/(dashboard)/dashboard/page.tsx`    | Diatur ke `true` oleh skrip bootstrap setelah pengaturan awal. Mengontrol visibilitas wizard pengaturan.                               |
| `AIRoute_ALLOW_BODY_PROJECT_OVERRIDE` | `0`              | `open-sse/executors/antigravity.ts`         | Escape hatch: izinkan body permintaan untuk menimpa field proyek Antigravity.                                                          |

### Jembatan CLI OAuth (Internal)

| Variable            | Default          | Source File                     | Deskripsi                                        |
| ------------------- | ---------------- | ------------------------------- | ------------------------------------------------ |
| `AIRoute_SERVER`  | deteksi otomatis | `src/lib/oauth/config/index.ts` | URL server untuk jembatan autentikasi CLI↔AIRoute. |
| `AIRoute_TOKEN`   | _(unset)_        | `src/lib/oauth/config/index.ts` | Token autentikasi untuk jembatan CLI.             |
| `AIRoute_USER_ID` | `cli`            | `src/lib/oauth/config/index.ts` | ID pengguna untuk sesi jembatan CLI.              |
| `SERVER_URL`        | _(unset)_        | `src/lib/oauth/config/index.ts` | Alias legacy untuk `AIRoute_SERVER`.            |
| `CLI_TOKEN`         | _(unset)_        | `src/lib/oauth/config/index.ts` | Alias legacy untuk `AIRoute_TOKEN`.             |
| `CLI_USER_ID`       | _(unset)_        | `src/lib/oauth/config/index.ts` | Alias legacy untuk `AIRoute_USER_ID`.           |
...
| `AIRoute_QODER_WORKSPACE`       | Qoder                   | Alias untuk `QODER_CLI_WORKSPACE`.                                                        |
...
Saat diaktifkan, AIRoute mengatur ulang urutan header HTTP dan field body JSON agar cocok dengan tanda tangan persis dari alat CLI resmi. Hal ini mengurangi risiko pemblokiran akun sambil mempertahankan IP proxy Anda.
...
| Variable                   | Default                         | Deskripsi                                                                                              |
| -------------------------- | ------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `AIRoute_MEMORY_MB`      | `512`                           | Batas heap V8 saat runtime. Docker standalone dan `AIRoute serve` menggunakannya untuk mengatur `--max-old-space-size`. |
...
| `NEXT_PUBLIC_AIRoute_E2E_MODE` | _(unset)_ | E2E test harness                          | Atur ke `true` untuk mengaktifkan mode pengujian E2E (autentikasi santai, test hook). |
...
| `INSTANCE_NAME=AIRoute`                             | Ada di template docs/env lama tetapi tidak digunakan saat runtime. Mungkin kembali dalam fitur multi-instansi di masa depan.      |
