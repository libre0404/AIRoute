...
/**
 * Endpoint suportado pelo Playground Studio. Reflete os 13 endpoints da API AIRoute
...
export const API_KEY_PLACEHOLDER = "$AIRoute_API_KEY";
...
function buildCurlSnippet(state: PlaygroundState): string {
  ...
  const lines: string[] = [
    `# Set your API key: export AIRoute_API_KEY="your-key-here"`,
    `curl -s -X POST \\`,
    `  "${url}" \\`,
