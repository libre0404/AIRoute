...
import { resolveModelAlias } from "@AIRoute/open-sse/services/modelDeprecation.ts";
...
function isRedisAuthCacheEnabled(): boolean {
  return (
    process.env.AIRoute_DISABLE_REDIS_AUTH_CACHE !== "1" &&
...
/**
 * Select an API key for internal AIRoute operations (combo health checks,
...
export async function getApiKeyMetadata(
  ...
  if (isConfiguredEnvApiKey(key)) {
    ...
    // The deployment-time env key (`AIRoute_API_KEY` / `ROUTER_API_KEY`)
