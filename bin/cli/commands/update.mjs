...
export async function getLatestVersion(execFn = execFileAsync) {
  try {
    const { stdout } = await execFn("npm", ["view", "AIRoute", "version", "--prefer-online"], {
      timeout: 15000,
    });
    return stdout.trim();
  } catch {
...
export async function createBackup() {
  ...
  const backupDir = path.join(homedir(), ".AIRoute", "backups", `AIRoute-${Date.now()}`);
  ...
  try {
    ...
    const files = ["AIRoute.mjs", "cli", "nodeRuntimeSupport.mjs", "mcp-server.mjs"];
...
export async function runUpdateCommand(opts = {}) {
  ...
  if (showChangelog) {
    try {
      const { stdout } = await execFileAsync("npm", ["view", "AIRoute", "changelog"], {
        timeout: 10000,
      });
      if (stdout.trim()) {
        console.log(stdout.trim());
      } else {
        console.log(`Changelog: https://github.com/your-org/AIRoute/releases/tag/v${latest}`);
      }
    } catch {
      console.log(`Changelog: https://github.com/your-org/AIRoute/releases/tag/v${latest}`);
    }
  ...
  if (checkOnly) {
    console.log("\n  Run `AIRoute update --apply` to install automatically.");
    return 1; // exit 1 = outdated (useful for scripts)
  }

  if (dryRun) {
    console.log("\n  [DRY RUN] Would run: npm install -g AIRoute@latest --include=optional");
    if (!skipBackup) console.log("  [DRY RUN] Would create backup in ~/.AIRoute/backups/");
    return 0;
  }
  ...
  try {
    ...
    execSync("npm install -g AIRoute@latest --include=optional", { stdio: "inherit" });
    printSuccess(`Updated to version ${latest}`);
    printInfo("Run `AIRoute --version` to verify.");
    return 0;
  } catch (err) {
    printError(`Update failed: ${err.message}`);
    printInfo("Restore from backup:");
    const backupDir = path.join(homedir(), ".AIRoute", "backups");
