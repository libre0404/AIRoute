...
// Opt-in native HTTPS (#5242). Resolved once at boot: when both AIRoute_TLS_CERT
// and AIRoute_TLS_KEY point at readable files we terminate TLS on the same
...
if (tlsOptions) {
  console.log(
    `[AIRoute][tls] HTTPS enabled — terminating TLS with cert=${tlsOptions.certPath}`
  );
}
