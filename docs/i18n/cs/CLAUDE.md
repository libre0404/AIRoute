...
**AIRoute** — jednotný AI proxy/router. Jeden koncový bod, 160+ poskytovatelů LLM, automatické zálohování.
...
AIRoute má tři související, ale odlišné mechanismy dočasného selhání. Udržujte jejich rozsah oddělený při ladění chování routování. Viz
...
- **Importy**: externí → interní (`@/`, `@AIRoute/open-sse`) → relativní
...
- **Cestovní aliasy**: `@/*` → `src/`, `@AIRoute/open-sse` → `open-sse/`, `@AIRoute/open-sse/*` → `open-sse/*`
- **Výchozí port**: 20128 (API + dashboard na stejném portu)
- **Adresář dat**: `DATA_DIR` env var, výchozí hodnota `~/.AIRoute/`
