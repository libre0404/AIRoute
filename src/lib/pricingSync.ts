...
// ─── Provider mapping: LiteLLM provider → AIRoute aliases ─────
...
/**
 * Transform LiteLLM raw data → AIRoute PricingByProvider format.
 *
 * Conversion: cost_per_token × 1_000_000 → $/1M tokens (AIRoute format).
 ...
 */
export function transformToAIRoute(raw: Record<string, LiteLLMModelInfo>): PricingByProvider {
  ...
  for (const [modelKey, info] of Object.entries(raw)) {
    ...
    // Map to AIRoute providers
    const litellmProvider = info.litellm_provider || "";
    const AIRouteProviders = LITELLM_PROVIDER_MAP[litellmProvider];

    if (AIRouteProviders) {
      for (const provider of AIRouteProviders) {
  ...
  try {
    ...
    for (const source of validSources) {
      if (source === "litellm") {
        ...
        const transformed = transformToAIRoute(raw);
