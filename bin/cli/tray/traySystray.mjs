...
// ~/.AIRoute/runtime by trayRuntime.ts (loadSystray). The previous inline
...
export async function initSystrayUnix(
  ...
  const items = [
    { title: `AIRoute  •  port ${port}`, tooltip: "Server running", enabled: false },
    ...
    { title: "Quit AIRoute", enabled: true },
  ];
  ...
  tray.ready().catch((err) => {
    process.stderr.write(`[AIRoute][tray] systray2 failed: ${err?.message ?? String(err)}\n`);
  });
