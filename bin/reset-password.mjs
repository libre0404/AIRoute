 ...
 *   printf 'NewPass123\nNewPass123\n' | AIRoute reset-password
 *   printf 'NewPass123' | AIRoute reset-password --password-stdin
 *
 * Resets the admin password for AIRoute.
...
async function main() {
  ...
  if (!passwordState.exists) {
    ...
    console.error(`   Make sure AIRoute has been started at least once.`);
  ...
  console.log("   Restart AIRoute for changes to take effect.\n");
}

main()
  .then(() => {
    // Explicit exit(0) so a caller that imports this module (bin/AIRoute.mjs
    // routes `AIRoute reset-password` here) terminates cleanly instead of
