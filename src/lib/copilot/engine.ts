...
Use \`createCombo\` tool or \`runAIRouteCli\` to create them.`;
  }
...
AIRoute supports 212+ providers across categories:
...
I can use CodeGraph to explore the AIRoute codebase. Just ask me:
...
function getHelpResponse(): string {
  return `## AIRoute Copilot — Comandos disponibles
...
### CLI
- "CLI health" → ejecuta \`AIRoute health\`
- "CLI list-combos" → ejecuta \`AIRoute list-combos\`
- "CLI set-budget 10" → ejecuta \`AIRoute set-budget 10\`

### Conocimiento
- "Cómo funciona AIRoute?" → explica la arquitectura
...
export async function processCopilotChat(request: CopilotRequest): Promise<CopilotResponse> {
  ...
  if (!intent) {
    ...
    return {
      message: `I understand you want help with AIRoute.\n\n${getHelpResponse()}`,
    };
  }
...
> Crea un combo llamado "fallback-claude" con estrategia priority y targets: [{"provider":"claude-code","model":"claude-sonnet-4"},{"provider":"openai","model":"gpt-4o"}]`,
  ...
  if (intent.tool === "runAIRouteCli") {
    const tool = getCopilotTool("runAIRouteCli");
    if (!tool) return { message: "Error: CLI executor not found." };

    const result = await tool.handler(intent.args);
    return {
      message: result,
      toolCalls: [{ name: "runAIRouteCli", args: intent.args, result }],
    };
  }
