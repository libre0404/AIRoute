 ...
 * source is the user's OWN AIRoute instance, so loopback/private hosts are the
...
/**
 * Fetch the live `/v1/models` catalog from AIRoute. The catalog is the
 ...
 */
export async function fetchAIRouteCatalog(
  baseUrl: string,
  apiKey: string,
  timeoutMs = 5_000
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `AIRoute /v1/models returned ${response.status} ${response.statusText}`
      );
    }
...
/**
 * Generate a full `opencode.json` document for AIRoute. The catalog is the
...
export async function generateOpencodeConfig(
  ...
  const providerId = options.providerId?.trim() || "AIRoute";
  ...
  if (fetchCatalog) {
    const result = await fetchAIRouteCatalog(baseURL, options.apiKey, timeoutMs);
  ...
  const provider: Record<string, unknown> = {
    name: existingProvider?.name ?? "AIRoute",
