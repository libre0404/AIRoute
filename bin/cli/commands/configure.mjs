...
/**
 * `AIRoute configure <cli>` — interactive provider+model picker that writes a
 * local CLI config pointed at the ACTIVE AIRoute context (local or remote).
 *
 * The model catalog comes from the active context's GET /v1/models, so when you
 * are in remote mode (`AIRoute connect ...`) you pick from the remote server's
 ...
 * is referenced by env var (AIRoute_API_KEY) — never written to disk.
 */
...
function buildCodexProfile(modelId, ctx) {
  const lines = [
    ...
    `# ${modelId} — generated by 'AIRoute configure codex'`,
    `model                          = "${modelId}"`,
...
async function configureCodex(modelId, ctxWindow, opts) {
  ...
  printInfo("Prereq: ~/.codex/config.toml must define the [model_providers.AIRoute] block");
