...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const tempDir = path.join(os.tmpdir(), `AIRoute-export-${timestamp}`);
    const zipPath = path.join(os.tmpdir(), `AIRoute-full-backup-${timestamp}.zip`);
    ...
    try {
      ...
      return new NextResponse(archiveBuffer, {
        ...
        headers: {
          ...
          "Content-Disposition": `attachment; filename="AIRoute-full-backup-${timestamp}.tar.gz"`,
