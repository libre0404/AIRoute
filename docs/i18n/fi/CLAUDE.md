...
**AIRoute** — yhtenäinen AI-proxy/reititin. Yksi päätepiste, yli 160 LLM-toimittajaa, automaattinen varajärjestelmä.
...
AIRoute:lla on kolme liittyvää mutta erilaista tilapäisen epäonnistumisen mekanismia. Pidä niiden
...
- **Tuonnit**: ulkoiset → sisäiset (`@/`, `@AIRoute/open-sse`) → suhteelliset
...
- **Polkualias**: `@/*` → `src/`, `@AIRoute/open-sse` → `open-sse/`, `@AIRoute/open-sse/*` → `open-sse/*`
- **Oletusportti**: 20128 (API + dashboard samalla portilla)
- **Tietohakemisto**: `DATA_DIR` ympäristömuuttuja, oletuksena `~/.AIRoute/`
