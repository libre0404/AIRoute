 ...
 *     is set explicitly. The AIRoute LOCAL_ONLY rule is enforced at the
...
declare global {
  var __AIRouteEmbedWsStarted: boolean | undefined;
}
...
export function initEmbedWsProxy(): void {
  if (globalThis.__AIRouteEmbedWsStarted) return;
  ...
  server.listen(port, host, () => {
    globalThis.__AIRouteEmbedWsStarted = true;
