 ...
 * Priority: system daemon socket → AIRoute custom socket.
...
async function getActiveSocketPath(): Promise<string> {
  ...
  // Fallback to AIRoute custom socket
...
async function getDefaultHostname() {
  try {
    ...
    const normalized = `AIRoute-${machineId.slice(0, 8)}`.replace(/[^a-zA-Z0-9-]/g, "-");
