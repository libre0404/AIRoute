 ...
 *
 * Closes the gap where `npm install -g AIRoute` ships the plugin
 * inside the AIRoute package (`@AIRoute/opencode-plugin/dist/`) but
...
// We walk up from this file to find the AIRoute package root. The script
// lives at `<AIRoute>/bin/cli/commands/setup-open-code.mjs`, so the
...
// The bundled plugin ships at PACKAGE_ROOT/@AIRoute/opencode-plugin/
// (see root package.json `files`: ["@AIRoute/", ...]). The env override
// exists so tests can point at a fixture without building the real plugin.
const BUNDLED_PLUGIN_DIR =
  process.env.AIRoute_OPENCODE_PLUGIN_DIR || join(PACKAGE_ROOT, "@AIRoute", "opencode-plugin");
...
/**
 * Locate the bundled @AIRoute/opencode-plugin dist. The plugin may be
 * present in two states:
 *
 *   - Built (`dist/index.cjs` + `dist/index.js` exist) — preferred,
 *     ships from a published AIRoute tarball after Step 8.8 of
...
function resolveBundledPlugin() {
  if (!existsSync(BUNDLED_PLUGIN_DIR)) {
    throw new Error(
      `Bundled @AIRoute/opencode-plugin not found at ${BUNDLED_PLUGIN_DIR}.\n` +
        `This usually means AIRoute was installed from a source tree that does not ` +
        `include the workspace package. Try reinstalling AIRoute (npm install -g AIRoute) ` +
        `or run \`cd @AIRoute/opencode-plugin && npm install && npm run build\` from the source repo.`
    );
  }
  ...
  if (!existsSync(esmEntry)) {
    throw new Error(
      `@AIRoute/opencode-plugin dist/ not built (looked for ${esmEntry}).\n` +
...
/**
 * Copy the plugin package into `<opencodeConfig>/plugins/AIRoute/`. We
 ...
 * resolve the peer deps from the AIRoute package's tree, which is
 * unreliable.
 */
function installPluginToOpenCode(pluginInfo, opencodeConfigDir) {
  const targetDir = join(opencodeConfigDir, "plugins", "AIRoute");
...
function registerPluginInOpenCodeConfig({
    ...
    try {
      ...
      throw new Error(
        `Failed to parse existing ${configPath}: ${err.message}\n` +
          `Fix or remove the file manually, then re-run \`AIRoute setup opencode\`.`
      );
    }
  }
  ...
  // drop a legacy `opencode-AIRoute-auth` entry if present — that
  // package is the obsolete predecessor of @AIRoute/opencode-plugin
  // and was the root cause of issue #3711.
  const filtered = plugins.filter((p) => {
    if (typeof p === "string") {
      return !p.includes("opencode-AIRoute-auth");
    }
    if (Array.isArray(p) && p[1] && typeof p[1] === "object") {
      ...
      if (typeof p[0] === "string" && p[0].includes("opencode-AIRoute-auth")) {
        return false;
      }
    }
    return true;
 ...
 *
 * @param {object} opts
 * @param {string} [opts.providerId="AIRoute"]
...
export async function runSetupOpenCodeCommand(opts = {}) {
  const providerId = opts.providerId || "AIRoute";
  // Remote-aware: explicit --remote/--base-url → active context → localhost.
  let baseURL = opts.remote || opts.baseURL || opts.baseUrl;
  if (!baseURL) {
    try {
      const ctx = resolveActiveContext(opts.context ?? process.env.AIRoute_CONTEXT);
      baseURL = ctx?.baseUrl;
    } catch {
      /* no context */
    }
  ...
  printHeading("AIRoute → OpenCode Plugin Setup");
...
/**
 * Register the `AIRoute setup opencode` subcommand on the parent
 * `setup` command. Commander builds the doc/help from the chain, so
 * `AIRoute setup --help` automatically shows the new subcommand.
 *
...
export function registerSetupOpenCode(setupCommand) {
  setupCommand
    ...
    .description(
      t("setup.opencode") ||
        "Install and register the bundled @AIRoute/opencode-plugin with a local OpenCode install"
    )
    .option(
      "--provider-id <id>",
      "OpenCode provider id to register (default: AIRoute)",
      "AIRoute"
    )
    .option(
      "--base-url <url>",
      "AIRoute base URL the plugin should talk to (default: active context or http://localhost:20128)"
    )
    .option(
      "--remote <url>",
      "Remote AIRoute URL, e.g. http://192.168.0.15:20128 (overrides --base-url and the context)"
    )
