...
> **TL;DR**: AIRoute tracks every request's token usage, computes cost, enforces per-API-key quota, and surfaces analytics in the dashboard. This guide explains how it all works.
...
Every request that flows through AIRoute generates a **usage record** that captures:
...
For providers that don't return usage (some web-cookie providers), AIRoute **estimates** tokens using a `~4 chars per token` heuristic (see `open-sse/services/autoCombo/pipelineRouter.ts`).

### Cached Tokens

AIRoute tracks `cached_tokens` separately from `prompt_tokens` because:
...
For models with no pricing data, AIRoute falls back to **estimating cost** using internal average rates (sourced from LiteLLM's pricing data).
...
| Tool                    | Description                                        |
| ----------------------- | -------------------------------------------------- |
| `AIRoute_cost_report` | Generates a per-key cost report for a given period |
| `AIRoute_check_quota` | Returns current quota status for an API key        |

Example agent invocation:

```json
{
  "tool": "AIRoute_cost_report",
  "args": { "period": "week" }
}
```
