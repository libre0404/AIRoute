...
> **Status:** Accepted · **Context:** [#5670](https://github.com/diegosouzapw/AIRoute/issues/5670),
> [#5603](https://github.com/diegosouzapw/AIRoute/issues/5603) · **Contract:** `domain/routing/routerBackends.ts`
> (typed registry — code lands with [#5868](https://github.com/diegosouzapw/AIRoute/pull/5868))
...
1. **Lifecycle** (`RouterBackendLifecycle`) — _how the engine runs_:
   - `in-process` — runs inside the AIRoute Node process (the native TS pipeline).
   - `supervised` — a local child process AIRoute installs/starts/stops/health-checks
     via `ServiceSupervisor`, then consumes as a provider connection.
   - `external` — an HTTP endpoint AIRoute dispatches to but does **not** manage
...
[#5868](https://github.com/diegosouzapw/AIRoute/pull/5868)) declares every engine once, with its
...
[#5817](https://github.com/diegosouzapw/AIRoute/pull/5817); until it merges,
...
- **Selection** (`resolveRelayRoutingBackend`): a single global env toggle —
  `AIRoute_RELAY_BACKEND` / `RELAY_ROUTING_BACKEND` ∈ {`ts`, `bifrost`, `auto`}.
...
([#5869](https://github.com/diegosouzapw/AIRoute/pull/5869) manifest +
[#5870](https://github.com/diegosouzapw/AIRoute/pull/5870) `shouldTryBifrostForRequest`),
