...
**Problem it solves:** AIRoute proxies many API keys against the same upstream
...
- All persistence is in the existing AIRoute SQLite DB (`DATA_DIR/storage.sqlite`).
...
1. `localStorage.getItem("AIRoute:quota-share:pools")` is non-empty.
