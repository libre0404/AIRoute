...
const HEADER_NAME = "x-AIRoute-cli-token";
...
/**
 * Validates the CLI machine-id token sent by the local AIRoute CLI.
 * Only accepted from loopback IPs. Disabled via AIRoute_DISABLE_CLI_TOKEN=true.
 */
export async function isCliTokenAuthValid(request: Request): Promise<boolean> {
  if (process.env.AIRoute_DISABLE_CLI_TOKEN === "true") return false;
