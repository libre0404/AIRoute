...
import { appendNoThinkingVariants } from "@AIRoute/open-sse/utils/noThinkingAlias";
import { getAllEmbeddingModels } from "@AIRoute/open-sse/config/embeddingRegistry";
import { getAllImageModels } from "@AIRoute/open-sse/config/imageRegistry";
import { getAllRerankModels } from "@AIRoute/open-sse/config/rerankRegistry";
import { getAllAudioModels } from "@AIRoute/open-sse/config/audioRegistry";
import { getAllModerationModels } from "@AIRoute/open-sse/config/moderationRegistry";
import { getAllVideoModels } from "@AIRoute/open-sse/config/videoRegistry";
import { getAllMusicModels } from "@AIRoute/open-sse/config/musicRegistry";
import { REGISTRY } from "@AIRoute/open-sse/config/providerRegistry";
import { CODEX_NATIVE_UNPREFIXED_MODELS } from "@AIRoute/open-sse/services/model";
import { resolveNestedComboTargets } from "@AIRoute/open-sse/services/combo";
import {
  AUTO_TEMPLATE_VARIANTS,
  AUTO_SUFFIX_VARIANTS,
  AUTO_FAMILY_IDS,
  createBuiltinAutoCombo,
} from "@AIRoute/open-sse/services/autoCombo/builtinCatalog";
...
import { getTokenLimit } from "@AIRoute/open-sse/services/contextManager";
...
async function buildUnifiedModelsResponseCore(
  ...
  try {
    ...
    const prefixRoutesToProvider = (prefix: string, providerId: string) => {
      const parsed = parseModel(`${prefix}/__AIRoute_probe__`);
    ...
    if (apiKey) {
      ...
      if (keyMeta && keyMeta.allowedQuotas && keyMeta.allowedQuotas.length > 0) {
        ...
        // (AIRoute_API_KEY / ROUTER_API_KEY per isValidApiKey). Those keys have no
