---
title: "AIRoute Fly.io Deployment Guide"
version: 3.8.40
lastUpdated: 2026-06-28
---

# AIRoute Fly.io Deployment Guide

This document describes the actual deployment process for AIRoute on Fly.io, covering two scenarios:
...
This guide is based on a verified working configuration for the current project. The application name is `AIRoute`.
...
- Access URL: `https://AIRoute.fly.dev/`
...
- `app = 'AIRoute'` determines which Fly application the deployment targets
...
```toml
app = 'AIRoute-yourname'
```

Note:

- Make sure the application you see in the console matches the `app` value in `fly.toml`
- If you previously used a different name, such as `oroute`, do not confuse it with `AIRoute`
...
```powershell
flyctl apps create AIRoute
```

If you changed the application name, replace `AIRoute` with your chosen name.
...
These parameters have been used in actual deployments on the current `AIRoute` application:
...
- `AIRoute_WS_BRIDGE_SECRET` (required in production — used for WebSocket bridge authentication)
...
| `AIRoute_WS_BRIDGE_SECRET`  | Required in production | WebSocket bridge authentication secret                |
...
| ---------------------- | --------------------------- |
| `DATA_DIR`             | `/data`                     |
| `NEXT_PUBLIC_BASE_URL` | `https://AIRoute.fly.dev` |
...
1. **Set `NEXT_PUBLIC_BASE_URL` to your public HTTPS domain**
   ...
   ```powershell
   flyctl secrets set NEXT_PUBLIC_BASE_URL=https://AIRoute.fly.dev -a AIRoute
   ```

   If you are using a custom domain, replace it with the corresponding domain (e.g. `https://AIRoute.yourdomain.com`).
...
2. **Configure the callback URL on the provider console**
   ...
   - `https://AIRoute.fly.dev/callback`
...
- Intended for the current project `AIRoute`
...
flyctl secrets set `
  ...
  AIRoute_WS_BRIDGE_SECRET=$wsBridgeSecret `
  DATA_DIR=/data `
  NEXT_PUBLIC_BASE_URL=https://AIRoute.fly.dev `
  -a AIRoute
```
...
```bash
flyctl secrets set AIRoute_WS_BRIDGE_SECRET=$(openssl rand -hex 32) -a AIRoute
```

Notes:

- `AIRoute_WS_BRIDGE_SECRET` is required in production; missing it will break the WebSocket bridge handshake

If you also want to set an initial password:

```powershell
flyctl secrets set INITIAL_PASSWORD=your-strong-password -a AIRoute
```
...
```powershell
flyctl secrets set KEY=value -a AIRoute
```
...
If the current repository is a fork and you want to sync updates from the upstream `https://github.com/diegosouzapw/AIRoute`, follow the workflow below.
...
```powershell
git remote add upstream https://github.com/diegosouzapw/AIRoute.git
```
...
5. `flyctl deploy`
6. `flyctl status -a AIRoute`
7. `flyctl logs --no-tail -a AIRoute`
...
```powershell
flyctl status -a AIRoute
```

### 10.2 View Startup Logs

```powershell
flyctl logs --no-tail -a AIRoute
```
...
```powershell
try {
  (Invoke-WebRequest -Uri "https://AIRoute.fly.dev" -MaximumRedirection 5 -UseBasicParsing).StatusCode
} catch {
...
- You are viewing a different application (e.g. `oroute` instead of `AIRoute`)
...
1. `flyctl auth login`
2. `flyctl apps create AIRoute`
3. `flyctl secrets set ... -a AIRoute`
4. `flyctl deploy`
5. `flyctl logs --no-tail -a AIRoute`
