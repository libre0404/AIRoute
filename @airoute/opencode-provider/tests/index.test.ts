...
import {
  buildAIRouteOpenCodeConfig,
  createAIRouteAgentBlock,
  createAIRouteComboConfig,
  createAIRouteMCPEntry,
  createAIRouteModesBlock,
  createAIRouteProvider,
  fetchLiveModels,
  listCombos,
  mergeIntoExistingConfig,
  normalizeBaseURL,
  AIRoute_DEFAULT_MODEL_CAPABILITIES,
  AIRoute_DEFAULT_MODEL_CONTEXT_LENGTHS,
  AIRoute_DEFAULT_OPENCODE_MODELS,
  AIRoute_MCP_DEFAULT_SCOPES,
  AIRoute_PROVIDER_NPM,
...
test("createAIRouteProvider validates required fields", () => {
  assert.throws(
    () => createAIRouteProvider({ baseURL: "", apiKey: "x" } as never),
    /baseURL is required/
  );
  assert.throws(
    () => createAIRouteProvider({ baseURL: "http://x", apiKey: "" } as never),
    /apiKey is required/
  );
});

test("createAIRouteProvider produces the OpenCode-compatible shape", () => {
  const provider = createAIRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_AIRoute",
  });

  assert.equal(provider.npm, AIRoute_PROVIDER_NPM);
  assert.equal(provider.name, "AIRoute");
  assert.equal(provider.options.baseURL, "http://localhost:20128/v1");
  assert.equal(provider.options.apiKey, "sk_AIRoute");
  assert.equal(typeof provider.models, "object");
});

test("createAIRouteProvider seeds the default model catalog", () => {
  const provider = createAIRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_AIRoute",
  });
  ...
  const defaultIds = [...AIRoute_DEFAULT_OPENCODE_MODELS].sort();
...
test("createAIRouteProvider honours a custom models list and labels", () => {
  const provider = createAIRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_AIRoute",
...
test("createAIRouteProvider deduplicates and trims model ids", () => {
  const provider = createAIRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_AIRoute",
...
test("createAIRouteProvider honours displayName override", () => {
  const provider = createAIRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_AIRoute",
    displayName: "Local AIRoute",
  });
  assert.equal(provider.name, "Local AIRoute");
});

test("buildAIRouteOpenCodeConfig wraps the provider with the OpenCode schema", () => {
  const doc = buildAIRouteOpenCodeConfig({
    baseURL: "http://localhost:20128/v1",
    apiKey: "sk_AIRoute",
  });

  assert.equal(doc.$schema, OPENCODE_CONFIG_SCHEMA);
  assert.equal(typeof doc.provider.AIRoute, "object");
  assert.equal(doc.provider.AIRoute.options.baseURL, "http://localhost:20128/v1");
});
...
test("buildAIRouteOpenCodeConfig emits model and small_model prefixed with provider key", () => {
  const doc = buildAIRouteOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_AIRoute",
  ...
  assert.equal(doc.model, "AIRoute/claude-sonnet-4-5-thinking");
  assert.equal(doc.small_model, "AIRoute/gemini-3-flash");
});

test("buildAIRouteOpenCodeConfig omits model and small_model when not supplied", () => {
  const doc = buildAIRouteOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_AIRoute",
  });
...
test("buildAIRouteOpenCodeConfig ignores blank model strings", () => {
  const doc = buildAIRouteOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_AIRoute",
    model: "   ",
...
test("mergeIntoExistingConfig preserves existing provider entries", () => {
  ...
  assert.ok("AIRoute" in (result.provider as Record<string, unknown>));
  assert.deepEqual((result as Record<string, unknown>).keybinds, { submit: "enter" });
});

test("mergeIntoExistingConfig overwrites existing AIRoute entry", () => {
  const existing = {
    provider: {
      AIRoute: {
  ...
  const AIRoute = (result.provider as Record<string, unknown>).AIRoute as { name: string };
  assert.equal(AIRoute.name, "NEW");
});
...
test("mergeIntoExistingConfig writes model and small_model when supplied", () => {
  ...
  assert.equal(result.model, "AIRoute/claude-sonnet-4-5-thinking");
  assert.equal(result.small_model, "AIRoute/gemini-3-flash");
});

test("mergeIntoExistingConfig does not add model keys when not supplied", () => {
  const result = mergeIntoExistingConfig(
    {},
    { baseURL: "http://localhost:20128", apiKey: "sk_AIRoute" }
  );
  assert.ok(!("model" in result));
  assert.ok(!("small_model" in result));
});

test("AIRoute_MCP_DEFAULT_SCOPES contains 7 read-only scopes", () => {
  assert.equal(AIRoute_MCP_DEFAULT_SCOPES.length, 7);
  assert.ok(AIRoute_MCP_DEFAULT_SCOPES.every((s) => s.startsWith("read:")));
});

test("createAIRouteMCPEntry defaults to tsx runtime", () => {
  const entry = createAIRouteMCPEntry({
    serverPath: "/path/to/server.ts",
    apiKey: "sk_AIRoute",
  });
  assert.equal(entry.command, "npx");
  assert.deepEqual(entry.args, ["tsx", "/path/to/server.ts"]);
  assert.equal(entry.env.AIRoute_API_KEY, "sk_AIRoute");
  assert.ok(!("AIRoute_MCP_ENFORCE_SCOPES" in entry.env));
  assert.ok(!("AIRoute_MANAGEMENT_API_KEY" in entry.env));
});

test("createAIRouteMCPEntry uses node runtime when specified", () => {
  const entry = createAIRouteMCPEntry({
...
test("createAIRouteMCPEntry sets management key and scopes when supplied", () => {
  const entry = createAIRouteMCPEntry({
    serverPath: "/path/to/server.ts",
    apiKey: "sk_AIRoute",
  ...
  assert.equal(entry.env.AIRoute_MANAGEMENT_API_KEY, "sk_manage");
  assert.equal(entry.env.AIRoute_MCP_ENFORCE_SCOPES, "true");
  assert.equal(entry.env.AIRoute_MCP_SCOPES, "read:health,read:combos,execute:completions");
});

test("createAIRouteMCPEntry rejects missing required fields", () => {
  assert.throws(
    () => createAIRouteMCPEntry({ serverPath: "", apiKey: "x" }),
    /serverPath is required/
  );
  assert.throws(
    () => createAIRouteMCPEntry({ serverPath: "/p", apiKey: "" }),
    /apiKey is required/
  );
});
...
test("createAIRouteComboConfig builds minimal payload", () => {
  const payload = createAIRouteComboConfig({ name: "my-combo", strategy: "priority" });
...
test("createAIRouteComboConfig includes optional fields when supplied", () => {
...
test("AIRoute_DEFAULT_OPENCODE_MODELS includes cc/ prefixed models", () => {
  const defaults = [...AIRoute_DEFAULT_OPENCODE_MODELS];
...
test("AIRoute_DEFAULT_MODEL_CONTEXT_LENGTHS covers every default model id", () => {
  for (const id of AIRoute_DEFAULT_OPENCODE_MODELS) {
    const ctx = AIRoute_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    assert.ok(
...
test("createAIRouteProvider emits limit.context on default model entries", () => {
  const provider = createAIRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_AIRoute",
  });
...
test("createAIRouteProvider omits limit.context for unknown model ids", () => {
  const provider = createAIRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_AIRoute",
...
test("createAIRouteProvider reads contextLength from a live model entry for ids absent from the static map", () => {
  // #3298 regression guard: the static AIRoute_DEFAULT_MODEL_CONTEXT_LENGTHS
...
test("createAIRouteProvider: a live model contextLength wins over the static default map", () => {
...
test("createAIRouteProvider serialises limit.context to JSON", () => {
  const provider = createAIRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_AIRoute",
  });
  ...
  for (const id of AIRoute_DEFAULT_OPENCODE_MODELS) {
    const expectedContext = AIRoute_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    assert.equal(
...
test("AIRoute_DEFAULT_MODEL_CAPABILITIES covers every default model id", () => {
  for (const id of AIRoute_DEFAULT_OPENCODE_MODELS) {
    const caps = AIRoute_DEFAULT_MODEL_CAPABILITIES[id];
...
test("createAIRouteProvider emits default capability flags inline with the model entry", () => {
  const provider = createAIRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_AIRoute",
  });
...
test("createAIRouteProvider modelCapabilities overrides defaults and merges per id", () => {
  const provider = createAIRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_AIRoute",
    modelCapabilities: {
...
test("createAIRouteProvider applies capability overrides to non-default model ids", () => {
  const provider = createAIRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_AIRoute",
    models: ["custom-model"],
...
test("createAIRouteProvider modelLabels still works when modelCapabilities omits label", () => {
  const provider = createAIRouteProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_AIRoute",
...
test("createAIRouteAgentBlock builds provider-prefixed entries per role", () => {
  const block = createAIRouteAgentBlock({
    roles: {
  ...
  assert.equal(block.build.model, "AIRoute/claude-sonnet-4-5-thinking");
  assert.equal(block.build.temperature, 0.2);
  assert.equal(block.plan.model, "AIRoute/claude-opus-4-5-thinking");
  assert.equal(block.plan.top_p, 0.95);
  assert.equal(block.review.model, "AIRoute/gemini-3-flash");
  assert.equal(block.review.temperature, 0.0);
});

test("createAIRouteAgentBlock omits optional fields when not supplied", () => {
  const block = createAIRouteAgentBlock({
...
test("createAIRouteAgentBlock skips roles with empty modelId", () => {
  const block = createAIRouteAgentBlock({
    roles: {
...
test("createAIRouteAgentBlock emits tools as Record<string, boolean> per OC schema", () => {
  const block = createAIRouteAgentBlock({
    roles: {
      build: {
...
test("createAIRouteAgentBlock filters invalid tool entries and omits empty maps", () => {
  const block = createAIRouteAgentBlock({
    roles: {
      build: {
        modelId: "claude-sonnet-4-5-thinking",
...
test("createAIRouteModesBlock builds provider-prefixed mode entries", () => {
  const block = createAIRouteModesBlock({
    modes: {
...
test("createAIRouteModesBlock skips modes with empty modelId", () => {
  const block = createAIRouteModesBlock({
    modes: {
      build: { modelId: "claude-sonnet-4-5-thinking" },
      plan: { modelId: "" },
...
test("createAIRouteModesBlock honours numeric overrides limited to OC schema", () => {
  const block = createAIRouteModesBlock({
    modes: {
      build: {
        modelId: "claude-sonnet-4-5-thinking",
...
// #3419 — soft-deprecation in favour of @AIRoute/opencode-plugin. Guard the
...
test("package is marked deprecated in favour of @AIRoute/opencode-plugin (#3419)", () => {
  const here = dirname(fileURLToPath(import.meta.url));
  const pkg = JSON.parse(readFileSync(join(here, "..", "package.json"), "utf8"));
  assert.match(pkg.description, /DEPRECATED/);
  assert.match(pkg.description, /@AIRoute\/opencode-plugin/);

  const readme = readFileSync(join(here, "..", "README.md"), "utf8");
  assert.match(readme, /Deprecated/i);
  assert.match(readme, /@AIRoute\/opencode-plugin/);
});
