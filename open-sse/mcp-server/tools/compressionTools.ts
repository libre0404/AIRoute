 ...
 *
 * Tools:
 *   1. AIRoute_compression_status   — Get compression config, analytics, and cache stats
 *   2. AIRoute_compression_configure — Update compression settings
 */
...
export async function handleCompressionStatus(
  ...
  try {
    ...
    await logToolCall("AIRoute_compression_status", args, result, duration, true);
...
export async function handleCompressionConfigure(
  ...
  try {
    ...
    const duration = Date.now() - start;
    await logToolCall("AIRoute_compression_configure", args, result, duration, true);
...
export async function handleRtkDiscover(
  ...
  await logToolCall("AIRoute_rtk_discover", args, result, Date.now() - start, true);
  return result;
}
...
export async function handleRtkLearn(
  ...
  await logToolCall("AIRoute_rtk_learn", args, result, Date.now() - start, true);
  return result;
}
...
export const compressionTools = {
  ...
  AIRoute_ccr_retrieve: {
    name: "AIRoute_ccr_retrieve",
    description:
    ...
    handler: async (args: z.infer<typeof ccrRetrieveInput>, extra?: McpToolExtraLike) => {
      ...
      // AIRoute auth is API-key not OAuth-clientId) — resolve it to the same key id
  ...
  AIRoute_rtk_discover: {
    name: "AIRoute_rtk_discover",
    description:
