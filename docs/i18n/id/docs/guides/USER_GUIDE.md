...
Panduan lengkap untuk mengonfigurasi penyedia, membuat combo, mengintegrasikan alat CLI, dan menerapkan AIRoute.
...
**Tips Pro:** Gunakan Opus untuk tugas kompleks, Sonnet untuk kecepatan. AIRoute melacak kuota per model!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [dari dasbor AIRoute]
...
```json
{
  ...
  "anthropic_api_key": "your-AIRoute-api-key"
}
```
...
```json
{
  "agents": {
    "defaults": {
      "model": { "primary": "AIRoute/if/glm-4.7" }
    }
  },
  "models": {
    "providers": {
...
CLI secara otomatis memuat `.env` dari `~/.AIRoute/.env` atau `./.env`.
...
| Perintah                 | Tindakan                                                                                           |
| ------------------------ | -------------------------------------------------------------------------------------------------- |
| `npm run uninstall`      | Menghapus aplikasi dari sistem tetapi **menyimpan DB dan konfigurasi** di `~/.AIRoute`.          |
| `npm run uninstall:full` | Menghapus aplikasi DAN secara permanen **menghapus semua konfigurasi, kunci, dan basis data**. |

> Catatan: Untuk menjalankan perintah ini, navigasikan ke folder proyek AIRoute (jika Anda telah meng-clone-nya) dan jalankan. Atau, jika diinstal secara global, Anda cukup menjalankan `npm uninstall -g AIRoute`.
...
Pengguna Void Linux dapat mengemas dan menginstal AIRoute secara native menggunakan framework kompilasi silang `xbps-src`. Ini mengotomasi build standalone Node.js beserta binding native `better-sqlite3` yang diperlukan.
...
| --------------------------------------- | ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| `JWT_SECRET`                            | `AIRoute-default-secret-change-me` | Rahasia penandatanganan JWT (**ubah di produksi**)                                                                         |
| `INITIAL_PASSWORD`                      | `123456`                             | Kata sandi login pertama                                                                                                   |
| `DATA_DIR`                              | `~/.AIRoute`                       | Direktori data (db, penggunaan, log)                                                                                       |
...
| `CLOUD_URL`                             | `https://AIRoute.dev`              | Cloud sinkronisasi titik akhir berbasis URL                                                                                      |
...
| `AIRoute_MEMORY_MB`                   | `512`                                | Batas heap Node.js dalam MB                                                                                                |
...
- Quick Tunnel tidak dipulihkan otomatis setelah AIRoute atau container di-restart; aktifkan kembali dari dasbor bila diperlukan
...
- **Cache Semantik** — Otomatis menyimpan respons non-streaming, temperature=0 (lewati dengan `X-AIRoute-No-Cache: true`)
- **Idempotensitas Permintaan** — Mendeduplikasi permintaan dalam 5 detik melalui header `Idempotency-Key` atau `X-Request-Id`
- **Pelacakan Progres** — Event SSE `event: progress` yang bisa diaktifkan melalui header `X-AIRoute-Progress: true`

---

### Translator Playground

Akses melalui **Dashboard → Translator**. Debug dan visualisasikan bagaimana AIRoute menerjemahkan permintaan API antar penyedia.
...
AIRoute juga menerima `x_session_id` dan mengembalikan kunci sesi efektif di `X-AIRoute-Session-Id`.
...
AIRoute mengimplementasikan ketahanan tingkat penyedia dengan lima komponen:
...
4. **Tunggu Cooldown** — Jika setiap kandidat koneksi sudah cooldown, AIRoute dapat menunggu cooldown paling awal dan mencoba kembali permintaan klien yang sama secara otomatis.
...
AIRoute mendukung transkripsi audio melalui titik akhir yang kompatibel dengan OpenAI:
...
AIRoute tersedia sebagai aplikasi desktop asli untuk Windows, macOS, dan Linux.
