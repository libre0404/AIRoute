 ...
 * `thinkingConfig`, …). The AIRoute router endpoint `/v1/chat/completions`
 ...
 *   - forward to `/v1/chat/completions` on the AIRoute router,
 ...
 * @param model      resolved AIRoute model string
  ...
  async intercept(
    ...
    try {
      ...
      if (!upstream.ok) {
        ...
        throw new Error(`AIRoute ${upstream.status}: ${errText}`);
      }
