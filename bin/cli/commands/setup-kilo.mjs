 ...
 * This writes BOTH (matching the AIRoute dashboard) and prints the UI settings.
 *
...
/** Merge the AIRoute openai-compatible provider into Kilo's CLI auth.json. */
export function buildKiloAuth(existing, { apiKey, baseUrl, model }) {
  const auth = { ...(existing || {}) };
  auth["openai-compatible"] = {
    ...(auth["openai-compatible"] || {}),
    apiKey: apiKey || "sk_AIRoute",
    baseUrl,
    model,
  };
  return auth;
...
export function buildKiloVscodeSettings(existing, { apiKey, baseUrl, model }) {
  const s = { ...(existing || {}) };
  s["kilocode.customProvider"] = { name: "AIRoute", baseURL: baseUrl, apiKey: apiKey || "sk_AIRoute" };
...
export async function runSetupKiloCommand(opts = {}) {
  ...
  printHeading("AIRoute → Kilo Code (OpenAI-compatible)");
  ...
  if (dryRun) {
    ...
    console.log(
      JSON.stringify(
        { "openai-compatible": { ...auth["openai-compatible"], apiKey: apiKey ? "set" : "sk_AIRoute" } },
        null,
        2
      )
    );
...
export function registerSetupKilo(program) {
  program
    .command("setup-kilo")
    .description(
      "Configure Kilo Code for AIRoute: write ~/.local/share/kilo/auth.json (CLI) + VS Code kilocode.* settings"
    )
