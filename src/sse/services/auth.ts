...
import { getQuotaScopeLabelForProvider } from "@AIRoute/open-sse/services/antigravityQuotaFamily.ts";
import {
...
import { isLocalProvider } from "@AIRoute/open-sse/config/providerRegistry.ts";
import { COOLDOWN_MS, RateLimitReason } from "@AIRoute/open-sse/config/constants.ts";
import {
  preflightQuota,
  isQuotaPreflightEnabled,
} from "@AIRoute/open-sse/services/quotaPreflight.ts";
import { resolveResilienceSettings } from "@/lib/resilience/settings";
import { resolveModelLockoutSettings } from "@/lib/resilience/modelLockoutSettings";
import { syncHealthFromDB, type KeyHealth } from "@AIRoute/open-sse/services/apiKeyRotator.ts";
import {
  classifyProviderError,
  PROVIDER_ERROR_TYPES,
} from "@AIRoute/open-sse/services/errorClassifier.ts";
...
} from "@AIRoute/open-sse/config/codexQuotaScopes.ts";
import {
  getProviderById,
  getProviderAlias,
  resolveProviderId,
...
export function extractSessionAffinityKey(
  ...
  const headerKey = normalizeSessionKey(
    readHeaderValue(headers, "x-codex-session-id") ??
      ...
      readHeaderValue(headers, "x-AIRoute-session"),
    "header"
  );
 ...
 * - `Authorization: Bearer <key>` (OpenAI / AIRoute / Codex CLI / Bearer clients)
 ...
 * rejected by per-route gates that compare against AIRoute keys.
 *
 ...
 * Feature #1350: Supports AIRoute_API_KEY / ROUTER_API_KEY env vars as
...
export async function isValidApiKey(apiKey: string) {
  ...
  const envKey = process.env.AIRoute_API_KEY || process.env.ROUTER_API_KEY;
