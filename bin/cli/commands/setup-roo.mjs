 ...
 * must support native OpenAI tool-calling (AIRoute does).
 */
...
export function buildRooImport({ baseUrl, apiKey, model }) {
  return {
    providerProfiles: {
      currentApiConfigName: "AIRoute",
      apiConfigs: {
        AIRoute: {
          apiProvider: "openai",
          openAiBaseUrl: baseUrl,
          openAiApiKey: apiKey || "sk_AIRoute",
          openAiModelId: model,
...
export async function runSetupRooCommand(opts = {}) {
  ...
  const importPath = opts.importPath ?? opts["import-path"] ?? join(os.homedir(), ".AIRoute", "roo-settings.json");
  const vscodePath =
    opts.vscodeSettings ?? opts["vscode-settings"] ?? join(os.homedir(), ".config", "Code", "User", "settings.json");

  printHeading("AIRoute → Roo Code (OpenAI-compatible)");
  ...
  if (dryRun) {
    console.log(`\n── [dry-run] ${importPath} ──`);
    console.log(JSON.stringify({ ...importDoc, providerProfiles: { ...importDoc.providerProfiles, apiConfigs: { AIRoute: { ...importDoc.providerProfiles.apiConfigs.AIRoute, openAiApiKey: apiKey ? "set" : "sk_AIRoute" } } } }, null, 2));
  ...
  printInfo(`  API Key:   <your AIRoute_API_KEY>`);
...
export function registerSetupRoo(program) {
  program
    .command("setup-roo")
    .description(
      "Configure Roo Code for AIRoute: write a Roo import JSON + autoImport pointer + print UI steps"
    )
    ...
    .option("--import-path <path>", "Roo import JSON path (default: ~/.AIRoute/roo-settings.json)")
