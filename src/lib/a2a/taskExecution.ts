...
export const A2A_SKILL_HANDLERS: Record<string, A2ASkillHandler> = {
  ...
  "coze-delegation": async (task) => {
    const skillModule = await import("./skills/cozeDelegation");
    return skillModule.executeCozeDelegation(task);
  },
  "dify-delegation": async (task) => {
    const skillModule = await import("./skills/difyDelegation");
    return skillModule.executeDifyDelegation(task);
  },
};
