...
import {
  ...
  getTargetFormat,
} from "@AIRoute/open-sse/services/provider.ts";
import {
  getModelTargetFormat,
  PROVIDER_ID_TO_ALIAS,
} from "@AIRoute/open-sse/config/providerModels.ts";
import { handleChatCore } from "@AIRoute/open-sse/handlers/chatCore.ts";
import {
  errorResponse,
  modelCooldownResponse,
  providerCircuitOpenResponse,
  unavailableResponse,
} from "@AIRoute/open-sse/utils/error.ts";
...
} from "@AIRoute/open-sse/utils/proxyFetch.ts";
...
import { getRuntimeProviderProfile } from "@AIRoute/open-sse/services/accountFallback.ts";
...
export async function safeLogEvents({
  ...
  try {
    ...
    try {
      ...
      const { proxyConfigToUrl } = await import("@AIRoute/open-sse/utils/proxyDispatcher.ts");
...
export function withSessionHeader(response: Response, sessionId: string | null): Response {
  ...
  try {
    response.headers.set("X-AIRoute-Session-Id", sessionId);
    return response;
  } catch {
    ...
    cloned.headers.set("X-AIRoute-Session-Id", sessionId);
    return cloned;
  }
}
...
export function withSelectedConnectionHeader(
  ...
  try {
    response.headers.set("X-AIRoute-Selected-Connection-Id", connectionId);
    return response;
  } catch {
    const cloned = new Response(response.body, {
      status: response.status,
    ...
    cloned.headers.set("X-AIRoute-Selected-Connection-Id", connectionId);
    return cloned;
  }
}
