...
async function resolveProviderInput(opts, prompt, nonInteractive) {
  ...
  if (!apiKey) {
    throw new Error("Provider API key is required. Pass --api-key or AIRoute_API_KEY.");
  }
...
export function registerSetup(program) {
  ...
  // Wire up `AIRoute setup opencode` subcommand. Kept inside registerSetup
...
export async function runSetupCommand(opts = {}) {
  ...
  try {
    printHeading("AIRoute Setup");
    const { db, dbPath } = await openAIRouteDb();
