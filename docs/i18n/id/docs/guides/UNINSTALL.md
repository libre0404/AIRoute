# AIRoute — Panduan Mencopot Pemasangan (Bahasa Indonesia)
...
Panduan ini menjelaskan cara mencopot pemasangan AIRoute dari sistem Anda secara bersih.

---

## Mencopot Pemasangan dengan Cepat (v3.6.2+)

AIRoute menyediakan dua skrip bawaan untuk penghapusan yang bersih:
...
Perintah ini menghapus aplikasi AIRoute tetapi **mempertahankan** basis data, konfigurasi, kunci API, dan pengaturan penyedia Anda di `~/.AIRoute/`. Gunakan ini jika Anda berencana memasang ulang nanti dan ingin menyimpan pengaturan yang ada.
...
- Berkas cadangan
- Berkas log
- Semua berkas di direktori `~/.AIRoute/`
...
- Buka `Settings → Apps → AIRoute → Uninstall`
- Atau jalankan uninstaller NSIS dari direktori instalasi

**macOS:**

- Seret `AIRoute.app` dari `/Applications` ke Trash
- Hapus data: `rm -rf ~/Library/Application Support/AIRoute`

**Linux:**

- Hapus berkas AppImage
- Hapus data: `rm -rf ~/.AIRoute`
...
AIRoute menyimpan data di lokasi-lokasi berikut secara default:
