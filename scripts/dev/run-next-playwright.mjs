...
const bootstrapMode = process.env.AIRoute_E2E_BOOTSTRAP_MODE || "auth";
const playwrightPassword =
  process.env.AIRoute_E2E_PASSWORD || process.env.INITIAL_PASSWORD || "AIRoute-e2e-password";
const testServerEnv = {
  ...
  NEXT_PUBLIC_AIRoute_E2E_MODE: process.env.NEXT_PUBLIC_AIRoute_E2E_MODE || "1",
  AIRoute_DISABLE_BACKGROUND_SERVICES:
    process.env.AIRoute_DISABLE_BACKGROUND_SERVICES || "true",
  AIRoute_DISABLE_TOKEN_HEALTHCHECK: process.env.AIRoute_DISABLE_TOKEN_HEALTHCHECK || "true",
  AIRoute_DISABLE_LOCAL_HEALTHCHECK: process.env.AIRoute_DISABLE_LOCAL_HEALTHCHECK || "true",
  AIRoute_HIDE_HEALTHCHECK_LOGS: process.env.AIRoute_HIDE_HEALTHCHECK_LOGS || "true",
  ...(bootstrapMode === "open"
    ? {
        INITIAL_PASSWORD: "",
        AIRoute_E2E_PASSWORD: "",
        AIRoute_API_KEY: "",
      }
    : {
        INITIAL_PASSWORD: playwrightPassword,
        AIRoute_E2E_PASSWORD: playwrightPassword,
      }),
  ...(process.env.AIRoute_USE_TURBOPACK
    ? {
        AIRoute_USE_TURBOPACK: process.env.AIRoute_USE_TURBOPACK,
      }
    : {}),
};
...
export function shouldUseWebpackForPlaywrightDev({ mode, env }) {
  ...
  return mode === "dev" && env.AIRoute_USE_TURBOPACK === "0";
}
...
async function runBuildForStart() {
  ...
  if (process.env.AIRoute_PLAYWRIGHT_SKIP_BUILD === "1") return;
