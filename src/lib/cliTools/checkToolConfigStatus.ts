...
/**
 * Check if a tool has AIRoute configured by reading its config file directly.
  ...
  try {
    ...
    if (toolId === "hermes") {
      ...
      const hasAIRoute =
        ...
        lower.includes(`localhost:${apiPort}`) ||
        lower.includes(`127.0.0.1:${apiPort}`);
      return hasAIRoute ? "configured" : "not_configured";
    }

    const config = JSON.parse(content) as Record<string, unknown>;

    // Each tool stores AIRoute config differently
    switch (toolId) {
      case "claude":
      ...
      case "cline":
      case "kilo": {
        // Generic check: look for AIRoute-specific markers in the config
        const configStr = JSON.stringify(config).toLowerCase();
        if (
          configStr.includes("AIRoute") ||
          configStr.includes("sk_AIRoute") ||
