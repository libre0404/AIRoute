...
export default function HermesAgentToolCard({
  ...
  const setRoleSelection = (roleId: string, model: string, provider = "AIRoute") => {
    setSelections((prev) => ({ ...prev, [roleId]: { model, provider } }));
  };

  const applyToAll = (model: string) => {
    const newSel: Record<string, RoleSelection> = {};
    HERMES_ROLES.forEach((r) => (newSel[r.id] = { model, provider: "AIRoute" }));
  ...
  const effectiveRoles = React.useMemo(() => {
    // If user has pending changes, treat selected roles as AIRoute
    if (Object.keys(selections).length > 0) {
      const map: Record<string, any> = {};
      HERMES_ROLES.forEach((r) => {
        if (selections[r.id]) {
          map[r.id] = { usingAIRoute: true };
  ...
  // Count of roles that are (or will be) routed via AIRoute
  const configuredRolesCount = HERMES_ROLES.filter((role) => {
    // Pending selection always counts as AIRoute intent
    if (selections[role.id]) return true;

    const info = effectiveRoles[role.id];
    if (!info) return false;

    // Support both shapes: detector shape (usingAIRoute) and settings shape (provider + base_url)
    if (typeof info.usingAIRoute === "boolean") {
      return info.usingAIRoute;
    }
    return (
      info?.provider === "AIRoute" ||
              ...
              <h3 className="font-medium text-sm flex items-center gap-2">
                ...
                {firstSetupAt && (
                  <span
                    ...
                    title={`First set up via AIRoute on ${new Date(firstSetupAt).toLocaleDateString()}`}
                  >
        ...
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            {HERMES_ROLES.map((role) => {
              ...
              // - If user has selected something in this session (pending): show as via AIRoute
              // - Else if current from disk: show real provider name + "(not AIRoute)" or "AIRoute"
              let badge: { label: string; pending: boolean } | null = null;

              if (sel) {
                // pending change made via the Select modal / quick apply → will be routed via AIRoute
                const prov = sel.provider || "AIRoute";
                badge = { label: `${prov} (via AIRoute)`, pending: true };
              } else if (current) {
                const isOmni =
                  current?.provider === "AIRoute" ||
                  (current?.base_url || "").includes("20128") ||
                  (current?.base_url || "").includes("localhost");

                if (isOmni) {
                  badge = { label: "AIRoute", pending: false };
                } else {
                  const realProvider = current.provider || "Other";
                  badge = { label: `${realProvider} (not AIRoute)`, pending: false };
                }
              }
                    ...
                    {badge && (
                      <div
                        className={`text-[10px] px-1.5 py-px rounded shrink-0 ${
                          badge.label.includes("not AIRoute")
        ...
        onSelect={(model: any) => {
          if (modalRole) {
            ...
            if (modelValue) {
              ...
              const prov =
                (model && (model.provider || model.providerId || model.group)) || "AIRoute";
