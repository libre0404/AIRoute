 ...
 *
 * Slim Docker images (e.g. `node:24-trixie-slim` used by AIRoute's runtime
...
/**
 * Truthy-env check for `AIRoute_NO_SUDO`. Inlined (not imported from
...
export function isNoSudoEnv(): boolean {
  const value = process.env.AIRoute_NO_SUDO;
 ...
 * the operator opts into root-less mode via `AIRoute_NO_SUDO` (#6122). No
...
export function execFileWithPassword(
  ...
  // `AIRoute_NO_SUDO` (root-less / user-namespace deployments — #6122), skip
...
export async function runElevatedPowerShell(script: string): Promise<string> {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "AIRoute-elevate-"));
  const scriptName = `AIRoute-elevate-${crypto.randomUUID()}.ps1`;
