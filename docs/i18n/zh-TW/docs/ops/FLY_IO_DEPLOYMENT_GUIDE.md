...
本文檔記錄 AIRoute 在 Fly.io 上的實際部署方法，適用於兩類場景：

- 首次把當前項目部署到 Fly.io
- 後續代碼更新後繼續發布
- 新項目參考同樣流程部署

本文基於當前項目已經驗證通過的設定整理，應用名為 `AIRoute`。
...
- 以前如果用過別的名字，例如 `oroute`，不要和 `AIRoute` 混淆
...
如果你已經改成別的應用名，把 `AIRoute` 替換成你的名字。
...
```powershell
flyctl secrets set INITIAL_PASSWORD=你的強密碼 -a AIRoute
```
...
如果當前倉庫是 fork，並且你要同步上遊 `https://github.com/diegosouzapw/AIRoute` 的更新，推薦按下面流程執行。
...
- 你打開的是另一個應用，例如 `oroute`，不是 `AIRoute`
