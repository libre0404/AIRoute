...
// ── Helper: check if AIRoute CLI is available ──────────────────────────────

function getAIRouteCliPath(): string | null {
  try {
    const result = execSync("which AIRoute 2>/dev/null || command -v AIRoute 2>/dev/null", {
      encoding: "utf-8",
      timeout: 3000,
    }).trim();
    return result || null;
...
export const COPILOT_TOOLS: CopilotTool[] = [
  ...
  // ── CodeGraph Tools ──
  {
    ...
    description:
      "Search for symbols in the AIRoute codebase by name (functions, classes, types, variables). Use this to understand how the app works internally.",
    parameters: [
      {
        name: "query",
        type: "string",
  ...
  },
  {
    name: "findCallees",
    description:
      "Find all functions/symbols that a specific function calls. Useful for understanding dependencies and code flow within AIRoute.",
    parameters: [
      {
        name: "symbol",
        type: "string",
  ...
  // ── CLI Execution Tool ──
  {
    name: "runAIRouteCli",
    description:
      "Execute an 'AIRoute' CLI command to configure or query the AIRoute app. Gives complete control over the app — use for advanced operations not covered by other tools. Common commands: AIRoute list-keys, AIRoute switch-combo [id], AIRoute set-budget 10, AIRoute set-strategy [id] priority, AIRoute health, AIRoute mcp (starts MCP server), AIRoute db-health, AIRoute reset-password.",
    parameters: [
      {
        name: "command",
        type: "string",
        description:
          "CLI command arguments (everything after 'AIRoute'). Example: 'list-keys', 'switch-combo abc123', 'health'",
        required: true,
      },
    ],
    handler: async (args) => {
      ...
      const cliPath = getAIRouteCliPath();
      if (!cliPath) return "AIRoute CLI not found in PATH. Install AIRoute first.";
