...
/**
 * AIRoute — Prepublish Build Script
 *
...
const cliSrcFile = join(ROOT, "bin", "AIRoute.ts");
const cliDestFile = join(ROOT, "bin", "AIRoute.mjs");
  ...
  try {
    execFileSync(
      ...
      [
        ...
        "--packages=external",
        "--format=esm",
        "--outfile=bin/AIRoute.mjs",
      ],
      { cwd: ROOT, stdio: "inherit" }
    );
    chmodSync(cliDestFile, 0o755);
    console.log("  ✅ CLI Entrypoint bundled to bin/AIRoute.mjs");
  } catch (err: any) {
    console.warn("  ⚠️  CLI bundle error:", err.message);
  }
}

// ── Step 8.8: Build @AIRoute/opencode-plugin ──────────────
// The plugin ships bundled inside the AIRoute npm package (see root
// package.json "files": ["@AIRoute/", ...]). Its built `dist/` MUST be
// present in the publish tarball so `AIRoute setup opencode` can copy it
...
const opencodePluginSrc = join(ROOT, "@AIRoute", "opencode-plugin");
...
if (existsSync(opencodePluginSrc) && existsSync(join(opencodePluginSrc, "package.json"))) {
  ...
  if (!pluginAlreadyBuilt) {
    console.log("\n  🔨 Building @AIRoute/opencode-plugin (tsup)...");
    try {
      ...
      console.log("  ✅ @AIRoute/opencode-plugin bundled to @AIRoute/opencode-plugin/dist/");
    } catch (err: any) {
      console.error("  ❌ Failed to build @AIRoute/opencode-plugin:", err.message);
      console.error("     The published package would be missing the plugin dist.");
      console.error(
        "     Run `cd @AIRoute/opencode-plugin && npm install && npm run build` to debug."
      );
      process.exit(1);
    }
  } else {
    console.log("  ✅ @AIRoute/opencode-plugin dist/ already present (skipping rebuild)");
  }
  ...
  if (existsSync(pluginNodeModules)) {
    ...
    console.log("  🧹 Removed @AIRoute/opencode-plugin/node_modules (hard link guard)");
  }
} else {
  console.log("  ⏭️  @AIRoute/opencode-plugin not found in workspace (skipping build)");
}
