...
- before the active account is exhausted, AIRoute generates a compact structured summary
- after authentication selects a different account for the same session, AIRoute injects
...
If the active provider is enabled in `handoffProviders`, AIRoute generates a structured
...
AIRoute prepends the stored handoff as a system message. Injection happens only after the
...
At injection time, AIRoute converts that payload into a `<context_handoff>` system
