...
Masalah umum dan solusinya untuk AIRoute.
...
| EACCES: permission denied                           | Atur `DATA_DIR=/path/to/writable/dir` untuk mengganti `~/.AIRoute`                                                                                            |
...
| `dlopen` / `slice is not valid mach-o file` (macOS) | Jalankan `cd $(npm root -g)/AIRoute/app && npm rebuild better-sqlite3 && AIRoute` — lihat [Pembangunan ulang modul native macOS](#macos-native-module-rebuild) di bawah |
...
**Penyebab:** Anda menjalankan versi Node.js di luar batas runtime aman yang disetujui AIRoute. Kasus paling umum adalah menjalankan Node 20, 22, atau 24 versi patch lama yang berada di bawah batas keamanan yang diperlukan AIRoute.
...
3. Instal ulang AIRoute: `npm install -g AIRoute`
4. Mulai ulang: `AIRoute`
...
**Solusi (v3.5.5+):** AIRoute sekarang menggunakan fungsi `fetch()` milik undici sendiri ketika dispatcher proxy aktif, memastikan perilaku yang konsisten. Perbarui ke v3.5.5+.
...
AIRoute memperbarui token secara otomatis. Jika masalah berlanjut:
...
2. Pastikan `CLOUD_URL` mengarah ke endpoint cloud Anda (misalnya, `https://AIRoute.dev`)
...
Ketika banyak permintaan bersamaan mengenai penyedia yang dibatasi lajunya, AIRoute menggunakan mutex + auto rate-limiting untuk membuat serialisasi permintaan dan mencegah kegagalan berantai. Ini berjalan otomatis untuk penyedia dengan API key.

---

## Taksonomi Kegagalan RAG / LLM Opsional (16 masalah)

Beberapa pengguna AIRoute menempatkan gateway di depan tumpukan RAG atau agen. Dalam pengaturan tersebut, umum terjadi pola yang aneh: AIRoute terlihat sehat (penyedia aktif, profil routing baik, tidak ada peringatan batas laju) tetapi jawaban akhir masih salah.
...
1. Saat Anda menyelidiki respons yang buruk, kumpulkan:
   ...
   - route atau combo penyedia di AIRoute
...
3. Simpan nomornya di dashboard, runbook, atau pelacak insiden Anda sendiri di samping log AIRoute.
...
Anda dapat mengabaikan bagian ini jika Anda tidak menjalankan pipeline RAG atau agen di belakang AIRoute.
