...
**AIRoute** — unified AI proxy/router. One endpoint, 237 LLM providers, auto-fallback.
...
AIRoute has three related but distinct temporary-failure mechanisms. Keep their
...
- **Imports**: external → internal (`@/`, `@AIRoute/open-sse`) → relative
...
repo** instead of polluting the main AIRoute tree.
...
- **Runtime**: Node.js ≥22.0.0 <23 || ≥24.0.0 <27, ES Modules. This is the **only** runtime for the published `AIRoute` CLI, the server, and the test suites (`node:test` + vitest) — `engines.node` is authoritative and end users never need Bun.
...
- **Data directory**: `DATA_DIR` env var, defaults to `~/.AIRoute/`
...
AIRoute has **~48 quality-gate scripts** (`scripts/check/` + `scripts/quality/`) wired
