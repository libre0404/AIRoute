# Panduan Deployment AIRoute di Fly.io
...
Dokumen ini menjelaskan metode deployment AIRoute di Fly.io yang telah terbukti berhasil, mencakup dua skenario utama:
...
Dokumen ini disusun berdasarkan konfigurasi yang telah diverifikasi pada proyek saat ini, dengan nama aplikasi `AIRoute`.
...
- Alamat akses: `https://AIRoute.fly.dev/`
...
- `app = 'AIRoute'` menentukan aplikasi Fly mana yang menjadi target deployment
...
- Jika sebelumnya Anda menggunakan nama lain seperti `oroute`, jangan sampai tertukar dengan `AIRoute`
...
Jika Anda sudah mengganti nama aplikasi, ganti `AIRoute` dengan nama Anda.
...
Parameter-parameter berikut telah digunakan secara nyata pada aplikasi `AIRoute` saat ini:
...
- Berlaku untuk proyek saat ini yaitu `AIRoute`
...
```powershell
flyctl secrets set INITIAL_PASSWORD=kata-sandi-kuat-anda -a AIRoute
```
...
- Apakah aplikasi yang sedang dilihat adalah `AIRoute`
...
Jika repositori saat ini adalah fork dan Anda ingin menyinkronkan pembaruan dari upstream `https://github.com/diegosouzapw/AIRoute`, ikuti alur berikut.
...
- Anda membuka aplikasi yang salah, misalnya `oroute`, bukan `AIRoute`
