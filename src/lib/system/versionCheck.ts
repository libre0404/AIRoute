 ...
 * ONLY from `npm info AIRoute version --json` (the `npm` CLI binary). When that binary
...
const NPM_REGISTRY_LATEST_URL = "https://registry.npmjs.org/AIRoute/latest";
...
const GITHUB_RELEASES_LATEST_URL =
  "https://api.github.com/repos/diegosouzapw/AIRoute/releases/latest";
  ...
  try {
    const res = await fetchImpl(GITHUB_RELEASES_LATEST_URL, {
      ...
      headers: {
        ...
        "User-Agent": "AIRoute-version-check",
