...
function resolveMigrationsDir(): string {
  ...
  throw new Error(
    "[Migration] Could not resolve migrations directory. Set AIRoute_MIGRATIONS_DIR."
  );
}
 ...
 *
 * Set the threshold to 0 (via `AIRoute_MAX_PENDING_MIGRATIONS`) to disable
 * this safety check.
 */
 ...
 * the default via the `AIRoute_MAX_PENDING_MIGRATIONS` env var (#3416). This
 ...
 * `AIRoute_MIGRATIONS_DIR` convention used in resolveMigrationsDir(). Falls
 * back to the default on missing or invalid (non-numeric / negative) input.
 */
function resolveMaxPendingMigrations(): number {
  const raw = process.env.AIRoute_MAX_PENDING_MIGRATIONS;
...
function supportsFts5(db: SqliteAdapter): boolean {
  ...
  try {
    const probeTable = `__AIRoute_fts5_probe_${crypto.randomUUID().replace(/-/g, "_")}`;
    db.transaction(() => {
...
function getMigrationFiles(): Array<{ version: string; name: string; path: string }> {
  ...
  // would record version=NNN in _AIRoute_migrations; the rest would never run).
...
function getAppliedVersions(db: SqliteAdapter): Set<string> {
  const rows = db.prepare("SELECT version FROM _AIRoute_migrations").all() as Array<{
    version: string;
  }>;
...
function getAppliedRecords(db: SqliteAdapter): Array<{ version: string; name: string }> {
  return db
    .prepare("SELECT version, name FROM _AIRoute_migrations ORDER BY version")
    .all() as Array<{
    version: string;
    name: string;
  }>;
...
function reconcileRenumberedMigrations(
  ...
  for (const compatibility of RENAMED_MIGRATION_COMPATIBILITY) {
    ...
    const legacyRow = db
      .prepare("SELECT version, name FROM _AIRoute_migrations WHERE version = ? AND name = ?")
    ...
    const targetRow = db
      .prepare("SELECT version FROM _AIRoute_migrations WHERE version = ?")
      .get(compatibility.toVersion) as { version: string } | undefined;

    const applyRepair = db.transaction(() => {
      if (targetRow) {
        db.prepare("DELETE FROM _AIRoute_migrations WHERE version = ? AND name = ?").run(
          compatibility.fromVersion,
          compatibility.fromName
        );
      } else {
        db.prepare(
          "UPDATE _AIRoute_migrations SET version = ?, name = ? WHERE version = ? AND name = ?"
        ).run(
    ...
    const residualRow = db
      .prepare("SELECT version, name FROM _AIRoute_migrations WHERE version = ?")
    ...
    if (residualRow) {
      ...
      db.prepare("DELETE FROM _AIRoute_migrations WHERE version = ?").run(
        compatibility.fromVersion
      );
    }
  }
...
function rehomeLegacyVersionSlotMigrations(
  ...
  for (const legacy of LEGACY_VERSION_SLOT_MIGRATIONS) {
    ...
    const applyRepair = db.transaction(() => {
      ...
      db.prepare("UPDATE _AIRoute_migrations SET version = ? WHERE version = ? AND name = ?").run(
        legacyVersion,
        legacy.version,
        legacy.name
      );
...
export function runMigrations(db: SqliteAdapter, options?: { isNewDb?: boolean }): number {
  ...
  if (mismatches.length > 0) {
    ...
    console.error(
      `[Migration] The version-only tracking will skip these (version already applied), ` +
        `but please report this to the AIRoute maintainers.`
    );
  }
  ...
  // from the _AIRoute_migrations table.
  ...
  // #3416: resolve the threshold at call time so AIRoute_MAX_PENDING_MIGRATIONS
  ...
  if (
    ...
    if (plausiblePendingCount !== null && actionablePending.length <= plausiblePendingCount) {
      ...
      const bypassHint =
        ...
        `tracking table was wiped), set AIRoute_MAX_PENDING_MIGRATIONS=0 in your ` +
  ...
  for (const migration of pending) {
    ...
    const applyMigration = db.transaction(() => {
      ...
      db.prepare("INSERT INTO _AIRoute_migrations (version, name) VALUES (?, ?)").run(
        migration.version,
        migration.name
      );
    });
    ...
    try {
      ...
      if (message.includes("duplicate column name")) {
        const applyMarkerOnly = db.transaction(() => {
          db.prepare(
            "INSERT OR IGNORE INTO _AIRoute_migrations (version, name) VALUES (?, ?)"
...
export function getMigrationStatus(db: SqliteAdapter): {
  ...
  const appliedRows = db
    .prepare("SELECT version, name, applied_at FROM _AIRoute_migrations ORDER BY version")
