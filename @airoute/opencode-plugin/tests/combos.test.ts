 ...
 *
 * Covers:
 *   - `defaultAIRouteCombosFetcher(baseURL, apiKey, timeoutMs?)`
 ...
 *   - `createAIRouteProviderHook(opts, deps)` extension
...
const MODEL_PRIMARY: AIRouteRawModelEntry = {
  id: "claude-primary",
  capabilities: {
    tool_calling: true,
    reasoning: true,
...
const MODEL_SECONDARY: AIRouteRawModelEntry = {
  id: "claude-secondary",
  capabilities: {
    tool_calling: true,
    reasoning: false,
...
const MODEL_NO_TOOLS: AIRouteRawModelEntry = {
  id: "gemini-3-flash",
...
): AIRouteModelsFetcher & { callCount: () => number } {
  let n = 0;
...
function failingCombosFetcher(
  err = new Error("boom")
): AIRouteCombosFetcher & { callCount: () => number } {
  let n = 0;
...
// ────────────────────────────────────────────────────────────────────────────
// defaultAIRouteCombosFetcher — envelope tolerance + error surfacing
// ────────────────────────────────────────────────────────────────────────────

test("defaultAIRouteCombosFetcher: parses {combos:[…]} envelope", async () => {
  ...
  try {
    const combos = await defaultAIRouteCombosFetcher("https://or.example.com", "sk-test");
...
test("defaultAIRouteCombosFetcher: parses bare array envelope", async () => {
  const originalFetch = globalThis.fetch;
  ...
  try {
    const combos = await defaultAIRouteCombosFetcher("https://or.example.com/v1", "sk-test");
...
test("defaultAIRouteCombosFetcher: strips trailing /v1 before /api/combos", async () => {
  const originalFetch = globalThis.fetch;
  let observedUrl = "";
  globalThis.fetch = (async (input: unknown) => {
  ...
  try {
    await defaultAIRouteCombosFetcher("https://or.example.com/v1/", "sk-test");
...
test("defaultAIRouteCombosFetcher: throws on non-2xx with status code in message", async () => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = (async () => {
  ...
  try {
    await assert.rejects(
      async () => {
        await defaultAIRouteCombosFetcher("https://or.example.com", "sk-bad");
      },
      (err: unknown) => {
...
test("defaultAIRouteCombosFetcher: throws when apiKey missing", async () => {
  await assert.rejects(
    async () => defaultAIRouteCombosFetcher("https://or.example.com", ""),
    /apiKey required/
  );
});

test("defaultAIRouteCombosFetcher: throws when baseURL missing", async () => {
  await assert.rejects(
    async () => defaultAIRouteCombosFetcher("", "sk-test"),
    /baseURL required/
  );
});
...
test("mapComboToModelV2: api block matches providerId + baseURL", () => {
  ...
  assert.equal(m.providerID, "AIRoute-preprod");
...
test("mapComboToModelV2: explicit member temperature=false drops combo temperature=false", () => {
  const tempFalse: AIRouteRawModelEntry = {
    id: "no-temp",
...
// ────────────────────────────────────────────────────────────────────────────
// createAIRouteProviderHook — combos merge + collision + soft-fail + cache
// ────────────────────────────────────────────────────────────────────────────
...
test("models() returns combo entries merged into the map", async () => {
  ...
  assert.ok(out["opencode-AIRoute/claude-primary"]);
  assert.ok(out["opencode-AIRoute/claude-secondary"]);
  assert.ok(out["opencode-AIRoute/gemini-3-flash"]);
  assert.ok(out["opencode-AIRoute/claude-tier"]);

  const combo = out["opencode-AIRoute/claude-tier"];
  assert.equal(combo.name, "Claude Tier");
  assert.equal(combo.providerID, "opencode-AIRoute");
...
test("models(): combo with unknown member ids degrades to all-false LCD posture", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  assert.ok(out["opencode-AIRoute/phantom-combo"]);
  // With zero resolvable members, LCD = all-false (defensive posture).
  assert.equal(out["opencode-AIRoute/phantom-combo"].capabilities.toolcall, false);
  assert.equal(out["opencode-AIRoute/phantom-combo"].capabilities.reasoning, false);
  assert.equal(out["opencode-AIRoute/phantom-combo"].limit.context, 0);
});
...
test("models(): hidden combos are excluded from the map", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  assert.ok(out["opencode-AIRoute/visible"]);
  assert.ok(!out["opencode-AIRoute/hidden"], "hidden combo must be omitted");
});
...
test("models(): combo name exactly matches raw model id → raw deleted, raw deleted, no warn", async () => {
  ...
  // the real AIRoute payload where /v1/models pre-mirrors combos as
  // no-slash raw entries whose ids match /api/combos friendly names.
  const colliderCombo: AIRouteRawCombo = {
    id: "uuid-collider",
  ...
  assert.ok(out["opencode-AIRoute/claude-primary"], "combo surfaces under prefixed key");
  assert.equal(out["opencode-AIRoute/claude-primary"].name, "claude-primary");
...
test("models(): two combos with same slug → second gets disambiguator suffix", async () => {
  ...
  assert.ok(out["opencode-AIRoute/claude"], "first combo at prefixed slug");
  assert.ok(out["opencode-AIRoute/claude-uuid"], "second combo disambiguated by id prefix");
});
...
test("models(): combos cached + reused within TTL (one combo fetch per TTL window)", async () => {
  ...
  assert.ok(second["opencode-AIRoute/claude-tier"]);
});
...
test("models(): nested combo-ref context is the min of nested + raw members", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  const masterLight = out["opencode-AIRoute/master-light"];
