...
> Complete setup reference for AIRoute. For the quick version, see the [Quick Start in README](../README.md#-quick-start).
...
The [AUR package](https://aur.archlinux.org/packages/AIRoute-bin) installs AIRoute and provides a systemd user service.
...
AIRoute ships a desktop wrapper built on Electron 41 + electron-builder 26.10. Available scripts (workspace root):
...
AIRoute providers test-batch
```

Combined with env vars (`INITIAL_PASSWORD`, `AIRoute_WS_BRIDGE_SECRET`, etc.), this lets you spin up an AIRoute instance fully scriptable.
...
| `AIRoute setup`       | Guided CLI onboarding for password and first provider          |
| `AIRoute doctor`      | Run local health checks without starting the server            |
| `AIRoute providers`   | Discover, list, validate, and test providers from CLI          |
| `AIRoute config`      | CLI tool configuration — list, get, set, validate configs      |
| `AIRoute status`      | Offline status dashboard — version, DB, tools, config          |
| `AIRoute logs`        | Stream usage logs from the API (supports `--follow`)           |
| `AIRoute update`      | Check for or apply AIRoute updates                           |
| `AIRoute provider`    | Manage provider connections — add, list, remove, test, default |
...
```bash
AIRoute setup --non-interactive --password "$AIRoute_PASSWORD"
AIRoute setup --non-interactive --add-provider --provider openai --api-key "$OPENAI_API_KEY"
AIRoute setup --non-interactive --add-provider --provider openai --api-key "$OPENAI_API_KEY" --test-provider
```
...
AIRoute providers test <id-or-name>
...
Instead of pasting the base URL and key by hand, let AIRoute write each tool's
own config from the live model catalog. One command per tool:

```bash
AIRoute setup-codex        # ~/.codex/<name>.config.toml profiles
AIRoute setup-claude       # ~/.claude/profiles/<name>/settings.json
AIRoute setup-opencode     # ~/.config/opencode/opencode.json (openai-compatible)
AIRoute setup-cline        # Cline CLI + VS Code extension settings
AIRoute setup-kilo         # Kilo Code
AIRoute setup-continue     # ~/.continue/config.yaml (Continue / cn)
AIRoute setup-cursor       # prints Cursor's in-app steps
AIRoute setup-roo          # Roo Code import + autoImport pointer
AIRoute setup-crush        # ~/.config/crush/crush.json
AIRoute setup-goose        # ~/.config/goose/config.yaml
AIRoute setup-qwen         # ~/.qwen/settings.json
AIRoute setup-aider        # ~/.aider.conf.yml
```

Each accepts `--remote <url> --api-key <key>` to configure a local tool against a
**remote** AIRoute, plus `--dry-run` to preview. The launchers
...
AIRoute_get_health        # Should return system health
AIRoute_list_combos       # Should return active combos
...
For Claude Code-compatible upstreams (`anthropic-compatible-cc-*`), AIRoute derives the outbound `X-Stainless-Timeout` header from the resolved fetch timeout so provider-side read timeouts stay aligned with your env configuration.

For third-party Claude Code-compatible reverse proxies, AIRoute keeps the default `anthropic-beta` set conservative and, when `Client Cache Control` is left on `Auto`, only forwards client-provided `cache_control` markers. Enable the per-connection "Enable redact-thinking beta" toggle only when the upstream specifically requires redacted Claude thinking streams.
...
> **Note:** For streaming requests, `FETCH_TIMEOUT_MS` only covers connection setup / waiting for the first upstream response. Once the stream is active, AIRoute will only abort on an actual stall (`STREAM_IDLE_TIMEOUT_MS`) or Undici body inactivity (`FETCH_BODY_TIMEOUT_MS`).

### Reverse Proxy Compatibility

If you run AIRoute behind Nginx, Caddy, Cloudflare, or another reverse proxy, make sure the proxy timeouts are also higher than your AIRoute stream/fetch timeouts.
...
#   curl -L -o /tmp/AIRoute.tar.gz "https://github.com/diegosouzapw/AIRoute/archive/refs/tags/v${version}.tar.gz" && sha256sum /tmp/AIRoute.tar.gz
