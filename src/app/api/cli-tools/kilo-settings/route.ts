...
// Check if AIRoute OpenAI-compatible provider is configured
const hasAIRouteConfig = (auth) => {
  if (!auth) return false;
  const routerEntry = auth["openai-compatible"] || auth["AIRoute"];
  if (!routerEntry) return false;
  const baseUrl = routerEntry.baseUrl || routerEntry.baseURL || "";
  return (
    baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("AIRoute")
  );
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasAIRoute: hasAIRouteConfig(auth),
      authPath: AUTH_PATH,
    });
  } catch (error) {
...
// POST - Configure Kilo Code to use AIRoute as OpenAI-compatible provider
export async function POST(request) {
  ...
  try {
    ...
    // Add/update AIRoute as openai-compatible provider
...
// DELETE - Remove AIRoute config from Kilo
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "AIRoute settings removed from Kilo Code",
    });
  } catch (error) {
