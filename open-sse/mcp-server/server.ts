...
const MCP_ENFORCE_SCOPES = process.env.AIRoute_MCP_ENFORCE_SCOPES === "true";
const MCP_ALLOWED_SCOPES = new Set(
  (process.env.AIRoute_MCP_SCOPES || "")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean)
);
...
function getAIRouteApiKey(): string {
  return process.env.AIRoute_API_KEY || "";
}

export async function AIRouteFetch(path: string, options: RequestInit = {}): Promise<unknown> {
  const url = `${AIRoute_BASE_URL}${path}`;
  const apiKey = getAIRouteApiKey();
  ...
  if (!response.ok) {
    ...
    throw new Error(`AIRoute API error [${response.status}]: ${errorText}`);
  }
...
async function handleGetHealth() {
  ...
  try {
    const [healthRaw, resilienceRaw, rateLimitsRaw] = await Promise.allSettled([
      AIRouteFetch("/api/monitoring/health"),
      AIRouteFetch("/api/resilience"),
      AIRouteFetch("/api/rate-limits"),
    ]);
    ...
    await logToolCall("AIRoute_get_health", {}, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("AIRoute_get_health", {}, null, Date.now() - start, false, msg);
...
async function handleListCombos(args: { includeMetrics?: boolean }) {
  const start = Date.now();
  try {
    const combosRaw = await AIRouteFetch("/api/combos");
    ...
    if (args.includeMetrics) {
      metrics = toRecord(await AIRouteFetch("/api/combos/metrics").catch(() => ({})));
    }
    ...
    await logToolCall("AIRoute_list_combos", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("AIRoute_list_combos", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleGetComboMetrics(args: { comboId: string }) {
  const start = Date.now();
  try {
    const result = await AIRouteFetch(
      `/api/combos/metrics?comboId=${encodeURIComponent(args.comboId)}`
    );
    await logToolCall("AIRoute_get_combo_metrics", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("AIRoute_get_combo_metrics", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleSwitchCombo(args: { comboId: string; active: boolean }) {
  const start = Date.now();
  try {
    const result = await AIRouteFetch(`/api/combos/${encodeURIComponent(args.comboId)}`, {
      method: "PUT",
      body: JSON.stringify({ isActive: args.active }),
    });
    await logToolCall("AIRoute_switch_combo", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("AIRoute_switch_combo", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleCheckQuota(args: { provider?: string; connectionId?: string }) {
  ...
  try {
    ...
    const result = normalizeQuotaResponse(await AIRouteFetch(path), {
      provider: args.provider || null,
      connectionId: args.connectionId || null,
    });

    await logToolCall("AIRoute_check_quota", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("AIRoute_check_quota", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleRouteRequest(args: {
  ...
  try {
    ...
    const raw = (await AIRouteFetch("/v1/chat/completions", {
      method: "POST",
...
async function handleCostReport(args: { period?: string }) {
  ...
  try {
    ...
    const raw = toRecord(
      await AIRouteFetch(`/api/usage/analytics?range=${encodeURIComponent(range)}`)
    );
    ...
    await logToolCall("AIRoute_cost_report", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("AIRoute_cost_report", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleListModelsCatalog(args: { provider?: string; capability?: string }) {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("AIRoute_list_models_catalog", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleWebSearch(args: {
  ...
  try {
    ...
    const result = await AIRouteFetch("/v1/search", {
      method: "POST",
      body: JSON.stringify(body),
      signal: AbortSignal.timeout(60000),
    });
    await logToolCall("AIRoute_web_search", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("AIRoute_web_search", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleWebFetch(args: {
  ...
  try {
    ...
    const result = await AIRouteFetch("/v1/web/fetch", {
      method: "POST",
      body: JSON.stringify(body),
      signal: AbortSignal.timeout(60000),
    });
    await logToolCall("AIRoute_web_fetch", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("AIRoute_web_fetch", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export function createMcpServer(): McpServer {
  ...
  server.registerTool(
    ...
    {
      description:
        "Returns AIRoute health status including uptime, memory, circuit breakers, rate limits, and cache stats",
      inputSchema: getHealthInput,
    },
    withScopeEnforcement("AIRoute_get_health", async (args) => {
  ...
  server.registerTool(
    ...
    withScopeEnforcement("AIRoute_check_quota", (args) =>
  ...
  server.registerTool(
    "AIRoute_route_request",
    {
      description: "Sends a chat completion request through AIRoute intelligent routing",
      inputSchema: routeRequestInput,
    },
    withScopeEnforcement("AIRoute_route_request", (args) =>
  ...
  server.registerTool(
    ...
    withScopeEnforcement("AIRoute_cost_report", (args) =>
  ...
  server.registerTool(
    ...
    withScopeEnforcement("AIRoute_set_resilience_profile", (args) =>
  ...
  server.registerTool(
    ...
    withScopeEnforcement("AIRoute_test_combo", (args) =>
      handleTestCombo(testComboInput.parse(args))
    )
  );

  server.registerTool(
    "AIRoute_get_provider_metrics",
    {
      description:
  ...
  server.registerTool(
    ...
    withScopeEnforcement("AIRoute_get_session_snapshot", async (args) => {
  ...
  server.registerTool(
    "AIRoute_db_health_check",
    {
      description:
        "Diagnoses or repairs AIRoute database drift, including broken combo references and orphan quota/domain rows",
      inputSchema: dbHealthCheckInput,
    },
    withScopeEnforcement("AIRoute_db_health_check", (args) =>
      handleDbHealthCheck(dbHealthCheckInput.parse(args ?? {}))
    )
  );

  server.registerTool(
    "AIRoute_sync_pricing",
    {
      description:
        "Syncs pricing data from external sources (LiteLLM) into AIRoute without overwriting user-set prices",
      inputSchema: syncPricingInput,
    },
    withScopeEnforcement("AIRoute_sync_pricing", (args) =>
  ...
  server.registerTool(
    ...
    {
      description:
        "Performs a web search using AIRoute's search gateway. Supports multiple providers (Serper, Brave, Perplexity, Exa, Tavily) with automatic failover. Returns search results with titles, URLs, snippets, and position data.",
      inputSchema: webSearchInput,
    },
    withScopeEnforcement("AIRoute_web_search", (args) =>
  ...
  server.registerTool(
    ...
    {
      description:
        "Fetches and extracts content from a URL using AIRoute's web fetch gateway. Supports multiple providers (Firecrawl, Jina Reader, Tavily) with automatic failover. Returns the page content as markdown, HTML, links, or screenshot, along with metadata.",
      inputSchema: webFetchInput,
    },
    withScopeEnforcement("AIRoute_web_fetch", (args) => handleWebFetch(webFetchInput.parse(args)))
  );
  ...
  server.registerTool(
    ...
    withScopeEnforcement("AIRoute_cache_stats", () => handleCacheStats())
  );
  ...
  server.registerTool(
    ...
    withScopeEnforcement("AIRoute_cache_flush", (args) =>
      handleCacheFlush(cacheFlushInput.parse(args))
    )
  );

  server.registerTool(
    "AIRoute_oneproxy_fetch",
    {
      description:
    ...
    withScopeEnforcement("AIRoute_oneproxy_fetch", (args) =>
      handleOneproxyFetch(oneproxyFetchInput.parse(args))
    )
  );

  server.registerTool(
    "AIRoute_oneproxy_rotate",
    {
      description:
    ...
    withScopeEnforcement("AIRoute_oneproxy_rotate", (args) =>
      handleOneproxyRotate(oneproxyRotateInput.parse(args))
    )
  );

  server.registerTool(
    "AIRoute_oneproxy_stats",
    {
      description:
    ...
    withScopeEnforcement("AIRoute_oneproxy_stats", (args) =>
 ...
 * Called when `AIRoute --mcp` is used.
 */
export async function startMcpStdio(): Promise<void> {
  ...
  console.error("[MCP] AIRoute MCP Server starting (stdio transport)...");
  try {
    await server.connect(transport);
    console.error("[MCP] AIRoute MCP Server connected and ready.");
  } finally {
    if (closeAuditDb()) {
