...
Embedded services are locally-installed process sidecar tools that AIRoute installs, supervises, and
exposes as first-class routing targets. Unlike external providers (which are reached over the internet
via API keys), embedded services run on the same machine as AIRoute and communicate over loopback.
...
| --------------- | ----------------------------------------------- | :----------: | ------------------------------------------------------------------------------------------------------------------ |
| **9Router**     | `9router`                                      |    20130     | AI router that AIRoute can use as a sub-provider. Models exposed as `9router/{sub}/{model}`                     |
...
- AIRoute installs them under `DATA_DIR/services/{name}/` (isolated from AIRoute's own `package.json`)
- AIRoute spawns and monitors them as child processes
- AIRoute injects an ephemeral API key into the child's environment and rotates it without downtime (where applicable)
...
| API key management                    | AIRoute generates, encrypts at-rest (AES-256-GCM), and injects via env |
...
`package.json` and `node_modules/`. Does not conflict with AIRoute's own deps.
...
the next time AIRoute boots (if the service is installed).
...
- Strips incoming `cookie` and `authorization` headers (no leakage of AIRoute session)
...
2. Verify `npm` is in PATH: `which npm` from the same user account that runs AIRoute.
...
- `DATA_DIR` or its parent is not writable by the AIRoute process.
- Running inside Docker rootless without write access to the mapped volume.

**Fix:**

1. Check `DATA_DIR` (default: `~/.AIRoute/`): `ls -la ~/.AIRoute/`
2. Ensure the AIRoute process user owns the directory: `chown -R $USER ~/.AIRoute/`
...
3. Try the install manually: `npm install {package}@latest --prefix ~/.AIRoute/services/{name}/`.
...
Yes. Both services follow the same local-first model as AIRoute itself. They run
on the same machine and communicate over loopback. "Production" here means the VPS
or local server where AIRoute is deployed, not a remote cloud provider.
...
2. Check structured logs in AIRoute's pino output filtered by
   `service:supervisor` namespace.
3. Inspect the DB row: `sqlite3 ~/.AIRoute/AIRoute.db "SELECT * FROM version_manager WHERE tool='9router'"`.
