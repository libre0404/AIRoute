...
import { logger } from "@AIRoute/open-sse/utils/logger.ts";
import { resolveProxy } from "@AIRoute/open-sse/utils/networkProxy.ts";
import { proxyConfigToUrl } from "@AIRoute/open-sse/utils/proxyDispatcher.ts";
...
function getRequestPath(body: JsonRecord): string {
  ...
  try {
    ...
    return new URL(requestUrl, "http://AIRoute.local").pathname;
  } catch {
    return "/v1/responses";
  }
}
...
function getBridgeSecret(): string {
  return process.env.AIRoute_WS_BRIDGE_SECRET || "";
}
...
function getAuthRequest(body: JsonRecord): Request {
  ...
  const url = new URL(requestUrl, "http://AIRoute.local");
...
async function prepare(body: JsonRecord) {
  // Global kill-switch (feature flag AIRoute_CODEX_WS_ENABLED, default ON).
  // When disabled, the public Responses-over-WebSocket endpoint is unavailable.
  if (!isFeatureFlagEnabled("AIRoute_CODEX_WS_ENABLED")) {
  ...
  // Responses WebSocket too. The downstream client→AIRoute hop works, but the
...
export async function POST(request: Request) {
  ...
  const receivedSecret = request.headers.get("x-AIRoute-ws-bridge-secret") || "";
