...
// Key derivation: scryptSync(secret, "AIRoute-field-encryption-v1", 32)
// ─────────────────────────────────────────────────────────────────────────────

const ENC_PREFIX = "enc:v1:";
const STATIC_SALT = "AIRoute-field-encryption-v1";
const KEY_LENGTH = 32;
...
/**
 * Decrypt a value that may be encrypted with the AIRoute enc:v1: format.
...
function getDefaultDataDir() {
  ...
  const legacyDir = path.join(homeDir, ".AIRoute"); // getLegacyDotDataDir()

  // 2) Preserve the legacy ~/.AIRoute path if it already exists (avoid data loss).
  try {
  ...
  if (typeof xdgConfigHome === "string" && xdgConfigHome.trim().length > 0) {
    return path.join(path.resolve(xdgConfigHome.trim()), "AIRoute");
  }

  // 5) Default → legacy ~/.AIRoute (even if it does not exist yet).
  return legacyDir;
}
...
function sendUnauthorized(res) {
  sendError(res, 401, "Unauthorized", {
    "WWW-Authenticate": 'Basic realm="AIRoute WebDAV"',
  });
}
...
async function handlePut(req, res, absPath) {
  ...
  const tmpPath = absPath + ".AIRoute-webdav-tmp-" + Date.now();
  let writeStream;
  try {
