...
const contentSecurityPolicy = [
  ...
  // AIRoute is reached from a non-loopback host. Same-origin HTTP fetches stay covered
...
// AIRoute_BUILD_PROFILE=minimal physically removes four optional privileged
...
// The resulting artifact is intended to be published as `AIRoute-secure`
// for security-sensitive environments. See docs/security/SOCKET_DEV_FINDINGS.md.
const isMinimalBuild = process.env.AIRoute_BUILD_PROFILE === "minimal";
...
const nextConfig = {
  ...
  // AIRoute under https://host/AIRoute/). Empty by default so root-path
  ...
  basePath: process.env.AIRoute_BASE_PATH || "",
  distDir,
  ...
  turbopack: {
    ...
    resolveAlias: {
      ...
      // (Docker sets AIRoute_MITM_STUB=1 — #3390 graceful degradation). The
  ...
  // AIRoute is a proxy for AI APIs — request bodies routinely include
  ...
  experimental: {
    serverActions: {
      bodySizeLimit: process.env.AIRoute_SERVER_ACTIONS_BODY_LIMIT || "50mb",
    },
    ...
    // Next's internal router proxy defaults to 30s when this is unset. AIRoute
    ...
    // PR-2 of diegosouzapw/AIRoute#3932: tree-shake barrel re-exports so
    ...
    // the internal `@AIRoute/open-sse` workspace here: optimizePackageImports
  ...
  transpilePackages: ["@AIRoute/open-sse", "@lobehub/icons", "fumadocs-ui", "fumadocs-core"],
  ...
  webpack(config, { webpack }) {
    ...
    config.optimization.splitChunks = {
      ...
      cacheGroups: {
        ...
        // PR-2 of diegosouzapw/AIRoute#3932: isolate the heavy long-tail
  ...
  async headers() {
    return [
      ...
      // G-10: allow AIRoute's own dashboard to embed the 9Router UI via our reverse proxy.
