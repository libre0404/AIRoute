...
declare global {
  var __AIRouteShutdown:
    | { init: boolean; shuttingDown: boolean; activeRequests: number }
    | undefined;
}

function getShutdownState() {
  if (!globalThis.__AIRouteShutdown) {
    globalThis.__AIRouteShutdown = { init: false, shuttingDown: false, activeRequests: 0 };
  }
  return globalThis.__AIRouteShutdown;
}
...
async function cleanup(): Promise<void> {
  try {
    const [{ closeAuditDb }, { closeDbInstance }, { flushSpendBatchWriter }, { closeLogRotation }] =
      await Promise.all([
        import("@AIRoute/open-sse/mcp-server/audit.ts"),
        import("@/lib/db/core"),
