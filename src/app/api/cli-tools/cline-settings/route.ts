...
// Check if AIRoute is configured as OpenAI-compatible provider
const hasAIRouteConfig = (globalState: any) => {
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasAIRoute: hasAIRouteConfig(globalState),
...
// POST - Configure Cline to use AIRoute as OpenAI-compatible provider
export async function POST(request: Request) {
  ...
  try {
    ...
    secrets.openAiApiKey = apiKey || "sk_AIRoute";
...
// DELETE - Remove AIRoute OpenAI-compatible provider config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "AIRoute settings removed from Cline",
    });
  } catch (error) {
