...
AIRoute resolves its SQLite driver at startup through a 5-step fallback chain:
...
2. **Runtime-installed `better-sqlite3`** (in `~/.AIRoute/runtime/`)
...
- **Windows EBUSY**: `npm install -g AIRoute@latest` can fail if the previous
  version's `better_sqlite3.node` is locked by a running process. The runtime
  install in `~/.AIRoute/runtime/` sidesteps the global npm cache.
- **No build tools**: Some environments (corporate Windows without VS Build
  ...
  drivers ensure AIRoute still boots even if that fails.
- **Air-gapped systems**: If the npm registry is unreachable, `node:sqlite`
  or `sql.js` guarantee baseline functionality.

## Magic-byte validation

Before loading a runtime-installed `.node` file, AIRoute reads the first 8
...
AIRoute_SKIP_POSTINSTALL=1 npm install -g AIRoute

# Force-reinstall runtime better-sqlite3
rm -rf ~/.AIRoute/runtime
AIRoute  # will reinstall on next start

# Check what driver is active
AIRoute config db-info  # (if CLI command exists)
```
