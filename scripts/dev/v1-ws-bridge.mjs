...
function normalizeEndpoint(rawEndpoint) {
  ...
  let parsed;
  try {
    parsed = new URL(endpoint, "http://AIRoute.local");
  } catch {
    return null;
  }

  if (parsed.origin !== "http://AIRoute.local") {
    return null;
  }
...
class WebSocketSession {
  ...
  async executeRequest(requestId, endpoint, payload, abortController) {
    const headers = {
      ...
      "content-type": "application/json",
      "x-AIRoute-ws-session-id": this.sessionId,
      "x-AIRoute-ws-request-id": requestId,
    };
...
export function createAIRouteWsBridge({
  baseUrl,
  fetchImpl = fetch,
  ...
  if (!isText(baseUrl)) {
    throw new Error("createAIRouteWsBridge requires a baseUrl");
  }
