 ...
 * with the running AIRoute server when saving tokens.
 */
...
export function getServerCredentials(): ServerCredentials {
  return {
    server: process.env.AIRoute_SERVER || process.env.SERVER_URL || getDefaultApiServer(),
    token: process.env.AIRoute_TOKEN || process.env.CLI_TOKEN || "",
    userId: process.env.AIRoute_USER_ID || process.env.CLI_USER_ID || "cli",
  };
}
