...
AIRoute controls which **browser origins** may read cross-origin responses
...
taking seriously, but AIRoute's exposure is **narrower than the generic case**,
...
> `Access-Control-Allow-Credentials: true`. AIRoute's shared CORS path never
> does.
...
- If AIRoute runs behind a reverse proxy / tunnel (nginx, Caddy, Cloudflare
...
```bash
# Vite (5173) + Next.js (3000) dev servers calling a local AIRoute
...
## Example: reverse proxy in front of AIRoute

CORS is enforced by AIRoute itself, so the proxy generally should **not** add or
rewrite `Access-Control-*` headers (double headers break browsers). Terminate TLS
and forward — let AIRoute answer preflight:

```nginx
# nginx — forward to AIRoute; do NOT inject Access-Control-* here
location / {
...
Set the allowed browser origins in AIRoute (`CORS_ALLOWED_ORIGINS` or the
