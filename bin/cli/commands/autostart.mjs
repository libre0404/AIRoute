...
export function registerAutostart(program) {
  const cmd = program
    .command("autostart")
    .description(t("autostart.description") || "Manage AIRoute autostart at login");

  // #3331 — autostart could previously only be toggled from the tray
  // (`serve --tray`) or the Electron Appearance tab; a plain `AIRoute serve`
  // user had no path. These subcommands (with `on`/`off`/`true`/`false`
  // aliases, e.g. `AIRoute autostart on`) make it a first-class CLI action.
  cmd
    .command("enable")
    .aliases(["on", "true"])
