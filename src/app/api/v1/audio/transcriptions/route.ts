...
import { handleAudioTranscription } from "@AIRoute/open-sse/handlers/audioTranscription.ts";
...
import { attachAIRouteMetaToResponse } from "@/domain/AIRouteResponseMeta";
...
export async function POST(request) {
  ...
  if (response?.ok) {
    ...
    response = attachAIRouteMetaToResponse(response, {
      provider,
      model: resolvedModel,
      costUsd: 0,
