...
import {
  ...
  refreshCopilotToken,
} from "@AIRoute/open-sse/services/tokenRefresh.ts";
...
function isHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("AIRoute_DISABLE_TOKEN_HEALTHCHECK") ||
    isBuildProcess() ||
 ...
 * AIRoute_DISABLE_TOKEN_HEALTHCHECK switch: it lets an operator keep the
...
function getHealthCheckSkipProviders(): Set<string> {
  const raw = process.env.AIRoute_HEALTHCHECK_SKIP_PROVIDERS || "";
  return new Set(
    raw
      .split(",")
...
async function shouldHideLogs(): Promise<boolean> {
  if (
    isEnvFlagEnabled("AIRoute_HIDE_HEALTHCHECK_LOGS") ||
    isBuildProcess() ||
    isAutomatedTestProcess()
  ) {
    return true;
...
declare global {
  var __AIRouteTokenHC:
    { initialized: boolean; interval: ReturnType<typeof setInterval> | null } | undefined;
}

function getHCState() {
  if (!globalThis.__AIRouteTokenHC) {
    globalThis.__AIRouteTokenHC = { initialized: false, interval: null };
  }
  return globalThis.__AIRouteTokenHC;
}
...
export async function checkConnection(conn) {
  ...
  // Codex). 9router did not have this background sweep; it was introduced in AIRoute
