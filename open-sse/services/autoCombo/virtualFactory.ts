...
export async function createVirtualAutoCombo(
  ...
  if (candidateFilter) {
    ...
    if (narrowed.length > 0) {
      ...
      !spec?.family &&
      (process.env.AIRoute_AUTO_FREE_FALLBACK_TO_FULL_POOL === "true" ||
        process.env.AIRoute_AUTO_FREE_FALLBACK_TO_FULL_POOL === "1")
    ) {
      // Opt-in legacy behavior (category/tier only): warn loudly, then keep the full pool.
      log.warn(
        "AUTO",
        `${label} matched no connected models; falling back to the full pool (AIRoute_AUTO_FREE_FALLBACK_TO_FULL_POOL=true)`
      );
    } else {
      ...
      log.warn(
        "AUTO",
        `${label} matched no connected models; returning an empty pool.${spec?.family ? "" : ' Set AIRoute_AUTO_FREE_FALLBACK_TO_FULL_POOL=true to restore the legacy "use full pool" behavior.'}`
      );
      effectivePool = [];
    }
  }

  let weights: ScoringWeights = { ...resolveRegionWeights() };
