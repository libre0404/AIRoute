...
import { PROVIDER_ID_TO_ALIAS } from "@AIRoute/open-sse/config/providerModels.ts";
...
export async function getSettings() {
  ...
  const settings: Record<string, unknown> = {
    ...
    preferClaudeCodeForUnprefixedClaudeModels: isTruthyEnvFlag(
      process.env.AIRoute_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS
    ),
...
export async function resolveProxyForConnection(connectionId: string, apiKeyId?: string) {
  ...
  try {
    const { selectWorkingProxyFallback } = await import("@AIRoute/open-sse/utils/proxyFallback");
