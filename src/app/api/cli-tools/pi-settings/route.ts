...
// POST — write AIRoute settings to Pi config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge AIRoute settings (pi uses OpenAI-compatible config)
...
// DELETE — remove AIRoute settings from Pi config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Pi AIRoute settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
