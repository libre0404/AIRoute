...
AIRoute connects to an **Obsidian** vault as a **context source** — a local Markdown
...
vault sync (driven by `src/lib/obsidianSync.ts`). Enabling it points AIRoute at a
...
per call (per-API-key first, then global). Tools that hit the AIRoute **sync server**
...
| `obsidian_sync_status`       | AIRoute sync server status: running, vault name, port, uptime, last sync.          |
...
`open-sse/mcp-server/server.ts`, gated on `AIRoute_MCP_ENFORCE_SCOPES=true`, with
allowed scopes sourced from `AIRoute_MCP_SCOPES` or the API key's scope context. See
