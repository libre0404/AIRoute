...
declare global {
  var __AIRouteMcpAuditDb: AuditDatabase | null | undefined;
}
...
function getCachedAuditDb(): AuditDatabase | null {
  return globalThis.__AIRouteMcpAuditDb ?? null;
}

function setCachedAuditDb(database: AuditDatabase | null): void {
  globalThis.__AIRouteMcpAuditDb = database;
}
  ...
  if (!nodeSqliteFallbackAvailable()) {
    console.error(
      `[MCP Audit] better-sqlite3 native binding unavailable and Node ${process.version} ` +
        ...
        "`npm rebuild better-sqlite3` in the AIRoute install root."
    );
    return null;
  }
 ...
 * Uses the same SQLite database as the main AIRoute app.
 *
 * Driver priority:
...
async function getDb(): Promise<AuditDatabase | null> {
  ...
  try {
    ...
    const dbPath = process.env.DATA_DIR
      ...
      : join(homedir(), ".AIRoute", "storage.sqlite");
...
export async function logToolCall(
  ...
  try {
    ...
    const apiKeyId = process.env.AIRoute_API_KEY_ID || null;
