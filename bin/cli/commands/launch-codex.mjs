 ...
 *  in the shell can't shadow the AIRoute provider (defense-in-depth). Mirrors
...
const NO_AUTH_SENTINEL = "AIRoute-no-auth";
...
/**
 * Resolve the AIRoute root base URL + auth for codex, honouring (in order):
...
export function resolveCodexTarget(opts = {}) {
  ...
  if (explicit) {
    ...
    let fromCtx;
    try {
      fromCtx = resolveActiveContext(opts.context ?? process.env.AIRoute_CONTEXT)?.baseUrl;
    } catch {
      /* no context */
    }
    baseUrl = fromCtx
  ...
  if (!authToken) authToken = process.env.AIRoute_API_KEY;
  return { baseUrl, authToken };
}

/** Health-check an AIRoute root URL before launching Codex. */
 ...
 * AIRoute_API_KEY (the provider env_key) to the resolved token or a sentinel.
...
export function buildCodexEnv(baseEnv, authToken) {
  ...
  env.AIRoute_API_KEY = (authToken && String(authToken).trim()) || NO_AUTH_SENTINEL;
  return env;
}

/**
 * Codex `-c` flags that define the `AIRoute` provider inline, so launch works
 * WITHOUT a pre-existing ~/.codex/config.toml. Mirrors free-claude-code.
 * @param {string} baseUrl  AIRoute root URL (no /v1)
 * @returns {string[]}
 */
export function buildCodexProviderArgs(baseUrl) {
  return [
    "-c",
    tomlAssign("model_provider", "AIRoute"),
    "-c",
    tomlAssign("model_providers.AIRoute.name", "AIRoute"),
    "-c",
    tomlAssign("model_providers.AIRoute.base_url", `${baseUrl}/v1`),
    "-c",
    tomlAssign("model_providers.AIRoute.env_key", "AIRoute_API_KEY"),
    "-c",
    tomlAssign("model_providers.AIRoute.wire_api", "responses"),
    "-c",
    tomlAssign("model_providers.AIRoute.requires_openai_auth", false),
  ];
}
...
export async function runLaunchCodexCommand(opts = {}, codexArgs = []) {
  ...
  if (!(await healthCheck(baseUrl))) {
    console.error(
      (
        t("launch.notRunning") ||
        "AIRoute is not reachable at {port}. Start it with 'AIRoute serve'."
...
export function registerLaunchCodex(program) {
  program
    ...
    .description(
      t("launchCodex.description") || "Launch Codex CLI pointed at AIRoute (local or remote VPS)"
    )
    .option("--port <port>", "Local AIRoute port (ignored when --remote is set)", "20128")
    .option(
      "--remote <url>",
      "Remote AIRoute base URL, e.g. http://192.168.0.15:20128 (overrides --port + context)"
    )
    .option("--profile <name>", "Codex profile to activate (passed as --profile <name>)")
    .option("-p, --p <name>", "Alias for --profile")
    .option(
      "--api-key <key>",
      "AIRoute API key (overrides AIRoute_API_KEY env var for this invocation)"
    )
