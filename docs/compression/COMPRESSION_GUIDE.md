---
title: "🗜️ Prompt Compression Guide — AIRoute"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🗜️ Prompt Compression Guide — AIRoute
...
AIRoute implements a modular prompt compression pipeline that runs **proactively** before requests hit upstream providers. This means your token savings happen transparently — no changes needed to your workflow.
...
AIRoute documents compression savings from two sources: upstream project benchmarks and
AIRoute's own engine composition.
...
For overlapping tool/context payloads, the default AIRoute combo stacks the engines:
...
Send the `x-AIRoute-compression` request header to override the compression plan for a single
...
The applied plan is echoed back in the `X-AIRoute-Compression: <mode>; source=<source>` response
...
Beyond the 7 standard modes, AIRoute includes several advanced compression
...
} from "@AIRoute/open-sse/services/compression/cachingAware";
...
```ts
import { applyAging } from "@AIRoute/open-sse/services/compression/progressiveAging";
