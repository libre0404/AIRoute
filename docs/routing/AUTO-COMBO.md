---
title: "AIRoute Auto-Combo Engine"
version: 3.8.40
lastUpdated: 2026-06-28
---

# AIRoute Auto-Combo Engine
...
1. AIRoute detects `auto/` prefix in `src/sse/handlers/chat.ts`
...
AIRoute's combo engine supports **18 routing strategies** (declared in `src/shared/constants/routingStrategies.ts` → `ROUTING_STRATEGY_VALUES`). The Auto Combo engine itself is exposed under the `auto` strategy; the others are available for persisted combos.
...
import {
  registerStrategy,
  type RouterStrategy,
} from "@AIRoute/open-sse/services/autoCombo/routerStrategy";
...
| :------------------------------------- | :----------------------------------------------------------------------------- |
| `npm run test:combo:live`              | In-process real routing with `RUN_COMBO_LIVE=1`; snapshots a live AIRoute DB |
| `npm run test:combo:live:vps`          | HTTP calls against a live AIRoute server (set `COMBO_LIVE_BASE_URL`)         |
