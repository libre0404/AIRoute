...
/**
 * `AIRoute connect <host>` — remote mode.
 *
 * Logs into a remote AIRoute server and saves the result as the active context
...
export async function runConnectCommand(host, opts = {}) {
  ...
  if (!baseUrl) {
    printError("A host is required, e.g. AIRoute connect 192.168.0.15");
    return 2;
  }
  ...
  cfg.contexts[name] = {
    ...
    accessToken,
    scope,
    description: `Remote AIRoute (${host})`,
  };
  ...
  printInfo("Switch back to local with: AIRoute contexts use default");
  return 0;
}
...
export function registerConnect(program) {
  program
    ...
    .description(
      t("connect.description") || "Connect to a remote AIRoute server and enter remote mode"
    )
