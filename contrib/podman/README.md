...
Run AIRoute with Podman via **Quadlet** (systemd integration) or **podman compose**.
...
podman build --target runner-web -t AIRoute:web .
# For CLI tool support:
podman build --target runner-cli -t AIRoute:cli .
```

### 2. Copy Quadlet files to the systemd directory

```bash
mkdir -p ~/.config/containers/systemd/AIRoute
cp contrib/podman/*.container ~/.config/containers/systemd/AIRoute/
cp contrib/podman/*.network ~/.config/containers/systemd/AIRoute/
cp contrib/podman/*.volume ~/.config/containers/systemd/AIRoute/
```

### 3. Mount the project .env for secrets

Edit `~/.config/containers/systemd/AIRoute/AIRoute.container` and
uncomment/replace the `EnvironmentFile` line with the absolute path to
your project `.env`:

```
EnvironmentFile=/home/USER/code/docker/AIRoute/.env
```
...
systemctl --user start AIRoute-redis
systemctl --user start AIRoute
```

### 5. Verify

```bash
systemctl --user status AIRoute
...
```bash
journalctl --user -u AIRoute -f
```

### 6. Enable on boot

```bash
systemctl --user enable AIRoute-redis
systemctl --user enable AIRoute
```
