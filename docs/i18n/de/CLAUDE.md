...
**AIRoute** — einheitlicher KI-Proxy/Router. Ein Endpunkt, 160+ LLM-Anbieter, automatischer Fallback.
...
AIRoute hat drei verwandte, aber unterschiedliche Mechanismen für temporäre Fehler. Halten Sie ihren
...
- **Imports**: extern → intern (`@/`, `@AIRoute/open-sse`) → relativ
...
- **Pfad-Aliase**: `@/*` → `src/`, `@AIRoute/open-sse` → `open-sse/`, `@AIRoute/open-sse/*` → `open-sse/*`
- **Standardport**: 20128 (API + Dashboard am selben Port)
- **Datenverzeichnis**: `DATA_DIR` Umgebungsvariable, standardmäßig `~/.AIRoute/`
