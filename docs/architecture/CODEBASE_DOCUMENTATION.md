---
title: "AIRoute Codebase Documentation"
version: 3.8.40
lastUpdated: 2026-06-28
---

# AIRoute Codebase Documentation

> **Version:** v3.8.0
> **Last updated:** 2026-06-28
> **Audience:** Engineers contributing to AIRoute or building integrations on top of it.
>
...
- `@/*` → `src/*`
- `@AIRoute/open-sse` → `open-sse/index.ts`
- `@AIRoute/open-sse/*` → `open-sse/*`

Default HTTP port: **`20128`** (API and dashboard share the same process). Data
directory is `DATA_DIR` env var, defaulting to `~/.AIRoute/`.
...
├── open-sse/             Streaming engine workspace (@AIRoute/open-sse)
├── electron/             Desktop wrapper (Electron 41 main + preload)
├── bin/                  CLI entry points (AIRoute, reset-password)
...
| `responses.ts`, `AIRouteResponseMeta.ts` | Response shape helpers                            |
...
Separate npm workspace published as `@AIRoute/open-sse`. Owns request
...
```
bin/
├── AIRoute.mjs           Main CLI entry (Node ESM)
...
- `AIRoute` → `bin/AIRoute.mjs`
- `AIRoute-reset-password` → `bin/reset-password.mjs`
