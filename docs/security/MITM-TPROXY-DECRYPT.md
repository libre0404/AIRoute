...
TPROXY transparent decrypt is AIRoute's **5th capture mode** for the
...
configuration change** — but the process must trust the dynamic CA AIRoute
...
- Generates a long-lived CA via `generateMitmCa()` (CN `"AIRoute MITM CA"`,
...
`AIRoute-tproxy-ca.crt` (constant `TPROXY_CA_CERT_NAME`) — kept separate from
the static MITM cert's slot (`AIRoute-mitm.crt`) so the two never clobber each
other.
...
| **Dedicated CA slot**            | The dynamic CA installs to `AIRoute-tproxy-ca.crt`, never clobbering the static MITM cert.                                                                                                                                                                                                                                                                       |
...
host means anything AIRoute intercepts can be decrypted. It is gated behind the
...
- Confirm the process trusts the dynamic CA. The CA is installed under
  `AIRoute-tproxy-ca.crt`; apps with their own trust store (Firefox/Chrome NSS)
...
mode reverts the rules; if AIRoute was killed mid-session, use the AgentBridge
