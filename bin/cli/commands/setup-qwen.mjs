...
/** Merge the AIRoute modelProvider into Qwen's settings.json (preserve rest). */
export function buildQwenSettings(existing, { baseUrl, model }) {
  ...
  const providers = Array.isArray(s.modelProviders)
    ? s.modelProviders.filter((p) => p?.id !== "AIRoute")
    : [];
  providers.push({
    id: "AIRoute",
    name: "AIRoute",
    authType: "openai",
    baseUrl,
    envKey: "AIRoute_API_KEY",
  });
  s.modelProviders = providers;
  if (model) {
    s.selectedProvider = "AIRoute";
    s.model = model;
  }
  return s;
}
...
export async function runSetupQwenCommand(opts = {}) {
  ...
  printHeading("AIRoute → Qwen Code (openai-compatible)");
  ...
  printInfo(
    "\nProvide the key (settings reference AIRoute_API_KEY):  export AIRoute_API_KEY=..."
  );
...
export function registerSetupQwen(program) {
  program
    .command("setup-qwen")
    .description(
      "Configure Qwen Code for AIRoute: write ~/.qwen/settings.json (openai modelProvider)"
    )
