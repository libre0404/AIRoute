 ...
 * `X-AIRoute-Compression: <mode>; source=<source>`. Internal paths (idempotency
...
const COMPRESSION_REQUEST_HEADER = "x-AIRoute-compression";
const COMPRESSION_RESPONSE_HEADER = AIRoute_RESPONSE_HEADERS.compression;
...
/**
 * Wrap a Response so it carries `X-AIRoute-Compression: <mode>; source=request-header`
 * when the request supplied `x-AIRoute-compression` and the inner pipeline did not
