...
All AIRoute management API routes are classified into one of three protection
...
the operator put AIRoute behind an nginx/Cloudflare/Tailscale tunnel and a JWT
leaked, or auth was misconfigured — the attacker turns "call an API" into "run a
command on the host" (remote code execution). AIRoute closes this by enforcing a
...
If you run AIRoute behind a reverse proxy or tunnel (nginx, Caddy, Cloudflare
...
- A `403 LOCAL_ONLY` in AIRoute's logs for one of these paths is the guard
