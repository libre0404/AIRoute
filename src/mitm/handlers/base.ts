 ...
 *   - forwarding to the AIRoute router (Next.js API)
 *   - SSE piping
 ...
 * Routes through `@AIRoute/open-sse/utils/error.sanitizeErrorMessage` (Hard Rule #12)
...
async function safeErrorMessage(err: unknown): Promise<string> {
  try {
    const mod = (await import("@AIRoute/open-sse/utils/error")) as {
...
export abstract class MitmHandlerBase {
   ...
   *   3. Call `this.fetchRouter(...)` for the AIRoute router round-trip.
  ...
  /**
   * Forward the prepared body to the AIRoute router (Next.js API).
   * Adds AgentBridge correlation headers (`x-AIRoute-source`, `x-AIRoute-agent`)
  ...
  protected async fetchRouter(
    ...
    const base = process.env.AIRoute_BASE_URL ?? "http://127.0.0.1:20128";
