...
// Check if settings has AIRoute customModels.
// Multi-model entries are stored as `custom:AIRoute-0`, `custom:AIRoute-1`, …
// (Ported from upstream PR decolua/9router#618.)
const hasAIRouteConfig = (settings: any) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(isAIRouteCustomModel);
};
...
// POST - Update AIRoute customModels (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Remove every existing AIRoute config (multi-model: index 0..N)
    settings.customModels = settings.customModels.filter((m) => !isAIRouteCustomModel(m));

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Build and prepend AIRoute entries (one per requested model)
...
// DELETE - Remove AIRoute customModels only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove AIRoute customModels (every index, multi-model)
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter((m) => !isAIRouteCustomModel(m));
