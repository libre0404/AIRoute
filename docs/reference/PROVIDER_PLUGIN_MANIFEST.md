...
AIRoute advertises that URL to Bifrost and CLIProxyAPI via the
`X-AIRoute-Provider-Manifest-Url` request header. Set
`AIRoute_PROVIDER_MANIFEST_URL` when the sidecar needs a public or container
...
eventually be owned by a lower-latency sidecar while AIRoute keeps the
...
2. Teach Bifrost or CLIProxyAPI to import the manifest for API-key/static
   providers.
3. Route eligible providers through the sidecar behind `AIRoute_RELAY_BACKEND`
