# AIRoute CLI — Internal Conventions
...
```
AIRoute keys add openai sk-xxx
AIRoute combo switch fastest
AIRoute memory search "react hooks"
```

**Not allowed**:

```
AIRoute --add-key openai sk-xxx     # ❌ flag-as-verb
AIRoute add-key openai sk-xxx       # ❌ hyphen at the top level
```
...
- Reads base URL from `AIRoute_BASE_URL` env or `~/.AIRoute/config.json`
  (active profile).
- Injects `Authorization: Bearer ${AIRoute_API_KEY}` when available.
- Injects `x-AIRoute-cli-token` when applicable (see task 8.12).
...
- Detection order: `--lang` flag → `AIRoute_LANG` env → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
- Locale persisted via `config lang set <code>` — saves `AIRoute_LANG` to `~/.AIRoute/.env`.
