...
This document is the authoritative reference for all CI quality gates in AIRoute.
...
| `nightly-schemathesis` | OpenAPI contract fuzzing (schemathesis) against a live AIRoute using `docs/openapi.yaml` — surfaces spec violations / unhandled 500s (Fase 8 B.4) | **Advisory** |
...
Beyond the CI gates, AIRoute ships an **opt-in** `agent-lsp` scaffold
