...
// the `AIRoute setup-crush` CLI command agree on one canonical location.
...
/**
 * Crush's config uses a `providers.<id>` map. AIRoute is registered under
 * the `AIRoute` provider id as an `openai-compat` provider — same shape
...
const hasAIRouteConfig = (settings: Record<string, unknown> | null): boolean => {
  if (!settings) return false;
  const providers = settings.providers as Record<string, unknown> | undefined;
  const AIRoute = providers?.AIRoute as Record<string, unknown> | undefined;
  return (
    !!AIRoute &&
    AIRoute.type === "openai-compat" &&
    typeof AIRoute.base_url === "string" &&
    AIRoute.base_url.length > 0
  );
};
...
// POST — write AIRoute settings to crush.json (providers.AIRoute)
export async function POST(request: Request) {
  ...
  try {
    ...
    const provider: CrushProvider = {
      ...
      api_key: apiKey,
      models: [{ id: model, name: `AIRoute: ${model}`, context_window: DEFAULT_CONTEXT_WINDOW }],
    };
...
// DELETE — remove AIRoute provider from Crush config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove only the AIRoute-managed provider entry — preserve the rest
    ...
    return NextResponse.json({ success: true, message: "Crush AIRoute settings removed" });
  } catch (err) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(err) } }, { status: 500 });
  }
}
