...
AIRoute compression is built around engine contracts. A mode can run one engine directly
...
with a `22-87%` range, and a `~46%` input-compression tool. AIRoute uses the Caveman input-side
...
AIRoute install; when the optional stack is absent, LLMLingua still fail-opens. Only the three
...
- **Global npm (`npm i -g AIRoute`) / standalone** — run the install command above inside
...
| Tool                                | Scope               | Purpose                          |
| ----------------------------------- | ------------------- | -------------------------------- |
| `AIRoute_compression_status`      | `read:compression`  | Settings, analytics, cache stats |
| `AIRoute_compression_configure`   | `write:compression` | Update global settings           |
| `AIRoute_set_compression_engine`  | `write:compression` | Set mode and optional pipeline   |
| `AIRoute_list_compression_combos` | `read:compression`  | List compression combos          |
| `AIRoute_compression_combo_stats` | `read:compression`  | Read combo/engine analytics      |
