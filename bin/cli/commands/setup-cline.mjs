 ...
 * writes the CLI-mode files (matching the AIRoute dashboard) AND prints the
...
export function resolveClineTarget(opts = {}) {
  ...
  if (opts.remote) baseUrl = stripToRoot(opts.remote);
    ...
    try {
      baseUrl = stripToRoot(resolveActiveContext(opts.context ?? process.env.AIRoute_CONTEXT)?.baseUrl);
    } catch {
      /* none */
    }
...
/** Merge AIRoute openai-compatible settings into Cline's globalState (Plan + Act). */
...
export function buildClineSecrets(existing, { apiKey }) {
  return { ...(existing || {}), openAiApiKey: apiKey || "sk_AIRoute" };
}
...
export async function runSetupClineCommand(opts = {}) {
  ...
  printHeading("AIRoute → Cline (OpenAI-compatible)");
  ...
  if (dryRun) {
    ...
    console.log(`\n── [dry-run] ${secPath} ── (openAiApiKey: ${apiKey ? "set" : "sk_AIRoute"})`);
  } else {
...
export function registerSetupCline(program) {
  program
    .command("setup-cline")
    .description(
      "Configure Cline for AIRoute: write ~/.cline/data (CLI mode) + print VS Code extension settings"
    )
