...
How AIRoute estimates, records, and reports the cost of every request — and why the
...
AIRoute attributes a per-request USD cost to every completion by multiplying token
counts by a model's pricing rates. These numbers power the **Costs** dashboard, the
`AIRoute cost` / `AIRoute usage` CLI, CSV/JSON exports, and per-API-key budgets.

> **The dashboard "cost" is a savings tracker, not a bill.** AIRoute never charges you
...
- It depends on the pricing table AIRoute has for each model. A model with no pricing
...
3. **Hardcoded defaults** — shipped with AIRoute.
...
- API-key spend is buffered and flushed in batches by the
  ...
  | Env var                             | Default | Purpose                            |
  | ----------------------------------- | ------- | ---------------------------------- |
  | `AIRoute_SPEND_FLUSH_INTERVAL_MS` | `60000` | Flush interval in milliseconds.    |
  | `AIRoute_SPEND_MAX_BUFFER_SIZE`   | `1000`  | Max buffered entries before flush. |
...
AIRoute's CLI exposes cost, usage, and pricing commands (registered in
...
```bash
AIRoute cost                          # last 30d, grouped by provider
AIRoute cost --period 7d              # last 7 days
AIRoute cost --group-by model         # group by provider | model | combo | api-key | day
AIRoute cost --since 2026-06-01 --until 2026-06-13
AIRoute cost --api-key <key> --limit 50
```
...
```bash
AIRoute usage analytics --period 30d [--provider <id>]   # per-provider cost summary
AIRoute usage logs [--limit 100] [--follow] [--api-key <k>] [--search <q>]
AIRoute usage quota [--provider <id>] [--check]
AIRoute usage utilization [--api-key <k>]
AIRoute usage history [--limit 100]
AIRoute usage proxy-logs [--limit 100]

# Budgets
AIRoute usage budget list
AIRoute usage budget get [scope]
AIRoute usage budget set <amount> [--scope global] [--period monthly]
AIRoute usage budget reset [scope]
```

### `AIRoute pricing`

```bash
AIRoute pricing list [--provider <p>] [--model <m>] [--limit 200]
AIRoute pricing get <model>
AIRoute pricing sync [--provider <p>] [--force]   # POST /api/pricing/sync
AIRoute pricing diff [--model <m>]
AIRoute pricing defaults show
AIRoute pricing defaults set [--input <p>] [--output <p>] [--cache-read <p>] [--cache-write <p>]
```
...
- **All costs show $0 / "Legacy / Free".** The models in use have no pricing entry.
  Enable external sync (`PRICING_SYNC_ENABLED=true`) and run `AIRoute pricing sync`, or
...
- **Spend lags behind real time.** Per-key spend is batched; lower
  `AIRoute_SPEND_FLUSH_INTERVAL_MS` if you need fresher numbers.
