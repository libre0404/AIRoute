...
const CRYPTO_SECRETS = {
  ...
  // Generated in bin/AIRoute.mjs:ensureStorageEncryptionKey() and persisted to
  // ~/.AIRoute/.env to survive across upgrades. This prevents credential loss
  // when upgrading AIRoute (issue #1622).
  MACHINE_ID_SALT: () => `AIRoute-${randomBytes(8).toString("hex")}`,
};
 ...
 * It's generated at server startup in bin/AIRoute.mjs and persisted to
 * ~/.AIRoute/.env to survive across upgrades.
 * @see https://github.com/diegosouzapw/AIRoute/issues/1622
 */
...
function resolveDataDir(env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return join(appData, "AIRoute");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return join(resolve(xdg), "AIRoute");

  return join(homedir(), ".AIRoute");
}
        ...
        if (ENCRYPTION_BOUND_KEYS.has(key) && dbHasEncrypted) {
          log(
            `⚠️  ${key} NOT generated — encrypted credentials exist in DB. ` +
              `Restore your previous key via ~/.AIRoute/server.env, ~/.AIRoute/.env, ` +
