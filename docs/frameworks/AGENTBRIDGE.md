...
AgentBridge is AIRoute's MITM (Man-in-the-Middle) proxy that intercepts HTTPS traffic from IDE AI agents and reroutes it through AIRoute's unified routing engine. It supports **9 IDE agents** — Antigravity, Kiro, GitHub Copilot, OpenAI Codex, Cursor, Zed, Claude Code, Open Code, and Trae (investigating) — making AIRoute the broadest-coverage MITM proxy for AI coding assistants on the market.
...
- **Reroute any agent to any provider**: Copilot talking to OpenAI? Redirect it to Anthropic Claude, Gemini, or any of AIRoute's 226+ providers.
...
- **Apply AIRoute resilience**: combo routing, circuit breakers, fallbacks, and cost tracking work for IDE agent traffic too.

### Positioning vs. the market

| Feature           | 9router | anti-api | llm-interceptor | **AIRoute AgentBridge** |
| ----------------- | :-----: | :------: | :-------------: | :-----------------------: |
| Antigravity       |    ✓    |    ✓     |        —        |             ✓             |
| GitHub Copilot    |    ✓    |    ✓     |        —        |             ✓             |
| Kiro (AWS)        |    ✓    |    ✓     |        —        |             ✓             |
...
src/mitm/server.cjs  (port 443, CJS child process)
    ...
    ├── Target match? → fetch → AIRoute router (port 20128)
...
```bash
certutil -A -d sql:$HOME/.pki/nssdb -n "AIRoute AgentBridge" -t CT,, -i ~/.AIRoute/mitm/ca.crt
```
...
sudo security add-trusted-cert -d -r trustRoot \
  -k /Library/Keychains/System.keychain ~/.AIRoute/mitm/ca.crt
```

**Windows (certmgr):**

```powershell
certutil -addstore -f Root $env:USERPROFILE\.AIRoute\mitm\ca.crt
```
...
1. Point the runtime at the CA explicitly:
   ```bash
   export NODE_EXTRA_CA_CERTS=/path/to/AIRoute-agentbridge-ca.crt
   ```
...
| Source model (agent native) | Target model (AIRoute) |
| --------------------------- | ------------------------ |
...
AgentBridge intercepts credentials (OAuth tokens, API keys) that the IDE uses to authenticate with upstream providers. These are **masked before logging** (see §2.7) but are visible to AIRoute's MITM layer. First activation of each agent shows a dismissible risk notice modal.
...
The bypass list ensures that financial institutions, OAuth/SSO providers, and other sensitive hosts are **never decrypted**. Their TLS traffic passes through as a transparent TCP tunnel — AIRoute never sees the plaintext.
...
1. Verify the cert was installed: `security find-certificate -c "AIRoute AgentBridge"` (macOS) or `certutil -L -d sql:$HOME/.pki/nssdb` (Linux/NSS)
...
```bash
grep "AIRoute\|127.0.0.1.*github\|127.0.0.1.*cursor" /etc/hosts
```
...
2. Check AIRoute server logs: `APP_LOG_LEVEL=debug` in `.env`
3. Verify `AIRoute_BASE_URL` points to the correct router endpoint (default: `http://127.0.0.1:20128`)
