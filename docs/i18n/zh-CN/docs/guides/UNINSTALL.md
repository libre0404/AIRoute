...
本指南介绍如何从系统中彻底移除 AIRoute。

---

## 快速卸载（v3.6.2+）

AIRoute 提供两个内置脚本用于干净移除：
...
此命令移除 AIRoute 应用程序，但**保留** `~/.AIRoute/` 中的数据库、配置、API Key 和服务商设置。如果你计划稍后重装并希望保留现有配置，请使用此方式。
...
- 将 `AIRoute.app` 从 `/Applications` 拖入废纸篓
- 删除数据：`rm -rf ~/Library/Application Support/AIRoute`
...
AIRoute 默认将数据存储在以下位置：

| 平台          | 默认路径                      | 覆盖方式                   |
| ------------- | ----------------------------- | ------------------------- |
| Linux         | `~/.AIRoute/`               | `DATA_DIR` 环境变量        |
| macOS         | `~/.AIRoute/`               | `DATA_DIR` 环境变量        |
| Windows       | `%APPDATA%/AIRoute/`        | `DATA_DIR` 环境变量        |
| Docker        | `/app/data/`（挂载卷）         | `DATA_DIR` 环境变量        |
| XDG 兼容      | `$XDG_CONFIG_HOME/AIRoute/` | `XDG_CONFIG_HOME` 环境变量 |
