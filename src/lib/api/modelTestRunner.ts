...
import { withRateLimit } from "@AIRoute/open-sse/services/rateLimitManager";

const INTERNAL_ORIGIN = "http://AIRoute.internal";
...
export async function runSingleModelTest(
  ...
  const testBody = isRerank
    ? {
        model: fullModelStr,
        query: "What is AIRoute?",
        documents: [
          "AIRoute routes AI requests across configured providers.",
