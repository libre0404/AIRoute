 ...
 *
 * When `AIRoute_BUILD_BACKEND_ONLY=1` (or `AIRoute_BUILD_PROFILE=backend`) is set,
 * `build-next-isolated.mjs` calls `stubDashboardPages()` BEFORE `next build` and
 * `restoreDashboardPages()` in a `finally` afterward.
 *
 * WHY: AIRoute embedders that only consume the HTTP API (`/api/*`, `/v1/*`, `/v1beta/*`)
...
export const BACKEND_ONLY_STUB_MARKER =
  "/* AIRoute:backend-only-stub (auto-restored after build) */";
...
/**
 * Strip a leading `"use server"` module directive. Some AIRoute API Route Handlers
...
export function isBackendOnlyBuild(env = process.env) {
  return env.AIRoute_BUILD_BACKEND_ONLY === "1" || env.AIRoute_BUILD_PROFILE === "backend";
}
