...
export default function DroidToolCard({
  ...
  // (#618) Match any custom:AIRoute-<i> entry (multi-model).
  const isAIRouteEntry = (m) => typeof m?.id === "string" && m.id.startsWith("custom:AIRoute");

  const getConfigStatus = () => {
    if (!cliReady) return null;
    const currentConfig = droidStatus.settings?.customModels?.find(isAIRouteEntry);
    ...
    try {
      ...
      const res = await fetch("/api/cli-tools/droid-settings", {
        ...
        body: JSON.stringify({
          ...
          apiKey: !cloudEnabled ? "sk_AIRoute" : null,
          keyId: selectedKeyId,
  ...
  const getManualConfigs = () => {
    ...
    const keyToDisplay =
      selectedKeyObj?.key || (!cloudEnabled ? "sk_AIRoute" : "<API_KEY_FROM_DASHBOARD>");
          ...
          {!checkingDroid && cliReady && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current Base URL — first AIRoute entry, any index (#618) */}
                {droidStatus?.settings?.customModels?.find(isAIRouteEntry)?.baseUrl && (
                  <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {apiKeys.length > 0 ? (
                    ...
                    <span className="flex-1 text-xs text-text-muted px-2 py-1.5">
                      {cloudEnabled ? t("noApiKeysCreateOne") : t("defaultAIRouteKey")}
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={!droidStatus?.hasAIRoute}
                  loading={restoring}
                >
