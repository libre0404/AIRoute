...
AIRoute CLI commands authenticate against the local management API using a
`HMAC-SHA256(machine-id, salt)` token sent via the `x-AIRoute-cli-token`
request header.

This allows CLI subcommands (`AIRoute status`, `AIRoute providers`, etc.)
...
3. The CLI sends the token as `x-AIRoute-cli-token` on every request to
...
Set `AIRoute_CLI_SALT` to rotate the derived token without code changes.
...
export AIRoute_CLI_SALT="my-secret-salt-2026"

# Verify new token is in use
AIRoute status
```

Default salt: `AIRoute-cli-auth-v1`
...
**Opt-out:** set `AIRoute_DISABLE_CLI_TOKEN=true` (env or `.env`) to disable the CLI
