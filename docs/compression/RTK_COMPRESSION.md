...
RTK compression is AIRoute's command-aware compression engine for terminal and tool output. It is
...
`~118,000` standard tokens to `~23,900` RTK tokens, which is `79.7%` saved (`~80%`). AIRoute uses
...
- `AIRoute_RTK_TRUST_PROJECT_FILTERS=1` is set.
...
`rtkEngine` (`@AIRoute/open-sse/services/compression/engines/rtk`) is a
`CompressionEngine` and has no `updateConfig` method. Update an engine's config
through the registry helper instead:

```ts
import { updateEngineConfig } from "@AIRoute/open-sse/services/compression/engines/registry";
...
```ts
import { runRtkFilterTests } from "AIRoute/compression/engines/rtk/verify";
...
```
~/.AIRoute/rtk/filters/my-filter.json     # User-level
...
- User directory: `~/.AIRoute/rtk/filters/`
- Project directory: `<project>/.rtk/filters/`

To load filters programmatically:

```ts
import { loadRtkFilters } from "@AIRoute/open-sse/services/compression/engines/rtk/filterLoader";
...
```ts
import { readRtkRawOutput } from "AIRoute/compression/engines/rtk/rawOutput";
