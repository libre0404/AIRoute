---
title: "Codex CLI — Configuration with AIRoute"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Codex CLI — Configuration with AIRoute

Complete guide for using the Codex CLI pointed at AIRoute as an OpenAI-compatible backend.
...
export AIRoute_API_KEY="<YOUR_KEY>"
```
...
**AIRoute solves this transparently:**

```
Codex CLI
  → wire_api = "responses"
  → POST /v1/responses (AIRoute)
    → AIRoute Responses ↔ Chat Completions transformer
    → POST /chat/completions (DeepSeek / Mistral / GLM / Kimi / any provider)
```

You never need a separate translation proxy when using AIRoute. **All models use `wire_api = "responses"`** — AIRoute handles the rest.
...
| Model                                | AIRoute ID                         | Context window         | `auto_compact` | `tool_output_limit` |
| ------------------------------------ | ------------------------------------ | ---------------------- | -------------- | ------------------- |
...
All Codex models in AIRoute use the `cx/` prefix:

| Codex CLI name          | AIRoute model    |
| ----------------------- | ------------------ |
...
Other providers use their own prefix (`kmc/`, `glm/`, `ds/`, `ollamacloud/`, `opencode-go/`, `mistral/`) — the prefix matches the AIRoute provider alias.
...
## Generating profiles automatically with `AIRoute setup-codex`

If you run AIRoute on a VPS, you can auto-generate profile files from the live model catalog:

```bash
# From a VPS (uses local AIRoute on port 20128)
AIRoute setup-codex

# From any machine — point at your VPS
AIRoute setup-codex --remote http://100.x.x.x:20128 --api-key sk-xxx

# Preview without writing files
AIRoute setup-codex --remote http://100.x.x.x:20128 --dry-run

# Only generate GLM and Kimi profiles
AIRoute setup-codex --only glm,kimi

# Write to a custom directory
AIRoute setup-codex --codex-home /path/to/.codex
```
...
AIRoute can also **auto-sync** these same profile files after a successful provider model discovery/import changes the live catalog. This is **opt-in and off by default**: toggle it from the **CLI Code dashboard** ("CLI profile auto-sync" → Codex), or set `AIRoute_AUTO_SYNC_CODEX_PROFILES=true` (it also honors `CLI_ALLOW_CONFIG_WRITES`, on by default). When enabled it only writes separate `~/.codex/*.config.toml` profile files; it never changes the active/default `~/.codex/config.toml`, Codex-lb settings, auth, or provider selection.

---

## Launching Codex with `AIRoute launch-codex`

Health-checks your AIRoute instance before launching Codex:

```bash
# Launch against local AIRoute (default port 20128)
AIRoute launch-codex

# Launch with a specific profile
AIRoute launch-codex --profile kimi-k27

# Launch against a remote VPS
AIRoute launch-codex --remote http://100.x.x.x:20128/v1 --api-key sk-xxx

# Pass extra args to codex
AIRoute launch-codex --profile glm52 -- --yolo "fix this bug"
```
...
[model_providers.AIRoute.http_headers]
"X-Custom-Header" = "value"

# Headers read from env vars
[model_providers.AIRoute.env_http_headers]
"X-Trace-Id" = "TRACE_ID"

# Extra URL query params (useful for Azure api-version)
[model_providers.AIRoute.query_params]
...
```toml
[model_providers.AIRoute-main]
base_url = "http://192.168.0.1:20128/v1"
env_key  = "AIRoute_API_KEY"

[model_providers.AIRoute-tailscale]
base_url = "http://100.x.x.x:20128/v1"
env_key  = "AIRoute_API_KEY"
```
...
Verify the model exists in AIRoute with the correct prefix. Use `AIRoute models list` or open `/dashboard/providers/<provider>`.

**`Authentication error`**
Confirm `AIRoute_API_KEY` is exported: `echo $AIRoute_API_KEY`.

**`Connection refused`**
Verify AIRoute is running and the `base_url` host/port is correct for your network (local vs Tailscale vs VPS).
