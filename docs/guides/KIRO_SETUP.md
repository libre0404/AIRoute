...
This guide covers adding Kiro (AWS-hosted AI coding assistant) accounts to AIRoute,
...
## How AIRoute Solves This (v3.8.0+)

Starting with v3.8.0, AIRoute calls `registerClient()` (AWS SSO OIDC) during every
Kiro connection import. This gives each AIRoute connection its own dedicated OIDC
...
2. Choose one of:
   ...
   - **Auto-Import** — click the button; AIRoute reads credentials from the
     local kiro-cli database or `~/.aws/sso/cache`.
3. The connection is saved. AIRoute automatically registers a dedicated OIDC client for it.
...
(`clientSecretExpiresAt`). AIRoute stores this timestamp in `providerSpecificData`
...
- Ensure AIRoute can reach `https://oidc.us-east-1.amazonaws.com` (or the configured
