...
/**
 * Render the AIRoute config block in CodeWhale TOML format.
...
function renderCodewhaleConfig(baseUrl: string, apiKey: string, model: string): string {
  return [
    "# CodeWhale config — managed by AIRoute (plan 14)",
    "",
    "[openai]",
    `base_url = "${baseUrl}"`,
    `api_key = "${apiKey}"`,
 ...
 */
const hasAIRouteConfig = (content: string | null): boolean => {
  if (!content) return false;
  return content.includes("managed by AIRoute");
};
...
// POST — write AIRoute settings to CodeWhale's config.toml (primary), and
...
// DELETE — remove AIRoute CodeWhale config (primary + legacy, if present)
