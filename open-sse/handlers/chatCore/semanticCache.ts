  ...
  if (semanticCacheEnabled && isCacheableForRead(body, clientRawRequest?.headers)) {
    ...
    if (cached) {
      ...
      const headers: Record<string, string> = {
        ...
        [AIRoute_RESPONSE_HEADERS.cache]: "HIT",
      };
      // A cache HIT serves WITHOUT an upstream call, so the incremental cost billed to
      // the client is 0 (consumers that sum X-AIRoute-Response-Cost must not charge for
      // hits). The original/would-have-been cost is surfaced via X-AIRoute-Cost-Saved.
      attachAIRouteMetaHeaders(headers, {
        provider,
        model,
        cacheHit: true,
