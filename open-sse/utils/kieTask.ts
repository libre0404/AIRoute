...
const FALLBACK_KIE_CALLBACK_URL = "https://AIRoute.local/api/kie/callback";
...
function getConfiguredKieCallbackUrl(): string {
  const explicit =
    process.env.KIE_CALLBACK_URL?.trim() || process.env.AIRoute_KIE_CALLBACK_URL?.trim();
  if (explicit) return explicit;

  return (
    callbackUrlFromBaseUrl(process.env.AIRoute_PUBLIC_URL) ||
