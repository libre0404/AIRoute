...
export const LINUX_EXECUTABLE_NAMES = ["AIRoute-desktop", "AIRoute", "AIRoute"];
...
function discoverMacExecutable() {
  ...
  const candidates = [
    ...
    join(distDir, "mac", "AIRoute.app", "Contents", "MacOS", "AIRoute"),
    join(distDir, "mac-arm64", "AIRoute.app", "Contents", "MacOS", "AIRoute"),
  ];
...
function discoverWindowsExecutable() {
  const distDir = join(ROOT, "electron", "dist-electron");
  const candidates = [
    join(distDir, "win-unpacked", "AIRoute.exe"),
    join(distDir, "win-x64-unpacked", "AIRoute.exe"),
    join(distDir, "win-arm64-unpacked", "AIRoute.exe"),
  ];

  return (
    candidates.find((candidate) => existsSync(candidate)) ||
    findExecutableByName(distDir, ["AIRoute.exe"]) ||
    candidates[0]
  );
}
...
async function assertPortIsFree(url) {
  try {
    ...
    throw new Error(
      `Smoke URL already responded with HTTP ${response.status}: ${url}. Stop the existing AIRoute process before running this check.`
    );
  } catch (error) {
...
async function ensureSmokeEnvDirs(smokeEnv, dataDir) {
  ...
  if (platform() === "win32" && smokeEnv.APPDATA) {
    for (const subdir of ["AIRoute-desktop", "AIRoute", "AIRoute"]) {
...
async function main() {
  ...
  if (!existsSync(appExecutable)) {
    throw new Error(
      `Packaged AIRoute executable not found at ${appExecutable}. Build it first with \`npm run build:<target> --prefix electron\` or set ELECTRON_SMOKE_APP_EXECUTABLE.`
    );
  }
  ...
  const dataDir =
    ...
    (await mkdtemp(join(tmpdir(), "AIRoute-electron-smoke-")));
  const removeDataDir =
