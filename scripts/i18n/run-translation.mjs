#!/usr/bin/env node
/**
 * AIRoute — Docs translation pipeline (hash-based, incremental).
 *
 ...
 *   AIRoute_TRANSLATION_API_URL     e.g. https://cloud.AIRoute.dev/v1
 *   AIRoute_TRANSLATION_API_KEY     bearer token (kept out of logs)
 *   AIRoute_TRANSLATION_MODEL       e.g. cx/gpt-5.4-mini
...
function backendConfig() {
  const apiUrl = requireEnv("AIRoute_TRANSLATION_API_URL").replace(/\/$/, "");
  const apiKey = requireEnv("AIRoute_TRANSLATION_API_KEY");
  const model = requireEnv("AIRoute_TRANSLATION_MODEL");
  const timeoutMs = Number(process.env.AIRoute_TRANSLATION_TIMEOUT_MS || 60000);
...
async function main() {
  ...
  if (!opts.dryRun) {
    ...
    else backend.concurrency = Number(process.env.AIRoute_TRANSLATION_CONCURRENCY || 4);
    logInfo(
      `backend: ${backend.apiUrl} (model=${backend.model}, concurrency=${backend.concurrency}, timeout=${backend.timeoutMs}ms)`
    );
  } else {
    const apiUrl = (process.env.AIRoute_TRANSLATION_API_URL || "").replace(/\/$/, "");
