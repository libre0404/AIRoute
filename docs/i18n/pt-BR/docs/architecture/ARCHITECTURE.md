...
title: "Arquitetura do AIRoute"
version: 3.8.2
lastUpdated: 2026-05-13

---

# Arquitetura do AIRoute
...
AIRoute é um gateway de roteamento de IA local e um painel construído sobre Next.js.  
...
flowchart LR
    ...
    subgraph Router[Processo Local AIRoute]
...
- Construtor de metadados de resposta AIRoute: `src/domain/AIRouteResponseMeta.ts`
...
- arquivo: `${DATA_DIR}/storage.sqlite` (ou `$XDG_CONFIG_HOME/AIRoute/storage.sqlite` quando definido, caso contrário `~/.AIRoute/storage.sqlite`)
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Runtime do AIRoute]
...
> AIRoute v3.8.0. Para a lista canônica e continuamente atualizada, consulte
...
- resposta final do cliente retornada pelo AIRoute; respostas transmitidas são armazenadas na mesma forma de resumo compacto
...
1. `usageDb` e `localDb` compartilham a mesma política de diretório base (`DATA_DIR` -> `XDG_CONFIG_HOME/AIRoute` -> `~/.AIRoute`) com migração de arquivos legados.
...
5. O diretório `open-sse/` é publicado como o pacote de **workspace npm** `@AIRoute/open-sse`. O código-fonte o importa via `@AIRoute/open-sse/...` (resolvido pelo Next.js `transpilePackages`). Os caminhos de arquivo neste documento ainda usam o nome do diretório `open-sse/` para consistência.
...
- Construir imagem Docker: `docker build -t AIRoute .`
