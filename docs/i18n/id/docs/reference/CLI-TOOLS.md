# Panduan Pengaturan Alat CLI — AIRoute (Bahasa Indonesia)
...
untuk menggunakan **AIRoute** sebagai backend terpadu, memberikan manajemen kunci terpusat,
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Kiro / Cursor / Copilot
           │
           ▼  (semua mengarah ke AIRoute)
    http://YOUR_SERVER:20128/v1
           │
           ▼  (AIRoute meneruskan ke penyedia yang tepat)
...
| **Antigravity**    | `antigravity` | internal   | mitm            | AIRoute        |
...
## Langkah 1 — Dapatkan API Key AIRoute

1. Buka dashboard AIRoute → **API Manager** (`/dashboard/api-manager`)
...
export ANTHROPIC_API_KEY="sk-your-AIRoute-key"
...
Atau gunakan dashboard AIRoute → **CLI Tools → Cline → Apply Config**.
...
Atau gunakan dashboard AIRoute → **CLI Tools → KiloCode → Apply Config**.
...
# CLI ini menggunakan autentikasinya sendiri — AIRoute tidak diperlukan sebagai backend untuk Kiro CLI itu sendiri.
# Gunakan kiro-cli bersama AIRoute untuk alat lainnya.
kiro-cli status
```
...
> **Catatan:** Cursor merutekan permintaan melalui cloudnya sendiri. Untuk integrasi AIRoute,
> aktifkan **Cloud Endpoint** di Pengaturan AIRoute dan gunakan URL domain publik Anda.

Melalui GUI: **Settings → Models → OpenAI API Key**

- Base URL: `https://your-domain.com/v1`
- API Key: kunci AIRoute Anda

---

## Konfigurasi Otomatis Dashboard

Dashboard AIRoute mengotomatiskan konfigurasi untuk sebagian besar alat:
...
**Droid** dan **OpenClaw** adalah agen AI yang dibangun langsung ke dalam AIRoute — tidak perlu instalasi.
Keduanya berjalan sebagai rute internal dan menggunakan perutean model AIRoute secara otomatis.
...
| ------------------------- | --------------------------------- | ------------------------------------------ |
| `Connection refused`      | AIRoute tidak berjalan          | `pm2 start AIRoute`                      |
...
```bash
# Instal semua CLI dan konfigurasi untuk AIRoute (ganti dengan kunci dan URL server Anda)
AIRoute_URL="http://localhost:20128/v1"
AIRoute_KEY="sk-your-AIRoute-key"
...
cat > ~/.claude/settings.json   <<< "{\"apiBaseUrl\":\"$AIRoute_URL\",\"apiKey\":\"$AIRoute_KEY\"}"
cat > ~/.codex/config.yaml      <<< "model: auto\napiKey: $AIRoute_KEY\napiBaseUrl: $AIRoute_URL"
cat >> ~/.bashrc << EOF
export OPENAI_BASE_URL="$AIRoute_URL"
export OPENAI_API_KEY="$AIRoute_KEY"
export ANTHROPIC_BASE_URL="$AIRoute_URL"
export ANTHROPIC_API_KEY="$AIRoute_KEY"
EOF

source ~/.bashrc
echo "✅ All CLIs installed and configured for AIRoute"
```
