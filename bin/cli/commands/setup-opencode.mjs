...
const ENV_KEY_REF = "{env:AIRoute_API_KEY}";
...
export function postProcessOpencodeConfig(rawJson, opts = {}) {
  ...
  const prov = config.provider?.AIRoute;
...
export async function runSetupOpencodeCommand(opts = {}) {
  ...
  printHeading("AIRoute → OpenCode provider (openai-compatible)");
  printInfo(`Connecting to ${baseUrl} …`);

  // Deferred import: opencode.ts is TypeScript; tsx is registered by
  // bin/AIRoute.mjs before any command runs, so importing here is safe.
  let raw;
  try {
    const { generateOpencodeConfig } = await import(
      "../../../src/lib/cli-helper/config-generator/opencode.ts"
    );
    raw = await generateOpencodeConfig({ baseUrl, apiKey, model: opts.model, providerId: "AIRoute" });
  } catch (err) {
  ...
  if (dryRun) {
    ...
    printInfo(`[dry-run] ${modelCount} model(s) under provider 'AIRoute' → ${configPath}`);
    return 0;
  }

  if (!existsSync(configDir)) mkdirSync(configDir, { recursive: true });
  writeFileSync(configPath, json, "utf8");
  printSuccess(`opencode.json updated at ${configPath} (${modelCount} models under 'AIRoute')`);
  printInfo('Use it:  opencode -m AIRoute/<model> "..."   (export AIRoute_API_KEY first)');
  return 0;
}
...
export function registerSetupOpencode(program) {
  program
    ...
    .description(
      "Generate the AIRoute openai-compatible provider in ~/.config/opencode/opencode.json " +
    ...
    .option("--model <id>", "Set the default top-level model (AIRoute/<id>)")
