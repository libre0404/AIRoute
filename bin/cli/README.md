# bin/cli — AIRoute CLI internals

This directory contains the CLI runtime, helpers, and commands for the `AIRoute` binary.
...
├── sqlite.mjs              ← openAIRouteDb() — DB bootstrap
...
All HTTP calls to the AIRoute server must go through this wrapper.
...
- `baseUrl` — override base URL (default: `AIRoute_BASE_URL` env or `localhost:20128`)
- `apiKey` — override API key (default: `AIRoute_API_KEY`)
...
Locale detection order: `AIRoute_LANG` → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
...
2. `AIRoute_LANG` environment variable
...
```bash
AIRoute config lang set pt-BR       # saves to ~/.AIRoute/.env
AIRoute config lang list            # show all 42 available locales
AIRoute config lang get             # show currently active locale
```

**One-time override:**

```bash
AIRoute --lang de providers list    # run in German, not persisted
AIRoute_LANG=ja AIRoute status    # same effect via env
```
