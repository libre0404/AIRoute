...
/**
 * Interprets the `x-AIRoute-compression` request header into a plan, or null when the
...
/** Renders the X-AIRoute-Compression response header value. */
export function formatCompressionMeta(plan: DerivedPlan): string {
  return `${plan.mode}; source=${plan.source ?? "off"}`;
}

/**
 * Builds the annotation suffix for X-AIRoute-Compression from compression stats.
 ...
 * ASCII-only: this string is appended to the X-AIRoute-Compression HTTP header,
