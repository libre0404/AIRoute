...
//   3. CLI commands `AIRoute ...`     → must exist in bin/cli/commands/ or bin/
...
const ENV_VAR_ALLOWLIST = new Set([
  ...
  "VERBOSE",
  "LOG_LEVEL",
  "PORT", // generic, not AIRoute-specific
  "DATA_DIR",
  "REQUIRE_API_KEY",
  "AIRoute_BUILD_PROFILE", // build-time only
  "AIRoute_BUILD_SHA",
  "AIRoute_URL", // used by ad-hoc tooling, validated elsewhere
  "AIRoute_KEY", // ditto
  ...
  // subprocess — never read via `process.env.X` in AIRoute's own source, so the
  ...
  "OPENAI_BASE_URL", // env var AIRoute passes to downstream CLIs (AGENT_PROTOCOLS_GUIDE.md)
...
const ENDPOINT_ALLOWLIST = new Set([
  ...
  // paths on the UPSTREAM service (Claude.ai web, Blackbox), not AIRoute routes.
...
// Env var helper wrappers used across AIRoute — envInt(NAME, 5), envBool(NAME),
...
export function buildCodebaseIndex(root = ROOT) {
  ...
  // documented AIRoute env var (including external-CLI / docker / electron vars
  ...
  // Set of `AIRoute <subcommand>` strings that exist in bin/
...
const COARSE_PATTERNS = {
  ...
  // AIRoute <verb> <sub> ... — only on the same line, captures first 2 tokens
  cliCmd: /\bAIRoute\s+([a-z][a-z0-9-]+)(?:\s+([a-z][a-z0-9-]+))?/g,
...
export function scanDocFile(absPath, index, root = ROOT) {
  ...
  // 3) CLI commands: `AIRoute foo bar` — only flag when the line is in
  //    a code-like context (inside backticks or a shell block). Bare prose
  //    like "we use AIRoute and..." is not a command claim.
  for (const m of textNoCode.matchAll(COARSE_PATTERNS.cliCmd)) {
    ...
    const isShellLike = /^[ \t]*\$\s|^```sh|^```bash|^```shell|`AIRoute/.test(lineText);
    ...
    findings.push({
      ...
      value: `AIRoute ${sub}`,
      line: ln,
      msg: `AIRoute subcommand '${sub}' not registered in bin/`,
    });
  }
  ...
  for (const m of textNoCode.matchAll(COARSE_PATTERNS.fileRef)) {
    ...
    // (`@AIRoute/opencode-provider/src/index.ts`). Neither resolves from repo root.
...
export function formatHumanReport(result) {
  ...
  const KIND_LABELS = {
    ...
    "cli-cmd": "AIRoute subcommands not registered",
