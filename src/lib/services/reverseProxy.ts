 ...
 * can be iframed inside the AIRoute dashboard without CORS issues.
 *
 * Security:
 ...
 *     to prevent credential leakage between AIRoute and the embedded service.
 ...
 * Prevents AIRoute session cookies and Authorization from leaking upstream.
 */
 ...
 *   AIRoute origin, which would conflict with session management.
 * - content-security-policy / content-security-policy-report-only: the
 *   embedded service's CSP is irrelevant inside the AIRoute iframe.
 * - x-frame-options: would block the iframe entirely if set to DENY/SAMEORIGIN
 *   by the embedded service (AIRoute controls framing via its own CSP).
