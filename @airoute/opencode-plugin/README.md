# @AIRoute/opencode-plugin

> **Recommended way to use AIRoute with OpenCode.** Pulls a live model catalog from `/v1/models` (including `-low`/`-medium`/`-high`/`-thinking` variants as first-class IDs), aggregates combos via `/api/combos` using a least-common-denominator capability/limit join, sanitizes Gemini tool schemas in flight, and supports multiple side-by-side AIRoute instances out of the box.

## Why this and not `@AIRoute/opencode-provider`?

`@AIRoute/opencode-provider` is the legacy config-generator package — it writes a frozen `provider.AIRoute` block into `opencode.json` with a **hardcoded list of 8 models** ([`AIRoute_DEFAULT_OPENCODE_MODELS`](https://github.com/diegosouzapw/AIRoute/blob/main/%40AIRoute/opencode-provider/src/index.ts#L48-L56)). It works on the CLI but in the **OpenCode Desktop / Web** builds (Tauri / Electron) the runtime re-runs the model picker and the static block surfaces only a few of those — and they drift behind the live AIRoute catalog.
...
- Emitting the provider block **dynamically** in the plugin's `config`/`provider` hook — so `opencode.json` only needs the plugin entry, not a static `provider.AIRoute`
- Re-fetching on a configurable TTL (default 5 min), so new models / combo changes in the AIRoute UI appear without restarting OpenCode
...
The plugin ships **pre-built inside the `AIRoute` npm package** since v3.8.23.
If you have AIRoute installed, the plugin is already on disk:

```sh
# 1. One command — copy the plugin into OpenCode and update opencode.json
AIRoute setup opencode --auth

# 2. Follow the interactive prompt to enter your AIRoute API key
# 3. Restart OpenCode — /models lists the full live catalog
```

The `--auth` flag runs `opencode auth login --provider AIRoute` automatically.
Use `--base-url` to point at a non-default AIRoute address:

```sh
AIRoute setup opencode --base-url https://or.example.com --auth
```

### What it does

1. Locates the bundled plugin inside the AIRoute installation
2. Copies `dist/` + `package.json` to `~/.config/opencode/plugins/AIRoute/`
...
`@AIRoute/opencode-provider` or the legacy `opencode-AIRoute-auth` package are
automatically cleaned up.

### Manual install (without AIRoute CLI)

If you cannot run `AIRoute setup opencode` (local dev, CI, air-gapped), reference
the built artifact directly:

```sh
cd @AIRoute/opencode-plugin && npm run build && npm pack
# then extract into ~/.config/opencode/plugins/AIRoute-opencode-plugin/
```
...
// opencode.json
{
  ...
  "plugin": [
    [
      "./plugins/AIRoute-opencode-plugin/dist/index.js",
      {
        "providerId": "AIRoute",
...
```sh
opencode auth login --provider AIRoute
# prompts for the AIRoute API key, writes to ~/.local/share/opencode/auth.json
```

> ⚠ Use the `--provider` flag explicitly. `opencode auth login AIRoute` is parsed as a positional `url` argument by current OC releases (≤1.15.5) and fails with `fetch() URL is invalid`. Tracked upstream.

Restart OpenCode. `/models` lists the full live catalog. Variants (`-low`, `-medium`, `-high`, `-thinking`) and combos appear as first-class IDs — AIRoute is the source of truth, no client-side synthesis.
...
cd /path/to/AIRoute/@AIRoute/opencode-plugin
npm run build
npm pack
# produces AIRoute-opencode-plugin-0.1.0.tgz

# 2. Extract one copy per AIRoute endpoint
mkdir -p ~/.config/opencode/plugins/AIRoute-opencode-plugin-prod
mkdir -p ~/.config/opencode/plugins/AIRoute-opencode-plugin-preprod
tar -xzf AIRoute-opencode-plugin-0.1.0.tgz -C ~/.config/opencode/plugins/AIRoute-opencode-plugin-prod    --strip-components=1
tar -xzf AIRoute-opencode-plugin-0.1.0.tgz -C ~/.config/opencode/plugins/AIRoute-opencode-plugin-preprod --strip-components=1
```
...
```jsonc
{
  ...
  "plugin": [
    [
      "./plugins/AIRoute-opencode-plugin-prod/dist/index.js",
      {
        "providerId": "AIRoute",
        "displayName": "AIRoute",
        "baseURL": "https://or.example.com",
    ...
    [
      "./plugins/AIRoute-opencode-plugin-preprod/dist/index.js",
      {
        "providerId": "AIRoute-preprod",
        "displayName": "AIRoute Preprod",
...
```sh
opencode auth login --provider AIRoute
opencode auth login --provider AIRoute-preprod
```
...
### After publish (`@AIRoute/opencode-plugin` npm)
...
mkdir -p ~/.config/opencode/plugins/AIRoute-opencode-plugin-prod
mkdir -p ~/.config/opencode/plugins/AIRoute-opencode-plugin-preprod
npm install --prefix ~/.config/opencode/plugins/AIRoute-opencode-plugin-prod    @AIRoute/opencode-plugin
npm install --prefix ~/.config/opencode/plugins/AIRoute-opencode-plugin-preprod @AIRoute/opencode-plugin
```

`opencode.json` paths become `./plugins/AIRoute-opencode-plugin-prod/node_modules/@AIRoute/opencode-plugin/dist/index.js` (and the preprod equivalent).
...
| Variants pass-through                       | `-low`/`-medium`/`-high`/`-thinking` ship as first-class IDs from AIRoute (no client synthesis)                                                                                                                                                                                                                                                                                                           | `provider.models`            |
...
| --------------- | -------- | ------------------------------------------ | ---------------------------------------------------------- |
| `providerId`    | `string` | `"AIRoute"`                              | OpenCode provider id; must be unique across plugin entries |
| `displayName`   | `string` | `"AIRoute"` or `AIRoute (<id>)`        | Label in the OC UI                                         |
| `modelCacheTtl` | `number` | `300000` (5 min)                           | `/v1/models` TTL in ms                                     |
| `baseURL`       | `string` | resolved from `auth.json` after `/connect` | Override AIRoute base URL                                |
...
| `diskCache`           | `boolean` | `true`  | Persist the last successful `/v1/models` + `/api/combos` + enrichment + connections + compression snapshot to `${OPENCODE_DATA_DIR ?? ~/.local/share/opencode}/plugins/AIRoute-<providerId>.json`. On a subsequent cold start where `/v1/models` throws (network down / IP whitelist drop / 5xx) the static block hydrates from the snapshot so OC's model picker survives offline. Soft-fail on read/write — never blocks publishing.                                                                                                                                     |
...
With `mcpAutoEmit: true`, the plugin synthesises an `mcp.AIRoute` entry equivalent to a manual:
...
If you want a narrower-scoped Bearer for MCP (different from the chat/inference key), set `features.mcpToken`. Operator overrides win: if you already set `mcp.AIRoute` in `opencode.json`, the plugin will not overwrite it.
...
- `usableOnly: true` drops models whose canonical provider has no healthy connection in your AIRoute instance — your `/models` picker stays focused on what you can actually call.
- `diskCache: true` (default) writes a snapshot to `${OPENCODE_DATA_DIR}/plugins/AIRoute-<providerId>.json` on every healthy refresh. On a cold start where `/v1/models` is unreachable (laptop offline, IP whitelist drop), the snapshot hydrates the static block so OC still shows the catalog instead of a stub.
- `compressionMetadata: true` annotates combo display names with their pipeline using traffic-light emoji for intensity (e.g. `Combo: claude-primary [rtk🟡 → caveman🟠]`) so the picker advertises which compression each combo applies and how heavy it is at a glance. Palette: 🟢 lite/minimal · 🟡 standard · 🟠 aggressive/full · 🔴 ultra. Unknown intensities fall through to raw text (`[rtk:custom-thing]`) so the plugin never hides a value AIRoute knows but the plugin doesn't.
...
## Comparison vs `@AIRoute/opencode-provider`

[`@AIRoute/opencode-provider`](https://github.com/diegosouzapw/AIRoute/tree/main/%40AIRoute/opencode-provider) is the existing config-generator package — it writes a frozen `provider.<id>` block into `opencode.json` at build time. This plugin is the runtime integration.

|                   | `@AIRoute/opencode-plugin` (this) | `@AIRoute/opencode-provider`    |
| ----------------- | ----------------------------------- | --------------------------------- |
