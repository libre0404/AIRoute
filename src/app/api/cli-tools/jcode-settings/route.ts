...
// POST — write AIRoute settings to jcode config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge AIRoute settings (jcode uses OpenAI-compatible config)
...
// DELETE — remove AIRoute settings from jcode config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "jcode AIRoute settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
