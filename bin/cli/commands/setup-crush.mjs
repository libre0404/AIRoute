 ...
 * include /v1; the api_key may reference an env var (`$AIRoute_API_KEY`) so the
...
const API_KEY_REF = "$AIRoute_API_KEY";
...
export function resolveCrushTarget(opts = {}) {
  ...
  if (opts.remote) root = String(opts.remote).replace(/\/+$/, "");
    ...
    try {
      root = resolveActiveContext(opts.context ?? process.env.AIRoute_CONTEXT)?.baseUrl;
    } catch {
      /* none */
    }
  ...
  if (!apiKey) {
    try {
      const c = resolveActiveContext(opts.context ?? process.env.AIRoute_CONTEXT);
      apiKey = c?.accessToken || c?.apiKey;
    } catch {
      /* none */
    }
  }
  if (!apiKey) apiKey = process.env.AIRoute_API_KEY || "";
...
export function buildCrushProvider(modelIds, baseUrl) {
  ...
  for (const id of modelIds) {
    ...
    if (!cfg) continue;
    models.push({ id, name: `AIRoute: ${id}`, context_window: cfg.ctx });
  }
  return {
    type: "openai-compat",
    base_url: baseUrl,
...
/** Merge the AIRoute provider into an existing crush.json (preserve the rest). */
export function mergeCrushConfig(existing, provider) {
  const cfg = existing && typeof existing === "object" ? { ...existing } : {};
  cfg.providers = { ...(cfg.providers || {}), AIRoute: provider };
  return cfg;
}
...
export async function runSetupCrushCommand(opts = {}) {
  ...
  printHeading("AIRoute → Crush (openai-compat)");
  ...
  try {
    ...
    printInfo("Make sure AIRoute is running and --remote/--api-key are correct.");
    return 1;
  }
  ...
  if (dryRun) {
    ...
    printInfo(`[dry-run] ${provider.models.length} model(s) under providers.AIRoute → ${configPath}`);
    return 0;
  }
  mkdirSync(join(configPath, ".."), { recursive: true });
  writeFileSync(configPath, out, "utf8");
  printSuccess(`Wrote ${configPath} (${provider.models.length} models under providers.AIRoute)`);
  printInfo("Provide the key (config references $AIRoute_API_KEY):  export AIRoute_API_KEY=...");
...
export function registerSetupCrush(program) {
  program
    .command("setup-crush")
    .description("Generate the AIRoute openai-compat provider in ~/.config/crush/crush.json")
    .option("--port <port>", "Local AIRoute port (ignored when --remote is set)", "20128")
    .option("--remote <url>", "Remote AIRoute URL, e.g. http://192.168.0.15:20128")
    .option("--api-key <key>", "AIRoute API key (defaults to AIRoute_API_KEY env var)")
