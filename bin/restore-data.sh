#!/usr/bin/env bash
# bin/restore-data.sh — restore the AIRoute SQLite data volume from a snapshot
...
Stop AIRoute before running, and restart it afterwards.
EOF
}
...
ops_log "restore source: $snap → $AIRoute_DATA_DIR"
ops_confirm "Overwrite storage.sqlite at $AIRoute_DATA_DIR from $snap?" || ops_die "aborted"

# Pre-restore safety copy of the live data.
if [ -f "$AIRoute_SQLITE" ]; then
  safety="$AIRoute_BACKUPS_DIR/pre-restore_$(date -u +%Y%m%dT%H%M%SZ)"
  mkdir -p "$safety"
  if command -v sqlite3 >/dev/null 2>&1; then
    sqlite3 "$AIRoute_SQLITE" "VACUUM INTO '$safety/storage.sqlite'" \
      || cp -a "$AIRoute_SQLITE" "$safety/storage.sqlite"
  else
    cp -a "$AIRoute_SQLITE" "$safety/storage.sqlite"
  fi
...
mkdir -p "$AIRoute_DATA_DIR"
# Drop stale WAL/SHM so the restored DB is authoritative, then copy in.
rm -f "$AIRoute_SQLITE" "${AIRoute_SQLITE}-wal" "${AIRoute_SQLITE}-shm"
cp -a "$snap/storage.sqlite" "$AIRoute_SQLITE"

# Restore sibling DBs captured in the snapshot.
for f in "$snap"/*.sqlite; do
  [ -e "$f" ] || continue
  [ "$(basename "$f")" = "storage.sqlite" ] && continue
  cp -a "$f" "$AIRoute_DATA_DIR/"
done

ops_log "restore complete — restart AIRoute to pick up the restored data"
