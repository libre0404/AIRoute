...
AIRoute ships a WordPress-style plugin system. Plugins are self-contained
...
Plugins live under the AIRoute data directory:

```
~/.AIRoute/plugins/<plugin-name>/
  ├─ plugin.json
  └─ index.js          # (or whatever manifest.main points to)
```

`getDefaultPluginDir()` (`src/lib/plugins/scanner.ts`) resolves this to
`<home>/.AIRoute/plugins`, where `<home>` is taken from the `HOME` /
...
| `requires`         | object?   | `{ AIRoute?, permissions[] }`                             |
...
install (POST /api/plugins, path)
  ...
  → atomic rename into ~/.AIRoute/plugins/<name> → insert DB row
