...
For most users, **no special setup is required**. AIRoute ships a built-in
...
AIRoute also supports AgentRouter (and similar relays) through the **Claude Code
...
- AIRoute running with the `ENABLE_CC_COMPATIBLE_PROVIDER` feature flag enabled
  (see below).
...
setting an environment variable before starting AIRoute:
...
**`Invalid JSON response from provider (reset after Ns)` from the AIRoute logs** —
