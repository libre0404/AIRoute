...
AIRoute ships a cross-platform desktop app (Windows / macOS / Linux) built on
...
| App version        | `3.8.0`                    |
| App id             | `online.AIRoute.desktop` |
| Product name       | `AIRoute`                |
...
- Windows: `%APPDATA%\AIRoute`
- Linux: `$XDG_CONFIG_HOME/AIRoute` or `~/.AIRoute`
- macOS: `~/.AIRoute`
...
- Close button minimizes to tray; the tray menu has **Open AIRoute**, **Open Dashboard** (external browser), **Server Port** submenu, **Check for Updates**, **Quit**.
...
- `connect-src 'self' http://localhost:* http://127.0.0.1:* ws://localhost:* ws://127.0.0.1:* https://*.AIRoute.online https://*.AIRoute.dev`
- Dev mode adds `'unsafe-eval'` to `script-src` only

## Auto-update

Uses `electron-updater` with the GitHub provider (`diegosouzapw/AIRoute`).
...
- `AIRoute Setup X.Y.Z.exe`, `AIRoute-X.Y.Z-portable.exe` (Windows)
- `AIRoute-X.Y.Z-mac.dmg`, `AIRoute-X.Y.Z-arm64-mac.dmg` (macOS)
- `AIRoute-X.Y.Z.AppImage`, `AIRoute-desktop_X.Y.Z_amd64.deb` (Linux)

Releases are published to GitHub Releases (`diegosouzapw/AIRoute`), which is also where `electron-updater` checks for new versions.
