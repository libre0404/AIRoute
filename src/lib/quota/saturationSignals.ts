...
async function fetchCodexSaturation(
  ...
  const mod = await import("@AIRoute/open-sse/services/codexQuotaFetcher");
...
async function fetchBailianSaturation(
  ...
  const mod = await import("@AIRoute/open-sse/services/bailianQuotaFetcher");
...
async function defaultAnthropicDeps(): Promise<AnthropicSaturationDeps> {
  const [providersMod, usageMod] = await Promise.all([
    ...
    import("@AIRoute/open-sse/services/usage"),
  ]);
  return {
...
async function defaultGenericUsageFetch(
  ...
  const mod = await import("@AIRoute/open-sse/services/usage");
