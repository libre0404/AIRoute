...
// #1731v2 guard: a provider circuit-breaker-open response (503 + `X-AIRoute-Provider-Breaker`
// header / `provider_circuit_open` error code, see providerCircuitOpenResponse) is an AIRoute
...
export function isProviderCircuitOpenResult(
  ...
  const breakerHeader = result.headers?.get?.("x-AIRoute-provider-breaker");
