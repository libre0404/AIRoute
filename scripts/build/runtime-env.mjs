 ...
 * `AIRoute_MEMORY_MB`, mirroring `AIRoute serve`. Clamped to [64, 16384];
 * invalid/unset → fallback (512). The standalone launcher uses this so
 * AIRoute_MEMORY_MB can override the Docker image's NODE_OPTIONS fallback
 ...
 * when `AIRoute_MEMORY_MB` is unset. A fixed 512MB default crashed boxes with
 ...
 * explicit AIRoute_MEMORY_MB override always wins.
 ...
 * (`--max-old-space-size=…`). Used to decide whether `AIRoute serve` may
 ...
 * the user already exported. #5238: `AIRoute serve` used to UNCONDITIONALLY
