...
export const AIRoute_WEB_SEARCH_FALLBACK_TOOL_NAME = "AIRoute_web_search";
...
function buildFallbackTool(tool: JsonRecord, targetFormat?: string | null): JsonRecord {
  const name = AIRoute_WEB_SEARCH_FALLBACK_TOOL_NAME;
...
// built-in web-search tool has to be converted to the AIRoute_web_search function
...
export function supportsNativeWebSearchFallbackBypass({
  ...
  // native tool untouched instead of rewriting it to AIRoute_web_search. Mirrors the
...
export function prepareWebSearchFallbackBody<T extends JsonRecord>(
  ...
  if (!toolNames.has(AIRoute_WEB_SEARCH_FALLBACK_TOOL_NAME)) {
  ...
  if (isBuiltInWebSearchToolChoice(body.tool_choice)) {
    ...
    nextBody.tool_choice = (
      isResponsesTarget
        ? { type: "function", name: AIRoute_WEB_SEARCH_FALLBACK_TOOL_NAME }
        : { type: "function", function: { name: AIRoute_WEB_SEARCH_FALLBACK_TOOL_NAME } }
    ) as T["tool_choice"];
  }
  ...
  return {
    ...
    fallback: {
      enabled: true,
      toolName: AIRoute_WEB_SEARCH_FALLBACK_TOOL_NAME,
