 ...
 *
 * Purely informational invocations must NOT create `~/.AIRoute/.env` or write
 * a key — they never touch encrypted storage. Generating a 32-byte key and a
 * `.env` file just to print `AIRoute --version` (or `--help`) is a surprising
 ...
 *
 * Returns TRUE for everything else, INCLUDING a bare `AIRoute` (no args) — the
...
export function shouldProvisionStorageKey(argv) {
  ...
  // Bare `AIRoute` runs the default `serve` command → must provision.
