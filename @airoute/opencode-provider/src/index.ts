/**
 * OpenCode provider plugin for AIRoute AI Gateway.
 *
 * Generates an OpenCode-compatible provider object that points to a running
 * AIRoute instance. The output follows the OpenCode config schema
 * (https://opencode.ai/config.json) and delegates the runtime to
 * `@ai-sdk/openai-compatible` so OpenCode can drive any AIRoute-exposed
 ...
 *
 *     ```ts
 *     import { buildAIRouteOpenCodeConfig } from "@AIRoute/opencode-provider";
 *     const config = buildAIRouteOpenCodeConfig({
 *       baseURL: "http://localhost:20128",
 *       apiKey: "sk_AIRoute",
 *     });
 *     // config -> { $schema, provider: { AIRoute: { npm, name, options, models } } }
 *     ```
 *
 *  2. As a single-provider entry to merge into an existing opencode.json:
 *
 *     ```ts
 *     import { createAIRouteProvider } from "@AIRoute/opencode-provider";
 *     const provider = createAIRouteProvider({ baseURL, apiKey });
 *     // provider -> the value to place under provider.AIRoute in opencode.json
 *     ```
 *
...
export const AIRoute_PROVIDER_NPM = "@ai-sdk/openai-compatible" as const;
 ...
 *
 * Curated set covering the most commonly deployed AIRoute models. Synced
 * with the Alph4d0g/opencode-AIRoute-auth AIRoute_DEFAULT_MODELS constant
 * (https://github.com/Alph4d0g/opencode-AIRoute-auth, MIT) and extended
 * with Claude Code passthrough models (`cc/` prefix).
 */
export const AIRoute_DEFAULT_OPENCODE_MODELS = [
  "cc/claude-opus-4-8",
  "cc/claude-opus-4-7",
 ...
 *
 * Mirrors the capability shape used by Alph4d0g/opencode-AIRoute-auth
 * (https://github.com/Alph4d0g/opencode-AIRoute-auth, MIT).
 */
 ...
 * Matches the context lengths used by AIRoute's provider registry.
 */
export const AIRoute_DEFAULT_MODEL_CONTEXT_LENGTHS: Record<string, number> = {
 ...
 * model via `AIRouteProviderOptions.modelCapabilities`.
 */
export const AIRoute_DEFAULT_MODEL_CAPABILITIES: Record<string, ModelCapabilities> = {
...
export interface AIRouteProviderOptions {
  /** AIRoute base URL, with or without trailing `/v1`. Required. */
  baseURL: string;
  /** AIRoute API key. Required. Use `sk_AIRoute` for local instances without REQUIRE_API_KEY. */
  apiKey: string;
  /** Override the display name shown in OpenCode. Default: `"AIRoute"`. */
  displayName?: string;
   ...
   * `AIRoute_DEFAULT_MODEL_CAPABILITIES` for ids in the default catalog;
   * for custom ids the override is used verbatim.
   */
  modelCapabilities?: Record<string, ModelCapabilities>;
  /**
   * Optional per-model context-length overrides (tokens). Takes precedence
   * over the static `AIRoute_DEFAULT_MODEL_CONTEXT_LENGTHS` map but is
   ...
   * Emitted as `"AIRoute/<id>"`. When omitted the key is not written.
   */
  model?: string;
  /**
...
/** Per-model entry written under `provider.AIRoute.models[id]`. */
...
export interface OpenCodeProviderEntry {
  /** Identifier of the OpenCode runtime package that will speak to AIRoute. */
  npm: typeof AIRoute_PROVIDER_NPM;
...
export interface OpenCodeConfigDocument {
  ...
  /** Primary model for OpenCode, e.g. `"AIRoute/claude-sonnet-4-5-thinking"`. */
  model?: string;
  /** Secondary / cheap model for OpenCode, e.g. `"AIRoute/gemini-3-flash"`. */
  small_model?: string;
  provider: {
    [AIRoute_PROVIDER_KEY]: OpenCodeProviderEntry;
  };
}

function requireNonEmpty(value: unknown, field: string): string {
  if (typeof value !== "string") {
    throw new TypeError(`@AIRoute/opencode-provider: ${field} must be a string`);
  }
  const trimmed = value.trim();
  if (!trimmed) {
    throw new Error(`@AIRoute/opencode-provider: ${field} is required and cannot be empty`);
  }
  return trimmed;
}
...
export function normalizeBaseURL(rawBaseURL: string): string {
  ...
  try {
    ...
    throw new Error(
      `@AIRoute/opencode-provider: baseURL is not a valid URL: ${JSON.stringify(rawBaseURL)}`
    );
  }
  let base = trimmed;
  let end = base.length;
...
/**
 * Build the `provider.AIRoute` entry for an OpenCode config document.
 * The returned object is JSON-serialisable and safe to embed verbatim.
 */
export function createAIRouteProvider(options: AIRouteProviderOptions): OpenCodeProviderEntry {
  ...
  const modelList =
    options.models && options.models.length > 0
      ? [...options.models]
      : [...AIRoute_DEFAULT_OPENCODE_MODELS];
  ...
  for (const raw of modelList) {
    ...
    seen.add(id);
    const defaults = AIRoute_DEFAULT_MODEL_CAPABILITIES[id] ?? {};
    ...
    const rawContextLength =
      ...
      AIRoute_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    const contextLength =
  ...
  return {
    npm: AIRoute_PROVIDER_NPM,
    name: options.displayName?.trim() || "AIRoute",
...
/**
 * Build a full OpenCode config document (with `$schema` + `provider.AIRoute`).
 * Useful when scaffolding a fresh `opencode.json`.
 *
 * When `options.model` / `options.smallModel` are supplied they are emitted as
 * top-level `model` / `small_model` keys prefixed with `"AIRoute/"` so
 * OpenCode resolves them through the configured provider.
 */
export function buildAIRouteOpenCodeConfig(
  options: AIRouteProviderOptions
): OpenCodeConfigDocument {
  const doc: OpenCodeConfigDocument = {
    $schema: OPENCODE_CONFIG_SCHEMA,
    provider: {
      [AIRoute_PROVIDER_KEY]: createAIRouteProvider(options),
    },
  };

  if (options.model !== undefined) {
    const id = options.model.trim();
    if (id) doc.model = `${AIRoute_PROVIDER_KEY}/${id}`;
  }

  if (options.smallModel !== undefined) {
    const id = options.smallModel.trim();
    if (id) doc.small_model = `${AIRoute_PROVIDER_KEY}/${id}`;
  }
...
/**
 * Merge the AIRoute provider entry (and optional `model` / `small_model`
 ...
 * present are not removed. If `existing.provider.AIRoute` already exists it
...
export function mergeIntoExistingConfig(
  ...
  const partial = buildAIRouteOpenCodeConfig(options);
  ...
  merged.provider = {
    ...existingProvider,
    [AIRoute_PROVIDER_KEY]: partial.provider[AIRoute_PROVIDER_KEY],
  };
 ...
 */
export const AIRoute_MCP_DEFAULT_SCOPES = [
  "read:health",
  "read:combos",
  "read:quota",
  "read:usage",
...
export type AIRouteMCPScope = (typeof AIRoute_MCP_DEFAULT_SCOPES)[number] | string;

export interface AIRouteMCPOptions {
  /** Absolute path to the MCP server entry point (TypeScript or compiled JS). */
  serverPath: string;
  /** AIRoute API key forwarded to the MCP server as `AIRoute_API_KEY`. */
  apiKey: string;
  /**
   * Management API key used for management-scoped operations.
   * When supplied it is forwarded as `AIRoute_MANAGEMENT_API_KEY`.
   */
  managementApiKey?: string;
  /**
   * Comma-separated scope list passed as `AIRoute_MCP_SCOPES`.
   * When omitted `AIRoute_MCP_ENFORCE_SCOPES` is not set and all scopes are
   * available (development default). Pass an explicit list to restrict access.
   */
  scopes?: AIRouteMCPScope[];
  /**
...
/**
 * Build the `mcp.servers.AIRoute` entry for an OpenCode config document.
 *
 * @example
 * ```ts
 * const mcpEntry = createAIRouteMCPEntry({
 *   serverPath: "/home/user/.local/share/AIRoute/open-sse/mcp-server/server.ts",
 *   apiKey: "sk_AIRoute",
 *   managementApiKey: "sk_manage_...",
 *   scopes: ["read:health", "read:combos", "execute:completions"],
 * });
 * // Place at config.mcp.servers.AIRoute
 * ```
 */
export function createAIRouteMCPEntry(options: AIRouteMCPOptions): OpenCodeMCPServerEntry {
  ...
  const env: Record<string, string> = {
    AIRoute_API_KEY: apiKey,
  };

  if (options.managementApiKey !== undefined) {
    const mgmtKey = options.managementApiKey.trim();
    if (mgmtKey) env.AIRoute_MANAGEMENT_API_KEY = mgmtKey;
  }

  if (options.scopes !== undefined && options.scopes.length > 0) {
    env.AIRoute_MCP_ENFORCE_SCOPES = "true";
    env.AIRoute_MCP_SCOPES = options.scopes.join(",");
  }
...
async function fetchJSON<T>(url: string, apiKey: string, timeoutMs: number): Promise<T> {
  ...
  try {
    ...
    throw new Error(`@AIRoute/opencode-provider: request to ${url} failed: ${message}`);
  } finally {
    clearTimeout(timer);
  }
}
 ...
 * The shape mirrors the subset of fields that AIRoute's `/v1/models`
 * endpoint reliably provides across versions, normalised from both
 * camelCase and snake_case variants used by different AIRoute releases.
 *
 * Attribution: field-variant normalisation logic adapted from
 * https://github.com/Alph4d0g/opencode-AIRoute-auth (MIT).
 */
export interface AIRouteLiveModel {
  id: string;
  name: string;
...
/**
 * Fetch the live model catalog from a running AIRoute instance.
 *
 ...
 * `display_name`) field variants across AIRoute versions.
 *
 * Useful for dynamically populating the `models` option of
 * `createAIRouteProvider` / `buildAIRouteOpenCodeConfig` instead of
 * relying on `AIRoute_DEFAULT_OPENCODE_MODELS`.
 *
 * @param baseURL   - AIRoute base URL (with or without `/v1`).
 * @param apiKey    - AIRoute API key.
 * @param timeoutMs - Request timeout in milliseconds (default 5000).
 *
 * @example
 * ```ts
 * const models = await fetchLiveModels("http://localhost:20128", "sk_AIRoute");
 * const config = buildAIRouteOpenCodeConfig({
 *   baseURL: "http://localhost:20128",
 *   apiKey: "sk_AIRoute",
 *   models,                    // AIRouteLiveModel[] — contextLength auto-extracted
...
export async function fetchLiveModels(
  ...
  for (const raw of rawList) {
    ...
    // Extract context_length from AIRoute's /v1/models response.
    // AIRoute returns context_length in snake_case for both synced
 ...
 */
export type AIRouteCompressionOverride =
  | ""
  | "off"
  | "lite"
  | "standard"
...
export interface AIRouteCombo {
  id: string;
  name: string;
  strategy: string;
  active: boolean;
  compressionOverride: AIRouteCompressionOverride;
}

/**
 * Fetch the active routing combo list from a running AIRoute instance.
 *
...
export async function listCombos(
  ...
  for (const raw of rawList) {
    ...
    const compressionOverride = VALID_COMPRESSION_OVERRIDES.has(rawOverride)
      ? (rawOverride as AIRouteCompressionOverride)
      : "";
...
/**
 * Options for `createAIRouteComboConfig`.
 * Mirrors the subset of combo fields exposed by the AIRoute `/api/combos`
 * PATCH / POST payload that are safe to set programmatically.
 */
export interface AIRouteComboConfigOptions {
...
/**
 * Build a typed combo payload suitable for AIRoute's management API.
 *
 ...
 * @example
 * ```ts
 * const payload = createAIRouteComboConfig({
 *   name: "claude-primary",
 *   strategy: "priority",
 ...
 * ```
 */
export function createAIRouteComboConfig(
  options: AIRouteComboConfigOptions
): Record<string, unknown> {
 ...
 */
export interface AIRouteRoleOverrides {
...
/** Per-role binding used by `createAIRouteAgentBlock`. */
export interface AIRouteAgentRole extends AIRouteRoleOverrides {
  /** AIRoute model id, e.g. `"claude-sonnet-4-5-thinking"`. */
  modelId: string;
...
/** Options for `createAIRouteAgentBlock`. */
export interface AIRouteAgentBlockOptions {
  /** Per-role bindings. Keys become entries under OpenCode's `agent` block. */
  roles: Record<string, AIRouteAgentRole>;
}

/** Single entry inside the emitted OpenCode `agent` block. */
export interface OpenCodeAgentEntry extends AIRouteRoleOverrides {
  /** Always emitted as `"AIRoute/<modelId>"`. */
  model: string;
...
function buildAgentEntry(role: AIRouteAgentRole): OpenCodeAgentEntry | undefined {
  if (!role || typeof role.modelId !== "string") return undefined;
  const modelId = role.modelId.trim();
  if (!modelId) return undefined;
  const entry: OpenCodeAgentEntry = { model: `${AIRoute_PROVIDER_KEY}/${modelId}` };
 ...
 * specific AIRoute model. Useful for `.opencode/agent/*.md` defaults and
 ...
 * @example
 * ```ts
 * const agentBlock = createAIRouteAgentBlock({
 *   roles: {
 ...
 *   },
 * });
 * // -> { build: { model: "AIRoute/claude-sonnet-4-5-thinking", temperature: 0.2 }, ... }
 * ```
 */
export function createAIRouteAgentBlock(
  options: AIRouteAgentBlockOptions
...
/**
 * Per-mode binding used by `createAIRouteModesBlock`.
 *
 * @deprecated OpenCode's top-level `mode` block is deprecated in favour of
 * `agent`. Prefer `AIRouteAgentRole` + `createAIRouteAgentBlock`. This
 * type and the corresponding helper are kept for back-compat with configs
 * still using `mode:`.
 */
export interface AIRouteMode extends AIRouteAgentRole {}

/**
 * Options for `createAIRouteModesBlock`.
 *
 * @deprecated See `AIRouteMode`.
 */
export interface AIRouteModesBlockOptions {
  /** Per-mode bindings. Keys become entries under OpenCode's deprecated top-level `mode` block. */
  modes: Record<string, AIRouteMode>;
}
 ...
 * a specific AIRoute model. Emits the same shape as the `agent` block since
 * OpenCode's schema treats them identically (both reference `AgentConfig`).
 *
 * Modes with empty / missing `modelId` are skipped.
 *
 * @deprecated OpenCode's top-level `mode` block is deprecated in favour of
 * `agent`. Prefer `createAIRouteAgentBlock`. This helper is kept for
 * back-compat with configs still using `mode:`.
 *
 * @example
 * ```ts
 * const modesBlock = createAIRouteModesBlock({
 *   modes: {
 ...
 * ```
 */
export function createAIRouteModesBlock(
  options: AIRouteModesBlockOptions
...
export default createAIRouteProvider;
