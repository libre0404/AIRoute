...
AIRoute provides persistent conversational memory keyed by API key (and
...
> A client can opt a single request out with the `x-AIRoute-no-memory`
...
- `searchSemanticMemory(query, topK, scope)` — embed the query, search the
  collection filtered by `kind = "AIRoute_memory"` and optionally by
...
`qdrantApiKey`, `qdrantCollection` default `"AIRoute_memory"`,
...
- `AIRoute_memory_search` — `{apiKeyId, query?, type?, maxTokens?, limit?}`
...
- `AIRoute_memory_add` — `{apiKeyId, sessionId?, type, key, content,
metadata?}` → wraps `createMemory()`. Accepts only the 4 canonical types:
  `factual`, `episodic`, `procedural`, `semantic` (D17).
- `AIRoute_memory_clear` — `{apiKeyId, type?, olderThan?}` → lists matching
...
- For hard deletion, use `DELETE /api/memory/[id]` or `AIRoute_memory_clear`.
...
AIRoute's memory engine supports **four embedding sources** (`src/lib/memory/embedding/`). Each has different trade-offs in **latency, cost, model quality, and setup complexity**.
