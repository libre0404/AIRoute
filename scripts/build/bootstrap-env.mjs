 ...
 *   3. Preferred config .env  (DATA_DIR/.env -> ~/.AIRoute/.env -> ./.env)
...
function writeEnvFile(filePath, env) {
  const lines = [
    "# Auto-generated by AIRoute bootstrap — do not delete",
...
export function bootstrapEnv({ dataDirOverride, quiet = false } = {}) {
  ...
  // This keeps run-next / run-standalone consistent with `bin/AIRoute.mjs`.
  //
  ...
  if (needsPersist) {
    merged.AIRoute_BOOTSTRAPPED = "true";
  }
        ...
        if (row) {
          ...
          if (ciphertext?.startsWith("enc:v1:")) {
            ...
            if (parts.length >= 5) {
              ...
              // Legacy salt (fallback): scryptSync(secret, "AIRoute-field-encryption-v1", 32)
              const legacySalt = "AIRoute-field-encryption-v1";
              ...
              if (!keyMatched) {
                ...
                log(
                  "   Either restore your previous key via ~/.AIRoute/server.env or ~/.AIRoute/.env,"
                );
                log(
                  "   or run: AIRoute reset-encrypted-columns --force  (wipes credentials, keeps provider config)"
                );
              }
            }
          }
