...
<!-- skill:custom-start -->
<!-- Migrated from skills/AIRoute-compression/SKILL.md (preserved curated content) -->
...
AIRoute compresses token payloads before forwarding to providers. No code changes required — set it once, it applies to all requests transparently.
...
```bash
curl $AIRoute_URL/api/settings/compression \
...
```bash
curl -X PUT $AIRoute_URL/api/settings/compression \
  -H "Authorization: Bearer $AIRoute_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST $AIRoute_URL/api/compression/preview \
  -H "Authorization: Bearer $AIRoute_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "mode": "rtk",
...
When AIRoute is used with browser/Playwright MCP tools, it automatically compresses verbose accessibility-tree tool results. Enabled by default; configure thresholds:
...
```
AIRoute_compression_status     → current settings + savings analytics
AIRoute_compression_configure  → update mode/threshold/language
AIRoute_set_compression_engine → switch engine at runtime
```
