 ...
 *
 * Covers the `createAIRouteAuthHook(opts)` factory and its loader behaviour
...
import { createAIRouteAuthHook } from "../src/index.js";

test("createAIRouteAuthHook: default providerId is 'AIRoute'", () => {
  const hook = createAIRouteAuthHook();
  assert.equal(hook.provider, "opencode-AIRoute");
});

test("createAIRouteAuthHook: custom providerId binds to hook.provider (multi-instance)", () => {
  const hook = createAIRouteAuthHook({ providerId: "AIRoute-preprod" });
  assert.equal(hook.provider, "opencode-AIRoute-preprod");
});

test("createAIRouteAuthHook: methods[0] is type 'api' with label including displayName", () => {
  const hook = createAIRouteAuthHook();
  ...
  assert.equal(m.label, "AIRoute API Key");

  const custom = createAIRouteAuthHook({ providerId: "AIRoute-preprod" });
  assert.equal(custom.methods[0].label, "AIRoute (opencode-AIRoute-preprod) API Key");
});

test("createAIRouteAuthHook: prompts[0] uses key='apiKey' per @opencode-ai/plugin contract", () => {
  ...
  assert.ok(
    typeof (p as { message: string }).message === "string" &&
      (p as { message: string }).message.includes("AIRoute"),
...
test("loader: valid api auth → {apiKey, baseURL, fetch} when baseURL option set (T-04)", async () => {
  const hook = createAIRouteAuthHook({ baseURL: "https://or.example.com/v1" });
  const result = await hook.loader!(
