# bin/_ops-common.sh — shared helpers for the AIRoute ops runbook scripts.
#
...
ops_set_data_dir() {
  AIRoute_DATA_DIR="$1"
  AIRoute_SQLITE="${AIRoute_DATA_DIR}/storage.sqlite"
  AIRoute_BACKUPS_DIR="${DB_BACKUPS_DIR:-${AIRoute_DATA_DIR}/db_backups}"
}
ops_set_data_dir "${DATA_DIR:-$HOME/.AIRoute}"
...
ops_find_snapshot() {
  ...
  for cand in \
    "$id" \
    "$id/" \
    "$AIRoute_BACKUPS_DIR/$id" \
    "$AIRoute_BACKUPS_DIR/snapshot_$id"; do
  ...
  if [ -d "$AIRoute_BACKUPS_DIR" ]; then
    for cand in "$AIRoute_BACKUPS_DIR"/snapshot_*"$id"*; do
      [ -f "$cand/storage.sqlite" ] && { printf '%s\n' "$cand"; return 0; }
    done
  fi
  ops_die "no snapshot matching '$id' under $AIRoute_BACKUPS_DIR (run bin/snapshot-data.sh first)"
}
