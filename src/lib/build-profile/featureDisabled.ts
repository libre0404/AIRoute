...
export class FeatureDisabledError extends Error {
  ...
  constructor(featureName: string) {
    super(
      `Feature "${featureName}" is disabled in this build (AIRoute_BUILD_PROFILE=minimal). ` +
        `Install the full AIRoute artifact instead of AIRoute-secure if you need this feature.`
    );
