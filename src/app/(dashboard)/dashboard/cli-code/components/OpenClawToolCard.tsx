...
export default function OpenClawToolCard({
  ...
  const getConfigStatus = () => {
    ...
    const currentProvider = openclawStatus.settings?.models?.providers?.["AIRoute"];
  ...
  useEffect(() => {
    if (openclawStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = openclawStatus.settings?.models?.providers?.["AIRoute"];
      if (provider) {
        const primaryModel = openclawStatus.settings?.agents?.defaults?.model?.primary;
        if (primaryModel) {
          const modelId = primaryModel.replace("AIRoute/", "");
  ...
  const getManualConfigs = () => {
    ...
    const settingsContent = {
      agents: {
        defaults: {
          model: {
            primary: `AIRoute/${selectedModel || "provider/model-id"}`,
          },
        },
      },
      models: {
          ...
          {!checkingOpenclaw && openclawStatus && !cliReady && (
            <div className="flex items-center gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
              ...
              {/*
                ...
                detected locally — typical of remote AIRoute
          ...
          {!checkingOpenclaw && cliReady && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current Base URL */}
                {openclawStatus?.settings?.models?.providers?.["AIRoute"]?.baseUrl && (
                  <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={!openclawStatus?.hasAIRoute}
                  loading={restoring}
                >
