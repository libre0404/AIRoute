...
AIRoute can connect to a **Notion** workspace as a **context source** — a read/write
...
blocks — all proxied through AIRoute (with retry, timeout, and error classification)
...
> AIRoute to access with that integration (Notion's permission model is share-based,
> not workspace-wide).
...
AIRoute saves the token and immediately runs a 1-result test search; if Notion
...
`open-sse/mcp-server/server.ts` only when `AIRoute_MCP_ENFORCE_SCOPES=true`; the
caller's allowed scopes come from `AIRoute_MCP_SCOPES` (comma-separated) or the
