...
export const SKIP_UNIVERSAL_HANDOFF_FLAG = "_AIRouteSkipUniversalHandoff";
...
async function generateHandoffAsync(options: {
  ...
  const summaryBody = {
    ...
    temperature: 0.1,
    _AIRouteSkipContextRelay: true,
    _AIRouteInternalRequest: "context-handoff",
  };
...
async function generateUniversalHandoffAsync(options: {
  ...
  const summaryBody: Record<string, unknown> = {
    ...
    temperature: 0.1,
    _AIRouteSkipContextRelay: true,
    _AIRouteInternalRequest: "universal-handoff",
  };
