---
title: "Remote Mode — Drive a remote AIRoute from your laptop"
version: 3.8.40
...
Run the `AIRoute` CLI on your laptop while AIRoute itself runs somewhere else
(a VPS, a home server, another machine on your Tailnet). You log in once with
`AIRoute connect`, and from then on **every** CLI command targets that remote
server — same commands, same output, just executed against the remote.

There is no second tool to install: remote mode is the regular `AIRoute` CLI
plus scoped **access tokens**.

```bash
npm install -g AIRoute                 # the normal CLI
AIRoute connect 192.168.0.15           # log in (password → scoped token)
AIRoute models list                    # ← now lists the REMOTE server's models
AIRoute configure codex                # ← writes a local Codex profile from the remote catalog
```
...
```
your laptop                              remote AIRoute (VPS)
┌────────────────────┐                   ┌───────────────────────────────┐
│ AIRoute CLI      │  POST /api/cli/connect  (password → token)         │
...
- **Contexts** store one server each (`~/.AIRoute/config.json`, `chmod 600`).
  `AIRoute contexts use <name>` switches the active server; `default` is local.
...
```bash
AIRoute connect 192.168.0.15 --scope write
```

Options: `--port <p>` (when the host has none), `--name <ctx>` (context name),
`--scope read|write|admin`. A full URL is honoured as-is:
`AIRoute connect https://omni.example.com`.

### With a pre-generated token

Generate a scoped token in the dashboard (or with `AIRoute tokens create`) and
paste it — no password needed:

```bash
AIRoute connect 192.168.0.15 --key oma_live_xxxxxxxx
```
...
that uses the bundled Antigravity desktop client, not just AIRoute.)

There are two supported ways to connect Antigravity to a remote AIRoute.
...
#
#   AIRoute-cred-v1.eyJ2IjoxLCJ...
```

Then, in the **remote** dashboard: **Providers → Antigravity → Connect**, and
paste the `AIRoute-cred-v1.…` blob into the **Step 2** field (it accepts either
a callback URL or a credential blob). AIRoute decodes it, runs the Cloud Code
...
```bash
AIRoute tokens create --name "laptop" --scope write [--expires 30]
#   ↳ prints the secret ONCE — copy it now
AIRoute tokens list                 # masked: id, name, scope, prefix, status, expiry
AIRoute tokens revoke <id|prefix>   # revoke immediately
AIRoute tokens scopes               # explain the three scopes
```
...
`AIRoute configure` reads the **active server's** live model catalog and writes
a config on **your** machine.

```bash
AIRoute configure codex
...
# non-interactive
AIRoute configure codex --provider glm --model glm/glm-5.2 --name glm52
```

The written profile references the inference key by env var
(`AIRoute_API_KEY`) — the secret is never written to disk. For the one-time
base Codex setup (the `[model_providers.AIRoute]` block), see
...
|-----|---------|----------------|
| Codex | `AIRoute setup-codex` | `~/.codex/<name>.config.toml` profiles (per model) |
| Claude Code | `AIRoute setup-claude` | `~/.claude/profiles/<name>/settings.json` (per model) |
| OpenCode | `AIRoute setup-opencode` | `~/.config/opencode/opencode.json` — the `AIRoute` openai-compatible provider with every catalog model (run `opencode -m AIRoute/<model>`) |
| Cline | `AIRoute setup-cline` | `~/.cline/data/{globalState,secrets}.json` (CLI mode) + prints the VS Code extension settings to paste (OpenAI-compatible, Base URL **without** `/v1`) |
| Kilo Code | `AIRoute setup-kilo` | `~/.local/share/kilo/auth.json` (CLI) + VS Code `kilocode.*` settings — OpenAI-compatible, Base URL **with** `/v1` |
| Continue | `AIRoute setup-continue` | `~/.continue/config.yaml` (VS Code/JetBrains + `cn` CLI) — `provider: openai`, `apiBase` **with** `/v1`, key via `${{ secrets.AIRoute_API_KEY }}` |
| Cursor | `AIRoute setup-cursor` | prints the in-app steps (Settings → Models → Override OpenAI Base URL **with** `/v1` + key + model). Cursor config is opaque SQLite — chat panel only |
| Roo Code | `AIRoute setup-roo` | writes a Roo import JSON (`~/.AIRoute/roo-settings.json`) + sets `roo-cline.autoImportSettingsPath` + prints UI steps (OpenAI-compatible, Base URL **with** `/v1`) |
| Crush | `AIRoute setup-crush` | `~/.config/crush/crush.json` — `openai-compat` provider, `base_url` **with** `/v1`, key via `$AIRoute_API_KEY` |
| Goose | `AIRoute setup-goose` | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER=openai` + `OPENAI_HOST` **without** `/v1` + `GOOSE_MODEL`) + env recipe |
| Qwen Code | `AIRoute setup-qwen` | `~/.qwen/settings.json` — openai `modelProvider`, `baseUrl` **with** `/v1`, key via `envKey` (AIRoute_API_KEY) |
| Aider | `AIRoute setup-aider` | `~/.aider.conf.yml` (`openai-api-base` **without** `/v1` + `model: openai/<id>`) + env recipe (`aider --message --yes`) |
...
AIRoute setup-opencode --only glm,kimi        # keep only matching models
opencode -m AIRoute/glm/glm-5.2 "..."          # export AIRoute_API_KEY first
```

> OpenCode also has a richer **plugin** integration: `AIRoute setup opencode`
> (now remote-aware via `--remote`) installs `@AIRoute/opencode-plugin`.
> `setup-opencode` is the lightweight openai-compatible alternative. The API key
> is referenced via `{env:AIRoute_API_KEY}` — never written to disk.

---

## Managing contexts (switch between servers)

A **context** is a saved server (baseUrl + credential + scope). `AIRoute connect`
creates one and makes it active; from then on every command targets it. Manage and
switch between them with `AIRoute contexts`:

```bash
AIRoute contexts list            # all contexts; the active one is marked ●
AIRoute contexts current         # the active server, auth status, scope
```

```text
  | Name    | Base URL                  | Auth  | Scope | Description
● | vps     | http://100.67.86.91:20128 | token | admin | Remote AIRoute (…)
...
```bash
AIRoute contexts use vps         # → all commands now hit the remote VPS
AIRoute tokens list              #   (runs against the VPS)

AIRoute contexts use default     # → back to localhost
AIRoute tokens list              #   (runs against the local server)
```

**Add a context manually** (instead of `connect`), inspect, or rename:

```bash
AIRoute contexts add staging --url https://staging.example.com:20128 \
  --access-token oma_live_xxxx --scope write --description "staging box"
AIRoute contexts show staging    # full details for one context
AIRoute contexts rename staging stg
```
...
```bash
AIRoute contexts remove stg --yes
```
...
> revoke the token on the server with `AIRoute tokens revoke <id>` to actually
> kill access.
...
```bash
AIRoute contexts export --out contexts.json     # default: stdout
AIRoute contexts import contexts.json            # overwrite; --merge to keep existing
```
...
AIRoute connect 192.168.0.15                 # or: --key oma_live_xxxx  (no password)
AIRoute contexts current                     # shows the remote server + scope

# 2. Use it — management commands now run against the remote
AIRoute tokens create --name laptop --scope read   # mint a narrower token
AIRoute tokens list                                 # masked list, from the remote

# 3. Switch back and forth
AIRoute contexts use default                 # → local
AIRoute contexts use 192-168-0-15            # → remote again (name from `contexts list`)
...
AIRoute tokens revoke <id|prefix>            # kills access on the server
AIRoute contexts remove 192-168-0-15 --yes   # drop the local context (even if active → falls back to default), no prompt
```
...
- `AIRoute connect` reuses the login brute-force lockout + audit logging.
...
- The local context file is `~/.AIRoute/config.json` (`chmod 600`); tokens are
