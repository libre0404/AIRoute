#!/usr/bin/env node
/**
 * Post-install warm-up for AIRoute native runtimes.
 *
 * Runs after the main `scripts/build/postinstall.mjs` binary-copy step.
 * Tries to pre-resolve better-sqlite3 into ~/.AIRoute/runtime/ so that
...
if (
  process.env.AIRoute_SKIP_POSTINSTALL === "1" ||
  process.env.CI === "true" ||
  process.env.CI === "1"
) {
  process.exit(0);
...
(async () => {
  try {
    ...
    console.warn(`[AIRoute] postinstall warm-up skipped: ${err?.message ?? err}`);
  }
})();
