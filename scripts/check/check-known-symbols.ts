 ...
 *
 * Sources:
 *   - MCP_TOOLS (33 base tools: AIRoute_* + compression + agent_skills)
 *   - memoryTools (3): AIRoute_memory_*
 *   - skillTools (4): AIRoute_skills_*
...
export const KNOWN_MCP_TOOL_NAMES: readonly string[] = [
  ...
  "AIRoute_cache_stats",
  "AIRoute_cache_flush",
  "AIRoute_compression_status",
  "AIRoute_compression_configure",
  "AIRoute_set_compression_engine",
  "AIRoute_list_compression_combos",
  "AIRoute_compression_combo_stats",
  ...
  // memoryTools (3)
  "AIRoute_memory_search",
  "AIRoute_memory_add",
  "AIRoute_memory_clear",
  // skillTools (4)
  "AIRoute_skills_list",
  "AIRoute_skills_enable",
  "AIRoute_skills_execute",
  "AIRoute_skills_executions",
  // gamificationTools (8)
...
async function main(): Promise<void> {
  ...
  const executorsMod = await import("@AIRoute/open-sse/executors/index.ts");
  ...
  // ── (3) Translator pairs ──────────────────────────────────────────────────
  await import("@AIRoute/open-sse/translator/bootstrap.ts").then((m) =>
    (m.bootstrapTranslatorRegistry as () => void)()
  );
  const formatsMod = await import("@AIRoute/open-sse/translator/formats.ts");
  const registryMod = await import("@AIRoute/open-sse/translator/registry.ts");
  ...
  const { MCP_TOOLS } = await import("@AIRoute/open-sse/mcp-server/schemas/tools.ts");
  const { memoryTools } = await import("@AIRoute/open-sse/mcp-server/tools/memoryTools.ts");
  const { skillTools } = await import("@AIRoute/open-sse/mcp-server/tools/skillTools.ts");
  const { gamificationTools } =
    await import("@AIRoute/open-sse/mcp-server/tools/gamificationTools.ts");
  const { pluginTools } = await import("@AIRoute/open-sse/mcp-server/tools/pluginTools.ts");
  const { notionTools } = await import("@AIRoute/open-sse/mcp-server/tools/notionTools.ts");
  const { obsidianTools } = await import("@AIRoute/open-sse/mcp-server/tools/obsidianTools.ts");
