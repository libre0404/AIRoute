...
const OPENCODE_GO_QUOTA_URL =
  process.env.AIRoute_OPENCODE_GO_QUOTA_URL ?? "https://api.z.ai/api/monitor/usage/quota/limit";
const OPENCODE_GO_DASHBOARD_BASE_URL =
  process.env.AIRoute_OPENCODE_GO_DASHBOARD_URL ?? "https://opencode.ai/workspace";
...
const OLLAMA_CLOUD_USAGE_URL =
  process.env.AIRoute_OLLAMA_CLOUD_USAGE_URL ?? "https://ollama.com/settings";
...
function resolveOpenCodeGoDashboardConfig(
  ...
  const workspaceId =
    process.env.AIRoute_OPENCODE_GO_WORKSPACE_ID?.trim() ||
  ...
  const authCookie =
    process.env.AIRoute_OPENCODE_GO_AUTH_COOKIE?.trim() ||
    ...
    if (!res.ok) {
      ...
      return {
        message:
          ...
          "Set AIRoute_OPENCODE_GO_QUOTA_URL to a working endpoint, or follow " +
...
function resolveOllamaCloudConfig(providerSpecificData?: JsonRecord): OllamaCloudConfig {
  const cookie =
    process.env.AIRoute_OLLAMA_USAGE_COOKIE?.trim() ||
