...
export default function CodexToolCard({
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_AIRoute for localhost if no key, otherwise use selected key
      const keyToUse =
  ...
  const getManualConfigs = () => {
    const keyToUse = !cloudEnabled ? "sk_AIRoute" : "<YOUR_AIRoute_API_KEY>";

    let configContent = `# AIRoute Configuration for Codex CLI
model = "${selectedModel || CODEX_DEFAULT_MODELS[0]}"`;
...
# Utilize the built-in OpenAI provider pointed to AIRoute
