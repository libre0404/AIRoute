 ...
 *
 * Reference: AIRoute issue #2260 + comment 4459544580 (Open WebUI bypass).
 */
 ...
 * verified against opencode→AIRoute→Anthropic with claude-opus-4-7 OAuth.
 */
...
export const DEFAULT_SYSTEM_TRANSFORMS_CONFIG: SystemTransformsConfig = {
  providers: {
    [PROVIDER_CLAUDE]: {
      ...
      // usage` when opencode (or any non-claude-cli client) hits AIRoute's
 ...
 *
 * `providerId` matches AIRoute's provider key (`claude`,
...
const GLOBAL_KEY = "__AIRoute_systemTransforms_config__";
