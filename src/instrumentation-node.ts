...
/**
 * Rename a Node process title so AIRoute is identifiable in `ps`/`htop`
...
export function renameProcessTitle(currentTitle: string): string {
  ...
  return `AIRoute${currentTitle.slice("next-server".length)}`;
}

function isBackgroundServicesDisabled(): boolean {
  const raw = process.env.AIRoute_DISABLE_BACKGROUND_SERVICES;
  if (!raw) return false;
...
export async function registerNodejs(): Promise<void> {
  // Rename the process title so AIRoute is identifiable in ps/htop instead
  ...
  await import("@AIRoute/open-sse/index.ts");
  ...
  // See: https://github.com/diegosouzapw/AIRoute/issues/3625 (Part A)
  try {
  ...
  if (!isBackgroundServicesDisabled()) {
    ...
    const { initBatchProcessor } = await import("@AIRoute/open-sse/services/batchProcessor");
    initBatchProcessor();
  ...
  try {
    ...
    if (settings.systemPrompt) {
      const { setSystemPromptConfig } =
        await import("@AIRoute/open-sse/services/systemPrompt.ts");
    ...
    const { hydrateThinkingBudgetConfig } = await import(
      "@AIRoute/open-sse/services/thinkingBudget.ts"
    );
  ...
  // Enforce credential encryption policy (PIPL/DSL compliance for cn region)
  try {
    const { enforceEncryptionPolicy } = await import("@/lib/db/encryption");
    enforceEncryptionPolicy();
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[STARTUP] Encryption policy enforcement failed — refusing to start:", msg);
    process.exit(1);
  }
  ...
  if (!isBackgroundServicesDisabled()) {
    ...
    try {
      const { autoRefreshDaemon } = await import("@AIRoute/open-sse/services/autoRefreshDaemon");
    ...
    // CN provider model version auto-sync: periodically polls Chinese LLM providers
    // (DeepSeek, Doubao, Moonshot, Tencent Hunyuan, GLM, Alibaba DashScope) for
    // model list changes. Auto-enabled when AIRROUTE_REGION=cn; opt-in elsewhere via
    // settings.cnModelSyncEnabled. Non-blocking, never fatal.
    try {
      const { initCnModelModelSync } = await import("@/lib/cnProviderModelSync");
      await initCnModelModelSync();
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      console.warn("[STARTUP] CN model sync failed to start (non-fatal):", msg);
    }
    ...
    // daemon on import (gated by AIRoute_ENABLE_LIVE_WS, default ON) — but NOTHING
    ...
    // ever pulled it into a module graph, so in the published `AIRoute` bin the
