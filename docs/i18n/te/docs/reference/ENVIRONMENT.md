...
> Complete reference for every environment variable recognized by AIRoute.
...
AIRoute uses **SQLite** (via `better-sqlite3`) for all persistence. These variables control data location, encryption, and lifecycle.

| Variable                         | Default              | Source File                                     | Description                                                                                                        |
| -------------------------------- | -------------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `DATA_DIR`                       | `~/.AIRoute/`      | `src/lib/db/core.ts`                            | Root directory for SQLite DB, backups, and data files. Override for Docker volumes or custom paths.                |
...
| `AIRoute_CRYPT_KEY`            | _(unset)_            | `src/lib/db/encryption.ts`                      | **Legacy alias** for `STORAGE_ENCRYPTION_KEY`. Accepted as a fallback when the primary variable is absent.         |
| `AIRoute_API_KEY_BASE64`       | _(unset)_            | `src/lib/db/encryption.ts`                      | **Legacy alias** (Base64-encoded form) accepted as a fallback. Decoded automatically before use.                   |

### Scenarios

| Scenario              | Configuration                                                                    |
| --------------------- | -------------------------------------------------------------------------------- |
| **Local development** | Leave all defaults. DB lives at `~/.AIRoute/AIRoute.db`.                     |
...
| **CI/Testing**        | `DATA_DIR=/tmp/AIRoute-test` — ephemeral, no encryption needed.                |
...
| `AIRoute_PORT`      | _(unset)_    | `src/lib/runtime/ports.ts` | Takes precedence over `PORT` when running inside Electron or other wrappers.           |
...
AIRoute provides a two-layer defense: request-side injection scanning and response-side PII stripping.
...
> [!IMPORTANT]
> When deploying behind a reverse proxy (nginx, Caddy), `NEXT_PUBLIC_BASE_URL` **must** be set to your public URL (e.g., `https://AIRoute.example.com`). Without this, OAuth callbacks will fail because the redirect_uri won't match.
...
Controls how AIRoute discovers and launches CLI sidecars (Claude Code, Codex, etc.).
...
| `CLI_ALLOW_CONFIG_WRITES` | `false`    | `src/shared/services/cliRuntime.ts` | Allow AIRoute to write CLI config files (token refresh, session data).   |
...
```bash
# Mount host binaries into the container and tell AIRoute where they are:
...
| Variable                                | Default     | Source File                                 | Description                                                                                                                   |
| --------------------------------------- | ----------- | ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `AIRoute_BASE_URL`                    | auto-detect | `open-sse/mcp-server/server.ts`             | Explicit URL for MCP/A2A tools to reach AIRoute. Overrides localhost auto-detection.                                        |
| `AIRoute_API_KEY`                     | _(unset)_   | MCP/A2A modules                             | API key for internal MCP tool and A2A skill calls.                                                                            |
| `AIRoute_API_KEY_ID`                  | _(unset)_   | `open-sse/mcp-server/audit.ts`              | Key ID for MCP audit log attribution.                                                                                         |
| `ROUTER_API_KEY`                        | _(unset)_   | Legacy                                      | Legacy alias for `AIRoute_API_KEY`.                                                                                         |
| `AIRoute_MCP_ENFORCE_SCOPES`          | `false`     | `open-sse/mcp-server/server.ts`             | Enforce scope-based access control on MCP tool calls.                                                                         |
| `AIRoute_MCP_SCOPES`                  | _(all)_     | `open-sse/mcp-server/server.ts`             | Comma-separated scopes: `admin`, `combos`, `health`, `models`, `routing`, `budget`, `metrics`, `pricing`, `memory`, `skills`. |
...
| `AIRoute_DISABLE_BACKGROUND_SERVICES` | `false`     | `src/instrumentation-node.ts`               | Disable all background services (sync, pricing, model refresh). Useful for CI/test.                                           |
| `AIRoute_BOOTSTRAPPED`                | `false`     | `src/app/(dashboard)/dashboard/page.tsx`    | Set `true` by bootstrap script after initial setup. Controls setup wizard visibility.                                         |
| `AIRoute_ALLOW_BODY_PROJECT_OVERRIDE` | `0`         | `open-sse/executors/antigravity.ts`         | Escape hatch: allow request body to override the Antigravity project field.                                                   |

### OAuth CLI Bridge (Internal)

| Variable            | Default     | Source File                     | Description                               |
| ------------------- | ----------- | ------------------------------- | ----------------------------------------- |
| `AIRoute_SERVER`  | auto-detect | `src/lib/oauth/config/index.ts` | Server URL for CLI↔AIRoute auth bridge. |
| `AIRoute_TOKEN`   | _(unset)_   | `src/lib/oauth/config/index.ts` | Auth token for CLI bridge.                |
| `AIRoute_USER_ID` | `cli`       | `src/lib/oauth/config/index.ts` | User ID for CLI bridge sessions.          |
| `SERVER_URL`        | _(unset)_   | `src/lib/oauth/config/index.ts` | Legacy alias for `AIRoute_SERVER`.      |
| `CLI_TOKEN`         | _(unset)_   | `src/lib/oauth/config/index.ts` | Legacy alias for `AIRoute_TOKEN`.       |
| `CLI_USER_ID`       | _(unset)_   | `src/lib/oauth/config/index.ts` | Legacy alias for `AIRoute_USER_ID`.     |
...
| `AIRoute_QODER_WORKSPACE`       | Qoder                   | Alias for `QODER_CLI_WORKSPACE`.                                                  |
...
When enabled, AIRoute reorders HTTP headers and JSON body fields to match the exact signature of official CLI tools. This reduces the risk of account flagging while preserving your proxy IP.
...
| Variable                   | Default                         | Description                                                            |
| -------------------------- | ------------------------------- | ---------------------------------------------------------------------- |
| `AIRoute_MEMORY_MB`      | `512`                           | Runtime V8 heap limit. Docker standalone and `AIRoute serve` use it to set `--max-old-space-size`. |
...
| `NEXT_PUBLIC_AIRoute_E2E_MODE` | _(unset)_ | E2E test harness                          | Set `true` to enable E2E test mode (relaxed auth, test hooks). |
...
| `INSTANCE_NAME=AIRoute`                             | Present in old docs/env templates but unused at runtime. May return in a future multi-instance feature. |
