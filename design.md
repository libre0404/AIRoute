...
> **Date:** 2026-06-16 · **Scope:** unify the AIRoute dashboard (`src/`) with the marketing site (`_mono_repo/AIRouteSite/`) into **one visual identity** — same graph-paper grid background, same color tokens, standardized components.
...
The exact recipe from the site (`_mono_repo/AIRouteSite/css/base.css`): a **fixed, full-viewport pseudo-element** painting two 1px line gradients, sitting at `z-index:-1` behind all content.
...
| Site reference                    | `_mono_repo/AIRouteSite/css/tokens.css`, `css/base.css`                                                            |
