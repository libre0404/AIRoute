...
> **TL;DR**: AIRoute's compression engine is **pluggable** — you can register custom engines, ship language packs for new languages, and compose stacked pipelines. This guide shows how.
...
````ts
import type { CompressionEngine } from "AIRoute/compression/engines/types";
import { registerCompressionEngine } from "AIRoute/compression/engines/registry";
...
```
~/.AIRoute/compression/engines/my-engine.ts    # User-level
...
} from "@AIRoute/open-sse/services/compression/engines/registry";
...
Caveman-style compression uses **language-specific rule packs** to handle fillers, hedging, and verbose patterns in each natural language. AIRoute ships with **6 language packs**: `en`, `es`, `fr`, `de`, `ja`, `pt-BR`.
...
```ts
import { loadRulePack } from "AIRoute/compression/ruleLoader";
...
```
~/.AIRoute/compression/rules/hi/filler.json  # User-level
...
AIRoute selects **ONE mode per request** based on configuration, auto-trigger thresholds, and combo overrides.
...
`@AIRoute/open-sse/services/compression/strategySelector`):

```ts
import { applyStackedCompression } from "@AIRoute/open-sse/services/compression/strategySelector";
...
AIRoute's compression engines credit several upstream projects in the README
...
pack, how does that reach AIRoute?** This section is the authoritative answer.
...
rule/filter in AIRoute's own format** (see
...
AIRoute's own `smartcrusher` / `toon` / `tabular` layers. There is no live
...
1. **Don't vendor** — re-express the upstream rule/filter in AIRoute's format.
