...
function getDataDir() {
  ...
  return path.join(os.homedir(), ".AIRoute");
}
...
// Inspector. The token is injected by manager.ts (same value the AIRoute
...
async function resolveTargetIP(targetHost) {
  ...
  // Region-aware DNS: use AliDNS in CN region, Google DNS elsewhere
  const dnsServer = process.env.AIRROUTE_REGION === "cn" ? "223.5.5.5" : "8.8.8.8";
...
async function passthrough(req, res, bodyBuffer) {
  ...
  // Defense-in-depth loop guard (Gap 14). The x-AIRoute-source header is the
...
async function intercept(req, res, bodyBuffer, mappedModel, sourceModel) {
  ...
  // The AIRoute router uses these to distinguish AgentBridge traffic from
  ...
  try {
    ...
    const response = await fetch(forward.url, {
      ...
      headers: {
        ...
        "x-AIRoute-source": "agent-bridge",
        "x-AIRoute-agent": agentId,
      },
    ...
    if (!response.ok) {
      ...
      throw new Error(`AIRoute ${response.status}: ${errText}`);
    }
...
const server = https.createServer(sslOptions, async (req, res) => {
  ...
  if (req.headers["x-AIRoute-source"] === "AIRoute") {
    vlog(1, `[MITM] → PASSTHROUGH (AIRoute source loop)`);
