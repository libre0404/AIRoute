...
/**
 * Render the AIRoute provider block in Forge TOML format.
...
function renderForgeConfig(baseUrl: string, apiKey: string, model: string): string {
  ...
  return [
    "# Forge config — managed by AIRoute (plan 14)",
    "",
    "[openai]",
    `api_key = "${apiKey}"`,
 ...
 * Looks for the managed-by-AIRoute marker comment.
 */
...
// POST — write AIRoute settings to Forge config.toml
...
// DELETE — remove Forge AIRoute config
