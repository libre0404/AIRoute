...
Complete guide for configuring providers, creating combos, integrating CLI tools, and deploying AIRoute.
...
models. Haiku models do not accept the `max` effort tier, so AIRoute downgrades that
...
```json
{
  "env": {
    ...
    "ANTHROPIC_AUTH_TOKEN": "your-AIRoute-api-key"
  }
}
```
...
```json
{
  "agents": {
    "defaults": {
      "model": { "primary": "AIRoute/if/kimi-k2" }
    }
  },
  "models": {
    "providers": {
...
The CLI automatically loads `.env` from `~/.AIRoute/.env` or `./.env`.
...
```bash
git clone https://github.com/diegosouzapw/AIRoute.git
cd AIRoute && npm install && npm run build

export JWT_SECRET="your-secure-secret-change-this"
export INITIAL_PASSWORD="your-password"
export DATA_DIR="/var/lib/AIRoute"
export PORT="20128"
...
npm run start
# Or: pm2 start npm --name AIRoute -- start
```
...
pm2 start npm --name AIRoute -- start

# Or with custom memory limit
AIRoute_MEMORY_MB=512 pm2 start npm --name AIRoute -- start
...
docker build -t AIRoute:cli .

# Portable mode (recommended)
docker run -d --name AIRoute -p 20128:20128 --env-file ./.env -v AIRoute-data:/app/data AIRoute:cli
```
...
Void Linux users can package and install AIRoute natively using the `xbps-src` cross-compilation framework. This automates the Node.js standalone build along with the required `better-sqlite3` native bindings.
...
| --------------------------------------- | ------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| `JWT_SECRET`                            | `AIRoute-default-secret-change-me` | JWT signing secret (**change in production**)                                                             |
| `INITIAL_PASSWORD`                      | `CHANGEME`                           | First login password                                                                                      |
| `DATA_DIR`                              | `~/.AIRoute`                       | Data directory (db, usage, logs)                                                                          |
...
| `NEXT_PUBLIC_CLOUD_URL`                 | `https://AIRoute.dev`              | Cloud sync endpoint base URL (replaces legacy `CLOUD_URL`)                                                |
...
| `AIRoute_MEMORY_MB`                   | `512`                                | Node.js heap limit in MB                                                                                  |
...
- **Semantic Cache** — Auto-caches non-streaming, temperature=0 responses (bypass with `X-AIRoute-No-Cache: true`)
- **Request Idempotency** — Deduplicates requests within 5s via `Idempotency-Key` or `X-Request-Id` header
- **Progress Tracking** — Opt-in SSE `event: progress` events via `X-AIRoute-Progress: true` header

---

### Translator Playground

Access via **Dashboard → Translator**. Debug and visualize how AIRoute translates API requests between providers.
...
AIRoute also accepts `x_session_id` and returns the effective session key in `X-AIRoute-Session-Id`.
...
AIRoute implements provider-level resilience with five components:
...
4. **Wait For Cooldown** — If every candidate connection is already cooling down, AIRoute can wait for the earliest cooldown and retry the same client request automatically.
...
- Migrate AIRoute between machines
...
AIRoute supports audio transcription via the OpenAI-compatible endpoint:
...
AIRoute clamps it down to that cap before sending the upstream request.
...
AIRoute is both an **MCP server** (Model Context Protocol) and an **A2A server** (Agent-to-Agent JSON-RPC 2.0). Any MCP-compatible IDE or agent host can call AIRoute tools directly — no extra wrapper required.

### MCP transports

- **SSE**: `http://localhost:20128/api/mcp/sse`
- **Streamable HTTP**: `http://localhost:20128/api/mcp/stream`
- **stdio**: `AIRoute --mcp` (for IDE plugins that prefer stdio)
...
AIRoute exposes an extensible **skill framework** (`src/lib/skills/`) so agents and the A2A endpoint can run domain-specific routines (e.g. `code-review`, `summarize`, `extract-facts`, `web-research`).
...
AIRoute persists **long-term conversational memory** with hybrid retrieval:
...
Subscribe to AIRoute events for real-time monitoring and automation.
...
- Every payload includes `X-AIRoute-Signature` (HMAC-SHA256) for verification
...
AIRoute integrates with cloud coding agents (**OpenAI Codex Cloud**, **Devin**, **Jules**, **Antigravity**) so you can dispatch long-running tasks from the same dashboard that handles your local routing.

- Create tasks in **Dashboard → Cloud Agents** or via `POST /api/v1/agents/tasks`
- Track status, logs, and artifacts per task
- Bring-your-own API key per provider — credentials never leave the AIRoute instance
...
You can manage every AIRoute resource (providers, combos, keys, settings) over HTTP using a **Bearer key with the `manage` scope**.
...
curl http://localhost:20128/api/providers \
  -H "Authorization: Bearer $AIRoute_MANAGE_KEY"
...
curl http://localhost:20128/api/keys -H "Authorization: Bearer $AIRoute_MANAGE_KEY"
curl -X POST http://localhost:20128/api/keys -H "Authorization: Bearer $AIRoute_MANAGE_KEY" \
...
AIRoute ships an internal CLI (`AIRoute …`) for setup, diagnostics, and runtime control. This is **separate from the "CLI Tools" page in the dashboard**, which configures third-party CLIs (Claude Code, Cursor, Codex, Cline, …) so they can talk to AIRoute.

```bash
AIRoute setup                    # Interactive wizard (password, providers, combos)
AIRoute setup --non-interactive  # CI-friendly
AIRoute doctor                   # Health diagnostics (data dir, DB, providers, ports)
AIRoute providers available      # List supported providers
AIRoute providers list           # List configured connections
AIRoute providers test <id>      # Live test a provider connection
AIRoute combos list              # List combos
AIRoute combos switch <name>     # Set default combo
AIRoute models                   # List available models (--json, --search)
AIRoute keys add | list | remove # Manage API keys from the terminal
AIRoute backup                   # Snapshot config + DB
AIRoute restore [<timestamp>]    # Restore from a snapshot
AIRoute health                   # Detailed health (breakers, cache, memory)
AIRoute quota                    # Provider quota usage
AIRoute mcp status               # MCP server status
AIRoute a2a status               # A2A server status
AIRoute tunnel list|create|stop  # Cloudflare/Tailscale/ngrok tunnels
AIRoute reset-password           # Reset the admin password
AIRoute --mcp                    # Start MCP server over stdio
AIRoute --port 3000              # Start the server on a custom port
```

Tip: pair `AIRoute doctor --json` with your monitoring tool to alert on unhealthy provider connections.

---

## 🖥️ Desktop Application (Electron)

AIRoute is available as a native desktop application for Windows, macOS, and Linux.
...
| Variable              | Default | Description                      |
| --------------------- | ------- | -------------------------------- |
| `AIRoute_PORT`      | `20128` | Server port                      |
| `AIRoute_MEMORY_MB` | `512`   | Node.js heap limit (64–16384 MB) |
