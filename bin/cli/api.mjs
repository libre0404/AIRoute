...
export function getBaseUrl(opts = {}) {
  ...
  const envUrl = process.env.AIRoute_BASE_URL;
  if (envUrl) return stripTrailingSlash(envUrl);

  // Resolve from the active context (canonical store + legacy profile fallback).
  // This is what makes "remote mode" work: `AIRoute contexts use <remote>`
...
export async function buildHeaders(opts) {
  ...
  //
  // The active context's scoped token MUST win over the ambient AIRoute_API_KEY:
  // `AIRoute connect <remote>` saves the context's token, but users keep
  // AIRoute_API_KEY in their shell. The global `--api-key` option is bound to
  // that env var (.env("AIRoute_API_KEY")), so commands that spread
  ...
  const ambientKey = process.env.AIRoute_API_KEY || null;
  ...
  const cliToken = opts.cliToken ?? process.env.AIRoute_CLI_TOKEN ?? (await getCliToken());
...
export async function apiFetch(path, opts = {}) {
  ...
  const timeout =
    opts.timeout ?? (Number.parseInt(process.env.AIRoute_HTTP_TIMEOUT_MS || "", 10) || 30000);
  const maxAttempts = opts.retry === false ? 1 : (opts.retryMax ?? RETRY_DEFAULTS.maxAttempts);
  const verbose = opts.verbose ?? process.env.AIRoute_VERBOSE === "1";
