 ...
 * trust store under a DEDICATED slot (`AIRoute-tproxy-ca.crt`), separate from
 * the static MITM cert (`AIRoute-mitm.crt` in `cert/install.ts`), so the two
...
export const TPROXY_CA_CERT_NAME = "AIRoute-tproxy-ca.crt";
...
export async function installTproxyCa(
  ...
  if (process.env.AIRoute_SKIP_SYSTEM_TRUST === "1" && deps.run === undefined) {
    console.log("[tproxy-ca] AIRoute_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
