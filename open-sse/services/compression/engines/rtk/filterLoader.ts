...
function getDataDir(): string {
  return process.env.DATA_DIR || path.join(os.homedir(), ".AIRoute");
}
...
function projectFiltersTrusted(
  ...
  if (process.env.AIRoute_RTK_TRUST_PROJECT_FILTERS === "1") return true;
...
export function loadRtkFilters(options: RtkFilterLoadOptions = {}): RtkFilterDefinition[] {
  const currentCacheKey = [
    ...
    process.env.AIRoute_RTK_TRUST_PROJECT_FILTERS === "1" ? "env-trust" : "env-normal",
  ].join("|");
