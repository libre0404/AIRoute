...
const PII_PATTERNS: PIIPattern[] = [
  ...
  // ── China PII patterns (GB/T 35273-2020 / 个人信息保护法) ──
  {
    ...
    regex: /(?<=^|[^A-Za-z0-9])[1-9]\d{5}(?:19|20)\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01])\d{3}[\dXx](?=$|[^A-Za-z0-9])/g,
  ...
  {
    name: "cn_bank_card",
    // Chinese bank card (UnionPay): 16-19 digits, starts with 62
    regex: /(?<=^|[^A-Za-z0-9])62\d{14,17}(?=$|[^A-Za-z0-9])/g,
...
export function sanitizePII(text: string, isStreaming = false): SanitizeResult {
  ...
  for (const pattern of PII_PATTERNS) {
    ...
    while ((match = pattern.regex.exec(cleanText)) !== null) {
      ...
      if (pattern.name === "cn_bank_card" && !isValidLuhn(match[0].replace(/[-\s]/g, ""))) {
        continue;
      }
      if (pattern.name === "cn_id_card" && !isValidCnIdCard(match[0])) {
        continue;
      }
...
/**
 * Validate 18-digit Chinese resident ID card (居民身份证号码) checksum.
 * Algorithm: GB 11643-1999 / ISO 7064:1983 MOD 11-2
 * Weight factors for positions 1-17: [7,9,10,5,8,4,2,1,6,3,7,9,10,5,8,4,2]
 * Check character map: ['1','0','X','9','8','7','6','5','4','3','2']
 */
function isValidCnIdCard(id: string): boolean {
  const digits = id.toUpperCase().replace(/[^0-9X]/g, "");
  if (digits.length !== 18) return false;

  // Basic date validation (positions 7-14: YYYYMMDD)
  const year = parseInt(digits.substring(6, 10), 10);
  const month = parseInt(digits.substring(10, 12), 10);
  const day = parseInt(digits.substring(12, 14), 10);
  ...
  const expectedCheck = CHECK_CHARS[sum % 11];
  return digits[17] === expectedCheck;
}
