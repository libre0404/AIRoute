 ...
 *
 * Feature request: https://github.com/diegosouzapw/AIRoute/issues/142
 */
export default function CopilotToolCard({
  ...
  const [selectedModels, setSelectedModels] = useState<Set<string>>(() => {
    ...
    try {
      const saved = localStorage.getItem("AIRoute-copilot-selected-models");
  ...
  const [selectedApiKeyId, setSelectedApiKeyId] = useState(() => {
    if (typeof window !== "undefined") {
      const savedKey = localStorage.getItem("AIRoute-cli-key-copilot");
  ...
  useEffect(() => {
    if (selectedModels.size > 0) {
      localStorage.setItem(
        "AIRoute-copilot-selected-models",
  ...
  const generateConfig = () => {
    ...
    const config = {
      name: "AIRoute",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.AIRoute}`,
      models,
    };

    const responsesConfig = {
      name: "AIRoute-responses",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.AIRoute}`,
  ...
  const handleApiKeyChange = (value: string) => {
    ...
    if (value) localStorage.setItem("AIRoute-cli-key-copilot", value);
  };
