...
> **TL;DR**: AIRoute ships with built-in health monitoring, provider autopilot, quota tracking, and observability hooks. This guide covers the dashboard, alerts, and troubleshooting.
...
AIRoute has **3 layers of monitoring**:
...
│  └─ MCP tools: AIRoute_get_health, AIRoute_get_session_snapshot │
└──────────────────────────────────────────────────────────────┘
```
...
AIRoute supports **3 alert channels**:
...
Create a `~/.AIRoute/dashboard.json`:
...
- AIRoute's quota tracking may be **stale** — the provider's truth is upstream
...
- Run `sqlite3 ~/.AIRoute/storage.sqlite "PRAGMA integrity_check;"`
- If "ok" — false alarm, the health check is being too strict
- If anything else — **stop AIRoute** and follow the [disaster recovery guide](./DATABASE_GUIDE.md#disaster-recovery)
