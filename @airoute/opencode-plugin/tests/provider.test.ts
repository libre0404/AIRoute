 ...
 *
 * Covers `createAIRouteProviderHook(opts, deps)`:
...
const FIXTURE: AIRouteRawModelEntry[] = [
  {
    id: "claude-primary",
    object: "model",
    owned_by: "combo",
...
function stubFetcher(payload: AIRouteRawModelEntry[]): AIRouteModelsFetcher & {
  callCount: () => number;
...
test("createAIRouteProviderHook: default providerId is 'AIRoute'", () => {
  const hook = createAIRouteProviderHook(undefined, { combosFetcher: async () => [] });
  assert.equal(hook.id, "opencode-AIRoute");
});

test("createAIRouteProviderHook: custom providerId binds to hook.id (multi-instance)", () => {
  const a = createAIRouteProviderHook(
    { providerId: "AIRoute-preprod" },
    { combosFetcher: async () => [] }
  );
  const b = createAIRouteProviderHook(
    { providerId: "AIRoute-local" },
    { combosFetcher: async () => [] }
  );
  assert.equal(a.id, "opencode-AIRoute-preprod");
  assert.equal(b.id, "opencode-AIRoute-local");
});
...
test("models: returns {} when no baseURL resolvable (no opts.baseURL and no auth.baseURL)", async () => {
  const fetcher = stubFetcher(FIXTURE);
  const hook = createAIRouteProviderHook({}, { fetcher, combosFetcher: async () => [] });
...
test("models: maps a sample /v1/models entry to ModelV2 (sanity)", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-abc") as never });
  const claude = out["opencode-AIRoute/claude-primary"];
  ...
  assert.equal(claude.id, "opencode-AIRoute/claude-primary");
  assert.equal(claude.name, "claude-primary");
  assert.equal(claude.providerID, "opencode-AIRoute");
  ...
  // cost is zeroed (AIRoute /v1/models has no pricing)
