---
title: "AIRoute — Dashboard Features Gallery"
version: 3.8.40
lastUpdated: 2026-06-28
---

# AIRoute — Dashboard Features Gallery
...
Visual guide to every section of the AIRoute dashboard.
...
- 💻 **CLI enhancement suite** — 20+ commands including `AIRoute providers`, `AIRoute combos`, `AIRoute doctor`, `AIRoute setup`
...
OpenRouter connections can store a per-connection `preset` in Advanced Settings. When set, AIRoute sends it as the OpenRouter top-level request field, for example `"preset": "email-copywriter"`, unless the client request already supplied its own `preset`.
...
Native Electron desktop app for Windows, macOS, and Linux. Run AIRoute as a standalone application with system tray integration, offline support, auto-update, and one-click install.
...
AIRoute now supports **OpenAI-compatible WebSocket clients** via the `/v1/ws` upgrade endpoint. The custom `scripts/dev/v1-ws-bridge.mjs` server wraps Next.js and upgrades WS connections to full bidirectional streaming sessions. Authentication uses the same API key or session cookie as HTTP requests.
...
**Hybrid token counting** also lands in v3.6.6: when a Claude-compatible provider exposes `/messages/count_tokens`, AIRoute calls it before large requests with graceful estimation fallback.
