#!/usr/bin/env bash
# =============================================================================
# codex-ws — roda a OpenAI Codex CLI contra um AIRoute LOCAL usando o
#            transporte Responses-over-WebSocket (em vez do HTTP da Cloud).
#
# POR QUE ESTE WRAPPER EXISTE
# ---------------------------
# O AIRoute expõe um proxy WebSocket para a API de Responses do Codex em
...
#     account"). É preciso mandar o id "puro" -> "gpt-5.5". (O AIRoute, no
...
#   AIRoute_WS_BASE   base URL do AIRoute local   (default abaixo)
#   AIRoute_WS_MODEL  modelo codex (id puro)        (default "gpt-5.5")
#   AIRoute_LOCAL_KEY API key (qualquer valor se REQUIRE_API_KEY=false)
...
AIRoute_WS_BASE="${AIRoute_WS_BASE:-http://127.0.0.1:20128/v1}"  # base do AIRoute local
AIRoute_WS_MODEL="${AIRoute_WS_MODEL:-gpt-5.5}"                  # id PURO (sem "codex/")
...
export AIRoute_LOCAL_KEY="${AIRoute_LOCAL_KEY:-local}"
...
# Gerado por codex-ws.sh — config isolada para o WS local do AIRoute.
model = "$AIRoute_WS_MODEL"
model_provider = "AIRoute-local"

[model_providers.AIRoute-local]
name = "AIRoute Local (WS)"
base_url = "$AIRoute_WS_BASE"   # a URL WebSocket é derivada desta base pela CLI
...
env_key = "AIRoute_LOCAL_KEY"   # a CLI lê a key Bearer desta env var
...
overrides=(-c model="$AIRoute_WS_MODEL" -c model_provider="AIRoute-local")
