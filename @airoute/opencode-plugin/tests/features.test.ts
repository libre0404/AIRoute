...
import {
  ...
  defaultAIRouteEnrichmentFetcher,
  defaultAIRouteCompressionMetaFetcher,
...
test("parseAIRoutePluginOptions: empty features object → preserved", () => {
  const r = parseAIRoutePluginOptions({ features: {} });
  assert.deepEqual(r, { features: {} });
});

test("parseAIRoutePluginOptions: all boolean features set → preserved", () => {
  const r = parseAIRoutePluginOptions({
    features: {
      combos: true,
      enrichment: true,
...
test("parseAIRoutePluginOptions: mcpToken string → preserved", () => {
  const r = parseAIRoutePluginOptions({
...
test("parseAIRoutePluginOptions: unknown features key → throws (strict)", () => {
  assert.throws(
    () =>
      parseAIRoutePluginOptions({
        features: { combos: true, unknown_field: "oops" },
      }),
    /Invalid @AIRoute\/opencode-plugin options/
  );
});

test("parseAIRoutePluginOptions: non-boolean for boolean feature → throws", () => {
  assert.throws(
    () =>
      parseAIRoutePluginOptions({
...
test("parseAIRoutePluginOptions: empty mcpToken → throws (min 1)", () => {
  assert.throws(
    () => parseAIRoutePluginOptions({ features: { mcpToken: "" } }),
    /Invalid @AIRoute\/opencode-plugin options/
  );
});
...
const SAMPLE_RAW: AIRouteRawModelEntry[] = [
  {
    id: "claude-sonnet-4-6",
    object: "model",
    created: 0,
...
test("provider hook: enrichment fetcher called when features.enrichment !== false", async () => {
  let called = 0;
  const enrichment: AIRouteEnrichmentMap = new Map([
    ["claude-sonnet-4-6", { name: "Claude Sonnet 4.6", pricing: { input: 3, output: 15 } }],
  ]);
  const hook = createAIRouteProviderHook(
    { providerId: "AIRoute", baseURL: "https://or.example.com/v1" },
    {
  ...
  const m = out["opencode-AIRoute/claude-sonnet-4-6"];
...
test("provider hook: enrichment fetcher NOT called when features.enrichment:false", async () => {
  ...
  assert.equal(
    out["opencode-AIRoute/claude-sonnet-4-6"].name,
    "claude-sonnet-4-6",
    "raw id preserved"
  );
});
...
test("provider hook: compression metadata fetcher called when opted in", async () => {
  let called = 0;
  const compressionCombos: AIRouteCompressionCombo[] = [
    {
      id: "default-caveman",
  ...
  const combo = out["opencode-AIRoute/claude-primary"];
...
const stubAuthJson = (apiKey: string) => async () => ({
  "opencode-AIRoute": { type: "api" as const, key: apiKey },
});
...
test("config hook: MCP auto-emit OFF by default (no mcp entry)", async () => {
  ...
  assert.ok(input.provider?.["opencode-AIRoute"], "provider block written");
...
test("config hook: features.mcpAutoEmit:true writes mcp entry with provider apiKey", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-AIRoute"] as
...
test("config hook: features.mcpToken overrides provider apiKey in mcp Bearer", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-AIRoute"] as { headers: Record<string, string> };
  assert.equal(
...
test("config hook: existing operator mcp.<providerId> wins (no overwrite)", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {
    mcp: { "opencode-AIRoute": { type: "custom-user-entry", url: "https://manual.example/mcp" } },
  };
  await hook(input as never);
  assert.deepEqual(
    input.mcp?.["opencode-AIRoute"],
...
test("config hook: features.mcpAutoEmit:true with /v1 in baseURL → strips correctly", async () => {
  const hook = createAIRouteConfigHook(
    ...
    {
      readAuthJson: async () => ({
        "opencode-AIRoute-preprod": { type: "api" as const, key: "sk-preprod" },
      }),
      fetcher: async () => SAMPLE_RAW,
      combosFetcher: async () => [],
      logger: { warn: () => {} },
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-AIRoute-preprod"] as { url: string };
  assert.equal(
    entry.url,
...
test("defaultAIRouteEnrichmentFetcher: empty baseURL → empty map", async () => {
  const m = await defaultAIRouteEnrichmentFetcher("", "sk", 100);
  assert.equal(m.size, 0);
});

test("defaultAIRouteEnrichmentFetcher: empty apiKey → empty map", async () => {
  const m = await defaultAIRouteEnrichmentFetcher("https://or.example.com", "", 100);
  assert.equal(m.size, 0);
});

test("defaultAIRouteCompressionMetaFetcher: empty baseURL → empty array", async () => {
  const arr = await defaultAIRouteCompressionMetaFetcher("", "sk", 100);
...
test("defaultAIRouteEnrichmentFetcher: merges names from /api/pricing/models and prices from /api/pricing", async () => {
  ...
  try {
    const map = await defaultAIRouteEnrichmentFetcher(
...
test("defaultAIRouteEnrichmentFetcher: name-only when pricing endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing/models")) {
  ...
  try {
    const map = await defaultAIRouteEnrichmentFetcher("https://or.example.com", "sk-test", 5_000);
    const opus = map.get("cc/claude-opus-4-7");
...
test("defaultAIRouteEnrichmentFetcher: pricing-only when catalog endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing")) {
...
): AIRouteEnrichmentMap {
  const map: AIRouteEnrichmentMap = new Map();
...
test("canonicalDedupSet: drops canonical row when alias twin present", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: AIRouteRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as AIRouteRawModelEntry,
    { id: "claude/claude-opus-4-7" } as AIRouteRawModelEntry,
  ];
...
test("canonicalDedupSet: keeps standalone canonical row (no alias twin) — never hides a model", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: AIRouteRawModelEntry[] = [
    { id: "claude/claude-opus-99" } as AIRouteRawModelEntry, // canonical only — no `cc/claude-opus-99`
  ];
  const drop = canonicalDedupSet(raw, c2a);
...
test("canonicalDedupSet: multi-provider — drops all canonical twins where alias exists", () => {
  ...
  const raw: AIRouteRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as AIRouteRawModelEntry,
    { id: "claude/claude-opus-4-7" } as AIRouteRawModelEntry,
    { id: "cx/gpt-5.5" } as AIRouteRawModelEntry,
    { id: "codex/gpt-5.5" } as AIRouteRawModelEntry,
    { id: "pol/openai-large" } as AIRouteRawModelEntry,
    { id: "pollinations/openai-large" } as AIRouteRawModelEntry,
  ];
  const drop = canonicalDedupSet(raw, c2a);
  assert.equal(drop.has("claude/claude-opus-4-7"), true);
