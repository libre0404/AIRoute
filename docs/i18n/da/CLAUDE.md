...
**AIRoute** — samlet AI proxy/router. Én endpoint, 160+ LLM udbydere, auto-fallback.
...
AIRoute har tre relaterede, men distinkte mekanismer til midlertidige fejl. Hold deres
...
- **Imports**: ekstern → intern (`@/`, `@AIRoute/open-sse`) → relativ
...
- **Sti aliaser**: `@/*` → `src/`, `@AIRoute/open-sse` → `open-sse/`, `@AIRoute/open-sse/*` → `open-sse/*`
- **Standardport**: 20128 (API + dashboard på samme port)
- **Data katalog**: `DATA_DIR` miljøvariabel, standard til `~/.AIRoute/`
