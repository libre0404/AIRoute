---
title: "AIRoute — Deployment Guide on VM with Cloudflare"
version: 3.8.40
lastUpdated: 2026-06-28
---

# AIRoute — Deployment Guide on VM with Cloudflare
...
Complete guide to install and configure AIRoute on a VM (VPS) with domain managed via Cloudflare.
...
AIRoute_WS_BRIDGE_SECRET=REPLACE-WITH-WS-BRIDGE-SECRET  # REQUIRED em produção: usado pelo Codex Responses WS bridge
...
# AIRoute_PUBLIC_BASE_URL=https://llms.seudominio.com

# === Cloud Sync (optional) ===
# CLOUD_URL=https://cloud.AIRoute.online
# NEXT_PUBLIC_CLOUD_URL=https://cloud.AIRoute.online
EOF
```
...
```bash
docker pull diegosouzapw/AIRoute:latest

docker run -d \
  --name AIRoute \
  --restart unless-stopped \
  --env-file /opt/AIRoute/.env \
  -p 20128:20128 \
...
```bash
docker ps | grep AIRoute
docker logs AIRoute --tail 20
```
...
```bash
cat > /etc/nginx/sites-available/AIRoute << 'NGINX'
...
Keep reverse-proxy stream timeouts aligned with your AIRoute timeout env vars. If you raise
`FETCH_TIMEOUT_MS` / `STREAM_IDLE_TIMEOUT_MS`, raise `proxy_read_timeout` / `proxy_send_timeout`
above the same threshold.

AIRoute uses `NEXT_PUBLIC_BASE_URL` as the canonical browser-facing origin for OAuth
...
`AIRoute_TRUST_PROXY` if AIRoute is not directly reachable by clients and your proxy
...
# Enable AIRoute
ln -sf /etc/nginx/sites-available/AIRoute /etc/nginx/sites-enabled/AIRoute
...
```bash
docker pull diegosouzapw/AIRoute:latest
docker stop AIRoute && docker rm AIRoute
...
```bash
docker logs -f AIRoute          # Real-time stream
docker logs AIRoute --tail 50   # Last 50 lines
```
...
docker cp AIRoute:/app/data ./backup-$(date +%F)

# Or compress the entire volume
docker run --rm -v AIRoute-data:/data -v $(pwd):/backup \
  alpine tar czf /backup/AIRoute-data-$(date +%F).tar.gz /data
```
...
docker stop AIRoute
docker run --rm -v AIRoute-data:/data -v $(pwd):/backup \
  alpine sh -c "rm -rf /data/* && tar xzf /backup/AIRoute-data-YYYY-MM-DD.tar.gz -C /"
docker start AIRoute
```
...
See also [TUNNELS_GUIDE.md](./TUNNELS_GUIDE.md) for the in-repo Cloudflare Tunnel walkthrough. The standalone `AIRouteCloud/` worker lives in a separate companion repo.
...
| 20128 | AIRoute   | Localhost only (via nginx) |
