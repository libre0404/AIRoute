 ...
 * types, the provider-id mapping table, and the raw→AIRoute transform
...
// ─── Provider mapping: models.dev provider ID → AIRoute provider IDs/aliases ──
//
// models.dev uses canonical provider IDs (e.g. "openai", "anthropic", "google").
// AIRoute uses both full IDs and short aliases (e.g. "cc" for claude, "cx" for codex).
// We map each models.dev provider to ALL AIRoute identifiers that should receive
...
export const MODELS_DEV_PROVIDER_MAP: Record<string, string[]> = {
  ...
  // that is the historical alias pair. But AIRoute's catalog & combo targets
  ...
  // Additional providers that may overlap with AIRoute
  alibaba: ["ali", "alibaba"],
...
/**
 * Map a models.dev provider ID to AIRoute provider IDs.
...
/**
 * Transform models.dev raw data → AIRoute PricingByProvider format.
 *
 * models.dev costs are already in $/1M tokens (same as AIRoute format).
...
export function transformModelsDevToPricing(raw: ModelsDevData): PricingByProvider {
  ...
  for (const [providerId, providerData] of Object.entries(raw)) {
    const AIRouteProviders = mapProviderId(providerId);
    ...
    for (const [modelId, model] of Object.entries(providerData.models || {})) {
      ...
      // Write to ALL mapped AIRoute providers
      for (const omniProvider of AIRouteProviders) {
