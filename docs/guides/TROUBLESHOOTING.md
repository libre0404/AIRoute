...
Common problems and solutions for AIRoute.

---

## Quick Reference

**New to AIRoute?** Start here — these solve 90% of problems:

| I see this              | What it means                       | What to do                                                                                        |
| ----------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------- |
| "Can't connect"         | AIRoute isn't running             | Run `AIRoute` or `docker restart AIRoute`                                                     |
...
| EACCES: permission denied                           | Set `DATA_DIR=/path/to/writable/dir` to override `~/.AIRoute`                                                                                          |
...
| `dlopen` / `slice is not valid mach-o file` (macOS) | Run `cd $(npm root -g)/AIRoute/app && npm rebuild better-sqlite3 && AIRoute` — see [macOS native module rebuild](#macos-native-module-rebuild) below |
...
**Cause:** You are running a Node.js version outside AIRoute's approved secure runtime floor. The most common case is running an older Node 22 or 24 patch level that falls below the patched security floor AIRoute requires.
...
3. Reinstall AIRoute: `npm install -g AIRoute`
4. Restart: `AIRoute`
...
```
dlopen(/Users/<user>/.nvm/versions/node/v24.14.1/lib/node_modules/AIRoute/app/node_modules/better-sqlite3/build/Release/better_sqlite3.node, 0x0001): tried: '...' (slice is not valid mach-o file)
```

**Fix — rebuild for your local environment (no Node.js downgrade required):**

```bash
cd $(npm root -g)/AIRoute/app
...
**Fix (v3.5.5+):** AIRoute now uses undici's own `fetch()` function when a proxy dispatcher is active, ensuring consistent behavior. Update to v3.5.5+.

### MITM proxy under WSL: desktop apps on the Windows host are not intercepted

**Cause:** The MITM proxy and its CA certificate install into the environment where AIRoute runs. Under WSL that environment is the Linux guest, while the AI desktop apps (Kiro, Trae, Copilot, Zed, …) run on the Windows host. The host apps do not trust the guest's certificate store and do not route through the guest's system proxy, so desktop interception does not engage there.

**Recommendation:** Run AIRoute natively on the same OS as the desktop apps you want to intercept (Windows for Windows apps; macOS/Linux likewise). Keeping AIRoute inside WSL while targeting host apps requires manually trusting the generated CA certificate on the Windows host and pointing each host app's network/proxy settings at the WSL proxy endpoint — an unsupported, fragile setup.
...
AIRoute auto-refreshes tokens. If issues persist:
...
2. Verify `CLOUD_URL` points to your cloud endpoint (e.g., `https://AIRoute.dev`)
...
When many concurrent requests hit a rate-limited provider, AIRoute uses mutex + auto rate-limiting to serialize requests and prevent cascading failures. This is automatic for API key providers.

---

## Optional RAG / LLM failure taxonomy (16 problems)

Some AIRoute users place the gateway in front of RAG or agent stacks. In those setups it is common to see a strange pattern: AIRoute looks healthy (providers up, routing profiles ok, no rate limit alerts) but the final answer is still wrong.
...
1. When you investigate a bad response, capture:
   ...
   - route or provider combo in AIRoute
   - any RAG context used downstream (retrieved documents, tool calls, etc)
2. Map the incident to one or two WFGY ProblemMap numbers (`No.1` … `No.16`).
3. Store the number in your own dashboard, runbook, or incident tracker next to the AIRoute logs.
...
You can ignore this section if you do not run RAG or agent pipelines behind AIRoute.
...
2. Restart AIRoute so the new env values are picked up
...
3. Restart AIRoute and re-test from **Dashboard → CLI Tools**
...
- Run `AIRoute providers` from the CLI to re-trigger the OAuth flow, or
...
### AIRoute_WS_BRIDGE_SECRET missing in production
...
**Cause:** The `AIRoute_WS_BRIDGE_SECRET` env var is missing from the production environment.

**Fix:**

1. Generate a random secret: `openssl rand -hex 32`
2. Set `AIRoute_WS_BRIDGE_SECRET=<random-secret>` in the production server env (and any client that talks to the bridge)
3. Restart AIRoute
...
- **GitHub Issues**: [github.com/diegosouzapw/AIRoute/issues](https://github.com/diegosouzapw/AIRoute/issues)
