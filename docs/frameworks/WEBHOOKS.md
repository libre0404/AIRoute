...
AIRoute can fire HTTP webhooks on platform events. Use them to integrate with
...
When a webhook has a `secret`, AIRoute signs the JSON body and sends:

```
Content-Type: application/json
User-Agent: AIRoute-Webhook/1.0
X-Webhook-Event: <event>
X-Webhook-Timestamp: <ISO-8601>
X-Webhook-Signature: sha256=<hex HMAC-SHA256(secret, body)>
```

> Header names use the `X-Webhook-*` prefix (not `X-AIRoute-*`). The signature
...
```json
{
  ...
  "data": {
    "message": "Test webhook delivery from AIRoute",
    "webhookId": "<uuid>"
  }
}
```
