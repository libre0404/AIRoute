...
import { getStaticQoderModels } from "@AIRoute/open-sse/services/qoderCli.ts";
import { deriveConfigFromRegistryModelsUrl } from "./discoveryConfig";
import { fetchGitHubCopilotModels } from "@AIRoute/open-sse/services/githubCopilotModels.ts";
import { fetchKiroAvailableModels } from "@AIRoute/open-sse/services/kiroModels.ts";
import {
  buildGlmCodingHeaders,
  buildGlmModelsUrl,
} from "@AIRoute/open-sse/config/glmProvider.ts";
...
} from "@AIRoute/open-sse/services/bedrock.ts";
import {
  AZURE_AI_DEFAULT_BASE_URL,
  buildAzureAiModelsUrl,
} from "@AIRoute/open-sse/config/azureAi.ts";
import {
  DATAROBOT_DEFAULT_BASE_URL,
  buildDataRobotCatalogUrl,
  isDataRobotDeploymentUrl,
} from "@AIRoute/open-sse/config/datarobot.ts";
import { OCI_DEFAULT_BASE_URL, buildOciModelsUrl } from "@AIRoute/open-sse/config/oci.ts";
import {
  SAP_DEFAULT_BASE_URL,
  buildSapModelsUrl,
  getSapResourceGroup,
} from "@AIRoute/open-sse/config/sap.ts";
import {
  WATSONX_DEFAULT_BASE_URL,
  buildWatsonxModelsUrl,
} from "@AIRoute/open-sse/config/watsonx.ts";
...
} from "@AIRoute/open-sse/config/audioRegistry.ts";
import {
    ...
    if (provider === "vertex" || provider === "vertex-partner") {
      ...
      try {
        const { parseSAFromApiKey, getAccessToken } =
          await import("@AIRoute/open-sse/executors/vertex.ts");
        if (accessToken) {
