 ...
 * `stream`. Without this hint, AIRoute previously routed those requests with
...
export function resolveStreamFlag(
  ...
  // AIRoute accumulates the provider stream and converts it to JSON for the client
  ...
  // AIRoute fallback would force SSE upstream and fail JSON-only providers as
  ...
  // No explicit stream param — preserve AIRoute's streaming default unless
