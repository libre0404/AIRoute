...
// Check if settings has AIRoute config
const hasAIRouteConfig = (settings: any) => {
  ...
  return openai.some((p: any) => {
    if (p.name?.includes("AIRoute") || p.id === "AIRoute") return true;
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      reason: runtime.reason,
      settings,
      hasAIRoute: hasAIRouteConfig(settings),
...
// POST - Write AIRoute config to settings.json + .env
export async function POST(request: Request) {
  ...
  try {
    ...
    const resolvedApiKey = apiKey || "sk_AIRoute";
    ...
    const envLines = envContent.split("\n").filter((line) => {
      // Remove old AIRoute-related keys we're about to write
      return (
    ...
    // openai provider — primary, supports all models via AIRoute
    const openaiEntry = {
      id: resolvedModel,
      name: `${resolvedModel} (AIRoute)`,
    ...
    const openaiIdx = openaiProviders.findIndex(
      (p: any) => p && (p.baseUrl === normalizedBaseUrl || p.id === "AIRoute")
    );
    if (openaiIdx >= 0) {
    ...
    // anthropic provider — for Claude models via AIRoute
    const anthropicEntry = {
      id: "claude-sonnet-4-6",
      name: "Claude Sonnet 4.6 (AIRoute)",
    ...
    // gemini provider — for Gemini models via AIRoute
    const geminiEntry = {
      id: "gemini-3-flash",
      name: "Gemini 3 Flash (AIRoute)",
...
// DELETE - Remove AIRoute config from settings.json and .env
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove AIRoute entries from each provider type
    ...
    for (const type of providerTypes) {
      if (Array.isArray(existingConfig.modelProviders?.[type])) {
        existingConfig.modelProviders[type] = existingConfig.modelProviders[type].filter(
          (p: any) => !p.name?.includes("AIRoute") && p.id !== "AIRoute"
        );
    ...
    return NextResponse.json({
      success: true,
      message: "AIRoute settings removed from Qwen Code",
    });
  } catch (error) {
