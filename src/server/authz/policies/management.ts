...
function requestPeerAddress(ctx: PolicyContext): string | null {
  ...
  const stamped = resolveStampedPeer(
    ...
    process.env.AIRoute_PEER_STAMP_TOKEN
  );
...
const WS_BRIDGE_SECRET_HEADER = "x-AIRoute-ws-bridge-secret";
...
// server-ws.mjs (AIRoute_WS_BRIDGE_SECRET). Without this carve-out the MANAGEMENT
...
function isValidWsBridgeRequest(ctx: PolicyContext): boolean {
  ...
  const expected = process.env.AIRoute_WS_BRIDGE_SECRET || "";
