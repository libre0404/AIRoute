 ...
 *
 * Enforces that every env var referenced in AIRoute source code appears in
...
const IGNORE_FROM_CODE = new Set([
  ...
  "USERPROFILE",
  "PREFIX",
  // X11 display server — set by the OS/session manager, not AIRoute config.
  "DISPLAY",
  ...
  // bundler is active — framework-internal. The AIRoute-facing knob is
  // AIRoute_USE_TURBOPACK (scripts/dev/run-next.mjs), which IS documented.
  "TURBOPACK",
  "NODE_TEST_CONTEXT",
  "VITEST",
  // Instruction snippet shown to users (Traffic Inspector HttpProxySnippetCard) — not AIRoute config.
  ...
  // points at AIRoute). A downstream client-tool var, not an AIRoute server
  ...
  // (scripts/check/check-openapi-breaking.mjs) — a build/check signal, not AIRoute runtime config.
  "BASE_REF",
  // PR body injected by GitHub Actions into the pr-evidence gate (github.event.pull_request.body);
  // a CI-only signal, never an AIRoute runtime config (Phase 7.10).
  "PR_BODY",
  // CLI machine-id token opt-out (server-side flag; not user-configurable via .env).
  "AIRoute_DISABLE_CLI_TOKEN",
  ...
  // ("http://192.168.0.15:20128" / null), never AIRoute runtime config (#5151).
  "COMBO_LIVE_BASE_URL",
  "COMBO_LIVE_API_KEY",
  // update-notifier opt-out for the CLI binary.
  "AIRoute_NO_UPDATE_NOTIFIER",
  ...
  // These are external signals set by the host OS or cloud provider — not AIRoute config.
  "CODESPACES",
  ...
  "AIRoute_DOCTOR_HOST",
  "AIRoute_DOCTOR_LIVENESS_URL",
  "AIRoute_PROVIDER_CATALOG_PATH",
  "AIRoute_PROVIDER_TEST_MODEL",
  // Test-only opt-out: instructs bin/AIRoute.mjs to skip auto-loading the
  // repository .env so isolation tests get a deterministic environment.
  "AIRoute_CLI_SKIP_REPO_ENV",
  ...
  // A dev/ops measurement tool, never AIRoute runtime config.
  "AIRoute_EVAL_CREDENTIALS",
  ...
  "OMNIROUT",
  // Static config alias path (the canonical var is AIRoute_PAYLOAD_RULES_PATH).
  "PAYLOAD_RULES_PATH",
  // Node.js module resolution path — OS/Node internal, not an AIRoute config var.
  ...
  "NVIDIA_BASE_URL",
  "NVIDIA_MODEL",
  // XDG standard data directory — set by OS/desktop session, not AIRoute config.
