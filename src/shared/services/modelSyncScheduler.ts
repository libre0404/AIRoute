...
const globalState = globalThis as typeof globalThis & {
  __AIRouteModelSyncInternalAuthToken?: string;
};
...
function getInternalAuthToken(): string {
  if (!internalAuthToken) {
    internalAuthToken = globalState.__AIRouteModelSyncInternalAuthToken || randomUUID();
    globalState.__AIRouteModelSyncInternalAuthToken = internalAuthToken;
  }
...
export function isModelSyncInternalRequest(request: { headers: Headers }): boolean {
  if (!internalAuthToken && globalState.__AIRouteModelSyncInternalAuthToken) {
    internalAuthToken = globalState.__AIRouteModelSyncInternalAuthToken;
  }
 ...
 * @param apiBaseUrl — internal base URL to call AIRoute's own API
