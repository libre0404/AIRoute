 ...
 *   model: string;                    // the model name the user chose from AIRoute
 * }
 *
 * interface HermesAgentConfigPayload {
 *   baseUrl: string;                  // usually the AIRoute base URL
...
export async function generateHermesAgentConfig(
  ...
  const resolvedKey = apiKey || "YOUR_AIRoute_API_KEY_HERE";
  ...
  // Build the providers.AIRoute entry (shared)
  const normalizedBase = normalizeBaseUrl(baseUrl);
  const AIRouteProvider = {
