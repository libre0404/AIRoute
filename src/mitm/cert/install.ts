...
const LINUX_CERT_NAME = "AIRoute-mitm.crt";
  ...
  return new Promise((resolve) => {
    exec(
      script,
      {
        ...
        env: {
          ...process.env,
          CERT_NAME: "AIRoute MITM Root CA",
          CERT_PATH: certPath || "",
          ACTION: action,
        },
      },
...
export async function installCert(sudoPassword: string, certPath: string): Promise<void> {
  ...
  if (process.env.AIRoute_SKIP_SYSTEM_TRUST === "1") {
    console.log("[cert] AIRoute_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
...
// They install / remove the AIRoute MITM root CA from the OS trust store and
