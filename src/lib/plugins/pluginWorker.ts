 ...
 *   2. The `exec` permission additionally requires AIRoute_PLUGINS_ALLOW_EXEC=1
...
function createSandbox(permissions: string[], pluginDir: string): Record<string, unknown> {
  ...
  if (permissions.includes("exec")) {
    if (process.env.AIRoute_PLUGINS_ALLOW_EXEC !== "1") {
      throw new Error(
        `Plugin '${name}' requested the 'exec' permission, which is disabled. Set AIRoute_PLUGINS_ALLOW_EXEC=1 to enable (local operator only).`
      );
    }
