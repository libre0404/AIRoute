...
export function getAIRouteTokenCounts(usage: UsageLike): { input: number; output: number } {
...
export function formatAIRouteCost(costUsd: unknown): string {
  const normalized = toFiniteNumber(costUsd);
  return normalized > 0 ? normalized.toFixed(10) : "0.0000000000";
}

export function buildAIRouteResponseMetaHeaders({
  cacheHit = false,
  costUsd = 0,
   ...
   * Emitted as `X-AIRoute-Cost-Saved` only when provided (omitted on normal
  ...
  const tokens = getAIRouteTokenCounts(usage);
  const headers: Record<string, string> = {
    [AIRoute_RESPONSE_HEADERS.cacheHit]: toHeaderValue(String(cacheHit)),
    [AIRoute_RESPONSE_HEADERS.latencyMs]: toHeaderValue(String(toNonNegativeInteger(latencyMs))),
    [AIRoute_RESPONSE_HEADERS.responseCost]: toHeaderValue(formatAIRouteCost(costUsd)),
    [AIRoute_RESPONSE_HEADERS.tokensIn]: toHeaderValue(String(tokens.input)),
    [AIRoute_RESPONSE_HEADERS.tokensOut]: toHeaderValue(String(tokens.output)),
    [AIRoute_RESPONSE_HEADERS.version]: toHeaderValue(APP_CONFIG.version),
  };

  if (typeof model === "string" && model.trim().length > 0) {
    headers[AIRoute_RESPONSE_HEADERS.model] = toHeaderValue(model);
  }

  if (typeof requestId === "string" && requestId.trim().length > 0) {
    headers[AIRoute_RESPONSE_HEADERS.requestId] = toHeaderValue(requestId);
  }

  if (typeof provider === "string" && provider.trim().length > 0) {
    headers[AIRoute_RESPONSE_HEADERS.provider] = toHeaderValue(getProviderAlias(provider));
  }
  ...
  if (costSavedUsd != null) {
    headers[AIRoute_RESPONSE_HEADERS.costSaved] = toHeaderValue(
      formatAIRouteCost(costSavedUsd)
    );
  }

  const attempts = toNonNegativeInteger(fallbackAttempts);
  if (attempts > 0) {
    headers[AIRoute_RESPONSE_HEADERS.fallbackAttempts] = toHeaderValue(String(attempts));
  }

  return headers;
}

export function buildAIRouteSseMetadataComment(
  options: Parameters<typeof buildAIRouteResponseMetaHeaders>[0]
): string {
  const headers = buildAIRouteResponseMetaHeaders(options);
...
/**
 * Single choke-point for attaching the X-AIRoute-* response meta headers.
 ...
 */
export function attachAIRouteMetaHeaders(
  headers: Headers | Record<string, string>,
  meta: Parameters<typeof buildAIRouteResponseMetaHeaders>[0]
): void {
  const built = buildAIRouteResponseMetaHeaders(meta);
...
/**
 * Attach the X-AIRoute-* meta headers onto an already-built Response, ADDING
 ...
 */
export function attachAIRouteMetaToResponse(
  response: Response,
  meta: Parameters<typeof buildAIRouteResponseMetaHeaders>[0]
): Response {
  ...
  try {
    attachAIRouteMetaHeaders(response.headers, meta);
    return response;
  } catch {
    ...
    attachAIRouteMetaHeaders(cloned.headers, meta);
    return cloned;
  }
}
