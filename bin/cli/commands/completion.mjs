...
function installPath(shell) {
  const home = homedir();
  if (shell === "zsh") return join(home, ".zsh", "completions", "_AIRoute");
  if (shell === "fish") return join(home, ".config", "fish", "completions", "AIRoute.fish");
  return join(home, ".bash_completion.d", "AIRoute");
}

function generateZshScript() {
  return `#compdef AIRoute

# AIRoute zsh completion (dynamic)
_AIRoute_get_cache() {
  local key="$1"
  local cache="$HOME/.AIRoute/completion-cache.json"
  ...
  if [[ $((now - mtime)) -gt 3600 ]]; then
    AIRoute completion refresh --quiet >/dev/null 2>&1
  fi
...
_AIRoute() {
  local -a commands
  commands=(
    'serve:Start the AIRoute server'
    'stop:Stop the server'
    'restart:Restart the server'
    'setup:Configure AIRoute'
  ...
  case $state in
    ...
    arg)
      case $words[1] in
        combo)
          case $words[2] in
            switch|delete|show)
              local -a combos
              combos=($(_AIRoute_get_cache combos))
        ...
        providers|keys)
          case $words[2] in
            add|remove|test)
              local -a providers
              providers=($(_AIRoute_get_cache providers))
      ...
      case $state in
        models)
          local -a models
          models=($(_AIRoute_get_cache models))
...
compdef _AIRoute AIRoute
`;
}

function generateBashScript() {
  return `#!/bin/bash
# AIRoute CLI bash completion (dynamic)
...
complete -F _AIRoute AIRoute
`;
}

function generateFishScript() {
  return `# AIRoute CLI fish completion (dynamic)
complete -c AIRoute -f
...
for cmd in $commands
  complete -c AIRoute -n '__fish_is_nth_token 1' -a $cmd
end

# Subcommands
complete -c AIRoute -n '__fish_seen_subcommand_from combo' -a 'list switch create delete show suggest'
complete -c AIRoute -n '__fish_seen_subcommand_from keys' -a 'add list remove regenerate revoke reveal usage'
complete -c AIRoute -n '__fish_seen_subcommand_from providers' -a 'available list test test-all'
complete -c AIRoute -n '__fish_seen_subcommand_from config' -a 'list get set validate contexts'
complete -c AIRoute -n '__fish_seen_subcommand_from completion' -a 'zsh bash fish install refresh'
complete -c AIRoute -n '__fish_seen_subcommand_from open' -a 'combos providers api-manager cli-tools agents settings logs memory skills evals audit cost resilience'

# Dynamic completions from cache (requires python3)
function __AIRoute_cache_get
  set -l key $argv[1]
  set -l cache "$HOME/.AIRoute/completion-cache.json"
...
complete -c AIRoute -n '__fish_seen_subcommand_from combo; and __fish_seen_subcommand_from switch delete' -a '(__AIRoute_cache_get combos)'
complete -c AIRoute -l model -a '(__AIRoute_cache_get models)'
complete -c AIRoute -l combo -a '(__AIRoute_cache_get combos)'
`;
}
...
export function registerCompletion(program) {
  ...
  // Backward-compat: `AIRoute completion <shell>` (positional arg form)
  comp
    .command("<shell>")
