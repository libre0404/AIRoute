...
**AIRoute** — birləşdirilmiş AI proxy/router. Bir uç nöqtə, 160+ LLM təminatçısı, avtomatik geri dönmə.
...
AIRoute-un üç əlaqəli, lakin fərqli müvəqqəti uğursuzluq mexanizmi var. Marşrut davranışını düzəldərkən onların
...
- **İdxallar**: xarici → daxili (`@/`, `@AIRoute/open-sse`) → nisbət
...
- **Yol aliasları**: `@/*` → `src/`, `@AIRoute/open-sse` → `open-sse/`, `@AIRoute/open-sse/*` → `open-sse/*`
- **Default port**: 20128 (API + dashboard eyni portda)
- **Məlumat qovluğu**: `DATA_DIR` env var, varsayılan `~/.AIRoute/`
