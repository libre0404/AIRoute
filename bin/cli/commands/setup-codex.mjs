 ...
 *   AIRoute setup-codex --remote http://100.67.86.91:20128 --api-key sk-xxx
 *
...
export async function runSetupCodexCommand(opts = {}) {
  ...
  printHeading(`AIRoute → Codex CLI profile generator`);
...
export function registerSetupCodex(program) {
  program
    ...
    .option(
      "--remote <url>",
      "Remote AIRoute URL, e.g. http://100.67.86.91:20128 — fetches models from there"
    )
    .option(
      "--api-key <key>",
      "AIRoute API key for the remote instance (defaults to AIRoute_API_KEY env var)"
    )
