...
export const NOAUTH_PROVIDERS = {
  ...
  auggie: {
    ...
    isLocalCli: true,
    freeNote:
      "Local passthrough — runs the Augment CLI (`auggie`) on this machine. Auth is handled by `auggie login`, not AIRoute.",
    authHint:
      "No API key stored by AIRoute. Install the Auggie CLI and run `auggie login` on this machine, then AIRoute spawns it locally for each request.",
    notice: {
      text: "Augment (Auggie CLI) requires the `auggie` binary installed and authenticated locally (`auggie login`). AIRoute spawns it as a subprocess and never sees or stores your Augment credentials.",
    },
  },
};
