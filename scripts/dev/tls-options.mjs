 ...
 *
 * Reads `AIRoute_TLS_CERT` and `AIRoute_TLS_KEY` (filesystem paths). Returns
...
export function resolveTlsOptions(
  ...
  const certPath = typeof env?.AIRoute_TLS_CERT === "string" ? env.AIRoute_TLS_CERT.trim() : "";
  const keyPath = typeof env?.AIRoute_TLS_KEY === "string" ? env.AIRoute_TLS_KEY.trim() : "";
  ...
  if (!certPath || !keyPath) {
    warn(
      `[AIRoute][tls] HTTPS not enabled: both AIRoute_TLS_CERT and AIRoute_TLS_KEY ` +
  ...
  try {
    ...
    warn(
      `[AIRoute][tls] HTTPS not enabled: could not read TLS cert/key ` +
