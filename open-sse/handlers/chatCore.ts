...
import {
  checkTokenLimits,
  recordTokenUsage,
} from "@AIRoute/open-sse/services/tokenLimitCounter.ts";
import {
  COOLDOWN_MS,
  HTTP_STATUS,
...
import { wasRefreshTokenRotated } from "@AIRoute/open-sse/services/refreshSerializer.ts";
...
export async function handleChatCore({
  ...
  const traceEnabled = process.env.AIRoute_TRACE === "true" || process.env.DEBUG === "true";
  ...
  if (webSearchFallbackPlan.enabled) {
    ...
    log?.info?.(
      "TOOLS",
      `Converted ${webSearchFallbackPlan.convertedToolCount} web_search tool(s) to AIRoute fallback for ${provider}`
    );
  }
  ...
  const pipelineSessionId =
    (clientRawRequest?.headers && typeof clientRawRequest.headers.get === "function"
      ? clientRawRequest.headers.get("x-AIRoute-session-id")
      : getHeaderValueCaseInsensitive(
          clientRawRequest?.headers ?? null,
          "x-AIRoute-session-id"
        )) || skillRequestId;
  ...
  // (#5312 / #5245): `x-AIRoute-thinking-marker: off` suppresses it for
  ...
  // They are accepted for compatibility at the AIRoute API boundary only.
  if (body && typeof body === "object") {
  ...
  // `x-AIRoute-no-memory: true` to skip memory+skills injection (a null owner
  ...
  if (body && Array.isArray(allMessages) && allMessages.length > 0) {
    ...
    try {
      ...
      if (compressionHeader) {
        log?.debug?.("COMPRESSION", `x-AIRoute-compression header: ${compressionHeader}`);
      }
  ...
  // rejects unknown/extra params with a strict "400 Param Incorrect". Map AIRoute's
  ...
  if (!stream) {
    ...
    if (clientResponseFormat === FORMATS.OPENAI_RESPONSES) {
      ...
      // Port of decolua/9router#517: opt-in `x-AIRoute-strip-reasoning` header
    ...
    // #6426: align response body `model` with the `X-AIRoute-Model` header
  ...
  if (needsResponsesTranslation) {
    ...
    transformStream = createSSETransformStreamWithLogger(
      ...
      // (e.g. OpenCode by UA; any client via `x-AIRoute-thinking-marker: off`);
