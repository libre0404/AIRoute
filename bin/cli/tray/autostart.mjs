...
const APP_LABEL = "com.AIRoute.autostart";
const WIN_REG_VALUE = "AIRoute";
const LINUX_SERVICE_NAME = "AIRoute.service";
const LINUX_DESKTOP_NAME = "AIRoute.desktop";

function resolveCliPath() {
  const candidates = [];
  if (process.argv[1]) candidates.push(process.argv[1]);
  try {
    const which = execSync("command -v AIRoute 2>/dev/null", { encoding: "utf8" }).trim();
    if (which) candidates.push(which);
  } catch {
    // command -v unavailable
  }
  candidates.push(join(dirname(fileURLToPath(import.meta.url)), "..", "..", "AIRoute.mjs"));

  for (const candidate of candidates) {
    if (!candidate) continue;
    try {
      const resolved = realpathSync(candidate);
      if (resolved.endsWith("AIRoute.mjs") && existsSync(resolved)) return resolved;
    } catch {
      // try next candidate
    }
  }
...
function writeLinuxSystemdUnit(cliPath) {
  ...
  const envFile = join(userHomeDir(), ".AIRoute", ".env");
  const lines = [
    "[Unit]",
    "Description=AIRoute AI proxy router",
 ...
 * running process. When the running AIRoute cli was itself spawned by the
