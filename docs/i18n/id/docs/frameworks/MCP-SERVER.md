# Dokumentasi Server MCP AIRoute (Bahasa Indonesia)
...
AIRoute MCP sudah tersedia secara bawaan. Jalankan dengan:
...
| Alat                            | Deskripsi                                                         |
| :------------------------------ | :---------------------------------------------------------------- |
| `AIRoute_get_health`          | Kesehatan gateway, pemutus sirkuit, uptime                        |
| `AIRoute_list_combos`         | Semua combo yang dikonfigurasi beserta modelnya                   |
| `AIRoute_get_combo_metrics`   | Metrik performa untuk combo tertentu                              |
| `AIRoute_switch_combo`        | Ganti combo aktif berdasarkan ID/nama                             |
| `AIRoute_check_quota`         | Status kuota per penyedia atau semua penyedia                     |
| `AIRoute_route_request`       | Kirim penyelesaian chat melalui AIRoute                         |
| `AIRoute_cost_report`         | Analitik biaya untuk periode waktu tertentu                       |
| `AIRoute_list_models_catalog` | Katalog model lengkap beserta kemampuannya                        |

## Alat Lanjutan (8)

| Alat                               | Deskripsi                                                              |
| :--------------------------------- | :--------------------------------------------------------------------- |
| `AIRoute_simulate_route`         | Simulasi routing percobaan dengan pohon fallback                       |
| `AIRoute_set_budget_guard`       | Anggaran sesi dengan tindakan degrade/block/alert                      |
| `AIRoute_set_resilience_profile` | Terapkan preset conservative/balanced/aggressive                       |
| `AIRoute_test_combo`             | Uji langsung semua model dalam combo melalui permintaan upstream nyata |
| `AIRoute_get_provider_metrics`   | Metrik terperinci untuk satu penyedia                                  |
| `AIRoute_best_combo_for_task`    | Rekomendasi kesesuaian tugas beserta alternatifnya                     |
| `AIRoute_explain_route`          | Jelaskan keputusan routing yang lalu                                   |
| `AIRoute_get_session_snapshot`   | Status sesi lengkap: biaya, token, kesalahan                           |
