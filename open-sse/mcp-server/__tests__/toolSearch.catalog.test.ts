...
describe("getAllToolDefinitions", () => {
  ...
  it("aggregates many tools across collections", () => {
    ...
    expect(all.find((t) => t.name === "AIRoute_get_health")).toBeTruthy();
  });
  ...
  it("includes compressionTools-only entries (AIRoute_ccr_retrieve, not in MCP_TOOLS)", () => {
    // Regression: compressionTools carries AIRoute_ccr_retrieve, which is absent from
    ...
    expect(all.find((t) => t.name === "AIRoute_ccr_retrieve")).toBeTruthy();
  });
});
