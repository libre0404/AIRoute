 ...
 * shapes), and attach an `_AIRoute` meta blob to a log payload. Side-effect-free; behaviour is
...
export function attachLogMeta(
  ...
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return { _AIRoute: compactMeta, _payload: payload ?? null };
  }
  const existing =
    payload._AIRoute &&
    typeof payload._AIRoute === "object" &&
    !Array.isArray(payload._AIRoute)
      ? payload._AIRoute
      : {};
  return {
    ...payload,
