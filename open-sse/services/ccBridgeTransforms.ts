 ...
 *
 * Goal: ensure the final request body AIRoute sends to Anthropic's
...
export interface InjectBillingHeaderOp {
   ...
   *   - AIRoute-daystamp: sha256(YYYY-MM-DD + version).slice(0,3)
   */
  versionFormat: "ex-machina" | "AIRoute-daystamp";
  /**
 ...
 * proven against the live AIRoute deployment (call log
...
/**
 * Compute the `cc_version` suffix per the AIRoute native-OAuth algorithm:
...
export function buildBillingHeaderValue(
  ...
  const suffix =
    options.versionFormat === "AIRoute-daystamp"
