 ...
 * #14 Removed dead AIRoute:// protocol (no handler existed)
...
function resolveNodeExecutable(env = process.env) {
  ...
  // (e.g. AIRoute.app/Contents/MacOS/AIRoute). Spawning it with
...
function resolveDataDir(overridePath, env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return path.join(appData, "AIRoute");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return path.join(path.resolve(xdg), "AIRoute");

  return path.join(require("os").homedir(), ".AIRoute");
}
    ...
    new Promise((r) =>
      setTimeout(() => {
        try {
          ...
          // bare SIGKILL of the direct child, keeping AIRoute.exe locked).
...
function installUpdate() {
  if (nextServer) {
    ...
    // grandchildren) keeps AIRoute.exe locked and the updater fails with "file in use".
...
function setupContentSecurityPolicy() {
  session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
    ...
    const csp = [
      ...
      `connect-src 'self' http://localhost:* http://127.0.0.1:* ws://localhost:* ws://127.0.0.1:* wss://localhost:* wss://127.0.0.1:* https://*.AIRoute.online https://*.AIRoute.dev`,
      scriptSrc,
...
function startNextServer() {
  ...
  if (changed) {
    serverEnv.AIRoute_BOOTSTRAPPED = "true";
    try {
      fs.mkdirSync(dataDir, { recursive: true });
      const lines = [
        "# Auto-generated by AIRoute bootstrap",
        "",
  ...
  // AIRoute_MEMORY_MB or a pre-set --max-old-space-size still wins. Mirrors
  ...
  const serverNodeOptions = (() => {
    ...
    const explicit = parseInt(serverEnv.AIRoute_MEMORY_MB, 10);
    let heapMb;
  ...
  nextServer.stdout?.on("data", (data) => {
    ...
    if (text.includes("Ready") || text.includes("started") || text.includes("listening")) {
      ...
      const isHeadless =
        ...
        process.env.AIRoute_HEADLESS === "true";
      if (isHeadless && !global.loggedHeadlessReady) {
        global.loggedHeadlessReady = true;
        console.log("\n\x1b[32m✔ AIRoute Headless CLI Server is ready and listening!\x1b[0m");
...
function stopNextServer() {
  if (nextServer) {
    ...
    // (AIRoute.exe-as-node) spawns grandchildren that a bare SIGTERM leaves alive,
    // holding a lock on AIRoute.exe and blocking updates.
    killProcessTree(nextServer, { signal: "SIGTERM" });
    nextServer = null;
  }
}
  ...
  try {
    ...
    const desktopFileContent =
      [
        ...
        "Type=Application",
        "Name=AIRoute",
        "Comment=AIRoute Desktop Client",
    ...
    fs.writeFileSync(path.join(autostartDir, "AIRoute-desktop.desktop"), desktopFileContent, {
      mode: 0o644,
    });
    return true;
  } catch (err) {
...
function isLinuxDesktopAutostartEnabled() {
  try {
    ...
    return fs.existsSync(
      path.join(os.homedir(), ".config", "autostart", "AIRoute-desktop.desktop")
    );
  } catch {
    return false;
  }
