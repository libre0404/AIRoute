...
const LOCAL_USAGE_MODEL = "AIRoute/local-usage";
  ...
  try {
    ...
    return {
      preferredConnectionId:
        ...
        readHeader(request, "x-AIRoute-connection")?.trim() ||
        null,
      preferredProvider: url.searchParams.get("provider")?.trim() || null,
    };
  } catch {
    return {
      preferredConnectionId: readHeader(request, "x-AIRoute-connection")?.trim() || null,
