  ...
  const handleExportJson = async () => {
    ...
    try {
      await fetchAndDownload(
        ...
        `AIRoute-legacy-backup-${new Date().toISOString().replace(/[:.]/g, "-")}.json`,
        "JSON Export failed"
      );
    } catch (err) {
  ...
  const handleExport = async () => {
    ...
    try {
      await fetchAndDownload(
        ...
        `AIRoute-backup-${new Date().toISOString().replace(/[:.]/g, "-")}.sqlite`,
        t("exportFailed")
      );
    } catch (err) {
      ...
      <div className="grid grid-cols-1 gap-3 mb-4">
        <div className="p-3 rounded-lg bg-bg border border-border">
          ...
          <p className="text-sm font-mono text-text-main break-all">
            {storageHealth.dbPath || "~/.AIRoute/storage.sqlite"}
          </p>
        </div>
      </div>
