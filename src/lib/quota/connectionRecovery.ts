...
import { cooldownUntilMs } from "@AIRoute/open-sse/services/accountFallback.ts";
...
declare global {
  var __AIRouteConnRecovery:
    | { initialized: boolean; interval: ReturnType<typeof setInterval> | null }
    | undefined;
}

function getRecoveryState() {
  if (!globalThis.__AIRouteConnRecovery) {
    globalThis.__AIRouteConnRecovery = { initialized: false, interval: null };
  }
  return globalThis.__AIRouteConnRecovery;
}
...
function isRecoverySchedulerDisabled(): boolean {
  return (
    isEnvFlagEnabled("AIRoute_DISABLE_CONNECTION_RECOVERY") ||
    isEnvFlagEnabled("AIRoute_DISABLE_BACKGROUND_SERVICES") ||
    isBuildProcess() ||
...
/**
 * Resolve the tick interval (ms) from AIRoute_CONNECTION_RECOVERY_INTERVAL_MS,
...
export function resolveConnectionRecoveryIntervalMs(
  rawValue: string | undefined = typeof process !== "undefined"
    ? process.env.AIRoute_CONNECTION_RECOVERY_INTERVAL_MS
    : undefined
): number {
