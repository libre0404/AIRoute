...
/**
 * Check if the config file contains AIRoute settings.
 */
...
// POST — write AIRoute settings to Smelt config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge AIRoute settings (smelt uses OpenAI-compatible config)
...
// DELETE — remove AIRoute settings from Smelt config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Smelt AIRoute settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
