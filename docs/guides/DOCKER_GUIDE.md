---
title: "🐳 Docker Guide — AIRoute"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🐳 Docker Guide — AIRoute
...
AIRoute ships four Compose profiles. Pick the one that matches your environment.

| Profile          | Service          | When to use                                                                                                                       | Command                                      |
| ---------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| `base` (default) | `AIRoute-base` | Headless server / minimal runtime, no provider CLIs bundled                                                                       | `docker compose --profile base up -d`        |
| `cli`            | `AIRoute-cli`  | Agentic workflows that call `AIRoute providers/setup/doctor` and bundled CLIs (Codex, Claude Code, Droid, OpenClaw)             | `docker compose --profile cli up -d`         |
| `host`           | `AIRoute-host` | Linux hosts that want `network_mode`-like access to host CLIs by mounting `~/.local/bin`, `~/.codex`, `~/.claude`, etc. read-only | `docker compose --profile host up -d`        |
...
AIRoute relies on Redis to back the distributed rate limiter and shared cache. The `redis` service is **always defined** in `docker-compose.yml` (it has no profile gate) and starts alongside any other profile.

| Detail               | Value                             |
| -------------------- | --------------------------------- |
| Image                | `redis:7-alpine`                  |
| Container name       | `AIRoute-redis`                 |
| Internal port        | `6379`                            |
| Host port (override) | `REDIS_PORT` (defaults to `6379`) |
| Volume               | `AIRoute-redis-data` → `/data`  |
...
| Image                  | `AIRoute:prod` (built from `runner-cli` target)                                  |
| Redis container        | `AIRoute-redis-prod` (`redis:8.6.2`, dedicated `redis-prod-data` volume)         |
| Data volume            | `AIRoute-prod-data` (named, persisted across rebuilds)                           |
...
```bash
docker build --target runner-base -t AIRoute:base .
docker build --target runner-cli  -t AIRoute:cli  .
```

Defaults exported by `runner-base`: `PORT=20128`, `HOSTNAME=0.0.0.0`, `NODE_OPTIONS=--max-old-space-size=512`, `DATA_DIR=/app/data`, `AIRoute_MIGRATIONS_DIR=/app/migrations`.

Memory behavior in Docker:

- `NODE_OPTIONS=--max-old-space-size=512` is baked into the image as a fallback.
- The actual server process is started by the standalone launcher, which reads `AIRoute_MEMORY_MB` and appends `--max-old-space-size=<AIRoute_MEMORY_MB>`.
- Node uses the last repeated `--max-old-space-size` value, so setting `AIRoute_MEMORY_MB` controls the effective Docker heap limit.
- If `AIRoute_MEMORY_MB` is unset, the launcher uses `512`.
...
| ----------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------ |
| `AIRoute_WS_BRIDGE_SECRET`  | Shared secret for the WebSocket bridge. **Required in production** — set to a strong random string. | unset (must be provided) |
...
| `AUTO_UPDATE_HOST_REPO_DIR`   | Host path mounted into `cli` profile at `/workspace/AIRoute` for self-update workflows            | `.` (current directory)  |
| `AIRoute_MEMORY_MB`         | Runtime Node heap ceiling for the Docker standalone server; overrides the image fallback above      | `512`                    |
...
Caddy sets the standard forwarding headers for the upstream container. AIRoute uses
...
protection. Only enable `AIRoute_TRUST_PROXY` for advanced deployments where you intentionally
want AIRoute to derive the public origin from trusted forwarded headers instead of explicit
configuration.
...
| `diegosouzapw/AIRoute` | `3.8.0`  | ~250MB | Current version       |
...
- **SQLite WAL Mode:** `docker stop` should be allowed to finish so AIRoute can checkpoint the latest changes back into `storage.sqlite`. The bundled Compose files already set a 40s stop grace period. If you run the image directly, keep `--stop-timeout 40`.
