...
export const PRIVATE_PROVIDER_URLS_ENV = "AIRoute_ALLOW_PRIVATE_PROVIDER_URLS";
...
// cloud-metadata endpoints stay blocked. Defaults ON (AIRoute is local-first); operators
// who only use public providers can disable it to restore strict SSRF blocking.
export const LOCAL_PROVIDER_URLS_ENV = "AIRoute_ALLOW_LOCAL_PROVIDER_URLS";
 ...
 * provider URLs (`AIRoute_ALLOW_PRIVATE_PROVIDER_URLS`, default OFF). Protocol and
 ...
 * (AIRoute is local-first — local OpenAI-compatible providers should validate out of the
 * box). Disable via the `AIRoute_ALLOW_LOCAL_PROVIDER_URLS` flag (DB toggle or env) to
