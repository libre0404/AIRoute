...
const backupRoot = path.join(os.tmpdir(), `AIRoute-build-isolated-${process.pid}-${Date.now()}`);
...
export function getTransientBuildPaths(rootDir = projectRoot, env = process.env) {
  ...
  if (env.AIRoute_BUILD_MOVE_TASKS === "1") {
    paths.push({
...
export function resolveNextBuildBundlerFlag(baseEnv = process.env) {
  ...
  return baseEnv.AIRoute_USE_TURBOPACK === "0" ? "--webpack" : "--turbopack";
}
...
export function resolveNextBuildEnv(baseEnv = process.env) {
  ...
  // sets one — don't clobber/duplicate) and let AIRoute_BUILD_MEMORY_MB override.
  if (!/--max-old-space-size/.test(env.NODE_OPTIONS || "")) {
    ...
    const heapMb = Number(baseEnv.AIRoute_BUILD_MEMORY_MB) || 8192;
...
export async function main() {
  ...
  try {
    ...
    if (isBackendOnlyBuild()) {
      console.log(
        "[build-next-isolated] AIRoute_BUILD_BACKEND_ONLY set — building API only (dashboard UI stubbed)"
      );
