 ...
 *      AIRoute itself, OpenRouter, a Gemini-OpenAI-compat URL).
 ...
 *      AIRoute's own `/v1` so the vision model can be any provider
 *      registered in AIRoute (`google/gemini-2.0-flash`,
 *      `openrouter/...`, etc.) instead of being limited to OpenAI/Anthropic.
 *   2. `OPENAI_API_URL` env var (legacy)
 *   3. AIRoute self-loop (`http://localhost:20128/v1`) — auto-detected when
 *      the model uses a known AIRoute-internal provider (e.g. `kr/`, `if/`,
 ...
 *                that require AIRoute self-loop routing.
 */
export function resolveVisionBridgeBaseUrl(model?: string): string {
  ...
  // anthropic/), it can only be resolved through AIRoute's own router,
  // not through a direct OpenAI/Anthropic endpoint. Use the operator-configured
  // port via AIRoute_PORT / PORT env vars, falling back to the default 20128.
...
async function callVisionModelSingle(
  ...
  try {
    ...
    if (isAnthropic) {
      ...
      // AIRoute itself or any other OpenAI-compatible endpoint instead of
      // hardcoded api.openai.com.
      const baseUrl = resolveVisionBridgeBaseUrl(config.model);

      // When routing through the AIRoute self-loop (non-standard provider),
      // keep the full provider-prefixed model ID so AIRoute can resolve the
      ...
      // When routing through AIRoute's own API, omit the vision-bridge
      // guardrail on the sub-request to prevent infinite recursion.
      // Use sk_AIRoute as fallback for self-loop if no API key is resolved.
      const selfLoopApiKey = resolvedApiKey || "sk_AIRoute";
      const headers: Record<string, string> = {
        "Content-Type": "application/json",
        Authorization: `Bearer ${selfLoopApiKey}`,
      };
      if (useFullModelId) {
        headers["x-AIRoute-disabled-guardrails"] = "vision-bridge";
      }
