---
title: "AIRoute Architecture"
version: 3.8.40
lastUpdated: 2026-06-28
---

# AIRoute Architecture
...
AIRoute is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[AIRoute Local Process]
...
AIRoute can install, supervise, and route to locally-running AI tool processes
...
- AIRoute response metadata builder: `src/domain/AIRouteResponseMeta.ts`
...
- file: `${DATA_DIR}/storage.sqlite` (or `$XDG_CONFIG_HOME/AIRoute/storage.sqlite` when set, else `~/.AIRoute/storage.sqlite`)
...
flowchart LR
    ...
    subgraph ContainerOrProcess[AIRoute Runtime]
        Next[Next.js Server\nPORT=20128]
...
> AIRoute v3.8.0. For the canonical and continuously-updated list, refer to
...
- final client response returned by AIRoute; streamed responses are stored in the same compact summary form
...
1. `usageDb` and `localDb` share the same base directory policy (`DATA_DIR` -> `XDG_CONFIG_HOME/AIRoute` -> `~/.AIRoute`) with legacy file migration.
...
5. The `open-sse/` directory is published as the `@AIRoute/open-sse` **npm workspace package**. Source code imports it via `@AIRoute/open-sse/...` (resolved by Next.js `transpilePackages`). File paths in this document still use the directory name `open-sse/` for consistency.
...
- Build Docker image: `docker build -t AIRoute .`
