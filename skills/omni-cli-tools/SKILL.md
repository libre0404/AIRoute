---
name: omni-cli-tools
description: "Manage CLI tool integrations exposed via the API. List, configure, and invoke CLI tool plugins that extend AIRoute's automation surface."
---
...
Read Crush CLI AIRoute config

Local-only. Reads the AIRoute provider block in Crush's config.
...
Write Crush CLI AIRoute config

Local-only. Registers AIRoute as an `openai-compat` provider in Crush's config.
...
Remove AIRoute from Crush CLI config

Local-only. Removes the AIRoute provider block from Crush's config.
...
Read CodeWhale CLI AIRoute config

Local-only. Reads the AIRoute config block from `~/.codewhale/config.toml` (with `~/.deepseek/config.toml` legacy fallback).
...
Write CodeWhale CLI AIRoute config

Local-only. Writes the AIRoute config block in CodeWhale TOML format.
...
Remove AIRoute from CodeWhale CLI config

Local-only. Removes the AIRoute config block from CodeWhale's config.
