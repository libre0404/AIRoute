...
export function resolveAiderTarget(opts = {}) {
  ...
  if (opts.remote) root = stripToRoot(opts.remote);
    ...
    try {
      root = stripToRoot(resolveActiveContext(opts.context ?? process.env.AIRoute_CONTEXT)?.baseUrl);
    } catch {
      /* none */
    }
...
export function buildAiderRecipe({ apiBase, model }) {
  return [
    ...
    "export OPENAI_API_KEY=$AIRoute_API_KEY",
...
export async function runSetupAiderCommand(opts = {}) {
  ...
  printHeading("AIRoute → Aider (openai-compatible via LiteLLM)");
...
export function registerSetupAider(program) {
  program
    .command("setup-aider")
    .description("Configure Aider for AIRoute: write ~/.aider.conf.yml + print the env recipe")
