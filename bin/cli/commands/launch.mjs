...
/**
 * Build a clean child env for Claude Code pointed at AIRoute.
 *
...
export function buildClaudeEnv(baseEnv, baseUrlOrPort, authToken, opts = {}) {
  ...
  // from stopping at its local login gate before it ever contacts AIRoute (an
  ...
  env.ANTHROPIC_AUTH_TOKEN = (authToken && String(authToken).trim()) || "AIRoute-no-auth";
...
/**
 * Resolve the AIRoute base URL + auth for launch, honouring (in order):
...
export function resolveLaunchTarget(opts = {}) {
  ...
  if (!authToken) authToken = process.env.ANTHROPIC_AUTH_TOKEN ?? process.env.AIRoute_API_KEY;
...
export async function runLaunchCommand(opts = {}, claudeArgs = []) {
  ...
  try {
    ...
    console.error(
      (t("launch.notRunning") || "AIRoute is not reachable at {port}. Start it with 'AIRoute serve'.").replace(
        "{port}",
        baseUrl
      )
    );
...
export function registerLaunch(program) {
  program
    .command("launch")
    .description(
      t("launch.description") || "Launch Claude Code pointed at AIRoute (local or remote)"
    )
    .option("--port <port>", t("serve.port") || "Proxy port", "20128")
    .option("--remote <url>", "Remote AIRoute base URL (overrides --port and the active context)")
    ...
    .option("--api-key <key>", "Alias for --token (AIRoute access token / API key)")
