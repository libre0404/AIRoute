...
Delegated **Context Editing** is a Claude-only context-management feature. Unlike AIRoute's local
...
tool-use / tool-result blocks from its own running context window. AIRoute only attaches a body
parameter (`context_management.edits[]`); Claude does the actual clearing against its own tokenizer.

This is a delegated capability by nature: other providers reject the parameter, so AIRoute scopes
...
AIRoute injects a single edit into the outbound Anthropic Messages body:
...
AIRoute already emits on Claude requests.
...
left as-is. If a `clear_thinking_20251015` edit is also present, AIRoute stable-sorts the
...
After a Claude response, AIRoute records how much context the provider actually cleared. This is
...
| ----------------- | --------------------------------------------------- | ------------------------------------------- |
| Where it runs     | In AIRoute, before the request leaves the proxy   | In the provider (Claude), server-side       |
...
The two are complementary: local engines compress the bytes AIRoute sends; Context Editing lets
