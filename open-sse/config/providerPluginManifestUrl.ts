export const PROVIDER_PLUGIN_MANIFEST_HEADER = "X-AIRoute-Provider-Manifest-Url";
...
export function resolveProviderPluginManifestUrl(origin?: string | null): string {
  const configured = process.env.AIRoute_PROVIDER_MANIFEST_URL?.trim();
  ...
  const protocol = process.env.AIRoute_PUBLIC_PROTOCOL || "http";
