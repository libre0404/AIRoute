...
export async function runEnvShowCommand(opts = {}) {
  const current = {};
  for (const key of AIRoute_ENV_VARS) {
...
export async function runEnvGetCommand(key) {
  if (!key) {
    console.error("Key is required. Usage: AIRoute env get <key>");
    return 1;
  }
...
export async function runEnvSetCommand(key, value) {
  if (!key || value === undefined) {
    console.error("Usage: AIRoute env set <key> <value>");
    return 1;
  }
