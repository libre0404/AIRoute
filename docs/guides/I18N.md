...
AIRoute supports **42 languages** with full dashboard UI translation, translated documentation, and RTL support for Arabic and Hebrew.
...
AIRoute uses a hash-based incremental translator for docs, backed by an
OpenAI-compatible LLM endpoint (typically `cx/gpt-5.4-mini` through AIRoute
Cloud):
...
| Variable                            | Purpose                                 |
| ----------------------------------- | --------------------------------------- |
| `AIRoute_TRANSLATION_API_URL`     | OpenAI-compatible base URL, e.g. `…/v1` |
| `AIRoute_TRANSLATION_API_KEY`     | bearer token (kept out of logs)         |
| `AIRoute_TRANSLATION_MODEL`       | model id, e.g. `cx/gpt-5.4-mini`        |
...
- **Chunked batching**: Joins multiple strings with `__AIRoute_I18N_SEPARATOR__` delimiters to minimize API calls (max 1800 chars per request)
...
**Secondary translator** — uses any OpenAI-compatible LLM API (including AIRoute itself) to translate existing `docs/i18n/` markdown files. Best for polishing or re-translating docs with better quality than Google Translate.
