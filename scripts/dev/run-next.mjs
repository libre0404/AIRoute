...
import { createAIRouteWsBridge } from "./v1-ws-bridge.mjs";
...
if (fs.existsSync(rootAppDir) && fs.statSync(rootAppDir).isDirectory()) {
  ...
  console.error("Please rename or delete the root 'app/' directory before starting AIRoute.\n");
  process.exit(1);
}
...
// build default in build-next-isolated.mjs); AIRoute_USE_TURBOPACK=0 is the
// webpack escape hatch.
const useTurbopack = dev && mergedEnv.AIRoute_USE_TURBOPACK !== "0";
process.env.AIRoute_WS_BRIDGE_SECRET ||= randomUUID();
...
async function start() {
  ...
  const responsesWsProxy = createResponsesWsProxy({
    ...
    bridgeSecret: process.env.AIRoute_WS_BRIDGE_SECRET,
  });
  const wsBridge = createAIRouteWsBridge({
    baseUrl: `http://127.0.0.1:${dashboardPort}`,
  });
