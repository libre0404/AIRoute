...
默认 HTTP 端口：**`20128`**（API 和仪表盘共享同一进程）。数据目录由 `DATA_DIR` 环境变量指定，默认为 `~/.AIRoute/`。
...
├── open-sse/             流式传输引擎工作空间（@AIRoute/open-sse）
├── electron/             桌面端封装（Electron 41 主进程 + preload）
├── bin/                  CLI 入口点（AIRoute、reset-password）
...
| `responses.ts`、`AIRouteResponseMeta.ts`   | 响应格式辅助工具                  |
...
独立的 npm workspace，发布为 `@AIRoute/open-sse`。负责请求处理、执行器、翻译器、服务、转换器和 MCP 服务器。
...
- **导入**：外部 → 内部（`@/`、`@AIRoute/open-sse`）→ 相对路径。
