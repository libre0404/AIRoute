...
<img src="../../screenshots/MainAIRoute.png" alt="AIRoute 儀表板" width="820"/>
...
**~1.6B 有記錄的免費 Token/月** — 首月透過註冊獎勵最高可達 **~2.1B** — 聚合所有免費層的配額，加上永久免費、無上限的供應商，而上述壓縮進一步延長每一分 Token。([統計方法 →](../../reference/FREE_TIERS.md#tldr--how-much-free-inference-does-AIRoute-actually-aggregate))
...
**問題、供應商技巧、路線圖與支援 → [Discord](https://discord.gg/EkzRkpzKYt) · [Telegram](https://t.me/AIRouteOficial) · WhatsApp [🌍 全球](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t) / [🇧🇷 巴西](https://chat.whatsapp.com/BTGJXIyjeNIIgExvTMGGhI)**
...
[**🚀 快速開始**](#-quick-start) • [**🎯 Combo**](#-combos--the-flagship) • [**🌐 供應商**](#-231-ai-providers--50-free) • [**🔌 CLI 與 MCP**](#-full-cli--a2a--mcp) • [**🗜️ 壓縮**](#%EF%B8%8F-save-1595-tokens--automatically) • [**🌍 網站**](https://AIRoute.online)

[💥 承諾](#-the-promise) • [🤔 為什麼](#-why-AIRoute) • [🏆 優勢](#-what-sets-AIRoute-apart) • [🤖 相容 CLI](#-compatible-clis--coding-agents) • [🖥️ 執行平台](#%EF%B8%8F-where-AIRoute-runs--anywhere) • [🔒 隱私](#-private--local-first) • [🎬 實際展示](#-AIRoute-in-action) • [📚 探索更多](#-explore-more) • [📧 支援](#-support--community)
...
> 手動堆疊免費層很痛苦 — 數十個 SDK、數十個速率限制，而且你不清楚自己到底有多少配額。AIRoute 將 **40+ 供應商池 / 500+ 模型**的**有記錄**免費層聚合為一個真實數字，並在儀表板上即時展示 (`/dashboard/free-tiers`)。
...
> 一個端點。**231 個供應商。** 永遠不要停止建構 — 讓 AIRoute 選擇最便宜且有效的方案。
...
> **Combo** 是 AIRoute **自動**路由的模型鏈。配額用盡、供應商失敗或成本飆升 — Combo 自動滑動到下一個模型。**這就是 AIRoute 不可中斷的原因。** 🛡️

### ⚡ 零設定 — 只需使用 `auto`

無需建立 Combo。將模型設定為 `auto`（或變體），AIRoute 會根據您連接的供應商即時評分建構虛擬 Combo：
...
<sub>📊 與 LiteLLM、OpenRouter 和 Portkey 的詳細比較 → [`docs/comparison/AIRoute_VS_ALTERNATIVES.md`](../../comparison/AIRoute_VS_ALTERNATIVES.md)</sub>
...
- **🛰️ 遠端模式** — 透過範圍存取令牌從任何機器驅動遠端 AIRoute（`AIRoute connect` / `AIRoute contexts` / `AIRoute tokens`）。→ [Remote Mode](../../guides/REMOTE-MODE.md)
...
- **⚡ 本地效能與基礎設施** — 一鍵本地 Redis 啟動器（`AIRoute redis up`，加上儀表板 Redis 面板）、一鍵 **Cloudflare Workers** 和 **Deno Deploy** 中繼部署器接入代理池，以及可選的 Bifrost Go sidecar，用於卸載最熱門的中繼路徑（`BIFROST_BASE_URL`，逾時時自動備援到 TypeScript 路徑）。→ [Environment](../../reference/ENVIRONMENT.md)
...
<sub>📖 所有 16+ 工具的個別設定 → [`docs/reference/CLI-TOOLS.md`](../../reference/CLI-TOOLS.md) · 🧩 OpenCode 插件 → [`@AIRoute/opencode-provider`](https://www.npmjs.com/package/@AIRoute/opencode-provider)</sub>
...
| 平台 | 安裝方式 | 亮點 |
|---|---|---|
| 📦 **npm（全域）** | `npm install -g AIRoute` | 一條命令，任何作業系統 |
| 🐳 **Docker** | `docker run … diegosouzapw/AIRoute` | 多架構 **AMD64 + ARM64** |
| 🖥️ **桌面（Electron）** | `npm run electron:build` | 原生視窗 + 系統匣 — **Windows / macOS / Linux** |
| 💪 **ARM** | 原生 `arm64` | Raspberry Pi、ARM 伺服器、Apple Silicon |
| 📱 **Android（Termux）** | `pkg install nodejs-lts && npx -y AIRoute` | **在手機上**執行，24/7，無需 root |
| 📲 **PWA** | "新增到主畫面" | 全螢幕、離線、可從瀏覽器安裝 |
| 🧩 **OpenCode 插件** | `@AIRoute/opencode-provider` | 原生 OpenCode 整合 |
...
> 您的金鑰、您的機器、您的資料。AIRoute 是**本地代理** — 絕不向外回報。

- 🏠 **100% 在您的硬體上執行** — npm、Docker、桌面或手機。AIRoute 雲端絕不介入請求路徑。
...
> AIRoute 不僅僅是伺服器 — 它擁有 **60+ 命令**的**完整命令列控制台**，以及開放的代理協定，讓 AI 代理可以**自行**驅動 AIRoute。

### ⌨️ 真正的 CLI（不僅僅是 `start`）

```bash
AIRoute               # 啟動閘道 + 儀表板（埠口 20128）
AIRoute chat          # 互動式 TUI 聊天客戶端（斜線指令：/model /combo /skill /memory）
AIRoute setup         # 引導式首次執行精靈
AIRoute doctor        # 診斷供應商、埠口、原生依賴
```

### 🛰️ 遠端模式 — 在此執行 CLI，AIRoute 在 VPS 上

AIRoute 在伺服器上？從筆記型電腦使用**相同 CLI** 驅動。登入一次
使用範圍存取令牌；之後每個命令都指向遠端。

```bash
AIRoute connect 192.168.0.15            # 密碼 → 範圍令牌，儲存為上下文
AIRoute models list                     # ← 針對遠端伺服器執行
AIRoute configure codex                 # ← 選擇遠端模型，寫入本地 Codex 設定檔
AIRoute tokens create --name ci --scope read   # 為其他機器建立更窄的令牌
AIRoute contexts use default            # ← 切換回本地伺服器
```
...
透過 **MCP** 或 **A2A** 公開 AIRoute，任何有能力的代理即可取得整個閘道的金鑰 — 路由、供應商、Combo、快取、壓縮、記憶 — 自主運作。

| 協定 | 端點 | 用途 |
|---|---|---|
| 🧰 **MCP（stdio）** | `AIRoute --mcp` | 接入 Claude Desktop、Cursor 等 MCP 客戶端 |
...
```bash
# 將完整 AIRoute 工具集透過 MCP 提供給 Claude Code：
...
> **為什麼用很多 Token 而不用少量 Token？** 每個請求**透明地**通過 AIRoute 的壓縮管線 — 無需變更客戶端。它現在是**9 個可組合引擎的堆疊**，按順序執行，並可依路由 Combo 混合搭配 — 建立在 [RTK](https://github.com/rtk-ai/rtk)、[Caveman](https://github.com/JuliusBrussee/caveman) (⭐ 51K+)、[LLMLingua-2](https://github.com/microsoft/LLMLingua) 和 [Troglodita](https://github.com/leninejunior/troglodita) (PT-BR) 的基礎上。
...
```
Client (10,000 tok) ──▶ AIRoute Compression (9 engines) ──▶ Provider (~1,080 tok, up to 95% saved)
```
...
- **🎛️ 壓縮決策位置**（_優先級，高 → 低_）— 每請求 `x-AIRoute-compression` 標頭 › 路由 Combo 覆寫 › 活動命名設定檔 › 自適應/自動觸發 › 面板預設 › 關閉。應用的計畫在 `X-AIRoute-Compression: <mode>; source=<source>` 回應標頭中回顯。
...
您應該會看到已連接的模型列表。🎉 就是這樣 — 開始編碼，AIRoute 會自動路由和備援。

如果您的客戶端無法傳送自訂標頭，AIRoute 也提供令牌化的相容別名：
...
> 🎬 **製作了關於 AIRoute 的影片？** 透過連結開啟 [issue](https://github.com/diegosouzapw/AIRoute/issues/new) 或 [discussion](https://github.com/diegosouzapw/AIRoute/discussions) — 我們會在此展示。
...
> 💡 儀表板上的"成本"是**節省追蹤器**，不是帳單 — AIRoute 從不向您收費。使用免費模型顯示的"$290 總成本"意味著**節省了 $290**。
...
🇷🇺 🇨🇳 🇮🇷 🇨🇺 🇹🇷 在被封鎖的地區？AIRoute 的 **3 層代理**（全域 / 每供應商 / 每連線）代理 API 請求、OAuth 流程、連線測試、Token 重新整理和模型同步。
...
| `DATA_DIR` | `~/.AIRoute` | 資料庫和設定儲存位置 |

**AIRoute 會向我收費嗎？** 不會 — 它是免費的開源軟體，在您的機器上執行。您只直接向付費供應商付費。AIRoute 沒有帳單系統。
...
- 🌍 **網站**：[AIRoute.online](https://AIRoute.online)
- 🐙 **GitHub**：[github.com/diegosouzapw/AIRoute](https://github.com/diegosouzapw/AIRoute)
- 🐛 **Issue**：[回報錯誤](https://github.com/diegosouzapw/AIRoute/issues)（請附上 `npm run system-info` 輸出）
...
- **網站**：[AIRoute.online](https://AIRoute.online)
- **套件**：[npmjs.com/package/AIRoute](https://www.npmjs.com/package/AIRoute)
...
| [Remote Mode](../../guides/REMOTE-MODE.md) | 從筆記型電腦 CLI 透過範圍存取令牌驅動遠端 AIRoute（VPS） |
| [Claude Code Config](../../guides/CLAUDE-CODE-CONFIGURATION.md) | 將 Claude Code 指向 AIRoute（本地/遠端），附 `launch` + 每模型設定檔 |
...
| [Termux Guide](../../guides/TERMUX_GUIDE.md) | 在 Android 上透過 Termux 執行 AIRoute |
...
> AIRoute 由充滿熱情的開源社群塑造。這些個人做出了卓越貢獻，直接影響了專案的品質、穩定性和影響力。**謝謝你們。**
...
> 🙏 這些貢獻者的功能、錯誤修復和基礎設施改進是 AIRoute 可靠且功能豐富的**核心部分**。每一個 Pull Request、每一個測試案例、每一個 i18n 翻譯檔案都很重要。開源是由像他們這樣的人建構的。
...
AIRoute 站在巨人的肩膀上。它始於 **[9router](https://github.com/decolua/9router)** 的分支和 Go 專案 **[CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI)** 的 TypeScript 移植 — 從那裡開始，下面的每個子系統都受到先行開源專案的啟發。每一個都塑造了 AIRoute 的具體部分。這是我們對它們所有人的感謝。🙏
...
AIRoute 是免費且開源的，在公開環境中建構和維護。如果它為您節省了時間或金錢，請考慮支援其開發：

- ⭐ **為倉庫加星** — 這確實有助於提高能見度
- 💖 **[GitHub Sponsors](https://github.com/sponsors/diegosouzapw)** — 資助持續維護和新供應商
- 🐛 **在 [Discussions](https://github.com/diegosouzapw/AIRoute/discussions) 中回報錯誤和分享意見回饋**
