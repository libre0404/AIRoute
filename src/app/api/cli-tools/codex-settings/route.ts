...
const parseToml = (content: string) => {
  ...
  content.split("\n").forEach((line) => {
    ...
    // Section header like [model_providers.AIRoute]
...
// Check if config has AIRoute settings
const hasAIRouteConfig = (config: string | null) => {
  if (!config) return false;
  return (
    config.includes("openai_base_url") ||
    config.includes('model_provider = "AIRoute"') ||
    config.includes("[model_providers.AIRoute]")
  );
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      reason: runtime.reason,
      config,
      hasAIRoute: hasAIRouteConfig(config),
...
// POST - Update AIRoute settings (merge with existing config)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Update only AIRoute related fields (api_key goes to auth.json, not config.toml)
    ...
    // Always create a custom provider to reliably pass wire_api and use AIRoute_API_KEY
    parsed._root.model_provider = "AIRoute";
    parsed._sections["model_providers.AIRoute"] = {
      name: "AIRoute",
...
// DELETE - Remove AIRoute settings only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove AIRoute related root fields
    delete parsed._root.openai_base_url;

    if (parsed._root.model_provider === "AIRoute") {
      delete parsed._root.model;
      delete parsed._root.model_provider;
    }

    // Remove AIRoute provider section
    delete parsed._sections["model_providers.AIRoute"];
    ...
    return NextResponse.json({
      success: true,
      message: "AIRoute settings removed successfully",
    });
  } catch (error) {
