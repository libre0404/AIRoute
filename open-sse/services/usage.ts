...
const CROF_USAGE_URL = process.env.AIRoute_CROF_USAGE_URL ?? "https://crof.ai/usage_api/";
 ...
 * API to call. Instead we count the tokens AIRoute itself routed to this
 ...
 * through AIRoute, not the provider's own dashboard figure.
 */
async function getXiaomiMimoUsage(connectionId: string) {
  ...
  try {
    ...
    return {
      plan: "Xiaomi MiMo Token Plan (AIRoute-tracked)",
      quotas: {
 ...
 * MiMo self-track pattern above — AIRoute sums the tokens it itself routed
 ...
 * the USD that AIRoute has spent through this connection since the account was added — summed
...
async function getVertexUsage(connectionId: string, provider: string) {
  ...
  try {
    ...
    if (requests === 0) {
      return {
        plan: "Vertex AI",
        message: "Vertex connected. No usage recorded through AIRoute yet for this account.",
        quotas: { spend },
      };
    }
